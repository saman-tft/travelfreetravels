			...
			<div class="multi_city_container" id="multi_city_container_<?=$multi_k?>" style="<?=$segment_visibility?>">
				...
				<div class="col-xs-6 padfive"> 
				 <?php if($multi_k == 1) { ?>
					...
					<div class="plcetogo deprtures sidebord">
						<input type="text" autocomplete="off" name="from[]" class="m_depcity m_arr_c  normalinput auto-focus valid_class fromflight form-control b-r-0 brleft30" id="m_from<?=$multi_k?>" placeholder="Type Departure City" value="<?php echo @$multicity_segment_search_params['from'][$multi_k-1]?>" required />
				...
				<div class="col-md-4 col-xs-3 nopad secndates mfulwdth">
					<div class="col-xs-12 padfive">
                        ...
                        <?php } ?>
						<div class="plcetogo datemark brright30 sidebord">
							<input type="text" name="depature[]" class="m_depature_date normalinput auto-focus hand-cursor form-control b-r-0 brright30 disable-date-auto-update" id="m_flight_datepicker<?=$multi_k?>" placeholder="Select Date" value="<?php echo @$multicity_segment_search_params['depature'][$multi_k-1] ?>" readonly required/>
						</div>
					</div>
                    <?php if($multi_k > 2) { ?>
	                    <button class="city_close_btn remove_city" type="button"> <span class="fa fa-times"></span></button>
					<?php } ?>
