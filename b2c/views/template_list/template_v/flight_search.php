				...
				<div class="col-md-7 nopad placerows">
					<div class="col-xs-6 padfive">
						...
						<div class="plcetogo deprtures sidebord">
							<input type="text" autocomplete="off" name="airlinefrom" class="normalinput auto-focus valid_class fromflight form-control b-r-0 brleft30" id="airlinefrom" placeholder="Type Departure City" value="<?php echo @$onw_rndw_segment_search_params['from'] ?>" required />
							<input class="hide loc_id_holder" id="airline_from_loc_id" name="airline_from_loc_id" type="hidden" value="<?=@$onw_rndw_segment_search_params['from_loc_id']?>" >
...
<script>
	...
	$(window).scroll(function(){
            if ($("input").is(":focus")) {
            $('#flight_datepicker1').blur();
            } 
            
        });
    $(window).scroll(function(){
            if ($("input").is(":focus")) {
            $('#flight_datepicker2').blur();
            } 
