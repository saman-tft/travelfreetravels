...
.pagi_nation .pagination{margin: 5px 0;}
...
$sr_count = ($searchResultcount)? count($searchResultcount) : 0;
$region_list =array();
...
//debug($temp_conversion);				

$minPrice_1 =sprintf("%.2f", min($atsArray));
$maxPrice_1= sprintf("%.2f", max($atsArray));
//debug($minPrice_1);
//debug($maxPrice_1);exit;
...
// $this->currency->get_currency_symbol($default_currency); 
   // debug($minPrice_1); 
   // debug($maxPrice_1);
   //$minPrice =   isset($minPrice_1)? number_format(get_converted_currency_value ( $currency_obj->force_currency_conversion ( $minPrice_1 ) ), 0):0;

   //$maxPrice =   isset($maxPrice_1)? number_format(get_converted_currency_value ( $currency_obj->force_currency_conversion ( $maxPrice_1 ) ), 0):0; 
  	
    $minPrice=str_replace(',','', $minPrice_1) + $admin_markup_holidaylcrs + $gst_value; 
    $maxPrice=str_replace(',','', $maxPrice_1) + $admin_markup_holidaylcrs2 + $gst_value2; 
...
<div class="colrit layout_upgrade">  
 ...
 <div class="vluendsort">
  <div class="col-md-2 col-xs-4 nopad"><span class="price_sort">Price Sort:</span></div>
  <div class="col-md-5 col-xs-8 nopad pull-right"> 
   <div class="filterforallnty" id="top-sort-list-wrapper">
    <div class="col-xs-12 nopad">
     <div class="insidemyt">
      <ul class="sortul" style="margin-bottom: 0px;">
       <li class="sortli threonly-srtmob" style="width: 46%; margin: 2px 6px;" data-sort="hn"> <a class="sorta price-l-2-h" data-toggle="tab"> <!-- <i class="fa fa-long-arrow-down"></i> --> <strong>Low to High</strong> </a></li>
       <li class="sortli threonly-srtmob" style="width: 46%; margin: 2px 6px;" data-sort="sr"> <a class="sorta price-h-2-l" data-toggle="tab"> <!-- <i class="fa fa-long-arrow-up"></i> --> <strong>High to Low</strong> </a></li>
      </ul>
     </div>
    </div>
   </div>
    ...
    if(!empty($searchResult))
    {
     ...
     //    debug($tour_price_changed);exit;
        ...
        if(valid_array($tour_price_changed))
        {
          ...
          $min_price = min($min_numbers); 
          $min_price_1 =   isset($min_price)? ( $min_price):0;  
           //debug($min_price);
        ...
        if(!empty($min_price))
        {
          ...
          <li <?=$tour_id?> class=" col-xs-12 col-sm-12 col-md-12 nopad padtwo container_li tour-item" data-price="<?=($min_price);?>" data-price-sort="<?php echo isset($min_price)? number_format($min_price, 2):0; ?>" data-duration="<?= $record['duration'];?>" data-region="<?= $record['tours_continent'];?>" data-country="<?= $record['tours_country'];?>" data-category="<?= $record['tour_type'];?>" data-theme="<?= $record['theme'];?>">
                ...
                if(!empty($min_price))
                  {
                    ...
                    <div class="pkprice">
                      ...
                      echo isset($min_price)? number_format($min_price, 0):0; ?>
           ...
           //  debug( $start_city_id);exit;
              ...
              ?>
              <div class="more_deat">
                <div class="mn_inclus"><h4><strong class="city-dtl">Start city: </strong> <?php if(!empty($start_city)){
                    echo $start_city; 
                  }else{
                    echo $palces_covered;
                  }
                  ?></h4></div>
                <div class="mn_inclus"><h4><strong class="city-dtl">End city: </strong> <?php if(!empty($end_city)){
                 echo $end_city; 
                 }else{
                  echo $palces_covered;
                 } 
...
<div class="pagi_nation">
   <div class="page_lft">
    <?php echo $this->pagination->create_links_holiday(); ?> 
    <br><span class="">Total <?php echo $total_rows ?> Tours Found</span>
</div>   
</div>
...
<!-- <div class="pull-right">
    <?php echo $this->pagination->create_links_holiday(); ?> <span class="">Total <?php echo $total_rows ?> Tours Found</span>
</div>  -->
