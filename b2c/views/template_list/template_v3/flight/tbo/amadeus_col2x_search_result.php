...
*/
        ...
        //changes start for baggage 2
        $baggage_arr = array();
        $i = 0;
        //changes end for baggage 2
        foreach ($cur_SegmentDetails as $__segment_k => $__segment_v) {
            ...
            foreach ($__segment_v as $__stop => $__segment_flight) {
                ...
                //     if($__segment_flight['AirlineDetails']['AirlineCode']=="YT")
                // {
                //      $__segment_flight['AirlineDetails']['AirlineName']="Yeti Airlines";
                // }
                // commented below line and added if condition for it and also cabinbaggage is added
                // $Baggage = trim($__segment_flight['Baggage']);
                //changes start for baggage 3
                if (empty($__segment_flight['Baggage']) == false) {
                    $baggage_arr[$i][$__stop]['Baggage'] = trim($__segment_flight['Baggage']);
                    $baggage_arr[$i][$__stop]['CabinBaggage'] = trim($__segment_flight['CabinBaggage']);
                    $Baggage = trim($__segment_flight['Baggage']);
                    $CabinBaggage = trim($__segment_flight['CabinBaggage']);
                }
                //changes end for baggage 3 
                ...
                $inner_summary .= '<div class="col-xs-5 nopad5">
                                        ...
                                        <div class="termnl">' . ucwords(strtolower($__segment_flight['OriginDetails']['AirportName'])) . ' (' . $__segment_flight['OriginDetails']['AirportCode'] . ')</div>
                                        </div>';
                //direction indicator
                ...
                $inner_summary .= '<div class="col-xs-5 nopad5">
                                        ...
                                        <div class="termnl">' . ucwords(strtolower($__segment_flight['DestinationDetails']['AirportName'])) . ' (' . $__segment_flight['DestinationDetails']['AirportCode'] . ')</div>
                                        </div>';
                $inner_summary .= '</div>';
                //changes added the div with value flight duration to label the time showing
                //Between Content -----
                ...
                if (empty($Baggage) == false) {
                    // $inner_summary .= '<div class="termnl1 flo_w"><em><i class="fa fa-suitcase bag_icon"></i>' . "Hello" . '</em></div>';
                    // here baggage informatin is not displying for 
                    $inner_summary .= '<div class="termnl1 flo_w"><em><i class="fa fa-suitcase bag_icon"></i>' . $Baggage . '</em></div>';
                }
                //changes start for baggage 4
                if (empty($CabinBaggage) == false) {
                    $inner_summary .= '<div class="termnl1 flo_w"><em><i class="fa fa-shopping-bag"></i>' . ($CabinBaggage) . '</em></div>';
                }
                //changes end for baggage 4
                ...
                if (isset($__segment_flight['WaitingTime']) == true) {
                    ...
                    //changes added the following if else for stop info message and removed the portion in the span tag as " Plane change at ' . $next_seg_info['OriginDetails']['CityName'] ." and replaced with the variable 
                    $inner_summary .= '
                <div class="clearfix"></div>
                <div class="layoverdiv">
                    <div class="centovr">
                    ';
                    if ($__segment_v[$__stop]['AirlineDetails']['FlightNumber'] == $__segment_v[$__stop + 1]['AirlineDetails']['FlightNumber']) {
                        $stopInfo = "Short stop at " . $__segment_v[$__stop + 1]['OriginDetails']['CityName'];
                    } else {
                        $stopInfo = "Plane change at " . $__segment_v[$__stop + 1]['OriginDetails']['CityName'];
                    }
                    $inner_summary .= '
                    <span class="fa fa-plane"></span>' . $stopInfo . ' | <span class="fa fa-clock-o"></span> Waiting: ' . $waiting_time . '
            ...
            $inner_summary .= '</div>';
            //start changes for baggage 5
            $i++;
            //end changes for baggage 5
        }
        //start changes for baggage 6
        $baggage_arr = base64_encode(json_encode($baggage_arr));
        //end changes for baggage 6
        ...
        // change for sess expiry 1 added search_hash in the following form submit params
        $outer_summary .= '<div class="madgrid" data-key="' . $__root_indicator . $__trip_flight_k . '">
        <div class="onlmob"><form method="POST" id="form-id-' . $__root_indicator . $__trip_flight_k . '" action="' . $booking_url . '" class="book-form-wrapper">
                                    
        ' . $GLOBALS['CI']->flight_lib->booking_form($IsDomestic, $__trip_flight_v['Token'], $__trip_flight_v['TokenKey'], $cur_ProvabAuthKey, '', $booking_source, $baggage_arr) . '
        ' . $booking_button . '
                                </form></div>';
        $outer_summary .= '<div class="f-s-d-w col-xs-8 nopad wayeght full_same">';
        $total_stop_count = 0;
        //changes end for baggage 7
        ...
        foreach ($cur_SegmentSummary as $__segment_k => $__segment_v) {
            ...
            if ($__stop_count > 0) {
                $stopcount_calss = "stop-value";
            } else {
                $stopcount_calss = "stop_zero";
            }
            //$outer_summary .= '<div class="clearfix visible-sm-block"></div>';
        ...
        if ($cur_IsRefundable == 0) {
            $ref_code = "nre";
        } else {
            $ref_code = "re";
        }
        $outer_summary .= '</div>';
        // changes start for baggage 8 changed the form
        // changes start session: = change 2
        // change for sess expiry 2 added search_hash in the following form submit params
        $outer_summary .= '
