...
?>
<form autocomplete="off" name="flight" id="flight_form" action="<?php echo base_url(); ?>general/pre_flight_search"
	method="get" class="activeForm oneway_frm form mainsearchform" style="">
	<div class="tabspl">
		<div class="tabrow nopad">
			<div class="waywy">
				...
				<div class="smalway">
					<div class="radio-item wament hand-cursor">
						<input class="et" type="radio" name="trip_type"
							<?= (isset($flight_search_params['trip_type']) == false ? 'checked' : ($flight_search_params['trip_type']) == 'oneway' ? 'checked="checked"' : '') ?> id="onew-trp"
							value="oneway" />
					...
					<div class="radio-item wament hand-cursor">
						<input class="et" type="radio" name="trip_type"
							<?= (@$flight_search_params['trip_type'] == 'circle' ? 'checked="checked"' : '') ?> id="rnd-trp"
							value="circle" />
					...
					<div class="radio-item wament hand-cursor">
						<input class="et" type="radio" name="trip_type"
							<?= (@$flight_search_params['trip_type'] == 'multicity' ? 'checked="checked"' : '') ?>
			...
			<div class="whitebgrad">
				<div id="onw_rndw_fieldset" class="col-md-8 nopad"><!-- Oneway/Roundway Fileds Starts-->
					<div class="col-md-7 nopad placerows">
						<div class="col-xs-6 padfive">
							<div class="formlabel clearable">From</div>
							<div class="plcetogo deprtures sidebord">
								<input type="text" autocomplete="off" name="from"
									class="normalinput clearable auto-focus valid_class fromflight form-control b-r-0 brleft30"
									id="from" placeholder="Type Departure City"
									value="<?php echo @$onw_rndw_segment_search_params['from'] ?>" required />
								<input class="hide loc_id_holder" id="from_loc_id" name="from_loc_id" type="hidden"
									value="<?= @$onw_rndw_segment_search_params['from_loc_id'] ?>">
						...
						<div class="col-xs-6 padfive">
							...
							<div class="plcetogo destinatios sidebord">
								<input type="text" autocomplete="off" name="to"
									class="normalinput auto-focus valid_class departflight form-control b-r-0" id="to"
									placeholder="Type Destination City"
									value="<?php echo @$onw_rndw_segment_search_params['to'] ?>" required />
								<input class="hide loc_id_holder" id="to_loc_id" name="to_loc_id" type="hidden"
									value="<?= @$onw_rndw_segment_search_params['to_loc_id'] ?>">
							</div>
							<div class="alert-box" id="flight-alert-box-to"></div>
						</div>
					</div>
					<div class="col-md-5 nopad secndates">
						<div class="col-md-6 col-xs-12 padfive" id="departure__container">
							<div class="formlabel">Departure</div>
							<div class="plcetogo datemark sidebord datepicker_new1">
								<input type="text" readonly
									class="normalinput auto-focus hand-cursor form-control b-r-0"
									id="flight_datepicker1" placeholder="Select Date"
									value="<?php echo @$onw_rndw_segment_search_params['depature'] ?>" name="depature"
									required />
							</div>
							<div class="alert-box" id="flight-alert-departure"></div>
						</div>
						<div class="col-md-6 col-xs-12 padfive date-wrapper" id="return__container">
							<div class="formlabel">Return</div>
							<div class="plcetogo datemark sidebord datepicker_new2">
								<input type="text" readonly
									class="normalinput auto-focus hand-cursor form-control b-r-0"
									id="flight_datepicker2" name="return" placeholder="Select Date"
									value="<?php @$onw_rndw_segment_search_params['return'] ?>" <?php echo (@$onw_rndw_segment_search_params['trip_type'] != 'circle' ? 'disabled="disabled"' : '') ?> />
							</div>
				...
				<div class="col-md-4 col-xs-12 nopad thrdtraveller" id="travellers__container">
					<div class="col-xs-7 padfive mobile_width" id="travellers__container__input">
						<div class="formlabel">Traveller</div>
						<div class="totlall widtl">
							<span class="remngwd"><span class="total_pax_count"></span> <span
									id="travel_textflight">Traveller</span></span>
							<div class="roomcount pax_count_div">
								...
								<div class="advance_opt">
									<div class="col-xs-12 nopad">
										...
										<div class="alladvnce">
											<span class="remngwd" id="choosen_airline_class">
												<?php echo $choosen_airline_class; ?>
											</span>
											<input type="hidden" autocomplete="off" name="v_class" id="class"
												value="<?php echo $irline_class_value; ?>">
										...
										<div class="alladvnce2">
											...
											<select class="js-example-basic-single" name="carrier[]">
												<?php if (isset($choosen_airline_name)) { ?>
													<option value="<?php echo $choosen_airline_name1; ?>">
														<?php echo $choosen_airline_name; ?>
													</option>
												...
												foreach ($airline_list as $airline_list_k => $airline_list_v) {
													...
													if ($airline_list_k != $flight_search_params['carrier'][0]) {
														...
														<option value="<?php echo $airline_list_k; ?>">
								...
								<div class="inallsn inslnformob">
									<div class="oneroom fltravlr">
												...
												<div class="input-group countmore pax-count-wrapper adult_count_div">
													<span class="input-group-btn">
														<button type="button" class="btn btn-default btn-number"
															data-type="minus" data-field="adult"> <span
																class="glyphicon glyphicon-minus"></span> </button>
													</span>
													<input type="text" id="OWT_adult" name="adult"
														class="form-control input-number centertext valid_class pax_count_value"
														value="<?= (int) @$flight_search_params['adult_config'] ?>" min="1"
														max="9" readonly>
													<span class="input-group-btn">
														<button type="button" class="btn btn-default btn-number"
											...
											<div class="celroe col-xs-5">
												<div class="input-group countmore pax-count-wrapper child_count_div">
													...
													<input type="text" id="OWT_child" name="child"
														...
														value="<?= (int) @$flight_search_params['child_config'] ?>" min="0"
														max="9" readonly>
													<span class="input-group-btn">
														<button type="button" class="btn btn-default btn-number"
										...
										<div class="roomrow last">
											...
											<div class="celroe col-xs-5">
												<div class="input-group countmore pax-count-wrapper infant_count_div">
													<span class="input-group-btn">
														<button type="button" class="btn btn-default btn-number"
															data-type="minus" data-field="infant"> <span
																class="glyphicon glyphicon-minus"></span> </button>
													</span>
													<input type="text" id="OWT_infant" name="infant"
														class="form-control input-number centertext pax_count_value"
														value="<?= (int) @$flight_search_params['infant_config'] ?>"
										...
										<a class="done1 comnbtn_room1"><!-- <span class="fa fa-check"></span> -->
											Apply</a>
										<!-- Infant Error Message-->
									</div>
								</div>
					...
					<div class="col-xs-5 padfive mobile_width">
						...
						<div class="form__button__container__large">
							<input type="submit" name="search_flight" id="flight-form-submit"
								class="form__button__submit__large" value="Search" />
						</div>
					</div>
					<div class="clearfix"></div>
					<div class="col-xs-6 padfive">
						<button style="display:none" class="add_city_btn" id="add_city"> <span
								class="fa fa-plus"></span> Add City</button>
					</div>
				</div>
		...
		<div class="nationality__label">Nationality</div>
		<div class="col-md-3 padfive mrtp-10" id="nationality__container">
			<div class="formlabel">Nationality</div>
			<div class="plcemarkk">
				<select oninvalid="this.setCustomValidity('Please select Nationality')" oninput="setCustomValidity('')"
					name="country" class="form-control normalinput auto-focus hand-cursor form-control b-r-0 " required>
					...
					foreach ($country_list as $cl_k => $cl_v) {
						if (isset($flight_search_params['country'])) {
							if ($cl_v['nationality'] != "") {
								?>
								<option <?php if ($flight_search_params['country'] == $cl_v['name']) {
				...
				<div class="form__button__container__small">
					<input type="submit" name="search_flight-small" id="flight-small"
						class="form__button__submit__small" value="Search" />
				</div>
			</div>
		</div>
		<!--<div class="form__button__container__small">-->
		<!--						<input type="submit" name="search_flight" id="flight-form-submit" class="form__button__submit__small" value="Search" />-->
		<!--					</div>-->
	</div>
</form>

<style>
	.form__button__container__large {
		padding: 0.8rem;
		margin-left: 0.2rem;
		font-size: 20px;
		border-radius: 5px;
	...
	.form__button__container__large .form__button__submit__large {
	...
	.form__button__container__small {
		padding: 0.8rem;
		margin-left: 0.2rem;
		margin-top: 2rem;
		font-size: 20px;
	...
	.form__button__container__small .form__button__submit__small {
	...
	@media(max-width:480px) {
		...
		.form__button__container__small {
			z-index: 100000011213213213 !important;
		}

		.form__button__submit__small {
			z-index: 10000001121321321000 !important;
	...
	@media only screen and (min-width: 480px) and (max-width: 768px) {
		...
		#travellers__container {
			min-width: 100%;
		}

		#travellers__container__input {
			min-width: 100%;
		}
	...
	@media only screen and (min-width:1022px) and (max-width:1600px) {
...
<script>
	$(document).ready(function () {
		...
		$("#travel_text").html("Travellers");
		var select_val = '<?php if (isset($flight_search_params['carrier'][0])) {
			echo $flight_search_params['carrier'][0];
		} ?>';
		//alert(select_val);
