					...
					<div class="trvlwrap">
						...
						if(valid_array($table_data['booking_details']) == true) {
							...
							foreach($booking_details as $parent_k => $parent_v) { 
								...
								extract($parent_v);
					//	debug($parent_v);die;
	//debug($parent_v['convinence_amount']);
						//	die;
						//		debug($parent_v);die;
							?>
        ...
        $convenience_fees = round($convenience_fees);
									...
									if(count($trans_attributes['conv_price_details'])===0)
									{
									//	debug(
//$trans_attributes['Fare']);die;
										  $BaseFare = $trans_attributes['Fare']['BaseFare'];
                                       $Tax = $trans_attributes['Fare']['Tax'];
                                       $grand_total = $trans_attributes['Fare']['PublishedFare'] + $markup_value +                                                      $convenience_fees;
									}
									else
									{
									...
									//die;
									$conv=$parent_v['convinence_amount']*$currency_conversion_rate;
									$adminmarkup=$parent_v['admin_markup']*$currency_conversion_rate;
											;
									$segment_discount=$segment_discount*$currency_conversion_rate;
								?>

												<?php /*echo round($BaseFare+$Tax+$conv+$adminmarkup-$discount-($segment_discount+$reward_amount));*/
												// 	changes added new total amount calculation to display in user porfile page
										echo number_format((@$booking_details[$parent_k]['grand_total'] - $booking_details[$parent_k]['segment_discount'] + $booking_details[$parent_k]['reward_amount']), 2);?>	
													<?php
									}
								else
								{
									...
									if($booking_source=="PTBSID0000000009")
								    {
										$finaltotal=$grand_total+$adminmarkup-($segment_discount+$reward_amount);
									}
									else
									{
										$trans_attributes = json_decode($booking_transaction_details[0]['attributes'], true);
									...
									//	debug($booking_details);die;
										$finaltotal=$BaseFare+$Tax+($parent_v['convinence_amount']*$currency_conversion_rate)-$parent_v['discount']-$parent_v['reward_amount'];
										if($booking_source=="PTBSID0000000021")
								    {
										//	debug($adminmarkup);
										//	debug($parent_v['admin_markup']);
																$finaltotal=round($BaseFare+$Tax+($parent_v['convinence_amount']*$currency_conversion_rate)-$parent_v['discount']-$parent_v['reward_amount']+$parent_v['admin_markup']);
										}
									}
								?>
													<?=$currency?>
