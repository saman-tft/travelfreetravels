...
// debug($raw_hotel_list['HotelSearchResult']['HotelResults']);
foreach ($raw_hotel_list['HotelSearchResult']['HotelResults'] as $hd_key=> $hd) {
	...
	//$file_headers = @get_headers($hd['HotelImage']);
	$image_found=1;
...
<!-- <img src="" alt="Hotel img" data-src="<?php //echo base_url().'index.php/hotel/image_cdn/'.$hd['ResultIndex'].'/'.$search_id.'/'.base64_encode($hd['HotelCode'])?>" class="lazy h-img"> -->
					<?php 
							...
							if(strpos($hd['HotelPicture'], 'HotelNA.jpg') !== false){
								if(ENVIRONMENT == 'testing')
								{

									$img_url=$hd['HotelPicture'];
								}
								else
								{

									$img_url=str_replace('https','http', $hd['HotelPicture']);
								}
							}else
							{
								$img_url=$hd['HotelPicture'];
					...
					?>
					<img src="<?=$img_url?>" alt="Hotel Image" data-src="<?=$img_url?>" class="lazy h-img load-image hide">
				...
				<?php else:?>
					...
					?>
					<img src="<?=$img_url?>" alt="Hotel Image" data-src="<?=$GLOBALS['CI']->template->template_images('default_hotel_img.jpg') ?>" class="lazy h-img">
				<?php endif;?>
				<?php
					/**
				...
				<div class="celhtl width70">
					<div class="innd">
					   <div class="imptpldz">
						...
						<div class="adreshotle h-adr" data-toggle="tooltip" data-placement="bottom" data-original-title="<?php echo $hd['HotelAddress']?>"><?php echo $hd['HotelAddress']?>
						</div>
						...
						<div class="maprew">
						  ...
						  <a href="<?php echo base_url().'index.php/hotel/map?lat='.$hd['Latitude'].'&lon='.$hd['Longitude'].'&hn='.urlencode($hd['HotelName']).'&sr='.intval($hd['StarRating']).'&c='.urlencode($hd['HotelLocation']).'&price='.$RoomPrice.'&img='.urlencode($hd['HotelImage'])?>" class="location-map  fa fa-map-marker" target="map_box_frame" data-key="<?=$hd_key?>" data-hotel-code="<?=$hotel_code?>" data-star-rating="<?=$hd['StarRating']?>" data-hotel-name="<?=$hd['HotelName']?>" id="location_<?=$hotel_code?>_<?=$hd_key?>" data-toggle="tooltip" data-placement="bottom" data-original-title="View Map"></a>
						   </div>
				...
				<div class="celhtl width30">
					<div class="sidepricewrp">
						...
						<div class="priceflights">
							<strong> <?php echo $currency_obj->to_currency; ?> </strong>
							<!-- <strong> <?php echo $currency_obj->get_currency_symbol($currency_obj->to_currency); ?> </strong> -->
							<span class="h-p"><?php echo roundoff_number(round($RoomPrice)); ?></span>
...
if($booking_source=='PTBSID0000000011')
{
...
else
{
	...
	$css="tmx_htl";
						...
						<form action="<?php echo base_url().'index.php/'.$urils.'/hotel_details/'.($search_id)?>">
							...
							<input type="hidden" value="<?=$hotel_code?>" name="hotel_code" class="operation">
		...
		<div class="col-xs-12 nopad listfull full_mobile indictnblck <?php echo $css; ?>">
			<h4>Book a Hotel now with Travel Free Travels</h4>
		</div>
...
<strong class="currency_symbol hide" > <?php echo $currency_obj->to_currency; ?> </strong>
