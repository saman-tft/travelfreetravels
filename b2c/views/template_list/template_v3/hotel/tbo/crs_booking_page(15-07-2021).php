...
$currency_symbol =$pre_booking_params['default_currency'];
// $currency_symbol = ($pre_booking_params['default_currency']);
...
// if ($_SERVER['REMOTE_ADDR'] == '42.109.144.160') {  
// 	debug($hotel_total_price);
//     debug($pre_booking_params['markup_price_summary']);exit();
// }
//debug($hotel_total_price);exit();
/********************************* Convenience Fees *********************************/

$subtotal = $hotel_total_price;
$pre_booking_params['hotel_total_price'] = $hotel_total_price;
...
// }
 //debug($pre_booking_params);exit();
  $gst_details = $this->custom_db->single_table_records('gst_master', '*', array('module' => 'hotel'));
 //debug($gst_details);exit();
 $gst_details=$gst_details['data'][0]['gst'];
 $gst_value=($pre_booking_params['convenience_fees']*$gst_details)/100;
...
$pre_booking_params['GST']=$gst_value;
$pre_booking_params['hotel_total_price']=$total_room_price;
?>
		   ...
		   <div id="slidebarscr" style="width:370px;">
		 	<table class="table table-condensed tblemd">
			 	<tbody>
			      ...
			      <tr>
			        <th>Total Price</th>
			        <td><span class="pricesyn"><?=($pre_booking_params['default_currency'])?></span> <?=($total_room_price)?></td>
			      </tr>
			      <tr class="texdiv">
			        <th>Convenience Fees</th>
			        <td><span class="pricesyn"><?=($pre_booking_params['default_currency'])?> </span><?=round($tax_total)?></td>
			      </tr>
			      
			      <?php if($GST > 0){// echo "string"; ?>
			      	
			      <tr class="">
			        <th>GST</th>
			        <td><span class=""><?=($pre_booking_params['default_currency'])?></span> <?=$GST?></td>
			      </tr>
			      <?php } ?>
				  <!-- <tr class="textdiv">
					<th>Convenience Fees</th>
			      ...
			      <tr class="grd_tol">
			        <th>Grand Total</th>
			        <td><span class="pricesyn"><?=($pre_booking_params['default_currency'])?></span> <?=($grand_total)?></td>
			      </tr>
			    </tbody>
			  </table>
				...
				<div class="col-md-8 padall10 xcel">
				  ...
				  <div class="bokkpricesml">
					<div class="travlrs"><span class="travlrsnms">Travelers:</span><span class="fa fa-male"></span> <?=array_sum($search_data['adult_config'])?><span class="fa fa-child"></span> <?=array_sum($search_data['child_config'])?></div>
					 <span id="total_booking_amount" class="hide"><?php echo round($grand_total); ?></span>
					<div class="totlbkamnt grandtotal"> <span class="ttlamtdvot">Total Amount</span><?=($pre_booking_params['default_currency'])?> <?=round($grand_total)?></div>
					</div>
					<div class="clearfix"></div>
						  <div class="sckint">
			  <div class="ffty">
...
$lead_pax_details = @$pax_details[0];
 ?>
		<form action="<?=base_url().'index.php/hotels/pre_booking/'.$search_data['search_id']?>" method="POST" autocomplete="off">
		<div class="hide">
		...
		<div class="col-xs-2 nopadding">
		 ...
		 <div class="inptalbox">
			...
			<div class="col-xs-4 spllty">
			 ...
			 <?php if(is_adult($pax_index, $total_adult_count)=='Adult' && $search_data['is_domestic']=='') { ?>
			 <div class="col-xs-4 spllty">
			 	<input required="required" type="text" name="pan_number[]" id="pan_number-<?=$pax_index?>" class="clainput "  maxlength="45" placeholder="Enter PAN Number" onkeyup="alphanumeric(this.value)"/>
			 </div>
			<?php } ?>
		</div>
		</div>
									                      ...
									                      <div class="cartprc">
									                        <div class="payblnhm singecartpricebuk ritaln">
									                    	 <input type="text" placeholder="Enter Promo" name="code" id="code" class="promocode" aria-required="true" />
									                          ...
									                          <input type="hidden" name="total_amount_val" id="total_amount_val" class="promocode" value="<?=@$subtotal+$GST;?>" />
...
<div class="row">
	<input type="hidden" name="selected_pm" value="PAYPAL" checked/> 
<!-- <div class="contbk">
			   ...
			   <div id="nxtbarslider" style="width:370px;">
			 	<table class="table table-condensed tblemd">
			      ...
			      <?php } ?>
					...
					<tr class="grd_tol">
				        <th>Grand Total</th>
				        <td class="grandtotal"><?=($pre_booking_params['default_currency'])?> <span class="grand_total_amount"><?=round($grand_total)?></span></td>
				      </tr>
				      				    </tbody>
				  </table>
			   </div>
