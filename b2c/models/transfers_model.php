 ...
 function bookingcrs($condition=array(), $count=false, $offset=0, $limit=100000000000)
    {
        ...
        if ($count) {
            ...
            $query = 'select count(distinct(BD.app_reference)) as total_records 
                    ...
                    where U.user_type='.B2C_USER.' and  BD.domain_origin='.get_domain_auth_id().' and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.' '.$condition;
            // echo $query;exit;
            ...
            $bd_query = 'select *,BD.created_datetime as created_datetime,U.first_name,U.last_name,U.user_type,U.agent_staff,U.agent_staff_id,U.agency_name from transfer_booking_details AS BD 
           				 left join user U on U.user_id = BD.created_by_id
							left join user_type UT on UT.origin = U.user_type
                        WHERE U.user_type='.B2C_USER.' and BD.domain_origin='.get_domain_auth_id().' and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.' and BD.module_type = "transfers" '.$condition.'
...
//transfer_vehicle_info
	...
	public function add_transfer_driver($data) {
	  ...
	  public function transfer_driver_list() {
		$q = $this->db->query ( "select *,country.name as country_name from transfer_driver_info INNER JOIN country ON country.country_id=transfer_driver_info.country where transfer_driver_info.created_by=".$GLOBALS['CI']->entity_user_id." order by id DESC" );
		if ($q->num_rows () > 0) {
			return $q->result ();
		}
		return array ();
...
// debug($response);exit;
	...
	{
		$search_chars = $this->db->escape('%'.$search_chars.'%');
		$query = 'Select hotel_name, hotel_city, hotel_code, origin,country_code from hb_hotel_details where hotel_city like '.$search_chars.'
