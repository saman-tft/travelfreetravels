...
class Activity_Model extends CI_Model {
	...
	function b2c_holidaycrs_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) AS total_records from activity_booking_details BD
					...
					where U.user_type='.B2C_USER.' and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.' AND BD.domain_origin='.get_domain_auth_id().''.$condition;
			 //debug($query);exit;
	...
	function save_activity_booking_details($booking_params=array()){
		...
		if($booking_params['booked_by_id']=="")
		{
			$booking_params['booked_by_id']=0;
		}
...
function emulate_b2c_holiday_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	...
	public function admin_markup($min_price, $supplier, $cntry_code, $city_id='', $product_id=''){
				...
				ML.module_type = "b2c_sightseeing" and ML.markup_level = "level_2"'; 
				// debug($query);  exit;
