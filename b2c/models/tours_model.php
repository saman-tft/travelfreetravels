...
class tours_model extends CI_Model {
...

	{
		...
		$query = "select * from tours_itinerary_dw where tour_id='$tour_id' order by id asc"; 
...

	{
		...
		$query = "select * from tours_itinerary where tour_id='$tour_id' order by dep_date"; 
...

	{
		...
		$query = "select * from tours where status=1 and perfect_holidays=1 order by rand() limit 5"; 
...

	{
		...
		$query = 'select * from tour_destinations order by destination'; 
...

	{
		...
		$query = 'select id,destination from tour_destinations order by destination'; 
	...
	public function tour_dep_dates($tour_id) {
		...
		$query = "select * from tour_dep_dates where tour_id='$tour_id' order by dep_date asc"; 
	...
	public function tour_visited_cities_details($id) {
		...
		$query = "select * from tour_visited_cities where id='$id'"; 
...

	{
		...
		$query = "select * from tour_visited_cities where tour_id='$tour_id' order by id asc"; 
...

	{
		...
		$query = "select * from tour_visited_cities order by id asc"; 
...

	{
		...
		$query = "select * from tour_dep_dates order by dep_date asc"; 
...

	{
		...
		$query = "select * from tours_itinerary where publish_status=1 order by dep_date asc"; 
...

	{
		...
		$query = "select * from tour_dep_dates where tour_id='$tour_id' and dep_date='$dep_date'"; 
...

	{
		...
		$query = "select * from tours_itinerary order by tour_id asc"; 
...

	{
		...
		$query = "select * from tours_enquiry order by id desc"; 
...

	{
		...
		$query = "select * from tours_itinerary order by id asc";
...

	{
		...
		$query = 'select * from tour_inclusions order by id desc'; 
...

	{
		...
		$query = 'select * from '.$table.$order_by.$order; 
...

	{
		...
		$query = 'select * from tour_activity order by tour_activity'; 
...

	{
		...
		$query = "select * from tours_city where country_id='$tours_country' order by CityName"; 
...

	{
		...
		$query = "select * from tours_country where continent='".$fetch['tours_continent']."' order by name"; 
...

	{
		...
		$query = "select * from tours_city where country_id='".$fetch['tours_country']."' order by CityName	";
...

	{
		...
		$query = 'select * from top_package_desination_title '; 
...
function guest_booking($condition = array(), $count = false, $offset = 0, $limit = 100000000000) {
...
//debug($response);die;
	...
	function clean_search_data($temp_search_data) {
		...
		if($temp_search_data ['package_type'] !="")

		{

			$clean_search ['package_type']=$temp_search_data ['package_type'];
...

		{
			...
			$clean_search ['package_type']="";

		}

		if($temp_search_data ['duration'] !="")

		{

			$clean_search ['duration']=$temp_search_data ['duration'];
...
/**
	...
	$data ['nationality'] = trim ( $search_data ['nationality'] );
	...
	//to find the tour price

	public function find_tourprice_by_categories($data){


		$date=date_create($data['date_of_travel']);

        $data['date_of_travel'] =  date_format($date,"Y-m-d");

		$condition = array(

					'from_date <='=>$data['date_of_travel'],

					'to_date >='=>$data['date_of_travel'],
	...
	public function tour_qoutation_price($table, $where){
   ...
   public function activity_qoutation_price($table, $where){
   ...
   public function tour_top_destinations()
   ...
   {  
   		...
   		$query = 'SELECT  *, t1.id as id, t1.inclusions_checks as inclusions_checks FROM tours AS t1 
				...
				GROUP BY tc2.CountryName order by t1.id DESC limit 12';   
   ...
   function activity_top_destinations()

	{



		$query = 'SELECT * FROM activity_top_destination WHERE STATUS='.ACTIVE.' ORDER BY order_by ASC LIMIT 12';
