...
Class Flight_Model extends Transaction
{
	 ...
	 *
	 */
	public function select_recent_search_history($id){
			$query = "Select * from search_history where origin IN ($id) order by origin DESC";
	...
	function booking_guest_user($app_reference, $booking_source='', $booking_status='')
	{
		...
		$app_reference=$booking_details1[0]['app_reference'];
	 ...
	 */
	function is_domestic_flighttmx($from_loc, $to_loc,$country='',$triptype='')
	{
					...
					$query1 = 'SELECT * FROM flight_airport_list WHERE airport_code IN ('.$this->db->escape($from_loc).')';
						$query2 = 'SELECT *  FROM flight_airport_list WHERE airport_code IN ('.$this->db->escape($to_loc).')';
								$data1 = $this->db->query($query1)->row_array();
									...
									if($data1['CountryCode']=="IN" && $data2['CountryCode']=="NP" && $country=="Indian"  && $triptype!='Round')
									{
                                                  $data['total']=0;
									}
									if($data1['CountryCode']=="IN" && $data2['CountryCode']=="NP" && $country=="Nepalese"  && $triptype!='Round')
									{
                                                  $data['total']=0;
									}
									if($data2['CountryCode']=="IN" && $data1['CountryCode']=="NP" && $country=="Indian"  && $triptype!='Round')
									{
                                                  $data['total']=0;
									}
									if($data2['CountryCode']=="IN" && $data1['CountryCode']=="NP" && $country=="Nepalese"  && $triptype!='Round')
									{
                                                  $data['total']=0;
									}
								//	debug($data1);die;
	...
	function is_domestic_flight($from_loc, $to_loc,$country='',$triptype='')
	{
		//debug($triptype);die;
		if(valid_array($from_loc) == true || valid_array($to_loc)) {//Multicity
			$airport_cities = array_merge($from_loc, $to_loc);
			...
			$query = 'SELECT count(*) total FROM flight_airport_list WHERE airport_code IN ('.$this->db->escape($from_loc).','.$this->db->escape($to_loc).') AND country != "Nepal"';
				$query1 = 'SELECT * FROM flight_airport_list WHERE airport_code IN ('.$this->db->escape($from_loc).')';
						$query2 = 'SELECT *  FROM flight_airport_list WHERE airport_code IN ('.$this->db->escape($to_loc).')';
								...
								//code for making nep-ind sector domestic
									// if($data1['CountryCode']=="IN" && $data2['CountryCode']=="NP" && $country=="Indian"  && $triptype!='Round')
									// {
                                    //               $data['total']=0;
									// }
 ...
 //  debug($data);die;
	...
	//	debug($temp_search_data);die;
		...
		if(isset($temp_search_data['trip_type']) == true){
			...
			$clean_search['is_domestic'] = $this->is_domestic_flight($clean_search['from_loc'], $clean_search['to_loc'],$temp_search_data['country'],$clean_search['trip_type_label']);
			 if($_SERVER['REMOTE_ADDR']=="106.203.17.103")
                {
                  //  debug($this->is_domestic_flight($clean_search['from_loc'], $clean_search['to_loc']));die;
                }
			} else {
				//multicity
...
function clean_search_datatmx($temp_search_data)
	{
		...
		if(isset($temp_search_data['trip_type']) == true){
			...
			$clean_search['is_domestic'] = $this->is_domestic_flighttmx($clean_search['from_loc'], $clean_search['to_loc'],$temp_search_data['country'],$clean_search['trip_type_label']);
			} else {
				//multicity
				$clean_search['trip_type_label'] = 'Multi City';
				for($i=0; $i<count($temp_search_data['depature']); $i++){
	...
	function update_transaction_gst($book_detail_table,$transaction_id,$gst=0)
	{
		$data = array();
		...
		$data['gst'] = $gst;
		$update_condtion['origin'] = $transaction_id;
		$this->custom_db->update_record($book_detail_table, $data, $update_condtion);
	...
	{
		$query = 'Select * from top_flight_destinations where status = '.ACTIVE;
		$data = $this->db->query($query)->result_array();
		return $data;
	}
	function flight_top_destinations_home()
	{
		$query = 'Select * from top_flight_destinations where home_status = '.ACTIVE;
		$data = $this->db->query($query)->result_array();
		return $data;
	}
		function transfer_top_destinations_home()
	{
		$query = 'Select * from top_transfercms_destinations where status = '.ACTIVE;
		$data = $this->db->query($query)->result_array();
		return $data;
	}
	/**
	 *  for flight perfect package
	 * */
	 
	function flight_perfect_packages()
	{
		$query = 'Select * from top_flight_destinations where perfect_package = '.ACTIVE;
		$data = $this->db->query($query)->result_array();
		return $data;
	}
	function flight_perfect_packages_home()
	{
		$query = 'Select * from top_flight_destinations where perfect_package = '.ACTIVE;
		$data = $this->db->query($query)->result_array();
		return $data;
