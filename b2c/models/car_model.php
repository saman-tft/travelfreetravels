...

{
	...
	public function get_car_booking($id)
	{
	...
	public function get_car_booking_update($id)
	{
	    $data=array(
	        'Status'=>0,
	        );
	    $this->db->where('car_id', $id);
		$this->db->update('crs_car',$data);
		return true;
	}
	public function get_car_response($request=array())
	{
...
//error_reporting(E_ALL);
	    ...
	    $query="select * from crs_car where Status='1' && pickuplocation LIKE '%$get->pickup_location%'";
	    $result=$this->db->query($query)->result_array();

	    for($i=0;$i<count($result);$i++)
	    {
	        $cat=$result[$i]['VehicleCategoryName'];
	        $catquery="select * from crs_car_category where category_name='$cat'";
	        //echo $result[$i]['VehicleCategoryName'];die;
	         $catresult=$this->db->query($catquery)->result_array();
	          //debug($get);
	  //   debug($get);die;
	         if($get->driver_age>=$catresult[0]['minimium_age'] && $get->driver_age<=$catresult[0]['maximium_age'])
	         {
    	    $result[$i]['PickUpDateTime']=$get->pickup_datetime; 
    	    $result[$i]['ReturnDateTime']=$get->return_datetime;
    	    if($result[$i]['Status'])
    	    {
    	    ...
    	    $result[$i]['PictureURL']='https://www.alkhaleejtours.com/dev/extras/custom/TMX9604421616070986/uploads/packages/'.$result[$i]['PictureURL'];
    	    $result[$i]['RateRestrictions']=json_decode($result[$i]['RateRestrictions'],true); 
    	    $result[$i]['PaymentRules']=json_decode($result[$i]['PaymentRules'],true);
    	    $result[$i]['CancellationPolicy']=json_decode($result[$i]['CancellationPolicy'],true);
    	    $result[$i]['TPA_Extensions']=json_decode($result[$i]['TPA_Extensions'],true);
    	    $result[$i]['TotalCharge']=json_decode($result[$i]['TotalCharge'],true);
    	    $result[$i]['PricedCoverage']=json_decode($result[$i]['PricedCoverage'],true);
    	    $result[$i]['OperationSchedules']=json_decode($result[$i]['OperationSchedules'],true);
    	    $result[$i]['PricedEquip']=json_decode($result[$i]['PricedEquip'],true);
    	    $result[$i]['AdditionalInfo']=json_decode($result[$i]['AdditionalInfo'],true);
    	    $result[$i]['dropuplocation']=json_decode($result[$i]['dropuplocation'],true);
    	    $result[$i]['LocationDetails']=json_decode($result[$i]['LocationDetails'],true);
    	   array_push($formatsearch,$result[$i]);
	         }
	    }
	    $formattedsearch['CarSearchResult']['CarResults']=$formatsearch;
	    $response['Status']=true;
	    $response['Message']="data fetched successfully";
	    $response['Search']=$formattedsearch;
	//  debug($response);die;
	    return $response;
	    
	}
    public function get_car_raterule_response($request=array(),$safe_search_data=array())
	{
	    $get=json_decode($request);
	 //   debug($safe_search_data);die;
	    ...
	    $formatsearch=array();
	    $formattedsearch=array();
	    $query="select * from crs_car where car_id='$get->ResultToken'";
	    $result=$this->db->query($query)->result_array();
	 //   debug($result);die;
	    for($i=0;$i<count($result);$i++)
	    {
    	    $result[$i]['PickUpDateTime']=$safe_search_data['data']['depature'].'T'.$safe_search_data['data']['depature_time']; 
    	    $result[$i]['ReturnDateTime']=$safe_search_data['data']['return'].'T'.$safe_search_data['data']['return_time'];
    	    if($result[$i]['Status'])
    	    {
    	        $result[$i]['Status']='Available';
    	    }
    	    ...
    	    $result[$i]['AdditionalInfo']=json_decode($result[$i]['AdditionalInfo'],true);
    	    $result[$i]['LocationDetails']=json_decode($result[$i]['LocationDetails'],true);
    	    $result[$i]['AdditionalInfo']['OpeningHours']=json_decode($result[$i]['AdditionalInfo']['OpeningHours'],true);
    	    $result[$i]['AdditionalInfo']['OperationSchedules']=json_decode($result[$i]['AdditionalInfo']['OperationSchedules'],true);
    	    $result[$i]['LocationDetails']['PickUpLocation']['AdditionalInfo']=$result[$i]['AdditionalInfo'];
    	    $result[$i]['LocationDetails']['PickUpLocation']['Address']=$result[$i]['LocationDetails']['PickUpLocation'][0];
    	       $result[$i]['LocationDetails']['DropLocation']['Address']=$result[$i]['LocationDetails']['DropLocation'][0];
     	    $result[$i]['LocationDetails']['DropLocation']['AdditionalInfo']=$result[$i]['AdditionalInfo'];
    	    $result[$i]['PricedEquip']=json_decode($result[$i]['PricedEquip'],true);
    	     array_pop($result[$i]['LocationDetails']['PickUpLocation'][0]);
    	    for($j=0;$j<count($result[$i]['PricedEquip']);$j++)
    	    {
    	        $result[$i]['PricedEquip'][$j]['policy_description']['InclusionsList']=json_decode($result[$i]['PricedEquip'][$j]['policy_description']['InclusionsList'],true);
    	    }
    	  //  $result[$i]['AdditionalInfo']=json_decode($result[$i]['AdditionalInfo'],true);
    	    $result[$i]['dropuplocation']=json_decode($result[$i]['dropuplocation'],true);
	    ...
	    $formattedsearch['CarRuleResult']=$formatsearch;
	    $response['Status']=true;
	    $response['Message']="data fetched successfully";
	    $response['RateRule']=$formattedsearch;
//	 debug($response);die;
	    return $response;
    ...
    function save_booking_details ( $domain_origin, $status, $app_reference, $booking_source, $currency, $phone_number, 
        ...
        $car_to_date, $pickup_time, $drop_time, $car_pickup_lcation, $car_drop_location, $car_drop_address, $car_pickup_address, $final_cancel_date, $transfer_type,$oneway_fee, $pay_on_pickup){
	...
	function privatebooking($condition=array(), $count=false, $offset=0, $limit=100000000000)
	...
	$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) as total_records
					...
					where  BD.booking_source="'.PROVAB_CAR_CRS_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.''.$condition;
	...
	$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) as total_records
					...
					where  BD.booking_source="'.PROVAB_CAR_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.''.$condition;
