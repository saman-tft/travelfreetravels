...
WHERE find_in_set('".$search_data['data']['nationality']."',all_nationality_country.include_countryCodes) and module='hotel'";
   $query = $this->db->query($sql);
    $natprice = $query->result();
		$q3=$this->db->get_where('crs_room_price',array('nationality'=>$natprice[0]->name));
...
WHERE find_in_set('".$search_data['data']['nationality']."',all_nationality_country.include_countryCodes) and module='hotel'";
   $query = $this->db->query($sql);
	...
	$q3=$this->db->get_where('crs_room_price',array('room_id'=>$price_id,'nationality'=>$natprice[0]->name));
	...
	public function get_room_cancellation_policy2($roomid='',$search_data=array())
	{
		...
		foreach ($q3->result() as $key => $value) 
		{
			...
			$data[]=array(
					...
					'ChargeType'=>2,
					'FromDate'=>$cancel_date,
					'FromDate'=>$cancel_date,
					'fdays'=>$value->cancel_before,
					'tdays'=>$value->cancel_to,
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $key => $value) {
               ...
               // debug($hotel_data['Price']);die;
                ...
                $hotel_data['LastCancellationDate'] = '2020-11-13';
