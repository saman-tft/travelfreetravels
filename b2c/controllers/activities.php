...
$this->load->model ( 'activity_model' );
	 // debug($app_reference);exit();
		if (empty($app_reference) == false && empty($booking_source) == false) {
			// echo "in";die;
			$page_data = array();
			if($booking_source=='PTBSID00000000012'){
// echo "in2";die;
					if($app_reference!=''){
						// echo "in3";die;
...
// debug($package_formate);exit;
	...
	//	debug(	$data ['package_cancel_policy']);die;
...
WHERE find_in_set('".$safe_search_data['data']['nationality']."',all_nationality_country.include_countryCodes) and module='tours'";
   $query =$this->db->query($sql);
    $natprice = $query->result();
	//debug($natprice);
		$data['activity_price'] = $this->custom_db->single_table_records('activity_price_management','*',array('activity_id'=>$package_id,'nationality_id'=>$natprice[0]->origin))['data'];
//debug($data['activity_price'] );
//	echo $this->db->last_query();die;
		...
		$data ['activity_id'] =$package_id;
			/*#############################FOR REWARDS SYSTEM #########################################*/
							...
							if(is_logged_in_user()){
									...
									$reward_values = $this->rewards->find_reward(META_SIGHTSEEING_COURSE,$page_data ['total_price']);
...
// debug($page_data['is_pickup_available']);die;
		...
		if($post_data['transport_type']!=$page_data["formated_data"][$key]['transfer_type'] || $post_data['seasonsDateValue']!=$post_data['seasonsDateValue_old']){
			...
			//debug($page_data["formated_data"]);die;
			for($i=0;$i<count($transfer_option);$i++){
				if(($transfer_option[$i]['transfer_option']==$post_data['transport_type']) && ($transfer_option[$i]['duration']==$post_data['seasonsDateValue'])){
					...
					$page_data["formated_data"][$key]['Price']['_AdminBuying'] =
					$transfer_option[$i]['total_price'];
					...
					$page_data["formated_data"][$key]['Price']['TotalDisplayFare'] =round($val);
				}
			}
		}
	//	debug($page_data["formated_data"][$key]['Price']['_AgentBuying']);die;
	/*#############################FOR REWARDS SYSTEM #########################################*/
//	echo "test";
//	debug($page_data["formated_data"][$key]['Price']['_AgentBuying']);
				            $page_data ['total_price'] =round($page_data["formated_data"][$key]['Price']['_AgentBuying']);
						    $page_data['reward_usable'] = 0;
							$page_data['reward_earned'] = 0;
							$page_data ['total_price_with_rewards'] = 0;
							if(is_logged_in_user()){
								...
								//	debug($usable_rewards);exit();
									 if($reward_details[0]['reward_min']<=$usable_rewards){
                                       $page_data['reward_usable'] = $reward_values['usable_reward'];
									}else{
										$page_data['reward_usable'] = 0;
									}
   ...
   public function get_countries() {
	...
	function pre_booking_itinary($id='') 
	{
		...
		$domain_balance_status = true;
			...
			//echo $book_id;die;
				...
				$convenience_fees =get_converted_currency_value ( $currency_obj->force_currency_conversion($booking_data['book_attributes']['booking_params']['formated_data'][$key]['Price']['ConvinienceFee']));
			
            $amnt_val=get_converted_currency_value ( $currency_obj->force_currency_conversion($booking_data['book_attributes']['total']))-$convenience_fees;
			//	$post_params ['payment_method']=OFFLINE_PAYMENT;
			//debug($post_params ['promo_code_discount_val']);die;
				switch ($post_params ['payment_method']) {
	                ...
	                $this->transaction->create_payment_record($book_id, $amnt_val-$reward_discount, $firstname, $email, $phone, $productinfo, $convenience_fees,$post_params['promo_code_discount_val'], $pg_currency_conversion_rate,$reward_point,$reward_amount,$reward_earned);
	...
	debug($post_params);
	 ...
	 //debug($book_params);exit();
		...
		$booking_source = PROVAB_SIGHTSEEN_SOURCE_CRS;
  ...
  //                       $booking = $this->sightseeing_lib->formate_booking_response_data($booking,$temp_booking);
	...
	$data = $this->save_booking($promocode_discount_val, $book_id, $book_params,$pack_id,$pax_count,$currency_obj, $current_module);
		...
		if($payment_status==SUCCESS_STATUS){		
			...
			redirect ( base_url () . 'index.php/voucher/activity_crs/' . $app_reference . '/' . PROVAB_SIGHTSEEN_SOURCE_CRS . '/' . $booking_status . '/show_voucher' );
			}}else{
