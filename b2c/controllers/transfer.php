...
class Transfer extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        //we need to activate transferv1 api which are active for current domain and load those libraries
    ...
    function search($search_id) {
        ...
        // Get all the transferv1s bookings source which are active
...
//             //We will load different page for different API providers... As we have dependency on API for transferv1 details page
//             // debug($params['booking_source']);exit;
//             load_sightseen_lib($params['booking_source']);
//             if ($params['booking_source'] == transferv1BED_TRANSFER_BOOKING_SOURCE &&  isset($params['op']) == true and
...
//     }
    function transfer_details_api($booking_source){
        ...
        $transferv1_data_array = array();
        $safe_search_data['data']['search_id'] = abs($search_id);
        if(isset($safe_search_data['data']['from_transfer_type']) && $safe_search_data['data']['from_transfer_type']=='ProductTransfertransferv1')
        {
            $transferv1_data_from = $this->transfer_model->get_searched_transferv1_address($safe_search_data['data']['from_code']);
            $transferv1_data_array['transferv1_data_from']= $transferv1_data_from[0];
        }

        if(isset($safe_search_data['data']['to_transfer_type']) && $safe_search_data['data']['to_transfer_type']=='ProductTransfertransferv1')
        {
            $transferv1_data_to = $this->transfer_model->get_searched_transferv1_address($safe_search_data['data']['to_code']);
            $transferv1_data_array['transferv1_data_to']= $transferv1_data_to[0];
        }

        $transferv1_data_array['search_data'] = $safe_search_data['data'];
        // debug($transferv1_data_array);exit;
        ...
        if (isset($params['booking_source']) == true) {
            ...
            if ($params['booking_source'] == transferv1BED_TRANSFER_BOOKING_SOURCE && isset($params['transfer_code']) == true) {
                ...
                if(isset($raw_transfer_details['status']) && $raw_transfer_details['status'] == true) {
                    ...
                    if(isset($transfer) && valid_array($transfer) && !isset($transfer[0]['ErrorList']) && !isset($transfer[1]['ErrorList']) && isset($transfer[0])){
                        ...
                        $page_data['transferv1_data_array']  = $transferv1_data_array;
                        ...
                        switch($params['booking_source']) {
                            case transferv1BED_TRANSFER_BOOKING_SOURCE :
                                ...
                                'trip_type' => $trip_type,'flight_time' => $flight_array,'transferv1_data_array' => $transferv1_data_array,
...
// debug($raw_product_deails);exit;
                $this->template->view('activity/transferv1bed/activity_details', array('currency_obj' => $currency_obj,'product_details' => $raw_product_deails, 'search_id'=>$search_id,'active_booking_source' => $params['booking_source'], 'params' => $params));
            } else {
    ...
    //         //debug($token);exit;
    //         if ($params['booking_source'] == transferv1BED_TRANSFER_BOOKING_SOURCE) {
           ...
           if($post_params ['payment_method'] =='')
            ...
            if ($post_params ['booking_source'] == PROVAB_TRANSFER_SOURCE_CRS) {
              ...
              //  debug($currency_obj);exit;
                ...
                $amount = get_converted_currency_value ( $currency_obj->force_currency_conversion($post_params['total_amount_val']));
                // debug($amount);exit;
        ...
        if ($domain_balance_status) {
            ...
            if ($temp_booking != false) {
				...
				//	}
                     ...
                     //   $this->accounting_api->process_service($book_id, $data,'TransactionCRUD',transferv1BED_TRANSFER_BOOKING_SOURCE);
        ...
        {   
          ...
          //  debug($admin_markup );
            ...
            {  
             ...
             //   debug($admin_markup_price);
                $price_with_agent_markup = $transfer_price+$admin_markup_price;
                        ...
                        { 
                            ...
                            if($i==$k)
                            {
                                ...
                                if($cancellation_policy[$i]['charge_type']=='%')
                                {
                                    ...
                                    $converted_currency_rate = $currency_obj->getConversionRate(false,$curncy,$agent_base_currency);
                                    // debug($payed_amount);exit;
                                //    debug($price_with_agent_markup);
                                  //  debug($cancellation_amount);exit;
                                    $cancellation_amount=$cancellation_amount*$converted_currency_rate;
                                    }
           ...
           // $currency_obj = new Currency(array('module_type' => 'sightseeing', 'from' => get_api_data_currency(), 'to' => get_application_currency_preference()));
            $currency_obj =new Currency(array('module_type' => 'transferv1', 'from' =>'NPR', 'to' => get_application_currency_preference()));
...
// debug($currency_obj);exit();
            ...
            // debug($search_id);
            // debug($filters); 
           $formated_data = $this->transfer_lib->format_search_response_detail($raw_sightseeing_result['data'], $currency_obj, $search_id, 'b2b', $filters);
...
function booking() {//error_reporting(E_ALL);
 if($this->input->post()){
    ...
    // debug($pre_booking_params['pickup_location']);exit();
                ...
                $currency_obj_m =new Currency(array('module_type' => 'transferv1', 'from' =>'NPR', 'to' => get_application_currency_preference()));
                             $strCurrency_m  = $currency_obj_m->get_currency($value->price, true, false, true, false, 1);
                $transfer_price = $strCurrency_m['default_value'];
                }
                $supplier = 'Voyage CRS';
                ...
                $admin_markup_details = round(get_converted_currency_value ( $currency_obj->force_currency_conversion ($this->transfer_model->get_markup_for_admin ( $transfer_price, $supplier, $cntry_code, $city_id, $product_id))));
            ...
            //    debug($currency_obj);die;
                ...
                //  debug($total_markup);
                   $admin_convenience_fees =($transfer_price+$total_markup)*($convenience_fees[0]['value']/100); 
                }
            ...
            //  debug($admin_convenience_fees );die;
                $total_pax = $safe_search_data['data']['adult']+$safe_search_data['data']['child'];
                ...
                $page_data['pre_booking_params']['convenience_fees']=round($total_convenience_fees);
                // $gst_amount = 0;
                $agent_buying_price = $transfer_price+$admin_markup_details+$gst_amount+$total_convenience_fees;                
                $page_data['pre_booking_params']['agent_buying_price']=round($agent_buying_price);
    ...
    $page_data['pre_booking_params']['markup_price_summary']['TotalDisplayFare'] =round($price_with_agent_markup);
    $page_data['pre_booking_params']['markup_price_summary']['NetFare'] =round($net_fare);
      ...
      //    error_reporting(E_ALL);
        ...
        foreach ($package_details as $key => $value) {
            $transfer_id = $value->id;
             ...
             $agent_base_currency = agent_base_currency();
                $currency_obj_m = new Currency(array('module_type' => 'transfers', 'from' => $value->currency, 'to' => $agent_base_currency));
                             $strCurrency_m  = $currency_obj_m->get_currency($value->price, true, false, true, false, 1);
                $transfer_price = $strCurrency_m['default_value'];
                $supplier = 'Voyage CRS';
                $product_id = '';
                    ...
                    for($i=0;$i<count($cancellation_policy);$i++)
                    {
                        if($number_of_days==$cancellation_policy[$i]['no_of_days'] || $number_of_days<$cancellation_policy[$i]['no_of_days'])
                        {
                            if($cancellation_policy[$i]['charge_type']=='%')
                            {
                                ...
                                $cancellation_details = 'Cancellation from today will charge AED'.round(($cancellation_amount),2);
                            }
                            else if($cancellation_policy[$i]['charge_type']=='Amount')
                            {
                                $cancellation_amount = $cancellation_policy[$i]['amount'];
                        ...
                        { 
                            ...
                            if($i==$k)
                            {
                                ...
                                {
                                    $cancellation_amount = ($admin_net_fare_markup*$cancellation_policy[$i]['amount'])/100;
                                    $cancellation_details = 'Cancellation from '.$startdate.' will charge AED'.round(($cancellation_amount),2);
                                }
                                else if($cancellation_policy[$i]['charge_type']=='Amount')
                                {
                                    $cancellation_amount = $cancellation_policy[$i]['amount'];
...
// debug($pax_insert_id);exit();
    ...
    function get_transfer_city_list() {
        ...
        foreach ($data_list as $city_list) {
            ...
            $data['value'] = transferv1_suggestion_value($city_list['city_name'], $city_list['country_name']);
            ...
            if (intval($city_list['cache_transferv1s_count']) > 0) {
                $data['count'] = $city_list['cache_transferv1s_count'];
            } else {
                $data['count'] = 0;
            }
            $result[] = $data;
                                    ...
                                    if($get_agent_module_status)
                                    {
                                        if($get_agent_module_status['status'])
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                $currency_obj_m = new Currency(array('module_type' =>'transferv1', 'from' => $agent_base_currency, 'to' =>'AED'));
