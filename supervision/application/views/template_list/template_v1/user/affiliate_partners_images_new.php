	...
	<div class="row">
		<div class="pull-left" style="margin:5px 0">
			<a href="<?=base_url().'index.php/user/add_affiliate_partners'?>">
				<button  class="btn btn-primary btn-sm pull-right amarg">Add Affiliate Partners</button>
			</a>
		</div>
	</div>
	<div class="panel <?=PANEL_WRAPPER?>"><!-- PANEL WRAP START -->
		<div class="panel-heading"><!-- PANEL HEAD START -->
			<div class="panel-title">
				<i class="fa fa-edit"></i> Affiliate Partners
			</div>
			...
			<table class="table table-condensed">
				...
				if (valid_array($data_list) == true) {
					foreach ($data_list as $k => $v) :
						$action = '<form method="post" action="'.base_url().'index.php/user/affiliate_partners_images">
	     <input type="submit" class="btn btn-default btn-sm btn-primary" value="Edit" />
	     <input type="hidden" value="'.$v['origin'].'"  name="id" />
		 </form><!--<a role="button" href="'.base_url().'index.php/user/edit_affiliate_partners/'.$v['origin'].'"><button class="btn btn-sm">Edit</button></a>-->';
						...
						$status_button = '<a role="button" href="'.base_url().'index.php/user/affiliate_partners_delete/'.$v['origin'].'"><button class="label label-danger">Delete</button></a>'; 
				?>
					<tr>
						<td><?=($k+1)?></td>
						<!-- <td><?=$v['subtitle']?></td> -->
