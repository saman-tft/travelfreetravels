...
<div class="bodyContent">
	<div class="row">
		<div class="pull-left" style="margin:5px 0">
			<a href="<?=base_url().'index.php/user/add_blog'?>">
				<button  class="btn btn-primary btn-sm pull-right amarg">Add Blog</button>
			</a>
		</div>
	</div>
			...
			<table class="table table-condensed">
				...
				if (valid_array($data_list) == true) {
					foreach ($data_list as $k => $v) :
						$action = '<form method="post" action="'.base_url().'index.php/user/blog_images">
	     <input type="submit" class="btn btn-default btn-sm btn-primary" value="Edit" />
	     <input type="hidden" value="'.$v['origin'].'"  name="id" />
		 </form><!--<a role="button" href="'.base_url().'index.php/user/edit_blog/'.$v['origin'].'"><button class="btn btn-sm">Edit</button></a>-->';
						...
						$status_button = '<a role="button" href="'.base_url().'index.php/user/blog_delete/'.$v['origin'].'"><button class="label label-danger">Delete</button></a>'; 
				?>
					<tr>
						<td><?=($k+1)?></td>
						<td><?=$v['title']?></td> 
						<td><?php if(strlen($v['description'])>185) { echo substr(strip_tags($v['description']),0,182) . "..."; } else { echo $v['description']; } ?></td> 
						<td><img src="<?php echo $GLOBALS ['CI']->template->domain_ban_images ($v['image']) ?>" height="200px" width="200px" class="img-thumbnail"></td>
