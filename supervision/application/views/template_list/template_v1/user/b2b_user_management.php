...
<form method="GET" autocomplete="off" id="search_filter_form">
	...
	<div class="col-sm-12 well well-sm">
		...
		<a href="<?php echo base_url(); ?>index.php/user/user_export/?op=csv&q=<?php echo B2B_USER; ?>&user_status=<?php echo @$user_status;?>" class="btn btn-success"><i class="fa fa-download" aria-hidden="true"></i> Export users csv</a>
        <a href="<?php echo base_url(); ?>index.php/user/user_export/?op=pdf&q=<?php echo B2B_USER; ?>&user_status=<?php echo @$user_status;?>" class="btn btn-success"><i class="fa fa-download" aria-hidden="true"></i> Export users pdf</a>
	</div>
</form>
          ...
          var regex = /([A-Z]){5}([0-9]){4}([A-Z]){1}$/;
			     ...
			     if (regex.test($(this).val().toUpperCase())) {
			    	 $('.pan_validation_error').addClass("hide");
			    	  $("#b2b_user_submit").removeAttr('disabled');
			    
			    } else {
			    	$('.pan_validation_error').removeClass("hide");
			    		 $("#b2b_user_submit").attr('disabled','disabled');
