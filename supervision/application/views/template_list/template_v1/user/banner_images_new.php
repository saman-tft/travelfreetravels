			...
			<table class="table table-condensed">
				...
				if (valid_array($data_list) == true) {
					foreach ($data_list as $k => $v) :
						$action = '<form method="post" action="'.base_url().'index.php/user/banner_images">
	     <input type="submit" class="btn btn-default btn-sm btn-primary" value="Edit" />
	     <input type="hidden" value="'.$v['origin'].'"  name="id" />
		 </form><!--<a role="button" href="'.base_url().'index.php/user/edit_banner/'.$v['origin'].'"><button class="btn btn-sm">Edit</button></a>-->';
