<?php 
 foreach ($room_types_list as $type){ 
	 if($type->id == $room_data->room_type_id){
		$roomname= $type->name;
	}  
} 
foreach ($board_types_list as $board){ 
	 if($board->id == $room_data->board_type){
...
<div class="bodyContent">
	<div class="panel panel-primary clearfix">
		...
		<a href="<?php echo site_url()."index.php/hotel/add_room_price/{$hotel_id}/{$room_id}"; ?>" class="btn btn-primary addnwhotl pull-right">Add Room Price</a>
		<!-- PANEL HEAD START -->
				...
				<form action="" method="POST" autocomplete="off">
					<table class="table table-bordered table-striped table-responsive" style="table-layout:fixed;">
						<tr>
							...
							<th>Converted Price<br>Adult 1 Price(NPR)</th>
							<th>Converted Price<br>Adult 2 Price(NPR)</th>
							<th>Converted Price<br>Adult 3 Price(NPR)</th>
							<th>Converted Price<br>Child Price(NPR)</th>
				...
				<?php if(!empty($price_list))
							{ 
								...
								{ 
									...
									//debug($list);exit;
									  $currency_obj = new Currency(array('module_type' => 'hotel','from' => $list->currency, 'to' => 'NPR')); 
								  ...
								  //    //debug($converted_currency_rate);exit;
							       ...
							       $converted_one_adult = $converted_currency_rate*$list->one_adult;
							       $converted_two_adult = $converted_currency_rate*$list->two_adult;
							       $converted_three_adult = $converted_currency_rate*$list->three_adult;
							       $converted_child_price = $converted_currency_rate*$list->child_price;
						...
						<tr>
							...
							<td><?php echo $list->seasons_name; ?></td>
							<td><?php echo $list->seasons_from_date; ?></td>
							<td><?php echo $list->seasons_to_date; ?></td>
							...
							<td><?php echo $list->child_price ?></td>
							<td><?php echo $list->converted_one_adult; ?></td>
							<td><?php echo $list->converted_two_adult; ?></td>
							<td><?php echo $list->converted_three_adult; ?></td>
							<td><?php echo $list->converted_child_price; ?></td>
							...
							<td class="center">
								...
								if($list->status == "ACTIVE")
									{ 
								?>
									<a href="<?php echo site_url()."index.php/hotel/inactive_room_price/".$list->hotel_id.'/'.$list->id; ?>" class="btn btn-orange btn-sm btn-icon icon-left my-inactve"><i class="entypo-eye"></i>InActive</a>
								<?php 
								}
								else
								{ ?>
									<a href="<?php echo site_url()."index.php/hotel/active_room_price/".$list->hotel_id.'/'.$list->id; ?>" class="btn btn-green btn-sm btn-icon icon-left my-actve"><i class="entypo-check"></i>Active</a>
								<?php 
								} 
								?>
							...
							<td>
							    <a href="<?php echo site_url()."index.php/hotel/edit_room_price/".$list->id; ?>" class="btn btn-primary btn-sm btn-icon icon-left my-actve"><i class="entypo-edit"></i>Edit</a>
							</td>
						</tr>
					<?php 
					}
