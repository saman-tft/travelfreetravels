...
<?php  if($menu == true){ ?>
  ...
  <!-- <div class="container">
    <div class="col-xs-11 text-center mt10">
      <ul class="list-inline btn_sec">
        ...
        <li>
        <a href="<?php echo base_url () . 'index.php/tours/voucher/'.$tour_id.'/'.$utype.'/show_pdf';?>"  ><button class="btn-sm btn-primary pdf">PDF</button></a>
       </li>
       <li>
 ...
 <div class="collapse" id="emailmodel">
  <div class="well max_wd20">
    ...
    <form name="agent_email" class="browidth" method="post" action="<?php echo base_url () . 'index.php/tours/voucher/'.$tour_id.'/'.$utype.'/show_broucher/mail';?>">
      <input id="inc_sddress" value="1" type="hidden" name="inc_sddress">
      <input id="inc_fare" value="1" type="hidden" name="inc_fare">
      <div class="row">
       <input id="email" placeholder="Please Enter Email" class="airlinecheckbox validate_user_register form-control" type="text" checked name="email">
      </div>
...
<?php } ?>
  <table cellspacing="0" cellpadding="10" border="0" style="border-collapse: collapse; background: #ffffff; font-size: 14px; line-height: 20px; width: 100%; max-width: 800px; margin: 0px auto; font-family: Aller, sans-serif; color:#405364;box-shadow:0 0 5px 0 rgba(0,0,0,.11);">
  <tbody>
  <tr>
  ...
  <table cellspacing="0" cellpadding="10" border="0" style="border-collapse: collapse; width: 100%; margin: 0px auto; font-family: Aller, sans-serif; color:#405364;">
    <tbody>
      <tr>
        <td style="padding:10px 10px 0px 0px;border-bottom:3px solid #b2b2b2;">
         <!--  <span><img style="margin-bottom:10px; height:100px;" src="<?php echo $GLOBALS['CI']->template->template_images('mascot.png'); ?>" alt="logo"></span> -->
          <span style="float: left;"><img style="margin-bottom:10px;width: 150px" src="<?php echo $GLOBALS['CI']->template->domain_images($GLOBALS['CI']->template->get_domain_logo())?>" alt="logo"></span><!--  <span style="float: right;"><img style="margin-bottom:10px; height:150px;" src="<?php echo $GLOBALS['CI']->template->domain_images('keWD7SNXhVwQmNRymfGN192.168.0.25logo_voucher.png'); ?>" alt="logo"></span> -->
     ...
     <!--  <tr style="border-bottom:3px solid #b2b2b2;">
      ...
      if($quotation_details){
       $user_attributes = json_decode($quotation_details[0]['user_attributes'],ture);
      ?>
      <tr>
        <td style="padding:0px 10px 20px 0px">
          <h3 style="margin:0 0 8px ">Quote Reference : <?=$quotation_details[0]['quote_reference']?></h3>
          <h3 style="margin:0 0 8px ">Holiday Quote (<?=$quotation_details[0]['currency_code']?>)</h3>
          <?php 
          ...
          ?>
          <h4 style="margin:0 0 2px;font-weight:bold">Total : <?=$quotation_details[0]['currency_code']?> <?=$quotation_details[0]['quoted_price']?></h4>          
        </td>
      </tr>
      <?php 
      ...
      <tr>
        <td style="padding:0px 10px 20px 0px">
          <h3 style="margin:0 0 8px ">Holiday Quote (<?=$booking_details['currency_code']?>)</h3>
          <?php 
      ...
      foreach ($tours_itinerary_dw as $key => $itinary) {
        ...
        <tr>
          <td style="padding:0px 10px 10px 0">
            <h2 style="margin:5px 0;font-size: 16.5px; line-height:20px; font-weight:600">HOLIDAY COST</h2>
            <?php 
            $arr_occ = array();
            ...
            <table class="table table-bordered sm_tab" style="margin-bottom:8px">
              <thead>
                <?php  $currencyp = ($this->session->userdata('currency') != "") ? $this->session->userdata('currency') : "INR"; ?>
                <tr>
                  <th> From Date</th>
                  <th> To Date</th>
              ...
              <tbody>
                ...
                foreach ($tour_price as  $value) {
                  ...
                  $child_pricing_x = explode(',',$value['child_pricing']);
                  ...
                  foreach ($arr_occ_x as $key => $value_x) {
                    $arr_pri[$value_x] = array(
                      ...
                      'child_pricing' => $child_pricing_x[$key],
                      'markup' => $markup[$key]
                    );
                  }
                  // debug($value);
                  ...
                  foreach ($arr_occ as $key => $occ) {
                    if(array_key_exists($occ, $arr_pri)){
                      ?>
                      <td>
                        INR <?=sprintf('%.2f',$arr_pri[$occ]['pricing'])?>
                  ...
                  foreach ($arr_occ as $key => $occ) {
                    if(array_key_exists($occ, $arr_pri)){
                      ?>
                      <td>
                        INR <?=sprintf('%.2f',$arr_pri[$occ]['child_pricing'])?>
                        <?php //echo (($arr_pri[$occ]['pricing'])+($arr_pri[$occ]['markup']));?>
                      </td>
                      <?php
                    }else{
       ...
       <?php  if($en_note){ ?>
        ...
        <tr>
          <td style="padding:0px 10px 5px 0">
            <h2 style="margin:5px 0;font-size: 16.5px; line-height:20px; font-weight:600">Holiday Inclusions</h2>
            <p style="margin:0;white-space: normal;">
              <?php 
              $tours_itinerary['inclusions'] = str_replace('\n', '', $tours_itinerary[0]['inclusions']);
        ...
        <tr>
          <td style="padding:0px 10px 5px 0">
            <h2 style="margin:5px 0;font-size: 16.5px; line-height:20px; font-weight:600">Holiday Exclusions</h2>
            <p style="margin:0;white-space: normal;">
              <?php 
              $tours_itinerary['exclusions'] = str_replace('\n', '', $tours_itinerary[0]['exclusions']);
          ...
          <tr style="border-bottom: 1px solid #b2b2b2;">
            <td style="padding:10px 20px 10px 0">
              <table>
                <tbody>
                  <tr>
                    <td colspan="3" width="100%"><span style="margin:5px 0;line-height:20px;">&copy; 2020 bookings.quaqua.com. All rights reserved.
</span></td>
                  </tr>
                </tbody>
              </table>
