...
<div id="Package" class="bodyContent col-md-12">
      ...
      <div class="panel-title">
        <ul class="nav nav-tabs nav-justified" role="tablist" id="myTab">
          ...
          <li role="presentation" class="active" id="add_package_li"><a
            ...
            data-toggle="tab">Tour List </a></li>
            <li aria-controls="home"> &nbsp;&nbsp;
              <button onclick="location.href='<?php echo base_url(); ?>index.php/tours/add_tour';" class='btn btn-primary'><a style="color:white;">Add Tour</a></button>
            </li>     
          ...
          <div role="tabpanel" class="tab-pane active" id="add_package">
           <div class="col-md-12">
            ...
            <div class='form-group'>
              <label class='control-label col-sm-3' for='validation_current'>Tour Type</label>
          ...
          <th>Home Page Publish Status</th>-->
            ...
            <tbody>
              ...
              // }
                ...
                // $home_status_list = 'Home Publish: <input type="checkbox" class="home_published_status" id="home_status'.$tour_id.'" value="1" data-tourid="'.$data['id'].'" data-depdate="'.$ddl_data.'" '.$checked.'><br>';
              $home_status_list = '<select id="'.$tour_id.'" class="home_published_status" name = "home_page">
                                    <option  value="0" '.($data['home_status']=="0"? ' selected' : '').'>Please Select</option> 
                                    <option  value="1" '.($data['home_status']=="1"? ' selected' : '').'>Flights</option>
                                    <option  value="2" '.($data['home_status']=="2"? ' selected' : '').'>Hotels</option>
                                    <option  value="3" '.($data['home_status']=="3"? ' selected' : '').'>Transfers</option>
                                    <option  value="5" '.($data['home_status']=="5"? ' selected' : '').'>Activities</option>
                                    <option  value="6" '.($data['home_status']=="6"? ' selected' : '').'>Holidays</option>
             ...
             /* $package_name='<a >'.$data['package_name'].'</a>';*/
              ...
              <!--<td>'.$tour_destinations[$data['destination']].'</td>-->
              <td>';
              // <td>'.$data['expire_date'].'</td>
                foreach ($tours_country as $key => $value) {
                  //print_r($tours_country_name[$value]) ;
               ...
               <!--<td>'.$home_status_list.'</td>
               <td>'.get_status_label1($data['home_page_status']).get_status_toggle_button1($data['home_page_status'], $data['id']).'</td>-->
 ...
 <div class="dropdown-content">
                ...
                <a class="btn btn-sm" href="'.base_url().'index.php/tours/price_management/'.$data['id'].'"><i class="far fa-money-bill"></i> Price Management
              </a>
              <a class="btn btn-sm" href="'.base_url().'index.php/tours/edit_tour_package/'.$data['id'].'"> <i class="glyphicon glyphicon-pencil" ></i> Edit
            </a> 
            <a class="btn btn-sm" href="'.base_url().'index.php/tours/tour_itinerary_p2/'.$data['id'].'"> <i class="glyphicon glyphicon-pencil" ></i> Edit Itinerary
            </a> 
            <a class="btn btn-sm" href="'.base_url().'index.php/tours/tour_dep_dates/'.$data['package_id'].'"> <i class="fa fa-calendar"aria-hidden="true"></i> Dep Dates
          </a>
          <a class="btn btn-sm" href="'.base_url().'index.php/tours/tour_visited_cities/'.$data['id'].'"> <i class="fa fa-building"aria-hidden="true"></i> Cities
        </a>           

        <a class="btn callDelete" id="'.$data['id'].'"> 
          <i class="glyphicon glyphicon-trash"></i> Delete</a>
          &nbsp; <br>
        <a  class="btn btn-sm" href="'.base_url().'index.php/tours/voucher/'.$data['id'].'"
        > <i class="fa fa-file" aria-hidden="true"></i> Brochure      
      </a> &nbsp; <br> 
...
{
  echo '<td class="center">
  <a class="btn" data-placement="top" href="'.base_url().'index.php/tours/tour_dep_dates_p2/'.$data['package_id'].'"
...
<div id="book_modal" class="modal fade" role="dialog">
      ...
      <div class="modal-body">
       ...
       <div id="user_holder">
        ...
        <div class="form-group">
         ...
         <input type="text" class="form-control user_info caps_first" required="required" name="name" id="name" placeholder="">
        </div>
        <div class="form-group">
         <label for="">Middle Name</label>
         <input type="text" class="form-control caps_first" name="mname" id="mname"  placeholder="" >
        </div>
        <div class="form-group">
         <label for="">Last Name <span style="color:red">*</span></label>
         <input type="text" class="form-control user_info caps_first" required="required" name="lname" id="lname" placeholder="" >
        </div>
        ...
        <div class="form-group">
         ...
         <select name="pn_country_code" class="form-control user_info" required="required" >
          ...
          <?php 
                      foreach($country_code as $values){
                       ?>
                      <option value="<?php echo $values['name']."".$values['country_code'] ?>"> <?php echo $values['name']." ".$values['country_code'] ?></option>
                      <?php } ?>
  ...
  <?php 
  function get_status_label1($home_page_status)
{
  if (intval($home_page_status) == ACTIVE) {
    return '<span class="label label-success"><i class="fa fa-circle-o">Active</i> '.get_enum_list('home_page_status', ACTIVE).'</span>
  <a role="button" href="" class="hide">'.get_app_message('AL0021').'</a>';
  } else {
    return '<span class="bg-red-active"><i class="fa fa-circle-o">Inactive</i> '.get_enum_list('home_page_status', INACTIVE).'</span>
    <a role="button" href="" class="hide">Deactivate</a>';
  }
}

function get_status_toggle_button1($home_page_status, $origin)
{
  if (intval($home_page_status) == ACTIVE) {
    return '<a role="button" href="'.base_url().'index.php/tours/deactivate_tours_home/'.$origin.'" class="text-danger">Deactivate</a>';
  } else {
    return '<a role="button" href="'.base_url().'index.php/tours/activate_tours_home/'.$origin.'" class="text-success">Activate</a>';
  }
}
...
<script type="text/javascript">  
  ...
  $('#price_type2').click(function(){
   ...
   $('#total').val(Math.round(price));
  });

    $('#new_price').on('keyup blur change', function(e) {

      var price_one = $(this).val();
      price_one = parseFloat(price_one);
      price_one = Math.round(price_one);
      $('#total').val(price_one.toFixed(2));
    });
    ...
    $('.book_tourid').click(function() {
      $('#tour_id').val($(this).data('tourid'));
        ...
        $(".published_status").change(function()
        {
         ...
         $(".home_published_status").change(function()
        {
         //  var id_X = $(this).attr('id');
         //  if($(this).is(":checked"))
         //  {
         ...
         $.ajax({
          url: '<?php echo base_url();?>index.php/tours/ajax_tour_home_publish/',
          method: 'post',
          data: {'tour_id':tour_id,'dep_date':dep_date,'publish_status':publish_status},
          dataType: 'json',
          success:function(data){
