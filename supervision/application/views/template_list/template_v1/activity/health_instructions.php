<style>table.dataTable thead .sorting{background-image:none!important;}</style>
...
<div id="Package" class="bodyContent col-md-12 yhgjk">
	<div class="panel panel-default">
			...
			<div class="panel-title">
				<ul class="nav nav-tabs nav-justified" role="tablist" id="myTab">
					...
					<li role="presentation" class="active" id="add_package_li"><a
						...
						data-toggle="tab">Health Restriction </a></li>
		...
		<div class="panel-body">
			<!-- PANEL BODY START -->
			<form
				action="<?php echo base_url(); ?>index.php/activity/health_instructions_save"
					...
					<div role="tabpanel" class="tab-pane active" id="add_package">
						   <div class="col-md-12 bxpd">
							...
							<div class='form-group'>
								<label class='control-label col-sm-3' for='validation_current'>Health Restriction
								</label>
								<div class='col-sm-8 controls'>
									<input type="text" name="health_instructions" id="health_instructions"
	...
	<!-- PANEL WRAP END -->
        ...
        foreach ($health_instructions as $key => $data) {
        if($data['status']==1)
        {
        ...
        echo '<tr>
				...
				<div class="dropdown2" role="group">
				   <div class="dropdown slct_tbl pull-left sideicbb">
					   <i class="fa fa-ellipsis-v"></i>  
					    <ul class="dropdown-menu sidedis" style="display: none;">
				           <li><a class="sidedis sideicbb1" data-placement="top" href="'.base_url().'index.php/activity/edit_health_instructions/'.$data['id'].'"
				data-original-title="Edit Health Restriction"> <i class="glyphicon glyphicon-pencil"></i> Edit
				</a> </li>
              ...
              <td>'.$data['health_instructions'].'</td>';
        //echo '<td>'.$galleryImages.'</td>';
        echo '<td>'.$status.'</td>';
        if($data['status']==1)
        {
        echo '<td>
              <a class="" data-placement="top" href="'.base_url().'index.php/activity/activation_health_instructions/'.$data['id'].'/0"
              data-original-title="Deactivate Health Restriction"> <i class="glyphicon glyphicon-th-large"></i></i> De-activate
              </a>
              </td>
              </tr>';
        }
        ...
        {
        echo '<td>
              <a class="" data-placement="top" href="'.base_url().'index.php/activity/activation_health_instructions/'.$data['id'].'/1"
              data-original-title="Activate Health Restriction"> <i class="glyphicon glyphicon-th-large"></i></i> Activate
              </a>
              </td>
              </tr>';
        } 
		...
		$(document).ready(function()
		{
          ...
          $(".callDelete").click(function() { 
            $id = $(this).attr('id'); //alert($id);
		        ...
		        if($response==true){ window.location='<?=base_url()?>index.php/activity/delete_health_instructions/'+$id; } else{}
           });
		});
        </script>
       <?php
...
<script> $(function () { $('.table').DataTable(); }); 
...
function manage_details(id,operation)
{
      ...
      if(checkval=='')
      {
        alert('Please Select Any Health Restriction!!')
        return false;
      }
              var url="<?php echo base_url().'index.php/activity/manage_activity_health_instructions'; ?>" ;
              $.ajax({
                      url :url,
                      type: 'POST',
