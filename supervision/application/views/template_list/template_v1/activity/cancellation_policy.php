...
$('.datepicker_to').datepicker({
   ...
   {
    var tran_id = $('#activity_id').val();
    if(tran_id=='')
    {
      alert('Please Select Excursion Type')
      $('#seasonality_from').val('');
      $('#seasonality_to').val('');
    }
    var seasonality_from = $('#seasonality_from').val();
     $.ajax({
      url: "<?php echo base_url();?>activity/check_seasonality",
      type: "POST",
   ...
   function get_seasonality_details()
   {
    ...
    var seasonality_to = $('#seasonality_to').val();
     $.ajax({
      url: "<?php echo base_url();?>activity/check_seasonality_detls",
      type: "POST",
...
<div id="Package" class="bodyContent col-md-12 yhgjk">
  ...
  <div class="panel-body">
    ...
    <!-- PANEL HEAD START -->
<form class='form form-horizontal validate-form' style='margin-bottom: 0;' action="<?php echo base_url(); ?>activity/cancellation_policy"  method="post" id="vehicle_form" name="frm1" enctype="multipart/form-data" novalidate> 
  <input type="hidden" value="<?=$tc_data['id'];?>" name="id">
 <div class="col-md-12 col-sm-12 col-xs-12 mtp20">
                                         <div class="form-group item">
                                            <label class="control-label col-md-2 col-sm-2 col-xs-12">Excursion Name<span class="text-danger">*</span></label>
                                           
                                            <div class="col-md-8 col-sm-8 col-xs-12">
                                             <select class="form-control" name="activity_id" id="activity_id" required >
                                              <option value="">Select Excursion</option>
                                             <?php $activity_name = $this->db->get_where('activity', array('status' => '1'))->result_array();  ?>
                                                     <?php for($car_sup=0;$car_sup<count($activity_name);$car_sup++){ ?>
                                                     <option value="<?php echo $activity_name[$car_sup]['package_id'];?>" <?php if($tc_data['activity_id']==$activity_name[$car_sup]['package_id']) { echo 'selected'; } ?> ><?php echo $activity_name[$car_sup]['package_name'];?></option>
                                                    <?php } ?>
                                                </select>
   ...
   <table class="table table-bordered">
    ...
    <tbody>     
     ...
     foreach ($transferdata as $data) 
     {
      ...
      <tr>
       <td><?=$sn?></td>
       <td><input type='checkbox' class='interested<?=$j?>' onclick="uncheck(<?=$j?>);"  id='interested_<?=$j?>_<?=$i?>'   value="<?=$data['id']?>" /></td>
       <td class="center">
        <div class="dropdown2" role="group">
         <div class="dropdown slct_tbl pull-left sideicbb rtghu">
           <i class="fa fa-ellipsis-v"></i>  
            <ul class="dropdown-menu sidedis" style="display: none;">
        <li><a class="sideicbb1 sidedis" href="<?=base_url()?>index.php/activity/cancellation_policy/<?=$data['id']?>" > 
         <i class="glyphicon glyphicon-pencil"></i> Edit
        </a></li>
        <li><a class="sideicbb3 sidedis" href="<?=base_url()?>index.php/activity/cancellation_policy/<?=$data['id']?>/delete" onclick="return confirm('Are you sure to delete this record?');"> 
       ...
       <td><?=$this->custom_db->single_table_records('activity','package_name',array('package_id'=>$data['activity_id']))['data']['0']['package_name'];?></td>
   ...
   $('#view_modal').modal('show');
    ...
    $(document).ready(function(){
      ...
      $("#multi_del").click(function(){
            ...
            if($("input[name='multi_delete']:checked").length >0)
            {
              $status=$.post(app_base_url + "index.php/activity/multi_delete_cancellation_policy", {id: favorite}, function(result){
...
function manage_details(id,operation)
{
      ...
      var theme_tbl = 'activity_cancellation_policy';
       if(checkval=='')
      {
