	...
	<div class="panel <?=PANEL_WRAPPER?>"><!-- PANEL WRAP START -->
		<div class="panel-heading"><!-- PANEL HEAD START -->
			<div class="panel-title">
				<i class="fa fa-edit"></i> Perfect Activities Packages
			</div>
		</div><!-- PANEL HEAD START -->
		<div class="panel-body"><!-- PANEL BODY START -->
			<fieldset><legend><i class="fa fa-hotel"></i> City List</legend>
				<form action="<?=base_url()?>index.php/cms/activity_perfect_packages" enctype="multipart/form-data" class="form-horizontal" method="POST" autocomplete="off">
			...
			<table class="table table-condensed">
				...
				?>
					<tr>
						...
						<td><img src="<?php echo $GLOBALS ['CI']->template->domain_images ($v['image_activity']) ?>" height="100px" width="100px" class="img-thumbnail"></td>
						<td><?php echo get_status_label1($v['activity_perfect_packages_status']).get_status_toggle_button1($v['activity_perfect_packages_status'], $v['origin']); ?></td>
						
						<?php /* <td><?php echo get_status_label($v['top_destination_activity']).get_status_toggle_button($v['top_destination_activity'], $v['origin']) ?></td> */ ?>
						<td><a role="button" href="<?php echo base_url()?>index.php/cms/delete_activity_perfect_packages/<?php echo $v['origin'] ?>" class="text-danger">Delete</a></td>
					</tr>
				<?php
					endforeach;
				} else {
...
{
	if (intval($status) == ACTIVE) {
		return '<a role="button" href="'.base_url().'index.php/cms/deactivate_top_destination_activity/'.$origin.'" class="text-danger">Deactivate</a>';
	} else {
		return '';		
	}
}
...
{
	if (intval($home_status) == ACTIVE) {
		return '<a role="button" href="'.base_url().'index.php/cms/deactivate_activity_perfect_packages/'.$origin.'" class="text-danger">Deactivate</a>';
	} else {
		return '<a role="button" href="'.base_url().'index.php/cms/activate_activity_perfect_packages/'.$origin.'" class="text-success">Activate</a>';
	}
}
