...
$table = '<div>
	...
	if (valid_array($sub_admin) == true) {
		foreach ($sub_admin as $key => $value) {
			...
			$action = '<form method="post" action="'.base_url().'index.php/cms/add_cms_page">
			<input type="submit" class="btn btn-sm" value="Edit">
			<input type="hidden" value="'.$value['page_id'].'" name="page_id">
			</form>';
...
<form method="post" autocomplete="off" action="<?php echo base_url();?>index.php/cms/add_cms_page" id="profile_form">
	<input type="hidden" value="<?php echo $ID;?>" name="page_id">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-condensed">
 <tr>
  ...
  <tr>
   ...
   <tr>
    <td>Page URL(pass empty if content available)</td>
    <td><input type="url" name="page_url" value="<?php echo isset($page_url) ? $page_url : '';?>">
    <font color="red" ><?=@form_error('page_url')?> </font>
    </td>
   </tr>
  <tr>
    ...
    <select name="page_position">
       ...
       <option value="tft" <?php if(isset($page_position)){ if($page_position == 'tft'){ echo 'selected="selected"'; }}?>>TFT</option>
        <option value="legals" <?php if(isset($page_position)){ if($page_position == 'legals'){ echo 'selected="selected"'; }}?>>Legals</option>
         <option value="helpline" <?php if(isset($page_position)){ if($page_position == 'helpline'){ echo 'selected="selected"'; }}?>>Helpline</option>
   
        <option value="travellertools" <?php if(isset($page_position)){ if($page_position == 'travellertools'){ echo 'selected="selected"'; }}?>>Traveller Tools</option>
    </select>
...
<script>
    CKEDITOR.replace('editor', {
        filebrowserUploadUrl: 'https://travelfreetravels.com/ckupload.php',
