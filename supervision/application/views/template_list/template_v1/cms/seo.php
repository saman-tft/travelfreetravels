			...
			<table class="table table-condensed">
				...
				if (valid_array($data_list) == true) {
					foreach ($data_list as $k => $v) :
						$action = '<form method="post" action="'.base_url().'index.php/cms/seo">
	     <input type="submit" class="btn btn-default btn-sm btn-primary" value="'.get_app_message('AL0041').'" />
	     <input type="hidden" value="'.$v['id'].'"  name="id" />
		 <span class="glyphicon glyphicon-pencil"></span></form><!--<a role="button" href="'.base_url().'index.php/cms/edit_seo/'.$v['id'].'"><button class="btn btn-sm">Edit</button></a>-->';
				?>
					<tr>
						<td><?=($k+1)?></td>
						<td><?=$v['title']?></td>
