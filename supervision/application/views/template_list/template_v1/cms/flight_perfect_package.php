	...
	<div class="panel <?=PANEL_WRAPPER?>"><!-- PANEL WRAP START -->
		<div class="panel-heading"><!-- PANEL HEAD START -->
			...
			<div class="panel-title">
				<i class="fa fa-edit"></i> Perfect Flights Packages
			</div>
		...
		<div class="panel-body"><!-- PANEL BODY START -->
			...
			<span style="color:#dd4b39"><?php if(isset($message)){ echo $message; } ?></span>
				<form action="<?=$_SERVER['PHP_SELF']?>/cms/flight_perfect_package" enctype="multipart/form-data" class="form-horizontal" method="POST" autocomplete="off">
		...
		<div class="panel-body">
			<table class="table table-condensed">
				<tr>
					...
					<th>To City</th>
					<th>Image</th>
					<?php /* <th>Home Page Publish Status</th> */ ?>
				...
				?>
					<tr>
						...
						<?php /* <td><?php echo get_status_label1($v['home_status']).get_status_toggle_button1($v['home_status'], $v['origin']); ?></td> */ ?>
						<td><?php echo get_status_label2($v['perfect_packages_status']).get_status_toggle_button2($v['perfect_packages_status'], $v['origin']); ?></td>
						<td><?php /* echo get_status_label($v['status']).get_status_toggle_button($v['status'], $v['origin']) */ ?>
						
						<a role="button" href="<?php echo base_url() ?>index.php/cms/delete_flight_perfect_packages/<?php echo $v['origin'] ?>"><button class="btn btn-sm">Delete</button></a>
...
function get_status_label2($home_status)
{
	if (intval($home_status) == ACTIVE) {
		return '<span class="label label-success"><i class="fa fa-circle-o">Active</i> '.get_enum_list('home_status', ACTIVE).'</span>
	<a role="button" href="" class="hide">'.get_app_message('AL0021').'</a>';
...
function get_status_toggle_button2($home_status, $origin)
{
	if (intval($home_status) == ACTIVE) {
		return '<a role="button" href="'.base_url().'index.php/cms/deactivate_perfect_packages_status/'.$origin.'" class="text-danger">Deactivate</a>';
	} else {
		return '<a role="button" href="'.base_url().'index.php/cms/activate_perfect_packages_status/'.$origin.'" class="text-success">Activate</a>';
	}
}
?>
