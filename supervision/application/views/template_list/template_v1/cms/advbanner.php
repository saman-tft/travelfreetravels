			...
			<span style="color:#dd4b39"><?php if(isset($message)){ echo $message; } ?></span>
				<form action="<?=$_SERVER['PHP_SELF']?>" enctype="multipart/form-data" class="form-horizontal" method="POST" autocomplete="off">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
						<input type="text" autocomplete="off" name="textbanner" class="normalinput auto-focus valid_class  form-control b-r-0" id="to" placeholder="Text" value="" required />
			...
			<table class="table table-condensed">
				...
				?>
					<tr>
						...
						<td><?php echo get_status_label1($v['home_status']).get_status_toggle_button1($v['home_status'], $v['id']); ?></td>
						<td><?php //echo get_status_label($v['status']).get_status_toggle_button($v['status'], $v['id']) ?>
						<a role="button" href="<?php echo base_url() ?>index.php/cms/delete_banner/<?php echo $v['id'] ?>"><button class="btn btn-sm">Delete</button></a>
						<a role="button" href="<?php echo base_url() ?>index.php/cms/edit_ad_banner/<?php echo $v['id'] ?>"><button class="btn btn-sm">Edit</button></a>
...
function get_status_toggle_button1($home_status, $id)
{
	if (intval($home_status) == ACTIVE) {
		return '<a role="button" href="'.base_url().'index.php/cms/deactivate_top_advers_home/'.$id.'" class="text-danger">Deactivate</a>';
	} else {
		return '<a role="button" href="'.base_url().'index.php/cms/activate_top_advers_home/'.$id.'" class="text-success">Activate</a>';
	}
}
