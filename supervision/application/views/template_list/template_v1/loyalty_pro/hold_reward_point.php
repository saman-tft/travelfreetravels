...
function get_table($table_data='', $total_rows=0)
{
   ...
   <th><button class="btn btn-success" type="button" id="releaseall" onclick="updaterate()">Release</button> </th>
   <th>Action</th>
   <th>Agency Name</th>
   ...
   // debug($table_data);exit;
	if (valid_array($table_data) == true) {
		...
		foreach ($table_data as $k => $v) {
			...
			$checkbox="<input type='checkbox' name='seletedid' value='".$v['id']."'>";
			$action="<a class='btn btn-success' type='button' href='".base_url()."index.php/loyalty_program/relese_singl_value/".$v['id']."'>Release</a> ";
			...
			<td>'.$v['booking_type'].'</td>
			<td>'.$v['reward_point'].'</td>
			<td>'.$v['description'].'</td>
...
function updaterate()
{
	// var rate=$("#seletedid"+id).val();

	var favorite = [];
            $.each($("input[name='seletedid']:checked"), function(){
                favorite.push($(this).val());
            });
            // alert(favorite);

	$status=$.post(app_base_url + "index.php/loyalty_program/relese_all_value", {id: favorite}, function(result){
