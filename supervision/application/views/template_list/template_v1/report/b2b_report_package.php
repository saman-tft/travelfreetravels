							...
							if(valid_array($table_data['booking_details']) == true) {
					        	...
					        	foreach($booking_details as $parent_k => $parent_v) {
					       		$lead_pax_namee = $parent_v['booking_itinerary_details'][0]['first_name']." ".$parent_v['booking_itinerary_details'][0]['last_name'];
									...
									if($status=="")
									{
									    $status="BOOKNG_INPROGRESS";
									}
								$customer_details = customer_details($app_reference, $booking_source, $status);
									$voucher_btn = holiday_voucher($app_reference, $booking_source, $status, 'b2c');
...
$(document).on('click', '.transfercustomer_details', function (e) {
...
function customer_details($app_reference, $booking_source = '', $status = '') {
        return '<a  target="_blank" class="btn btn-sm btn-primary" data-app-reference="' . $app_reference . '" data-booking-status="' . $status . '" data-booking-source="' . $booking_source . '" class="btn flight_u transfercustomer_details"><i class="fa fa-file"></i>Pax profile</a>';
}
