		...
		if (isset($table_data) == true and valid_array($table_data['data']) == true) {
		    ...
		    foreach($booking_details as $parent_k => $parent_v) { 
				...
				$pack_attr=json_decode($parent_v['booking_details']['attributes'],1);
		        	// debug($pack_attr['booking_source']);exit;
				$action = '';
				$email='';
				$tdy_date = date ( 'Y-m-d' );
				$diff = get_date_difference($tdy_date,$hotel_check_in);
				$customer_details = customer_details($parent_v['booking_details']['app_reference'], $pack_attr['booking_source'], $parent_v['booking_details']['status']);
				$action .= holiday_voucher($parent_v['booking_details']['app_reference'],$pack_attr['booking_source'],$parent_v['booking_details']['status'],'b2c');
				$action.='<br/>';
				$action .= holiday_pdf($parent_v['booking_details']['app_reference'], $pack_attr['booking_source'], $parent_v['booking_details']['status'],'b2c');
				$action.='<br/>';
				$action .= holiday_voucher_email($parent_v['booking_details']['app_reference'],$pack_attr['booking_source'],$parent_v['booking_details']['status'],$parent_v['booking_details']['email']);
				//$action .= '<br />' . $customer_details;
				$action.='<br/>';
		    	if($status == 'BOOKING_CONFIRMED' && $diff > 0) {
					$action .= cancel_hotel_booking($parent_v['booking_details']['app_reference'], $pack_attr['booking_source'],$parent_v['booking_details']['status']);
				}
				$action .=get_booking_pending_status($parent_v['booking_details']['app_reference'], $pack_attr['booking_source'], $parent_v['booking_details']['status']);
				$
				$email = hotel_email_voucher($parent_v['booking_details']['app_reference'],$pack_attr['booking_source'], $parent_v['booking_details']['status']);
				...
				// debug($total_no_of_night);
 				...
 				// debug($parent_v['app_reference']);exit;
				...
				$voucher_date=$parent_v['booking_details']['created_datetime'];
				 $grand_total=$pack_attr['markup']+$pack_attr['cv']+$pack_attr['tour_amount']+$pack_attr['gst_value']-$pack_attr['convenience_fee'];
						$report_data .= '<tr>
