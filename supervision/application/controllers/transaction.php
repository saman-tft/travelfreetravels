...
class Transaction extends CI_Controller {
	...
	{	
		...
		$page_data['agent_list'] = magical_converter(array('k' => 'user_id', 'v' => 'first_name'), $agent_list);
//debug($page_data);die;
	...
	function search_history()
	{
		...
		if(is_active_package_module()){
			array_push($monthly_series_data, $this->monthly_holiday_search_history_log($year_start, $year_end));
			$page_data['transfers_top_search'] = json_encode($this->transfer_top_search($year_start, $year_end));
		}
	...
	function top_destinations()
	{
		...
		if(is_active_package_module()){
			array_push($monthly_series_data, $this->holiday_top_search($year_start, $year_end));
			$page_data['holiday_top_search'] = json_encode($this->holiday_top_search($year_start, $year_end));
		}
		if(is_active_car_module()){
			array_push($monthly_series_data, $this->car_top_search($year_start, $year_end));
	...
	private function holiday_top_search($year_start,$year_end){
		$this->load->model('tours_model');
		$temp_data = $this->tours_model->top_search($year_start, $year_end);
		return $this->group_top_search_data($temp_data);

	}
	private function car_top_search($year_start,$year_end){
	...
	private function monthly_holiday_search_history_log($year_start, $year_end)
	{
		$this->load->model('tours_model');
		$data['name'] = 'Holiday';
		$temp_data = $this->tours_model->monthly_search_history($year_start, $year_end);
		$data['data'] = $this->distribute_monthly_values($temp_data);
		$data['color'] = '#344236';
		return $data;
	}
	private function monthly_car_search_history_log($year_start, $year_end)
	{
		$this->load->model('car_model');
		$data['name'] = 'Car';
