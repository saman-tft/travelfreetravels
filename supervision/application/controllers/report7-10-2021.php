...
class Report extends CI_Controller {
	...
	function viewpaymentmode($app_reference)
   {
		...
		function b2c_carcrs_report($offset=0)
	{
        ...
        $table_data = $this->car_model->b2c_carcrs_report($condition, false, $offset, RECORDS_RANGE_2);
	...
	function b2b_carcrs_report($offset=0)
	{
        ...
        $table_data = $this->car_model->b2b_carcrs_report($condition, false, $offset, RECORDS_RANGE_2);
       	// echo $this->current_module;exit;
        $table_data = $this->booking_data_formatter->format_car_booking_datas($table_data , $this->current_module);
       	// debug($table_data);exit;
       	$page_data['table_data'] = $table_data['data'];
...
//echo $module;die;
        if($module == 'flight'){
			...
			else if($module == 'privatetransfer'){
		...
		$booking_details['module'] = $module;
            ...
            if($booking_details['status'] == SUCCESS_STATUS && valid_array($booking_details['data']) ==true){
                if($module == 'privatetransfer'){
				$response['data'] = get_compressed_output(
				$this->template->isolated_view('report/transfcustomer_details',
	...
	function b2c_holiday_report($module_type='b2c',$offset=0)
	{
		// error_reporting(E_ALL);
	...
	public function b2b_holiday_report($module_type='b2c',$offset = 0)
	{
		//debug($module_type);exit();
		if (!check_user_previlege('34'))
	 	{
	  ...
	  'override_app_msg' => true
	   ...
	   redirect(base_url());
		...
		$table_data = $this->tours_model->b2b_booking ( $condition, false, $offset, RECORDS_RANGE_2 );
		$page_data ['table_data'] = $table_data ['data'];
		$x = count ( $table_data );
		$this->load->library ( 'pagination' );
		if (count ( $_GET ) > 0)
