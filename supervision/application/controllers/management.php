...
class Management extends CI_Controller {
				...
				case 'specific':
					if (valid_array($page_data['form_data']['airline_origin'])) {
						foreach($page_data['form_data']['airline_origin'] as $__k => $__domain_origin) {
							if ($page_data['form_data']['specific_value'][$__k] != '' && empty($page_data['form_data']['value_type_'.$__domain_origin]) == false
							) {
								$this->domain_management_model->save_markup_data(
	...
	function b2c_amadeus_airline_markup()
	{
		$markup_module_type = 'b2c_flight';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
					$this->domain_management_model->save_amadeus_markup_data(
					$page_data['form_data']['markup_origin'], $page_data['form_data']['form_values_origin'], $markup_module_type, 0,
					$page_data['form_data']['generic_value'], $page_data['form_data']['value_type'], get_domain_auth_id());
					break;
				case 'specific':
				...
				case 'add_airline';//Balu A
					if(isset($page_data['form_data']['airline_code']) == true && empty($page_data['form_data']['airline_code']) == false) {
						$airline_code = trim($page_data['form_data']['airline_code'] = $page_data['form_data']['airline_code']);
						$markup_details = $this->domain_management_model->amadeus_individual_airline_markup_details($markup_module_type, $airline_code);
						$airline_list_origin= intval($markup_details['airline_list_origin']);
						if(intval($markup_details['markup_list_origin']) > 0) {
							$markup_list_origin = intval($markup_details['markup_list_origin']);
						} else {
		...
		$view_data = array();
		//Airline would have All - general and domain wise markup
		$data_list = $this->domain_management_model->b2c_amadeus_airline_markup();
		$airline_list = $this->db_cache_api->get_airline_list();
		$data_list['data']['airline_list'] = $airline_list;
		$this->template->view('management/b2c_amadeus_airline_markup', $data_list['data']);
	}
	function b2c_plazma_airline_markup()
	{
		$markup_module_type = 'b2c_plazma_flight';
		...
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
					$this->domain_management_model->save_plazma_markup_data(
					$page_data['form_data']['markup_origin'], $page_data['form_data']['form_values_origin'], $markup_module_type, 0,
					$page_data['form_data']['generic_value'], $page_data['form_data']['value_type'], get_domain_auth_id());
					break;
				case 'specific':
					if (valid_array($page_data['form_data']['airline_origin'])) {
						...
						//	debug(	$page_data['form_data']['specific_value'][$__k]);die;
							}
						}
					}
					break;
				case 'add_airline';//Balu A
					if(isset($page_data['form_data']['airline_code']) == true && empty($page_data['form_data']['airline_code']) == false) {
						$airline_code = trim($page_data['form_data']['airline_code'] = $page_data['form_data']['airline_code']);
						$markup_details = $this->domain_management_model->plazma_individual_airline_markup_details($markup_module_type, $airline_code);
						$airline_list_origin= intval($markup_details['airline_list_origin']);
						if(intval($markup_details['markup_list_origin']) > 0) {
							$markup_list_origin = intval($markup_details['markup_list_origin']);
						} else {
		...
		$view_data = array();
		//Airline would have All - general and domain wise markup
		$data_list = $this->domain_management_model->b2c_plazma_airline_markup();
		$airline_list = $this->db_cache_api->get_airline_list();
		$data_list['data']['airline_list'] = $airline_list;
		$this->template->view('management/b2c_plazma_airline_markup', $data_list['data']);
	}
	/**
	 * Balu A
	 * Manage domain markup for provab - Domain wise and module wise
	 */
	function b2c_hotelcrs_markup()
	{
		
		//Hotel would have All - general and domain wise markup
		$markup_module_type = 'b2c_hotelcrs';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
		...
		//Airline would have All - general and domain wise markup
		$data_list = $this->domain_management_model->b2c_hotelcrs_markup();
		$this->template->view('management/b2c_hotelcrs_markup', $data_list['data']);
	}
	...
	function b2c_carcrs_markup()
	{
		
		//Car would have All - general and domain wise markup
		$markup_module_type = 'b2c_carcrs';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			// debug($page_data);exit;
			switch($page_data['form_data']['form_values_origin']) {
		...
		//Airline would have All - general and domain wise markup
		$data_list = $this->domain_management_model->b2c_carcrs_markup();
		$this->template->view('management/b2c_carcrs_markup', $data_list['data']);
	}
	...
	function b2b_amadeus_airline_markup()
	{
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//FIXME : Airline Markup - agent wise and general markup
		...
		//Airline would have All - general and Agent wise markup
		$data_list = $this->domain_management_model->b2b_amadeus_airline_markup();
		$airline_list = $this->db_cache_api->get_airline_list();
		$data_list['airline_list'] = $airline_list;
		$this->template->view('management/b2b_amadeus_airline_markup', $data_list);
	}
	function b2b_plazma_airline_markup()
	{
...
error_reporting (E_ALL);  
		...
		$this->domain_management_model->markup_level = 'level_3';
		//FIXME : Airline Markup - agent wise and general markup
		$markup_module_type = 'b2b_plazma_flight';
		$page_data['form_data'] = $this->input->post();
	//	debug($page_data['form_data'] );die;
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
		...
		//Airline would have All - general and Agent wise markup
		$data_list = $this->domain_management_model->b2b_plazma_airline_markup();
		//debug($data_list);die;
		$airline_list = $this->db_cache_api->get_airline_list();
		$data_list['airline_list'] = $airline_list;
		$this->template->view('management/b2b_plazma_airline_markup', $data_list);
	}
	/**
	 * Balu A
	 * Manage domain markup for B2B - Domain wise and module wise
	...
	function b2b_hotel_markup()
	{
		...
		function b2b_hotelcrs_markup()
	{
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//Hotel would have All - general and domain wise markup
		$markup_module_type = 'b2b_hotelcrs';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
		...
		//Hotel would have All - general and domain wise markup
		$data_list = $this->domain_management_model->b2b_hotelcrs_markup();
		$this->template->view('management/b2b_hotelcrs_markup', $data_list);
	}
	/**
	 * Elavarasi
	 * Manage domain markup for B2B - Domain wise and module wise
	...
	function b2b_car_markup(){
		...
		function b2b_carcrs_markup(){
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//Sightseeing would have All - general and domain wise markup
		$markup_module_type = 'b2b_carcrs';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			// debug($page_data);exit;
			switch($page_data['form_data']['form_values_origin']) {
		...
		//Sightseeing would have All - general and domain wise markup
		$data_list = $this->domain_management_model->b2b_carcrs_markup();
		$this->template->view('management/b2b_carcrs_markup', $data_list);
	}
	/**
	 * Balu A
	 * Manage domain markup for B2B - Domain wise and module wise
...
/**
	...
	*/
	...
	function segment_wise_discount(){
		$post_params = $this->input->post();
		$data = array();
		if((isset($post_params)) && !empty($post_params)){
			
			if((int)$post_params['form_values_origin'] > 0){
				$update_data = array('value_type' => $post_params['value_type'], 'value' => $post_params['discount_value']);
				$update_cond = array('origin' => $post_params['form_values_origin']);
				$this->custom_db->update_record('segment_wise_discount', $update_data, $update_cond);
			}else{
				$insert_data = array();
				$insert_data['value_type'] = $post_params['value_type'];
				$insert_data['value'] = $post_params['discount_value'];
				$this->custom_db->insert_record('segment_wise_discount', $insert_data);	
			}
			set_update_message();
			redirect(base_url().'index.php/management/'.__FUNCTION__);
		}
		$discount_value = $this->custom_db->single_table_records('segment_wise_discount', '*');
		if($discount_value['status'] == SUCCESS_STATUS){
			$data['discount'] = $discount_value['data'][0];
		}
		$this->template->view('management/segment_wise_discount', $data);
	}
	//new function that handles logic for b2b topup Report
	function b2b_topup_report(){
        // error_reporting(E_ALL);
       try{
                 
        $topupReportList = $this->domain_management_model->getAllTopUpReport();
   
        if($topupReportList['status'] === 1 && count($topupReportList['data'] > 0)){
            $page_data = array();
            $page_data['data'] = $topupReportList['data'];
            $page_data['total_rows'] = count($topupReportList['data']);
              
    
            	$this->template->view('management/b2b_topup_report', $page_data);
