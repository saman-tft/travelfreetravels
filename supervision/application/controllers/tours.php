...
/*  public function add_tour_save() {
    ...
    {   //echo 'saved'; exit;
      ...
      public function add_tour_save() {
        ...
        $this->form_validation->set_rules('package_name', 'package_name', 'required|is_unique[tours.package_name]');
        if ($this->form_validation->run() == FALSE)
        {
                $this->session->set_flashdata('message', UL0098);
                redirect('tours/add_tour');
        }
        else
        { 
          $package_description = sql_injection($data['package_description']);
          ...
          redirect('tours/tour_dep_dates_p2/'.$tour_id); 
        }
        else
        { 
         echo $return; exit; 
...
public function testpri(){
 
  
  $query = 'select * from privilege_list_new';
    // echo $query;exit;
    $privileges = $this->db->query($query)->result_array($query);
    debug($privileges);
}
public function testupdatedb(){
  $this->db->where('origin','121');
  $this->db->set('url','tours/tour_list');
  $this->db->update('privilege_list_new');

  $this->db->where('origin','122');
  $this->db->set('url','tours/tour_type');
  $this->db->update('privilege_list_new');

   $this->db->where('origin','123');
  $this->db->set('url','tours/tour_subtheme');
  $this->db->update('privilege_list_new');

   $this->db->where('origin','124');
  $this->db->set('url','tours/tour_region');
  $this->db->update('privilege_list_new');

  $this->db->where('origin','125');
  $this->db->set('url','tours/tour_country');
  $this->db->update('privilege_list_new');

  $this->db->where('origin','126');
  $this->db->set('url','tours/tour_city');
  $this->db->update('privilege_list_new');

  $this->db->where('origin','127');
  $this->db->set('url','supplier/enquiries');
  $this->db->update('privilege_list_new');

  $this->db->where('origin','128');
  $this->db->set('url','supplier/general_enquiries');
  $this->db->update('privilege_list_new');
...
public function tour_list() { 
 ...
 $page_data['tour_list'] = $tour_list;
      ...
      public function activate_tours_home($origin) {
    $status = ACTIVE;
    $info = $this->tours_model->update_top_tour__home( $status, $origin );
    redirect ( base_url () . 'tours/tour_list' );
  }
   public function deactivate_tours_home($origin) {
    $status = INACTIVE;
    $info = $this->tours_model->update_top_tour__home( $status, $origin );
    redirect ( base_url () . 'tours/tour_list' );
  }
...
public function tour_dep_dates_p2($package_id="") { 
    //debug($_POST); exit();
    $package_data = $this->tours_model->package_data($package_id); 
//debug($package_data[0]['id']);exit;
  ...
  // debug($page_data['tour_data']);exit;
   ...
   $tour_dep_dates = $this->tours_model->tour_dep_dates($package_id);
   $page_data['tour_dep_dates'] = $tour_dep_dates;
 ...
 //debug("dfgdf");exit;
  ...
  //added
  if(($data['tour_dep_date']=="")||($data['tour_id']==""))
  {
  ...
  $seat_count=$data['seat_count'];
   ...
   if(empty($data['tour_dep_date'])){
  //  debug("if");exit;
    $this->session->set_flashdata('error_message', 'Fill the date!!');
      redirect('tours/tour_dep_dates/'.$tour_id); exit;
    }

  $check_tour_dep_dates = $this->tours_model->check_tour_dep_dates($tour_id,$dep_date);
  //debug($check_tour_dep_dates);exit;
  if($check_tour_dep_dates>0)
  {
   redirect('tours/tour_dep_dates/'.$tour_id); exit;
 }

 $query  = "insert into tour_dep_dates set tour_id='$tour_id',dep_date='$dep_date',seat_count='$seat_count'";
...
public function tour_dep_dates_p2_save() {
 ...
 $seat_count=$data['seat_count'];
   ...
   {
      //  echo $tour_id;die;
    redirect('tours/tour_dep_dates_p2/'.$tour_id); exit;
  }
 ...
 public function tour_dep_date_saveoldy() {
  $data = $this->input->post();
...
public function tour_dep_dates_p2_saveoldy() {
  $data = $this->input->post();   
    // debug($data); exit();
  $dep_date = sql_injection($data['tour_dep_date']);
  $tour_id  = sql_injection($data['tour_id']);
...
// echo $tour_id;exit;
  ...
  if($return)
  {
       ...
       $package_id = $tour_data[0]['package_id'];
       redirect('tours/tour_dep_dates_p2/'.$package_id);
     } 
     else
     {
  ...
  public function tour_visited_cities_p2($tour_id="") {
    //echo $tour_id;exit;  
   $page_data['tour_id']   = $tour_id;
   $page_data['tour_data'] = $this->tours_model->tour_data($tour_id)[0];   
   // debug($page_data['tour_data']);exit;     
...
<div class="col-sm-4 controls">
  ...
  public function edit_tour_package_save() {
   ...
   //debug( $data );die();
    $tour_id = $data['tour_id'];
        ...
        $fetch_x = mysql_fetch_array($exe_x);*/
          ...
          $old_video = $fetch_x['video'];

        $package_name          = ucwords($data['package_name']);
        $package_name_org          = ucwords($data['package_name_org']);

        $this->load->library('form_validation');
        if($package_name != $package_name_org) {
           $is_unique =  '|is_unique[tours.package_name]';
        } else {
           $is_unique =  '';
        }
        $this->form_validation->set_rules('package_name', 'package_name', 'required'.$is_unique);
        if ($this->form_validation->run() == FALSE)
        {
                $this->session->set_flashdata('message', UL0098);
                redirect('tours/edit_tour_package/'.$tour_id);
        }
        else
        {
              ...
              {       
                ...
                for($i=0;$i<$total_images;$i++)
                {
                  ...
                  $folderpath= trim($folder.$new);
                  $path      = addslashes($folderpath);
                  move_uploaded_file($_FILES['video']['tmp_name'][$i], $folderpath);  
                  array_push($arr_video,$new);
             ...
             $Video_list = $Video_list_arr = array_merge($arr_video,$old_video);
             $Gallery_list = implode(',', $Gallery_list);
             $Gallery_list_arr = array_filter($Gallery_list_arr);

              $Video_list = implode(',', $Video_list);
             $Video_list_arr = array_filter($Video_list_arr);
   ...
   public function tour_pricing_p2($tour_id) {
     ...
     $page_data['terms_n_Conditions'] = $get_tc;
     	...
     	if(empty($page_data['terms_n_Conditions']))
     	{
     	    $page_data['terms_n_Conditions'][0]['terms_n_conditions']=$terms[0]['page_description'];
     	}
 ...
 public function tour_pricing_p2_save() {
  ...
  $list_video  = $_FILES['video']['name'];

  $total_videos = count($list_video);
  for($i=0;$i<$total_videos;$i++)
  {
     $filename  = basename($list_video[$i]);
     $extension = pathinfo($filename, PATHINFO_EXTENSION);
     $uniqueno  = substr(uniqid(),0,5);
     $randno = substr(rand(),0,5);
...
$Video_list_check = explode(".", $Video_list);
if(end($Video_list_check)!=''){
if($Video_list){
     $video_name_db = explode(",",$Video_list);
 }
}else{
  $Video_list="";
}
...
public function ajax_tour_home_publish() {
  $data = $this->input->post();
  // debug($data); exit('');
  $tour_id        = sql_injection($data['tour_id']);
  $dep_date       = sql_injection($data['dep_date']);
  ...
  $query4 = "select * from tours_itinerary where tour_id=".$tour_id; 
  $num_ajax_tour_publish_4 = $this->tours_model->ajax_tour_publish_1($query4);
 ...
 if($num_ajax_tour_publish_4 == 0)
 {  
  $message['first'][]= "Unable to publish to home page the Tours not published.";
 }

 if($num_ajax_tour_publish_1 !=0 && $num_ajax_tour_publish_3!=0 && $num_ajax_tour_publish_4!=0 && ($publish_status == 1 || $publish_status == 2 || $publish_status == 3 || $publish_status == 4 || $publish_status == 5 || $publish_status == 6 || $publish_status ==0))
 {
   $query  = "update tours set home_status='$publish_status' where id='$tour_id'";
   $return = $this->tours_model->query_run($query);
   if($return)
   {
       ...
       if($publish_status != 0){
    $message['sec'][]= "Thanks! This tour is successfully published for home page.";
       }
  }else{
    $message['sec'][]= "Sorry|| some techinal .";
  }
...
if($publish_status == 0){
    $message['sec'][]= "Thanks! This tour is successfully unpublished for home page.";
}
if($publish_status != 1){
 $query  = "update tours set home_status='$publish_status' where id='$tour_id'";
 $return = $this->tours_model->query_run($query);
...
public function tour_itinerary_p2($tour_id) { 
 ...
 $page_data['tours_city_name'] = $this->tours_model->tours_city_name();
 // debug($page_data['tour_visited_cities']);exit;
...
public function tour_itinerary_p2_save() {
  ...
  foreach($id as $index => $record)
  {
    ...
    foreach ($program_title as $key => $program_title_val) 
    {
      ...
      $tours_itinerary_dw_data  =array( 
       ...
       'visited_city'=>$tour_visited_cities[0]['city'],
       'visited_city_day'=>$data['city_day_list'][$i],
       'program_title'=>$itinerary[$key]['program_title'],
       'program_des'=>$itinerary[$key]['program_des'],
       'hotel_name'=>$itinerary[$key]['hotel_name'],
       'rating'=>$itinerary[$key]['rating'],
  ...
  if($this->session->userdata('edit_itinary')){
    $this->session->unset_userdata('edit_itinary');
     ...
     // set_update_message();
     $this->session->set_flashdata('message', UL0013);
   //redirect('tours/tour_pricing_p2/');  
...
public function tour_type_save() {
  ...
  $this->load->library('form_validation');
        $this->form_validation->set_rules('tour_type_name', 'tour_type_name', 'required|is_unique[tour_type.tour_type_name]');
        if ($this->form_validation->run() == FALSE)
        {
                $this->session->set_flashdata('message', UL0098);
...
public function edit_tour_type_save() {
  ...
  $tour_type_name = sql_injection($data['tour_type_name']);
  $tour_type_name_org = sql_injection($data['tour_type_name_org']);
  $this->load->library('form_validation');
  if($tour_type_name != $tour_type_name_org) {
     $is_unique =  '|is_unique[tour_type.tour_type_name]';
  } else {
     $is_unique =  '';
  }
  $this->form_validation->set_rules('tour_type_name', 'tour_type_name', 'required'.$is_unique);
  if ($this->form_validation->run() == FALSE)
  {
          $this->session->set_flashdata('message', UL0098);
          redirect('tours/edit_tour_type/'.$id);
  }
  else
  {
  ...
  public function tour_country_save() {
    ...
    { redirect('tours/tour_country/'); }
     ...
     public function tour_subtheme_save() 
     {
      ...
      $this->load->library('form_validation');
        $this->form_validation->set_rules('tour_subtheme', 'tour_subtheme', 'required|is_unique[tour_subtheme.tour_subtheme]');
        if ($this->form_validation->run() == FALSE)
        {
                $this->session->set_flashdata('message', UL0098);
    ...
    //debug($data); exit;
      ...
      $this->load->library('form_validation');
      $tour_subtheme  = sql_injection($data['tour_subtheme']);
      $tour_subtheme_org  = sql_injection($data['tour_subtheme_org']);
      // debug($tour_subtheme);
      // debug($tour_subtheme_org);exit;
        if($tour_subtheme != $tour_subtheme_org) {
           $is_unique =  '|is_unique[tour_subtheme.tour_subtheme]';
        } else {
           $is_unique =  '';
        }

        $this->form_validation->set_rules('tour_subtheme', 'tour_subtheme', 'required'.$is_unique);
        ...
        if ($this->input->post()) {
           $post_data = $this->input->post();
            ...
            foreach ($city_arr as $city) {
                ...
                $this->db->where("country_id",$post_data['country_id']);
                $this->db->where("CityName",$city);
                $qur = $this->db->get ("tours_city");
                $count=$qur->num_rows();
...
public function edit_tour_city($id) {  
  if ($this->input->post()) {
    ...
    $this->db->where("CityName",$post_data['CityName']);
    $this->db->where("id !=",$id);
  ...
  {
     $this->session->set_flashdata('error_message', 'Duplicate data!!');
      redirect('tours/edit_tour_city/'.$id);
  }
...
public function edit_tour_region_save() {
  ...
  $id             = $data['id'];
  $tour_region  = sql_injection($data['tour_region']);
  $this->db->where("name",$tour_region);
    $this->db->where("id !=",$id);
    // $this->db->where("continent",$tours_continent);
...
public function delete_tour_country($id){
  $this->db->where('id',$id);
  $this->db->delete('tours_country');
  redirect('tours/tour_country/');
}
...
public function tour_country_save() {
  ...
  public function edit_tour_country_save() {
    ...
    {
       $this->session->set_flashdata('error_message', 'Duplicate data!!');
        redirect('tours/edit_tour_country/'.$id);
    }              
  }
...
public function save_price_management_old() 
{
 ...
 $insert_data = array('from_date' => $from_date,
   ...
   'Type'=>$Type,
   'tour_id' => $tour_id);
 $count = $this->custom_db->single_table_records('tour_price_management','*',['tour_id' => $tour_id,'from_date'=>$from_date]);
// debug($count);exit();
 if(empty($count))
 {
 ...
 {
     redirect(base_url().'index.php/tours/price_management/'.$tour_id);
 }
...
//--------currency---------//
 ...
 $insert_data = array('from_date' => $from_date,
   ...
   'convt_adult_airliner_price' => round($converted_airliner_price),
   'convt_child_airliner_price' => round($converted_child_sessional_price),
   'convt_infant_airline_price' => round($converted_infant_sessional_price),
   'budget_hotel_price' => round($budget_hotel_price),
   'standard_hotel_price' => round($standard_hotel_price),
   'deluxe_hotel_price' => round($deluxe_hotel_price),
   '4_star_hotel_price' => round($four_star_hotel_price),
 ...
 header('Location: '.base_url().'index.php/tours/price_management/'.$tour_id);
     ...
     public function voucher($tour_id,$operation='show_broucher',$mail = 'no-mail',$quotation_id = '',$app_reference = '',$email = '',$redirect = '',$ex_data = array())
    {
      // error_reporting(E_ALL);
        ...
        $where = ['id'=>$tour_id];
        // debug($where); exit();
        $page_data ['tour_data']    = $this->tours_model->holiday_tour_data_new('tours', $where);

        // debug($page_data ['tour_data'] );exit;
        $page_data ['tours_itinerary']      = $this->tours_model->tours_itinerary($tour_id,$dep_date="");
        // debug($page_data ['tours_itinerary']);exit;
        $page_data ['tours_itinerary_dw']   = $this->tours_model->tours_itinerary_dw($tour_id,$dep_date="");

        $page_data ['tours_itinerary_wd']   = $this->tours_model->tours_itinerary_dw($tour_id,$dep_date="");

        $page_data ['tours_date_price']     = $this->tours_model->tours_date_price($tour_id);

        $tour_data = $this->custom_db->get_result_by_query("select group_concat(adult_airliner_price) pricing,group_concat(child_airliner_price) child_pricing, group_concat(occupancy) occ, group_concat(markup) markup ,tour_id, from_date, to_date , currency from tour_price_management where tour_id = ".$tour_id." group by from_date, to_date ");
        ...
        foreach ($tour_cities_array as $t_city) {
            ...
            $result = $this->db->query ( $query_x )->row_array();
           
            $visited_city[] = $result;
        }
        // debug($visited_city);exit;
        $page_data['visited_city'] = $visited_city;
        // debug($page_data['visited_city']);exit;
        if ($quotation_id!='') {
        ...
        // debug($page_data['quotation_details']);exit;
        if ($app_reference!='') {
...
public function tours_delete_video_id()
{
...
$query = 'SELECT tql.*, u.title as a_title,u.first_name as a_f_name,u.last_name as a_l_name,t.package_name FROM tours_quotation_log AS tql LEFT JOIN tours AS t ON tql.tour_id = t.id  LEFT JOIN user AS u ON tql.created_by_id = u.user_id ORDER BY tql.id DESC';
 ...
 $this->session->set_userdata( $array );
  ...
  public function add_agent_remark(){
     ...
     // exit;
     $this->db->where(array('id'=>$r_id));
      if( $this->db->update('tours_enquiry',array('agent_remark'=> $update_data,'status'=>1,'created_by_name'=>$name))){
