...
Class Flight_Model extends CI_Model
{
	...
	function b2c_flight_report_invoice($user_type='',$condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		//$b2c_condition_array = array('U.user_type', '=', B2C_USER, ' OR ', 'BD.created_by_id', '=', 0);
		...
		// }
			...
			{
			    //change 1 for invoice: replaced following queries
				// $bd_query = 'select BD.* ,U.user_name,U.first_name,U.last_name from flight_booking_details AS BD
				// 	     left join user U on U.user_id = BD.created_by_id
				// 	     left join user_type UT on UT.origin = U.user_type
				// 	     join flight_booking_transaction_details as BT on BD.app_reference = BT.app_reference		
				...
				$bd_query = 'select BD.* ,U.user_name,U.first_name,U.last_name from flight_booking_details AS BD
                         ...
                         left join user_type UT on UT.origin = U.user_type
                         join flight_booking_transaction_details as BT on BD.app_reference = BT.app_reference   
                         join payment_gateway_details as PGD on BD.app_reference = PGD.app_reference    
                         WHERE  (U.user_type='.B2C_USER.' OR BD.created_by_id = 0) AND (BT.status="BOOKING_CONFIRMED" OR PGD.status="accepted")  AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
                         group by BT.app_reference order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			}
			else
			{
				...
				// 		  order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
				 $bd_query = 'select BD.*,U.agency_name,U.first_name,U.last_name from flight_booking_details AS BD
                          ...
                          join flight_booking_transaction_details as BT on BD.app_reference = BT.app_reference 
                          join payment_gateway_details as PGD on BD.app_reference = PGD.app_reference                         
                          WHERE  U.user_type='.B2B_USER.' AND (BT.status="BOOKING_CONFIRMED" OR PGD.status="accepted") AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
                          group by BT.app_reference order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			}
	...
	function b2c_flight_report_id($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		//$b2c_condition_array = array('U.user_type', '=', B2C_USER, ' OR ', 'BD.created_by_id', '=', 0);
