 ...
 * @author     Balu A<balu.provab@gmail.com>func
 * @version    V2
 */
...
/**
	...
	{
		if (empty($this->airline_markup) == true) {
			$response['specific_markup_list'] = $this->specific_amadeus_airline_markup('b2b_flight');
	...
	{
		if (empty($this->airline_markup) == true) {
			$response['specific_markup_list'] = $this->specific_plazma_airline_markup('b2b_plazma_flight');
		...
		function update_transaction_details($transaction_type, $app_reference, $fare, $domain_markup, $level_one_markup=0, $convinence=0, $discount=0, $currency='INR', $currency_conversion_rate=1,$remark)
	{
...
//All active supplier list
	...
	{
		$response['data'] = array();
		$response['data']['specific_markup_list'] = $this->specific_amadeus_airline_markup('b2c_flight');
		$response['data']['generic_markup_list'] = $this->generic_amadeus_domain_markup('b2c_flight');
		return $response;
	}
	function b2c_plazma_airline_markup()
	{
		$response['data'] = array();
		$response['data']['specific_markup_list'] = $this->specific_plazma_airline_markup('b2c_plazma_flight');
		$response['data']['generic_markup_list'] = $this->generic_plazma_domain_markup('b2c_plazma_flight');
		return $response;
	}
	/**
	 * Balu A
    ...
    function b2c_hotelcrs_markup()
	{
		$response['data'] = array();
		$response['data']['specific_markup_list'] = '';
		$response['data']['generic_markup_list'] = $this->generic_domain_markup('b2c_hotelcrs');
		return $response;
	}
	/**
	 * Elavarasi
	...
	function b2c_carcrs_markup()
	{
		$response['data'] = array();
		$response['data']['specific_markup_list'] = '';
		$response['data']['generic_markup_list'] = $this->generic_domain_markup('b2c_carcrs');
		return $response;
	}
	/**
	 * Balu A
	...
	{
		$query = 'SELECT ML.origin AS markup_origin, ML.type AS markup_type, ML.reference_id, ML.value, ML.value_type
		FROM amadeus_markup_list AS ML where ML.module_type = "'.$module_type.'" and
		ML.markup_level = "'.$this->markup_level.'" and ML.type="generic" and ML.domain_list_fk='.get_domain_auth_id();
		$generic_data_list = $this->db->query($query)->result_array();
		return $generic_data_list;
	}
	...
	function specific_amadeus_airline_markup($module_type)
	{
		$sub_query = 'SELECT AL.origin 
		FROM airline_list AS AL 
		JOIN amadeus_markup_list AS ML ON
		ML.module_type = "'.$module_type.'" and ML.markup_level = "'.$this->markup_level.'" and AL.origin=ML.reference_id and ML.type="specific"
		and ML.domain_list_fk != 0  and ML.domain_list_fk='.get_domain_auth_id();
		
		$query = 'SELECT AL.origin AS airline_origin, AL.name AS airline_name, AL.code AS airline_code,
		ML.origin AS markup_origin, ML.type AS markup_type, ML.reference_id, ML.value, ML.value_type
		FROM airline_list AS AL LEFT JOIN amadeus_markup_list AS ML ON
		ML.module_type = "'.$module_type.'" and ML.markup_level = "'.$this->markup_level.'" and AL.origin=ML.reference_id and ML.type="specific"
		and ML.domain_list_fk != 0  and ML.domain_list_fk='.get_domain_auth_id().' 
		where (AL.has_specific_markup='.ACTIVE.' OR AL.origin in ('.$sub_query.')) order by AL.name ASC';
		$specific_data_list = $this->db->query($query)->result_array();
	...
	function amadeus_individual_airline_markup_details($module_type, $airline_code)
	{
		$query = 'SELECT ML.origin as markup_list_origin,AL.origin as airline_list_origin 
		FROM airline_list AS AL 
		left JOIN amadeus_markup_list AS ML ON
		ML.module_type = "'.$module_type.'" and ML.markup_level = "'.$this->markup_level.'" and AL.origin=ML.reference_id and ML.type="specific"
		and ML.domain_list_fk != 0  and ML.domain_list_fk='.get_domain_auth_id().' where AL.code="'.$airline_code.'"';
		$specific_data_list = $this->db->query($query)->row_array();
		return $specific_data_list;
	}
	function plazma_individual_airline_markup_details($module_type, $airline_code)
	{
		$query = 'SELECT ML.origin as markup_list_origin,AL.origin as airline_list_origin 
		FROM airline_list AS AL 
		left JOIN markup_list AS ML ON
	...
	function master_transaction_request_list($type='provab', $data_list_filt=array(), $credit='')
	{
		...
		if(!empty($credit)){
			...
			$query = 'select MTD.*, CONCAT(U.first_name, " ", U.last_name) request_user, U.email, U.agency_name AS requested_from from master_transaction_details MTD, user U
			where MTD.created_by_id=U.user_id AND MTD.type="'.$type.'" and MTD.domain_list_fk = '.get_domain_auth_id().' '.$data_list_cond.'
			and MTD.transaction_type ="Credit" order by MTD.created_datetime DESC';
		}
		else{
			// $query = 'select MTD.*, CONCAT(U.first_name, " ", U.last_name) request_user, U.email, U.agency_name AS requested_from from master_transaction_details MTD, user U
			// where MTD.created_by_id=U.user_id AND MTD.type="'.$type.'" and MTD.domain_list_fk = '.get_domain_auth_id().' '.$data_list_cond.'
	...
	function bank_account_details_supplier() 
	{
		$query='SELECT BAD.*,CONCAT(U.first_name," ",U.last_name) as created_by_name, U.email
		        FROM bank_account_details_supplier BAD
		        JOIN user U on U.user_id=BAD.created_by_id
		        where BAD.domain_list_fk='.get_domain_auth_id();
		$tmp_data = $this->db->query($query);
		if($tmp_data->num_rows()>0) {
	...
	*/
	function agent_list(){
		...
		// $query = 'select user_id, agency_name FROM user WHERE status = '.ACTIVE.' AND user_type = '.B2B_USER.' order by user_id asc';

		$query = 'select user_id, agency_name,first_name FROM user WHERE status = '.ACTIVE.' AND agency_name!="" AND (user_type ='.B2B_USER.' OR user_type ='.B2C_USER.') order by user_id asc';
	...
	function process_direct_credit_debit_transaction($details, $type='')
	{
		...
		//debug($master_transaction_details);die;
		$insert_id = $this->custom_db->insert_record('master_transaction_details', $master_transaction_details);
	...
	{
		$response['data'] = array();
		$response['data']['specific_markup_list'] = $this->specific_domain_markup('b2c_holiday');
		$response['data']['generic_markup_list'] = $this->generic_domain_markup('b2c_holiday');
	/*	debug($this->generic_domain_markup('b2c_holiday')[0]);
		debug($response['data']['generic_markup_list']);
		exit;*/
		return $response;
	}
	...
	{
		$response['data'] = array();
		$response['data']['specific_markup_list'] = $this->specific_domain_markup('b2b_holiday');
		$response['data']['generic_markup_list'] = $this->generic_domain_markup('b2b_holiday');
		return $response;
	}
	//new function for retrieving the topup data
		function getAllTopUpReport(){
	    try{
	    $result = $this->custom_db->single_table_records('offline_payment', '*', array(), 0, 1000000000, array('id'=>'desc'));
	    } catch(Exception $e){
