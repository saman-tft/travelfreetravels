...
Class Module_Model extends CI_Model
{
	...
	/*
	 * Update hotel perfect packages
	 */
	function update_hotel_perfect_packages($status,$origin){
	    $data = array(
				'perfect_packages_status' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('all_api_city_master', $data);
	}
	function update_top_destination_home($status,$origin)
	{
		$data = array(
				'home_status' => $status
		);
		...
		function delete_top_destination($status,$origin)
	{
		/*$data = array(
				'home_status' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('all_api_city_master', $data);*/
		$this->db->delete('all_api_city_master', array('origin' => $origin)); 
	}
	function update_top_destination_activity($status,$origin)
	{
		$data = array(
				'top_destination_activity' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	function update_top_destination_activity_home($status,$origin)
	{
		$data = array(
				'home_status' => $status
		);
	...
	/**
	 * Update Activity Perfect Packages
	 */
	function update_activity_perfect_packages($status,$origin)
	{
		$data = array(
				'activity_perfect_packages_status' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	function delete_activity_perfect_packages($status,$origin)
	{
		$data = array(
				'activity_perfect_packages_status' => $status,
				'activity_perfect_package'  => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	...
	function update_flight_top_destination($status,$origin)
	{
		...
		function update_transfercms_top_destination($status,$origin)
	{
		$data = array(
				//'top_destination' => $status
			'status' => $status
		...
		$this->db->where('origin', $origin);
		$this->db->update('top_transfercms_destinations', $data);
	}
	function update_transfercms_top_destination_home($status,$origin)
	{
		// debug($status);
		// debug($origin);
		// exit;
	...
	function update_flight_top_destination_home($status,$origin)
	{
		// debug($status);
		// debug($origin);
		// exit;
	...
	/**
	 * Ashwani
	 * update perfect packages status public
	 * 
	*/
	function update_flight_perfect_packages_status($status,$origin)
	{
		// debug($status);
		// debug($origin);
		// exit;
			...
			if(in_array($k, $inactive_module) == false) {
				switch($k){
					...
					case META_PACKAGE_COURSE:
    					$module_name_value = 'holiday';
    					$val= $v;
    					break;
    				case META_AIRLINE_PJ_COURSE:
					  //  $module_name_value = 'privatejet';
						//$val= $v;
						break;
					case META_CAR_PC_COURSE:
					//    $module_name_value = 'privatecar';
					//	$val= $v;
						break;	
				}
			
				$promocode_module_options[$module_name_value] = $val;
...
//	debug($privileges);exit;		
	...
	function update_top_destination_transfers($status,$origin)
	{
		$data = array(
				'top_destination_transfers' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	function update_top_destination_transfers_home($status,$origin)
	{
		$data = array(
				'home_status_transfer' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	 ...
	 */
	  function delete_transfer_perfect_packages($status,$origin)
	{
		$data = array(
				'transfer_perfect_packages_status' => $status,
				'transfer_perfect_package'  => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	function update_transfer_perfect_packages_status($status,$origin)
	{
		$data = array(
				'transfer_perfect_packages_status' => $status
		);
	...
	function update_promocode_home($status,$origin)
	{
		$data = array(
				'home_status' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('promo_code_list', $data);
	}
	/**
	 * for module promocode module_status
	 */
	function update_promocode_module_status($status,$origin)
	{
		$data = array(
	...
	function update_top_advers_home($status,$origin)
	{
		$data = array(
				'home_status' => $status
		);
		$this->db->where('id', $origin);
		$this->db->update('adv_banner', $data);
	}
	function update_top_destination_car($status,$origin)
	{ 
		$data = array(
				'top_destination' => $status
		);
	 ...
	 */
	 function update_car_perfect_packages($status,$origin){
	     $data = array(
	...
	{
		// $query = "select * from general_user_review order by origin desc"; //echo $query; exit;
		...
		$query = 'select * from general_user_review order by origin desc';
		return $this->db->query($query)->result_array();
