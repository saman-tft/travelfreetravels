	...
	$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records
					...
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code
					left join user as U on BD.created_by_id = U.user_id 
					where (U.user_type='.B2C_USER.' OR BD.created_by_id = 0) AND BD.booking_source="PTBCSID00000000017" AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'';
...
function b2c_car_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records
					...
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code
					left join user as U on BD.created_by_id = U.user_id 
					where (U.user_type='.B2C_USER.' OR BD.created_by_id = 0) AND BD.booking_source="'.PROVAB_CAR_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'';
...
function monthly_search_history($year_start, $year_end)
	{
		...
		AND search_type="'.META_CAR_COURSE.'"
		GROUP BY YEAR(created_datetime), MONTH(created_datetime)';
		return $this->db->query($query)->result_array();
	}
	function top_search($year_start, $year_end)
	{
		$query = 'select count(*) AS total_search, CONCAT(from_location) label from search_car_history where
		(YEAR(created_datetime) BETWEEN '.$year_start.' AND '.$year_end.') AND domain_origin='.get_domain_auth_id().' 
		AND search_type="'.META_CAR_COURSE.'"
		GROUP BY CONCAT(from_location) order by count(*) desc, created_datetime desc limit 0, 15';
		return $this->db->query($query)->result_array();
	}
function b2b_car_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join car_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.created_by_id = U.user_id  where U.user_type='.B2B_USER.' AND BD.booking_source="'.PROVAB_CAR_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			return $data['total_records'];
		} else {
			$this->load->library('booking_data_formatter');
			...
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from car_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id 					     
						 WHERE  U.user_type='.B2B_USER.' AND BD.booking_source="'.PROVAB_CAR_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						 order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			$booking_details = $this->db->query($bd_query)->result_array();
			$app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
			if(empty($app_reference_ids) == false) {
...
function b2b_carcrs_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join car_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.created_by_id = U.user_id  where U.user_type='.B2B_USER.' AND BD.booking_source="'.PROVAB_CAR_CRS_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			return $data['total_records'];
		} else {
			$this->load->library('booking_data_formatter');
			...
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from car_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id 					     
						 WHERE  U.user_type='.B2B_USER.' AND BD.booking_source="'.PROVAB_CAR_CRS_BOOKING_SOURCE.'" AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						 order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			$booking_details = $this->db->query($bd_query)->result_array();
			$app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
			if(empty($app_reference_ids) == false) {
