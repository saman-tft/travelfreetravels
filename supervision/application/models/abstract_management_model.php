...
abstract Class Abstract_Management_Model extends CI_Model
{
	...
	function save_amadeus_markup_data($markup_origin, $type, $module_type, $reference_id, $value, $value_type, $domain_origin, $user_oid=0)
	{
		$markup_data['origin']			= intval($markup_origin);
		$markup_data['markup_level']	= $this->markup_level;
		$markup_data['type']			= strtolower($type);
		...
		if (empty($markup_data['type']) == false && empty($markup_data['value_type']) == false) {
			if (intval($markup_origin) > 0) {
				//update
				$this->custom_db->update_record('amadeus_markup_list', $markup_data, array('origin' => intval($markup_origin)));
			} else {
				//insert
				$this->custom_db->insert_record('amadeus_markup_list', $markup_data);
			}	
		}
	}
	function save_plazma_markup_data($markup_origin, $type, $module_type, $reference_id, $value, $value_type, $domain_origin, $user_oid=0)
	{
		$markup_data['origin']			= intval($markup_origin);
		$markup_data['markup_level']	= $this->markup_level;
		$markup_data['type']			= strtolower($type);
