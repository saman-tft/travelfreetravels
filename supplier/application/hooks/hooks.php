...
class application {
	...
	function initialize_domain_previleges() {
		...
		if (! empty ( $user_id )) {
			...
			$user_previleges = $this->CI->module_model->get_user_privillages ( AUTH_USER_POINTER );
			//debug(ADMIN);exit;
			...
			/*	if($_SERVER['REMOTE_ADDR']=="27.60.210.76")
		    {
		       debug($active_user_previleges);exit;
		    }*/
...
function initialize_domain_previlegesoldb2c() {
		...
		$user_id = $this->CI->session->userdata ( AUTH_USER_POINTER );
		if (! empty ( $user_id )) {
			$active_user_previleges = $this->CI->module_model->get_user_privillages ();
			// debug($active_user_previleges);
			// die;
			$this->CI->active_user_previleges = $active_user_previleges;
		}
	}
	...
	function bouncer_page_validation() {
		$skip_validation_list = array (
				'forgot_password','send_otp','change_admin_password','supplier_term','supplierRegister','get_city_lists_supplier','emailvalidate'
				); // SKIP LIST
...
// debug($user_id);exit();
			...
			if (isset ( $user_details ) == true and valid_array ( $user_details ) == true and count ( $user_details ) > 0) {
				...
				// debug($this->CI->entity_domain_id);exit();
				...
				$this->CI->entity_branch_id = $user_details['branch_id'];
				$this->CI->entity_supplier_id = $user_details['supplier_id'];
				$this->CI->entity_city_id = $user_details['city'];				
				$this->CI->payment_privilege_action = $user_details ['Payment_privilege_action'];

				$data= $this->CI->user_model->get_supplier_privillages( $user_details ['user_id']);
				// debug($user_details);exit();
				 

				if(!empty($data['supplier_privailage']))
				{ 
					$this->CI->supplier_privillages=explode(",",$data['supplier_privailage']);	  
				} 
