...
class Utilities extends CI_Controller {
	...
	function events_notification()
	{
		...
		$oe_start = 0;
		$event_limit = 10;
		$notification_list = $this->application_logger->get_events_notification_supplier ($oe_start, $event_limit);
	...
	function manage_promo_code($offset = 0) 
	{
		...
		if(isset($get_data['eid']) == true && intval($get_data['eid']) > 0 && valid_array($post_data ) == false) {
			...
			if ($this->form_validation->run ()) {
				// debug($page_data['from_data']);
				// debug($post_data);
				// debug($get_data);die;
				if(!isset($get_data['eid'])) {
					$this->form_validation->set_rules('promo_code_image', 'Promo code Image', 'required');
				}
	...
	public function promo_show_on_modal(){
		extract($this->input->post());
		//debug($this->input->post());
		if($showonmodal== 'no'){
			$showonmodal = 'yes';
		}elseif($showonmodal== 'yes'){
			$showonmodal= 'no';
		}
		
		$chk = $this->module_model->promo_show_on_modal($origin,$showonmodal);
		echo $chk;//ajax response
	}
	
			function manage_blog($id = '') 
	{
		...
		if (valid_array ( $post_data ) == false) {
			if (intval ( $id ) > 0) {
				...
				if (valid_array ( $tmp_data ['data'] [0] )) {
					$data ['blog_title'] = $tmp_data ['data'] [0] ['blog_title'];
					$data ['blog_description'] = $tmp_data ['data'] [0] ['blog_description'];
					$data ['blog_image'] = $tmp_data ['data'] [0] ['blog_image'];



					
				} else {
					redirect ( 'utilities/manage_blog' );
				}
			}
			...
			$data ['blog_title'] = $title = $this->input->post ( 'blog_title' );
			$data ['blog_description'] = $this->input->post ( 'blog_description' );
			
			//debug($data);exit;
			if ($this->form_validation->run ()) {
				// add / update data
	if (valid_array($_FILES) == true and $_FILES['blog_image']['error'] == 0 and $_FILES['blog_image']['size'] > 0) {
		//debug("ffffffff");exit();
					$config['upload_path'] = $this->template->domain_blog_upload_path();
					//debug($config['upload_path'] );exit();
					// debug($_FILES);exit;
					$temp_file_name = $_FILES['blog_image']['name'];
					...
					if ( ! $this->upload->do_upload('blog_image')) {
						echo $this->upload->display_errors();//exit('2');
					} else {
						$image_data =  $this->upload->data();
		...
		//debug($data);exit;
		$this->template->view ( 'utilities/manage_blog', $data );
	}

		function blog_status($id = '', $status = 'D') {
		if ($id > 0) {
			...
			$this->custom_db->update_record ( 'blog', array (
					'page_status' => $status 
			), array (
					'id' => $id 
			) );
