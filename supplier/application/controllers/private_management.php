...
class Private_Management extends CI_Controller {
	...
	function package_domain_markup()
	{
		...
		//Airline would have All - general and domain wise markup
		$data_list = $this->private_management_model->package_domain_markup();
		$this->template->view('private_management/package_domain_markup', $data_list['data']);
	}
...
function b2bpackage_domain_markup()
	{
		...
		//Airline would have All - general and domain wise markup
		$data_list = $this->private_management_model->b2bpackage_domain_markup();
		$this->template->view('private_management/b2bpackage_domain_markup', $data_list['data']);
	}




function corporatepackage_domain_markup()
	{
		//Hotel would have All - general and domain wise markup
		$page_data['form_data'] = $this->input->post();
		// debug($page_data);
		...
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
					$this->private_management_model->save_markup_data(
					$page_data['form_data']['markup_origin'], $page_data['form_data']['form_values_origin'], 'corporate_holiday', 0,
					$page_data['form_data']['generic_value'], $page_data['form_data']['value_type'], $page_data['form_data']['domain_origin']
					);
					break;
				case 'specific':
					if (valid_array($page_data['form_data']['domain_origin'])) {
						foreach($page_data['form_data']['domain_origin'] as $__k => $__domain_origin) {
							...
							&& empty($page_data['form_data']['value_type_'.$__domain_origin]) == false
								...
								$this->private_management_model->save_markup_data(
								$page_data['form_data']['markup_origin'][$__k], $page_data['form_data']['form_values_origin'], 'corporate_holiday', $page_data['form_data']['domain_origin'][$__k],
								$page_data['form_data']['specific_value'][$__k], $page_data['form_data']['value_type_'.$__domain_origin], $page_data['form_data']['domain_origin'][$__k]
								);
							}
						}
		...
		//Airline would have All - general and domain wise markup
		$data_list = $this->private_management_model->corporatepackage_domain_markup();
		$this->template->view('private_management/corporatepackage_domain_markup', $data_list['data']);
	}
	/**
	 * Manage domain markup for provab - Domain wise and module wise
	 */
