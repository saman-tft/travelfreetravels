...
class Supplier extends CI_Controller {
	...
	public function update_tstatus($package_id, $status) {
		$this->Supplierpackage_Model->update_tstatus ( $package_id, $status );
		redirect ( 'supplier/view_with_price/', 'refresh' );
	}
	...
	public function agent_enquiries() {
		$data ['enquiries'] = $this->Supplierpackage_Model->agent_enquiries ();
		// debug($data);exit;
		$this->template->view ( 'suppliers/agent_enquiries', $data );
	}
	public function delete_enquiry($id, $package_id) {
		$this->Supplierpackage_Model->delete_enquiry ( $id );
		redirect ( 'supplier/enquiries/' );
	}
	public function delete_agent_enquiry($id, $package_id) {
		$this->Supplierpackage_Model->delete_agent_enquiry ( $id );
		redirect ( 'supplier/agent_enquiries/' );
	}

	public function delete_packenquiry($id, $package_id) {
		$this->Supplierpackage_Model->delete_enquiry ( $id );
		redirect ( 'supplier/view_enquiries/'.$package_id.'/w' );
	}

	public function corporate_enquiries() {
		$data ['enquiries'] = $this->Supplierpackage_Model->corporate_enquiries ();

		 //debug($data);exit;
		$this->template->view ( 'suppliers/corporate_enquiries', $data );
	}
	...
	public function update_package($package_id) {
		...
		$rating = $this->input->post ( 'rating' );
		$p_price = $this->input->post ( 'p_price' );
		$top_destination = $this->input->post ( 'top_dest' );

if($top_destination=='on')
$top_destination=1;
else
$top_destination=0;
		$data = array (
	...
	public function images($package_id) {
		...
		if ($_FILES) {
			$cp = count ( $_FILES['traveller']['name'] );
		// print_r($cp);exit;
			$tra = $_FILES;
		for($i = 0; $i < $cp; $i ++) {
			...
			$_FILES ['traveller'] ['error'] = $tra ['traveller'] ['error'][$i];
			$_FILES ['traveller'] ['size'] = $tra ['traveller'] ['size'][$i];
			// debug($_FILES ['traveller']);exit;
			$name_of_traveller_img = 'traveller-' . time () . $_FILES ['traveller'] ['name'];

			// debug($name_of_traveller_img);
			move_uploaded_file ( $_FILES ['traveller'] ['tmp_name'], $this->template->domain_uploads_packages ( $name_of_traveller_img ) );
			$traveller_img = $name_of_traveller_img;
			$sup_id = $this->entity_user_id;
	...
	function enquiry_status()
	{
		try 
		{
			...
			$this->db->where('id',$id);
			if(!$this->db->update('package_enquiry',$data))
			{
				throw new Exception("Status Update Failed", 1);
			}
	...
	function update_comments()
	{
		try 
		{
			...
			if(!$this->db->update('package_enquiry',$data))
			{
				throw new Exception("Comments Update Failed", 1);
			}
	...
	function agent_enquiry_status()
	{
		try 
		{
			...
			$data['status']= $this->input->post('status');
			$this->db->where('id',$id);
			if(!$this->db->update('b2b_package_enquiry',$data))
			{
				throw new Exception("Status Update Failed", 1);
			}
			$msg='Status Successfully Updated';			
	...
	function agent_update_comments()
	{
		try 
		{
			$msg="Failed";
