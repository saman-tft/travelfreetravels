	...
	function index()
	{
		...
		if (web_page_access_privilege('p1')) {
			...
			if (!empty($active_package)) {
				$module_total_earning[6]['package'] = 'Package';
				$module_total_earning[6]['y'] = 0;
				$time_line_report[6]['name'] = 'Package';
				$time_line_report[6]['data'] = array();
				$time_line_report[6]['color'] = '#a8835b';
				$tmp_package_booking = $this->package_model->get_monthly_booking_summary();
				//debug($tmp_package_booking);

				$month_index_package = index_month_number($tmp_package_booking);
			}
			...
			if (!empty($active_package)) {
				$time_line_report[6]['data'] = $monthly_transfers_booking;
				$module_total_earning[6]['color'] = $time_line_report[6]['color'];
				$group_time_line_report[] = array('type' => 'column','name' => 'Package', 'data' => $transfers_earning, 'color' => $time_line_report[6]['color']);
			}
...
$condition=array();
			...
			if (!empty($active_hotel)) {
				...
				$page_data['hotel_booking_count'] = $this->hotels_model->b2c_crs_hotel_report($condition=array(), true);
			}
			if (!empty($active_airline)) {
			...
			if (!empty($active_sightseeing)) {
				$condition = array();//array('BD.module_type', '=', '"activity"');
		        $this->load->model('package_model');
				$page_data['sightseeing_booking_count'] =  $this->activity_model->b2c_holidaycrs_report($condition, true);
			}
			if (!empty($active_transfers)) {
				//	$condition[] = array('BD.module_type', '=', '"transfers"');//array('BD.module_type', '=', '"transfers"');
		        $this->load->model('package_model');
				$page_data['transfers_booking_count'] =  $this->transfers_model->bookingcrs($condition, true);
			}

			
		    $this->load->model('tours_model');
			$page_data['holiday_count'] = $this->tours_model->booking ( $condition=array(), true );	
$page_data['hotel_count'] = $this->hotel_model->b2c_hotelcrs_report($condition, true);	
