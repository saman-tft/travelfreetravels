...
class Management extends CI_Controller {
	...
	function b2c_airline_markup()
	{
		...
		//$data_list = array('data' => array('airline_list' => $airline_list));
		//debug($data_list);die;
	...
	function corporate_airline_markup()
	{
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//FIXME : Airline Markup - agent wise and general markup
		$markup_module_type = 'corporate_flight';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
		...
		//Airline would have All - general and Agent wise markup
		$data_list = $this->domain_management_model->corporate_airline_markup();
		$airline_list = $this->db_cache_api->get_airline_list();
		$data_list['airline_list'] = $airline_list;
		$this->template->view('management/corporate_airline_markup', $data_list);
	}
	...
	function corporate_hotel_markup()
	{
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//Hotel would have All - general and domain wise markup
		$markup_module_type = 'corporate_hotel';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
		...
		//Hotel would have All - general and domain wise markup
		$data_list = $this->domain_management_model->corporate_hotel_markup();
		$this->template->view('management/corporate_hotel_markup', $data_list);
	}
	/**
	 * Elavarasi
	 * Manage domain markup for B2B - Domain wise and module wise
	...
	function corporate_sightseeing_markup(){
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//Sightseeing would have All - general and domain wise markup
		$markup_module_type = 'corporate_sightseeing';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
		...
		//Sightseeing would have All - general and domain wise markup
		$data_list = $this->domain_management_model->corporate_sightseeing_markup();
		$this->template->view('management/corporate_sightseeing_markup', $data_list);
	}
	/**
	 * Elavarasi
	 * Manage domain markup for B2B - Domain wise and module wise
	...
	function corporate_transfer_markup(){
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//Sightseeing would have All - general and domain wise markup
		$markup_module_type = 'corporate_transfer';
		$page_data['form_data'] = $this->input->post();
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
		...
		//Sightseeing would have All - general and domain wise markup
		$data_list = $this->domain_management_model->corporate_transferv1_markup();
		$this->template->view('management/corporate_transfers_markup', $data_list);
	}
	...
	function corporate_bus_markup()
	{
		$user_oid = 0;//defining general only as of now
		$this->domain_management_model->markup_level = 'level_3';
		//Bus would have All - general and domain wise markup
		$page_data['form_data'] = $this->input->post();
		$markup_module_type = 'corporate_bus';
		if (valid_array($page_data['form_data']) == true) {
			switch($page_data['form_data']['form_values_origin']) {
				case 'generic':
					$this->domain_management_model->save_markup_data(
		...
		//Airline would have All - general and domain wise markup
		$data_list = $this->domain_management_model->corporate_bus_markup();
		$this->template->view('management/corporate_bus_markup', $data_list);
	}
	...
	public function b2b_balance_manager($balance_request_type="Cash"){
		...
		if (valid_array($page_data['form_data']) == true) {
			if (intval($page_data['form_data']['request_origin']) > 0) {
				...
				$process_details = $this->domain_management_model->process_balance_request($page_data['form_data']['request_origin'], $page_data['form_data']['system_request_id'], $page_data['form_data']['status_id'], $page_data['form_data']['update_remarks'],'b2b');
				// debug($process_details);exit;
			} else {
	...
	public function corporate_balance_manager($balance_request_type="Cash"){
		$page_data['form_data'] = $this->input->post();
		// debug($page_data);
		// die;
		if (valid_array($page_data['form_data']) == true) {
			if (intval($page_data['form_data']['request_origin']) > 0) {
				...
				$process_details = $this->domain_management_model->process_balance_request($page_data['form_data']['request_origin'], $page_data['form_data']['system_request_id'], $page_data['form_data']['status_id'], $page_data['form_data']['update_remarks'],'corporate');
			...
			// echo 'herrere';exit;
// debug($process_details);
// die;
			if ($process_details['status'] == SUCCESS_STATUS) {
				$data_list_filt = array();
				$data_list_filt[] = array('MTD.origin', '=',trim($page_data['form_data']['request_origin']));
				$data = $this->domain_management_model->master_transaction_request_list('corporate', $data_list_filt);
				if(!empty($data) && isset($data[0])){
					$master_transaction['master_transaction'] = $data[0];
					$email = $page_data['form_data']['request_user_email'];
					//$email=  "sagar@mailinator.com";
		...
		$page_data['table_data'] = $this->domain_management_model->master_transaction_request_list('corporate', $data_list_filt);
		...
		$page_data['status_options'] = get_enum_list('provab_balance_status');
		$page_data['heading'] = 'Corporate Balance Request';
		$page_data['search_params'] = $params;
		// debug($page_data);
		// die;
		$this->template->view('management/b2b_balance_manager', $page_data);
	 ...
	 */
	function bank_account_details_old()
	{
	...
	function bank_account_details()
	{
		...
		//debug($get_data);die;
	//	debug($_FILES);
		//debug($post_data['form_data']);die;
		$get_data = $this->input->get();
		$page_data = array();
		if(valid_array($post_data['form_data']) == false && isset($get_data['eid']) && intval($get_data['eid'])>0) {
			$temp_data=$this->custom_db->single_table_records('bank_account_details_supplier', '*', array('origin' => $get_data['eid']));
			$page_data['form_data']['origin']=$temp_data['data'][0]['origin'];
			$page_data['form_data']['en_account_name']=$temp_data['data'][0]['en_account_name'];
			$page_data['form_data']['en_bank_name']=$temp_data['data'][0]['en_bank_name'];
			$page_data['form_data']['en_branch_name']=$temp_data['data'][0]['en_branch_name'];
			...
			$page_data['form_data']['account_number']=$temp_data['data'][0]['account_number'];
			$page_data['form_data']['ifsc_code']=$temp_data['data'][0]['ifsc_code'];
			$page_data['form_data']['iben_number']=$temp_data['data'][0]['iben_number'];
			$page_data['form_data']['sort_code']=$temp_data['data'][0]['sort_code'];
			$page_data['form_data']['swift_code']=$temp_data['data'][0]['swift_code'];
			$page_data['form_data']['beneficiary_name']=$temp_data['data'][0]['beneficiary_name'];
			$page_data['form_data']['country_name']=$temp_data['data'][0]['country_name'];
			$page_data['form_data']['city']=$temp_data['data'][0]['city'];
			$page_data['form_data']['status']=$temp_data['data'][0]['status'];
			// debug($page_data);die;
		} else if( valid_array($post_data['form_data']) ) {
			$this->current_page->set_auto_validator();
			if ($this->form_validation->run()) {
				...
				if($origin > 0) {
					...
					$this->custom_db->update_record('bank_account_details_supplier', $post_data['form_data'],array('origin' => $origin) );
					set_update_message();
				} elseif($origin == 0){
					/** INSERT **/
					$post_data['form_data']['domain_list_fk'] = get_domain_auth_id();;
					...
					$post_data['form_data']['created_datetime'] = date('Y-m-d H:i:s');
					//debug($post_data['form_data']);die;
					$insert_id=$this->custom_db->insert_record('bank_account_details_supplier',$post_data['form_data']);
					//echo $insert_id;die;
					set_insert_message();
				}
				...
				if (valid_array($_FILES) == true and $_FILES['bank_icon']['error'] == 0 and $_FILES['bank_icon']['size'] > 0) {
					...
					$config['upload_path'] =$this->template->domain_image_upload_path ().'/bank_logo/';
					...
					if ( ! $this->upload->do_upload('bank_icon')) {
						// set_error_message();
						$this->session->set_flashdata(array('message' =>$this->upload->display_errors(), 'type' => FAILURE_MESSAGE, 'override_app_msg' => true));
						redirect('management/bank_account_details?eid='.$origin);
					} else 
					{
						...
						if (empty($insert_id) == true) {
						//UPDATE
						$temp_record = $this->custom_db->single_table_records('bank_account_details_supplier', 'bank_icon', array('origin' => $origin));
						$icon = $temp_record['data'][0]['bank_icon'];
						//DELETE OLD FILES
						if (empty($icon) == false) {
							if (file_exists($config['upload_path'].$icon)) {
						...
						$this->custom_db->update_record('bank_account_details_supplier', array('bank_icon' => $image_data['file_name']), array('origin' => $origin));
					}
	...
	*/
	public function account_ledger($offset=0)
	{
		...
		// debug($condition);
		// die;
		if($get_data['agent_type']==B2B_USER)
		{
		...
		{
			$complete_agent_details = $this->domain_management_model->get_corp_details($agent_id);
		}
		...
		{
		$total_records = $this->domain_management_model->corp_account_ledger($condition, true);	
		}
	...
	{
	$transaction_logs = $this->domain_management_model->corp_account_ledger($condition, false, $offset, RECORDS_RANGE_3);	
	}
...
// debug($get_data);die;
		...
		{
			$agent_list['data'] = $this->domain_management_model->corp_list();	
		}
	...
	*/
	...
	public function account_ledger_oldd($offset=0)
	{
		// echo "in";
		// die;
		$get_data = $this->input->get();
	...
	*Export Account Ledger details to Excel Format
	*/
	public function export_account_ledger_oldd($op=''){
