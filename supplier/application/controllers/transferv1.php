...
class Transferv1 extends CI_Controller 
{
	...
	public function pre_cancellation_new($app_reference,$url="")
	{
	echo 1;die;
	...
	public function update_refund_details()
	{
		...
		if ($this->form_validation->run()) {
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				...
				// die;
				$is_agent = false;
				if(valid_array($booked_user_details) == true && ($booked_user_details[0]['user_type'] == B2B_USER || $booked_user_details[0]['user_type'] == EMPLOYEE)){
					$is_agent = true;
				}
...
// die;
					...
					if($booked_user_details[0]['user_type'] == EMPLOYEE)
					$this->domain_management_model->update_agent_balance($agent_refund_amount, $booked_user_id,'corporate');
				else
				$this->domain_management_model->update_agent_balance($agent_refund_amount, $booked_user_id,'b2b');
				}
