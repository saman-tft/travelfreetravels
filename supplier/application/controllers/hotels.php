...
class Hotels extends CI_Controller {
    ...
    function add_hotel_type() {
        ...
        $this->template->view('hotel/hotel_types/add_hotel_type', $data = array());
    }
    ...
    function save_hotel_type_data() {
        try {
            ...
            $check_availibility = $this->hotels_model->check_type_exist($data['name'], $data['created_by']);
            if (!$check_availibility) {
                ...
                $this->session->set_flashdata('error_message', 'Hotel Type already exist');
                redirect('hotels/hotel_types');
            }
        } catch (Exception $e) {
    ...
    function add_room_types() {
        ...
        $this->template->view('hotel/room_type/add_room_type', $data = array());
    }
    ...
    function save_room_type_data() {
        try {
            ...
            $check_availibility = $this->hotels_model->check_room_type_exist($data['name'], $data['created_by']);
            if (!$check_availibility) {
                ...
                $this->session->set_flashdata('error_message', 'Room Type already exist');
                redirect('hotels/room_types');
            }
        } catch (Exception $e) {
            $this->session->set_flashdata('error_message', $e->getMessage());
    ...
    function add_hotel_ammenties() {
        $this->template->view('hotel/hotel_ammenities/add_hotel_ammenity', $data = array());
    }
...
// debug($data);die;
            ...
            $check_availibility = $this->hotels_model->check_hotelamenities_exist($data['name'], $data['created_by']);
            if (!$check_availibility) {
                ...
                $this->session->set_flashdata('error_message', 'Hotel Amenities already exist');
                redirect('hotels/hotel_ammenities');
            }
        } catch (Exception $e) {
            $this->session->set_flashdata('error_message', $e->getMessage());
    ...
    function room_meal_type() {


        $ammenities['meal_type_list'] = $this->hotels_model->get_room_meal_list();
        $this->template->view('hotel/room_meal_type/meal_type_list', $ammenities);
    }

    function add_room_ammenties() {
        $this->template->view('hotel/room_ammenities/add_hotel_ammenity', $data = array());
    }

    function add_room_meal_type() {
        $this->template->view('hotel/room_meal_type/add_meal_type', $data = array());
    }
    ...
    function save_room_amenities_data() {
        ...
        try {
            ...
            $data['status'] = $_POST['status'];
// debug($data);die;
            $check_availibility = $this->hotels_model->check_roomamenities_exist($data['name'], $data['created_by']);
            if (!$check_availibility) {
                ...
                $this->session->set_flashdata('error_message', 'Room Amenities already exist');
                redirect('hotels/room_ammenities');
            }
        } catch (Exception $e) {
            $this->session->set_flashdata('error_message', $e->getMessage());
    ...
    function save_room_meal_data() {
        //debug($_POST);die;
        try {

            if (isset($_POST['hotel_meal_name']) == FALSE || empty($_POST['hotel_meal_name']) == TRUE) {
                throw new Exception("Error Processing Request", 1);
            }
            $data['name'] = $_POST['hotel_meal_name'];
            $data['created_by'] =$this->entity_user_id;
            $data['status'] = $_POST['status'];

            $check_availibility = $this->hotels_model->check_roommeal_exist($data['name'], $data['created_by']);

            if (!$check_availibility) {

                if ($this->hotels_model->add_room_meal_details($data) == false) {
                    throw new Exception('Room Meal Type  Adding Failed', 1);
                }
                $this->session->set_flashdata('success_message', 'Inserted Successfully!!');
                redirect('hotels/room_meal_type', 'refresh');
            } else {
                $this->session->set_flashdata('error_message', 'Room Meal Type already exist');
                redirect('hotels/room_meal_type');
            }
        } catch (Exception $e) {
            $this->session->set_flashdata('error_message', $e->getMessage());
            redirect('hotels/room_meal_type', 'refresh');
    ...
    function inactive_room_meal($meal_id1) {
        $meal_id = json_decode(base64_decode($meal_id1));
        if ($meal_id != '') {
            $this->hotels_model->inactive_room_meal($meal_id);
            $this->session->set_flashdata('success_message', 'Updated Successfully!!');
        }
        redirect('hotels/room_meal_type', 'refresh');
    }
    ...
    function active_room_meal($meal_id1) {
        $meal_id = json_decode(base64_decode($meal_id1));
        if ($meal_id != '') {
            $this->hotels_model->active_room_meal($meal_id);
            $this->session->set_flashdata('success_message', 'Updated Successfully!!');
        }
        redirect('hotels/room_meal_type', 'refresh');
    }
    ...
    function edit_room_meal($meal_id1) {
        $meal_id = json_decode(base64_decode($meal_id1));
        if ($meal_id != '') {

            $hotel_meal['meal_list'] = $this->hotels_model->get_room_meal_list($meal_id);
            $this->template->view('hotel/room_meal_type/edit_meal_type', $hotel_meal);
        } else {
            redirect('hotels/room_meal_type', 'refresh');
        }
    }
    ...
    function update_room_meal($meal_id1) {
        $meal_id = json_decode(base64_decode($meal_id1));
        if ($meal_id != '') {
            if (count($_POST) > 0) {
                $this->hotels_model->update_room_meal($_POST, $meal_id);
                $this->session->set_flashdata('success_message', 'Updated Successfully!!');
                redirect('hotels/room_meal_type', 'refresh');
            } else if ($hotel_type_id != '') {
                redirect('hotels/edit_room_meal/' . $meal_id, 'refresh');
            } else {
                redirect('hotels/room_meal_type', 'refresh');
            }
        } else {
    ...
    function delete_room_meal($meal_id1) {
        $meal_id = json_decode(base64_decode($meal_id1));
        if ($meal_id != '') {
            $this->hotels_model->delete_room_meal($meal_id);
            $this->session->set_flashdata('success_message', 'Deleted Successfully!!');
        }
        redirect('hotels/room_meal_type', 'refresh');
    }
...
//			 debug($_POST);die;
            ...
            $data['cut_off'] = $this->input->post('cut_off');
            $data['checkIn_time'] = $this->input->post('checkIn_time');
            $data['checkOut_time'] = $this->input->post('checkOut_time');

            $data['sup_fname'] = $this->input->post('sup_fname');
            $data['sup_lname'] = $this->input->post('sup_lname');
            $data['sup_phone'] = $this->input->post('sup_phone');
            $data['sup_desig'] = $this->input->post('sup_desig');
            $data['sup_dep'] = $this->input->post('sup_dep');
            $data['sup_mail'] = $this->input->post('sup_mail');
            $data['manager_fname'] = $this->input->post('manager_fname');
            $data['manager_lname'] = $this->input->post('manager_lname');
...
//             debug($_POST);die;
            ...
            $data['manager_mail'] = $this->input->post('manager_mail');
            // debug($_FILES);exit;
            if (isset($_FILES['image']) == true && $_FILES['image']['error'] == 0) {
                $config['upload_path'] = DOMAIN_HOTEL_UPLOAD_DIR;
                $config['allowed_types'] = 'gif|jpg|png';
                $config['max_size'] = 1024;
                $config['max_width'] = 1024;
            ...
            if ($_POST['submit'] == 'Update') {
                ...
                redirect('hotels/hotel_crs_list');
            }
    ...
    function inactivate_multiple_hotels() {
        if (isset($_POST) && isset($_POST['data'])) {
            ...
            $this->db->query($sql);
            $data['status'] = 1;
            $data['success_url'] = site_url() . "index.php/hotels/hotel_crs_list";
            print json_encode($data);
            return;
        }
    }
...
//            $data['extra_bed'] = $this->input->post('extra_bed');
        ...
        redirect("hotels/room_crs_list/" . $hotel_id);
    }
...
//            $this->form_validation->set_rules('extrabed', 'extrabed', 'required');
            ...
            $child_age = explode('-', $this->input->post('child_age'));
            $data['child_age_from'] = $child_age[0];
            $data['child_age_to'] = $child_age[1];
            $data['child_price'] = $this->input->post('child_price');
            $data['child_commission'] = $this->input->post('child_commission');
            if (!empty($this->input->post('child_age_1'))) {
                $child_age_1 = explode('-', $this->input->post('child_age_1'));
                $data['child_age_from_1'] = $child_age_1[0];
                $data['child_age_to_1'] = $child_age_1[1];
                $data['child_price_1'] = $this->input->post('child_price_1');
                $data['child_commission_1'] = $this->input->post('child_commission_1');
            } else {
...
// 		$data['latest_season_date'] = $this->hotels_model->season_date($hotel_id);
        $data['hotels_data'] = $this->hotels_model->get_hotel_data($data['hotel_id'])->row();
        //debug($data['hotels_data']);exit;
    ...
    function edit_seasons($id) {
        ...
        $data['hotels_data'] = $this->hotels_model->get_hotel_data($data['data']->hotel_details_id)->row();
    ...
    function check_availability() {
        ...
        $end = date("Y-d-m", strtotime($this->input->post('end')));
        $hotel_id = $this->input->post('hotel_id');
        $id = $this->input->post('id');
        $count = $this->hotels_model->check_availability($start, $end, $hotel_id, $id);
        echo $count;
    }
    ...
    function unpublish($hotel_id) {

        $this->hotels_model->unpublish_hotel($hotel_id);
        $this->session->set_flashdata('success_message', 'Hotel Un-Published');
        redirect('hotels/hotel_crs_list');
    }

    function publish($hotel_id) {

        $this->hotels_model->publish_hotel($hotel_id);
        $this->session->set_flashdata('success_message', 'Hotel Published');
        redirect('hotels/hotel_crs_list');
    }

    function update_weekend_hotel($hotel_id = '') {

        $data = $this->input->post();

        $this->hotels_model->weekend_hotel($data);
        $this->session->set_flashdata('success_message', 'Hotel Published');
        $data = array();
        $data['status'] = true;
        echo json_encode($data);
