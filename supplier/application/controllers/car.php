...
class Car extends CI_Controller 
{
	...
	function getsupplier(){
        ...
        $supplier_id = $this->user_model->getsupplier($branch_id);
	...
	function pre_cancellation($app_reference, $booking_source)
	{
		if (empty($app_reference) == false && empty($booking_source) == false) {
			...
			if ($booking_details['status'] == SUCCESS_STATUS) {
				...
				$assembled_booking_details = $this->booking_data_formatter->format_car_booking_data($booking_details,$this->current_module);
	...
	function cancel_booking($app_reference, $booking_source)
	{
		if(empty($app_reference) == false) {
			...
			if ($master_booking_details['status'] == SUCCESS_STATUS) {
				...
				$this->load->library('booking_data_formatter');
				$master_booking_details = $this->booking_data_formatter->format_car_booking_data($master_booking_details, 'b2c');
