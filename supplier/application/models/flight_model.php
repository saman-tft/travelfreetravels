...
Class Flight_Model extends CI_Model
{
	...
	function b2b2b_flight_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		//$b2c_condition_array = array('U.user_type', '=', B2C_USER, ' OR ', 'BD.created_by_id', '=', 0);
	// debug($condition);die;
		...
		if ($count) {
			...
			//echo debug($condition);exit;
			$query = 'select count(distinct(BD.app_reference)) AS total_records from flight_booking_details BD
					  join user U on U.user_id = BD.sub_agent_id
					  join flight_booking_transaction_details as BT on BD.app_reference = BT.app_reference						
			...
			//Booking Details
			$bd_query = 'select BD.*,U.agency_name,U.first_name,U.last_name from flight_booking_details AS BD
					      join user U on U.user_id = BD.sub_agent_id join flight_booking_transaction_details as BT on BD.app_reference = BT.app_reference					      
						  WHERE  U.user_type='.SUB_AGENT.' AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						  order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
...
/**
	...
	function corporate_flight_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) AS total_records from flight_booking_details BD
					  join user U on U.user_id = BD.created_by_id
					  join flight_booking_transaction_details as BT on BD.app_reference = BT.app_reference						
					  where (U.user_type='.EMPLOYEE.' OR U.user_type='.CORPORATE_USER.' OR U.user_type='.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().''.$condition;
	...
	function sub_corporate_flight_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		//$b2c_condition_array = array('U.user_type', '=', B2C_USER, ' OR ', 'BD.created_by_id', '=', 0);
