...
/**
	...
	{
		if (empty($this->airline_markup) == true) {
			$response['specific_markup_list'] = $this->specific_airline_markup('corporate_flight');
			$response['generic_markup_list'] = $this->generic_domain_markup('corporate_flight');
			$this->airline_markup = $response;
		} else {
			$response = $this->airline_markup;
		}
	...
	function corporate_hotel_markup()
	{
		if (empty($this->hotel_markup) == true) {
			$response['specific_markup_list'] = '';
			$response['generic_markup_list'] = $this->generic_domain_markup('corporate_hotel');
			$this->hotel_markup = $response;
		} else {
			$response = $this->hotel_markup;
		}
...
//All active supplier list
	...
	function corporate_sightseeing_markup()
	{
		if (empty($this->sightseeing_markup) == true) {
			$response['specific_markup_list'] = '';
			$response['generic_markup_list'] = $this->generic_domain_markup('corporate_sightseeing');
			$this->sightseeing_markup = $response;
		} else {
			$response = $this->sightseeing_markup;
		}
	...
	function corporate_transferv1_markup(){
		if (empty($this->transferv1_markup) == true) {
			$response['specific_markup_list'] = '';
			$response['generic_markup_list'] = $this->generic_domain_markup('corporate_transfer');
			$this->transferv1_markup = $response;
		} else {
			$response = $this->transferv1_markup;
		}
	...
	function corporate_bus_markup()
	{
		if (empty($this->bus_markup) == true) {
			$response['specific_markup_list'] = '';
			$response['generic_markup_list'] = $this->generic_domain_markup('corporate_bus');
			$this->bus_markup = $response;
		} else {
			$response = $this->bus_markup;
		}
	...
	function master_transaction_request_list($type='provab', $data_list_filt=array(), $credit='')
	{
		...
		if($type!='corporate')
		{
			...
			where MTD.created_by_id=U.user_id AND U.user_type!='.SUB_AGENT.' AND MTD.type="'.$type.'" and MTD.domain_list_fk = '.get_domain_auth_id().' '.$data_list_cond.'
			and MTD.transaction_type ="Credit" order by MTD.status DESC, MTD.created_datetime DESC';
		}
		else{
		...
		else
		{
			...
			$query = 'select MTD.*, CONCAT(U.first_name, " ", U.last_name) request_user, U.email, U.agency_name AS requested_from from master_transaction_details MTD, user U
			where MTD.created_by_id=U.user_id AND U.user_type!='.SUB_CORPORATE.' AND MTD.type="'.$type.'" and MTD.domain_list_fk = '.get_domain_auth_id().' '.$data_list_cond.'
			and MTD.transaction_type ="Credit" order by MTD.status DESC, MTD.created_datetime DESC';
		}
		else{
...
// error_reporting(E_ALL);
		...
		$transaction_details_cond = array('origin' => intval($origin), 'system_transaction_id' => $system_request_id, 'type' =>$type);
		//Depending on status update
		// debug($transaction_details_cond);die;
		...
		// die;
				...
				if((!empty($type) == true) && ($type == 'Debit')){
					...
					// echo "In";
					// die;
// debug($transaction_owner_id);
// debug($amount);
// debug($type);
					$response['data']['agent_balance'] = $this->private_management_model->update_b2b_balance($transaction_owner_id, $amount,$type);
					// echo "AASA";
					// debug($response);
					// die;
	...
	function bank_account_details() 
	{
		$query='SELECT BAD.*,CONCAT(U.first_name," ",U.last_name) as created_by_name 
		        ...
		        where (U.user_type!=3 AND U.user_type!=12 AND U.user_type!=1) and BAD.created_by_id= '.intval($this->entity_user_id).' and  BAD.domain_list_fk='.get_domain_auth_id();
	...
	function bank_account_details1() 
	{
		$query='SELECT BAD.*,CONCAT(U.first_name," ",U.last_name) as created_by_name 
		        FROM bank_account_details BAD
		        JOIN user U on U.user_id=BAD.created_by_id
		        where BAD.domain_list_fk='.get_domain_auth_id().' and BAD.usr_type=0';
		$tmp_data = $this->db->query($query);
		if($tmp_data->num_rows()>0) {
			$tmp_data=$tmp_data->result_array();
			$data = array('status' => QUERY_SUCCESS, 'data' => $tmp_data);
	 ...
	 */
	function update_agent_balance($amount, $agent_user_id,$usr)
	{
		$current_balance = 0;
		$cond = array('user_oid' => intval($agent_user_id));
		if($usr=='corporate')
			$details = $this->custom_db->single_table_records('corporate_user_details', 'balance', $cond);
		else
		$details = $this->custom_db->single_table_records('b2b_user_details', 'balance', $cond);
		if ($details['status'] == true) {
			$details['data'][0]['balance'] = $current_balance = ($details['data'][0]['balance'] + $amount);
...
if($usr=='corporate')
	$this->custom_db->update_record('corporate_user_details', $details['data'][0], $cond);
else
			$this->custom_db->update_record('b2b_user_details', $details['data'][0], $cond);
...
/*
	...
	*/
	function get_corp_details($agent_id){
	...
	public function corp_account_ledger($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$data = array();
		$condition = $this->custom_db->get_custom_condition($condition);
		$agent_filter = '';
		$transaction_activated_from_date = '2017-01-10';//DONT REMOVE THIS CONDITION
		$agent_filter = ' AND U.user_type ='.CORPORATE_USER;	
	...
	*/
	...
	function corp_list(){
		...
		$query = 'select user_id, agency_name FROM user WHERE status = '.ACTIVE.' AND user_type = '.CORPORATE_USER.'
					order by user_id asc';
		$data = $this->db->query($query)->result_array();
		return $data;
	}
	 ...
	 * @param array $details
	 */
	function process_direct_credit_debit_transaction($details, $type='b2b')
	{
		// error_reporting(E_ALL);
		...
		$master_transaction_details['transaction_number'] = isset($details['transaction_number']) ? $details['transaction_number'] : 'N/A';
		$master_transaction_details['status'] = 'pending';
		$master_transaction_details['type'] = $type;
