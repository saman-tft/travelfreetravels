...
function corporate_bus_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) as total_records from bus_booking_details BD 
					...
					join bus_booking_itinerary_details AS ID on BD.app_reference=ID.app_reference 
					join payment_option_list as POL on POL.payment_category_code=BD.payment_mode 
					left join user as U on BD.created_by_id = U.user_id where U.user_type IN ('.EMPLOYEE.', '. CORPORATE_USER .', '.SUB_CORPORATE.') 
			...
			$booking_customer_details	= array();
			//debug($condition);die;
			$bd_query = 'select BD.*, BD.created_datetime as booked_on ,U.*,BD.status as status from bus_booking_details AS BD
					     left join user U on U.user_id = BD.created_by_id
						 WHERE  U.user_type IN ('.EMPLOYEE.', '. CORPORATE_USER .', '.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
	...
	function sub_corporate_bus_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) as total_records from bus_booking_details BD 
					...
					join bus_booking_itinerary_details AS ID on BD.app_reference=ID.app_reference 
					join payment_option_list as POL on POL.payment_category_code=BD.payment_mode 
					left join user as U on BD.created_by_id = U.user_id where U.user_type='.SUB_CORPORATE.' 
					AND  domain_origin='.get_domain_auth_id().''.$condition;
//debug($query); die;
			...
			$booking_itinerary_details	= array();
			$booking_customer_details	= array();
			$bd_query = 'select BD.* ,U.*,BD.status as status from bus_booking_details AS BD
					     left join user U on U.user_id = BD.created_by_id
	...
	function corporate_bus_report_old($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) as total_records from bus_booking_details BD 
					...
					join bus_booking_itinerary_details AS ID on BD.app_reference=ID.app_reference 
					join payment_option_list as POL on POL.payment_category_code=BD.payment_mode 
					left join user as U on BD.created_by_id = U.user_id where U.user_type='.CORPORATE_USER.' 
					AND  domain_origin='.get_domain_auth_id().''.$condition;
//debug($query); die;
...
//debug($query); die;
	...
	function b2b2b_bus_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$query = 'select count(distinct(BD.app_reference)) as total_records from bus_booking_details BD 
					...
					join bus_booking_itinerary_details AS ID on BD.app_reference=ID.app_reference 
					join payment_option_list as POL on POL.payment_category_code=BD.payment_mode 
					left join user as U on BD.sub_agent_id = U.user_id where U.user_type='.SUB_AGENT.' 
					AND  domain_origin='.get_domain_auth_id().''.$condition;
//debug($query); die;
			...
			$booking_customer_details	= array();
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from bus_booking_details AS BD
					     left join user U on U.user_id = BD.sub_agent_id
