...
function get_room_meal_list($id = '') {
    ...
    function b2b_crs_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	  {
		...
		if ($count) {
			$query = 'select count(distinct(BG.ref_id)) as total_records
					...
					where HD.hotel_type_id !='."'".VILLA."'".' AND U.user_type='.B2B_USER.$condition;
	...
	function b2b_crs_hotel_report_old($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
	...
	function get_ammenities_list_add($ammenities_id=''){
		$this->db->select('*');
		$this->db->from('hotel_hotel_amenities');
	...
	function get_ammenities_list($id='')
	{
		...
		return $this->db->get('hotel_hotel_amenities')->result();
	}
	...
	{
		if($this->db->insert('hotel_hotel_amenities',$data))
		{
			return TRUE;
		}
		return false;
	...
	function inactive_hotel_ammenity($ammenity_id){
		...
		$this->db->update('hotel_hotel_amenities', $data);
	...
	function get_room_ammenities_list($id='')
	{
		...
		return $this->db->get('hotel_room_amenities')->result();
	}
	function get_active_room_ammenities_list($ammenities_id=''){
		$this->db->select('*');
		$this->db->from('hotel_room_amenities');
		if($ammenities_id !='')
			$this->db->where('hotel_amenities_id', $ammenities_id);
		$this->db->where('status', 'ACTIVE');
	...
	{
		if($this->db->insert('hotel_room_amenities',$data))
		{
			return true;
		}
		return false;		
	...
	function inactive_room_ammenity($ammenity_id){
		...
		$this->db->where('id', $ammenity_id);
		$this->db->update('hotel_room_amenities', $data);
	...
	function inactive_room_price($id){
		...
		$this->db->where('id', $id);
		$this->db->update('hotel_room_prices', $data);
	...
	function active_room_cancellation($id){
		...
		$this->db->where('id', $id);
		$this->db->update('hotel_room_cancellation_policies', $data);
...
// 		echo $this->db->last_query();die;
	...
	function update_room_ammenity($input, $ammenity_id){
		...
		$this->db->where('id', $ammenity_id);
		$this->db->update('hotel_room_amenities', $update_data);
		
	}
	
	function delete_room_ammenity($ammenity_id){
		$this->db->where('id', $ammenity_id);
		$this->db->delete('hotel_room_amenities'); 
	...
	function get_hotel_types_list($id='')
	{
		...
		return $this->db->get('hotel_hotel_types')->result();
	}
	
	function add_hotel_type_details($data)
	{
		if($this->db->insert('hotel_hotel_types',$data))
		{
			return TRUE;
		}
		return false;
	...
	function inactive_hotel_type($hotel_type_id)
	{
		...
		$this->db->update('hotel_hotel_types', $data);		
	}
	...
	function update_hotel_type($input, $hotel_type_id){
		...
		$this->db->where('id', $hotel_type_id);
		$this->db->update('hotel_hotel_types', $update_data);
	}
	...
	{
		$this->db->where('created_by',$this->entity_user_id);
		return $this->db->get('hotel_hotels')->result();
	...
	function inactive_hotel($hotel_id){
		...
		$this->db->update('hotel_hotels', $data);
	...
	function get_hotel_crs_list($hotel_id='', $hotel){	
		...
		$this->db->where('h.created_by_id', $this->entity_user_id);
  ...
  function get_contatc_list($hotel_id){
	...
	{
		$this->db->delete('hotel_hotel_images', array('id' => $image_id));
	}
...
function get_active_room_types_list($room_type_id = ''){
	...
	function get_room_types_list($id='')
	{
		...
		return $this->db->get('hotel_room_types')->result();
	}
	...
	function inactive_room_types($room_type_id){
		...
		$this->db->update('hotel_room_types', $data);
	...
	function update_room_types($input, $room_type_id)
	{
		...
		$this->db->where('id', $room_type_id);
		$this->db->update('hotel_room_types', $update_data);		
	}
  ...
  // echo "in model";
   ...
   //debug($result);exit;
	...
	{
		if($this->db->insert('hotel_hotels',$data))
		{
	...
	{
			if($this->db->update('hotel_hotels',$data,['id'=>$id]))
			{
				return	$this->db->insert_id();
			}
			return	false;
	...
	{
			if($this->db->insert('hotel_hotel_images',$data))
			{
				return	TRUE;
			}
			return	false;
	...
	{
		return $this->db->get_where('hotel_room_prices',array('hotel_id'=>$hotel_id,'room_id'=>$room_id));
	}	
	
		function room_price_single($id)
	{
		return $this->db->get_where('hotel_room_prices',array('id'=>$id));
	}	
		
	function room_cancellation_data($id)
	{
		return $this->db->get_where('hotel_room_cancellation_policies',array('id'=>$id));
	}	
			
	function room_cancellation_list($hotel_id=0,$room_id=0)
	{
		return $this->db->get_where('hotel_room_cancellation_policies',array('hotel_id'=>$hotel_id,'room_id'=>$room_id));
	}	
	
	function get_hotel_data($hotel_id=0)
	{
		return $this->db->get_where('hotel_hotels',array('id'=>$hotel_id));
	}
	function get_hotel_images($hotel_id=0)
	{
		return $this->db->get_where('hotel_hotel_images',array('hotel_id'=>$hotel_id))->result();	
	}
	function delete_hotel_images($id=0)
	{
		return $this->db->delete('hotel_hotel_images',array('id'=>$id));	
	}
	function inactive_hotel_image($image_id){
		...
		$this->db->update('hotel_hotel_images', $data);		
	}
	...
	{
		if($this->db->insert('hotel_room_prices',$data))
		{
			return	true;
		}
		return	false;
	...
	{
		if($this->db->update('hotel_room_prices',$data,['id'=>$id]))
		{
			return	true;
		}
		return	false;
	...
	{
		if($this->db->insert('hotel_room_cancellation_policies',$data))
			{
				return	true;
			}
			return	false;
	...
	{
		if($this->db->update('hotel_room_cancellation_policies',$data,['id'=>$id]))
			{
				return	true;
			}
			return	false;
	...
	function get_crs_room_list($hotel_id)
	{
		...
		$this->db->from('hotel_rooms r');
		$this->db->join('hotel_room_types t','t.id=r.room_type_id');
	...
	{
		return $this->db->get_where('hotel_rooms',array('id'=>$id));
	}	
	...
	{
		if($this->db->insert('hotel_rooms',$data))
		{
			return	true;
		}
		return	false;
	...
	{
		if($this->db->update('hotel_rooms',$data,['id'=>$id]))
		{
			return	true;
		}
		return	false;
