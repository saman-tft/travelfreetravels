...
public function nationality_region()
 ...
 function get_supplier_details($id)
	...
	function booking($condition = array(), $count = false, $offset = 0, $limit = 100000000000) {		
		...
		if ($count) {
			$query = 'select COUNT(*) AS total_records from tour_booking_details AS BD  left join tours tr on tr.id = BD.tours_id WHERE 1=1 AND BD.supplier_id= "'.$this->entity_user_id.'" AND user_type="4" ' . $condition;
			//debug($query);exit();
			$data = $this->db->query ( $query )->row_array ();
			return $data ['total_records'];
		} else {		
			$bd_query = 'select BD.*,tr.agent_id from tour_booking_details AS BD  left join tours tr on tr.id = BD.tours_id WHERE 1=1  ' . $condition . ' and BD.supplier_id='."'".$this->entity_user_id."'".' order by BD.origin desc limit ' . $limit;
			$booking_details = $this->db->query ( $bd_query )->result_array ();
			//debug($bd_query);exit;
		...
		if ($count) {
			...
			$bd_query = 'select BD.*,tr.agent_id from tour_booking_details AS BD  left join tours tr on tr.id = BD.tours_id WHERE 1=1 AND user_type="3" ' . $condition . ' and tr.agent_id='."'".$this->entity_user_id."'".' order by BD.origin desc limit ' . $limit;
			$booking_details = $this->db->query ( $bd_query )->result_array ();
		//	debug($bd_query);exit;
	...
	public function get_tour_destinations()
	{
		...
		{
			$result[$res1['id']] = $res1['destination'];
		}
		return $result;
	}
	...
	public function AUTO_INCREMENT($table) {
        ...
        { 
	     ...
	     public function tour_list() {
	     	$query = 'select * from tours where admin_approve_status = 1 AND agent_id="'.$this->entity_user_id.'" AND status_delete != "1" order by id desc'; 
	...
	public function tours_enquiry($condition)
	{
		...
		$this->db->select('te.*,t.package_name');
		$this->db->from('tours t');
		$this->db->join('tours_enquiry te','t.id=te.tour_id');
		$this->db->where('t.supplier_id',$this->entity_user_id);
		...
		// // debug($data);exit();
		// $this->db->join('tours t', 't.id = te.tour_id');
		// $this->db->order_by('id','desc');
		// $query = $this->db->get("tours_enquiry te");
		// if($query->num_rows > 0)
		// {
		// 	$result['tours_enquiry']=$query->result_array();
...
/*		$exe   = mysql_query($query);
	...
	public function table_record_detailsold2022($table,$id)
	{
