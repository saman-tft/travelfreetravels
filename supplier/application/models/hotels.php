...
// 		debug($data['price_list']);die;
	...
	function save_room_price_data()
	{
		try 
		{
			...
			$this->form_validation->set_rules('four_adult', 'Four Adult Price', 'required');
			$this->form_validation->set_rules('one_child', 'One Child Price', 'required');
			$this->form_validation->set_rules('two_child', 'Double Child Price', 'required');
			if ($this->form_validation->run() == FALSE)
			{
				throw new 	Exception(validation_errors(), 1);
			}
			...
			$data['four_adult']=$this->input->post('four_adult');
			$data['one_child']=$this->input->post('one_child');
			$data['two_child']=$this->input->post('two_child');
			$data['status']=$this->input->post('status');
			// debug($data);die;
