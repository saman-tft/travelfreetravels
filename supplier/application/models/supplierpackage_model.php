...
class Supplierpackage_Model extends CI_Model {
	...
	public function with_price() {
		$q = $this->db->query ( 'select * from package where supplier_id="'.$this->entity_user_id.'" and module_type="activity" order by package_id DESC' );
		if ($q->num_rows () > 0) {
			return $q->result ();
		}
		return array ();
	...
	public function update_package_type($add_package_data, $pack_id) {
		//debug($pack_id);exit();
		$this->db->where ( 'package_types_id', $id );
