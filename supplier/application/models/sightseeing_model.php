...
function b2b_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	...
	function b2b2b_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
	...
	function corporate_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join sightseeing_booking_itinerary_details AS SBID on BD.app_reference=SBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.created_by_id = U.user_id  where U.user_type IN ('.EMPLOYEE.', '. CORPORATE_USER .', '.SUB_CORPORATE.')  AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			...
			$cancellation_details = array();
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from sightseeing_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id WHERE  U.user_type IN ('.EMPLOYEE.', '. CORPORATE_USER .', '.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						 order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			$booking_details = $this->db->query($bd_query)->result_array();
			$app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
			if(empty($app_reference_ids) == false) {
	...
	function sub_corporate_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
