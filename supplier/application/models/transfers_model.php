...
class Transfers_Model extends CI_Model {
    ...
    public function transfer_list($user_type) {
		$q = $this->db->query ( "select a.*,U.first_name,U.last_name from transfer_info as a join user U on U.user_id = a.created_by_id where a.user_type=".$user_type." and a.created_by_id=".$this->entity_user_id." order by a.id DESC" );
		if ($q->num_rows () > 0) {
			return $q->result ();
		}
		return array ();
 ...
 function bookingcrs($condition=array(), $count=false, $offset=0, $limit=100000000000)
    {
        ...
        if ($count) {
            $query = 'select count(distinct(BD.app_reference)) as total_records 
                    ...
                    left join user U on U.user_id = BD.created_by_id
					left join user_type UT on UT.origin = U.user_type
                    left join transfer_info TI on TI.id = BD.tours_id
                    where (U.user_type='.B2B_USER.' OR BD.created_by_id = 0) and  BD.domain_origin='.get_domain_auth_id().'  and TI.created_by_id="'.$this->entity_user_id.'" '.$condition;
            // echo $query;exit;
                    // join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
            $data = $this->db->query($query)->row_array();
            // debug($data);exit;
            ...
            $booking_customer_details   = array();
            $cancellation_details = array();
            $bd_query = 'select *,BD.created_datetime as created_datetime,BD.status as status from transfer_booking_details AS BD 
           				 left join user U on U.user_id = BD.created_by_id
							left join user_type UT on UT.origin = U.user_type
							left join transfer_info TI on TI.id = BD.tours_id
                        WHERE (U.user_type='.B2B_USER.' OR BD.created_by_id = 0) and BD.domain_origin='.get_domain_auth_id().' and TI.created_by_id="'.$this->entity_user_id.'" and BD.module_type = "transfers" '.$condition.'
                        order by BD.origin desc limit '.$offset.', '.$limit;
            $booking_details = $this->db->query($bd_query)->result_array();
            $app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
            if(empty($app_reference_ids) == false) {
...
// debug($response);exit;
	...
	{
		$query = 'select * from all_nationality_region where  module="transfers" order by name';
		$result = $this->db->query($query);
     	return  $result->result_array();
	}
