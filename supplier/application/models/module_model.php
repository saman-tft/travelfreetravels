...
//		}
	...
	function get_user_privillages(){
		...
		$query = 'select PL.privilege_key,PL.url from privilege_list_new PL JOIN privileges AS P ON P.p_no = PL.origin where  P.user_id = '.$user;
// echo $query;die;
...
//	debug($privileges);exit;		
	...
	function get_user_privillagesold2022(){
		$CI = & get_instance ();
		...
		// debug($user_type);
		// die;
		$query = 'SELECT privilleges  FROM `sub_admin_privillege_list` WHERE `uid` = '.$user;

		$privileges = $this->db->query($query)->result_array($query);
		// debug($privileges);
		// exit;
		if(count($privileges)>0)
		$arr = explode(",", $privileges[0]['privilleges']);
	else
		$arr=0;
		return $arr;
		}
	...
	function promo_code_list($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if(!$count) {
			//debug($condition);die;
			$query = 'select * from promo_code_list WHERE 1=1 '.$condition. ' order by created_datetime desc limit '.$offset.', '.$limit;
			//echo $query;die;
			return $this->db->query($query)->result_array();
		} else {
	...
	public function promo_show_on_modal($origin,$value) {
		$this->db->set('display_home_page', $value); 
		$this->db->where('origin', $origin); 
		$this->db->update('promo_code_list');
