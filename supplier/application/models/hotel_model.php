...
$booking_source='PTBSID0000000011';
		...
		if ($count) {
			...
			$query = "select count(distinct(BD.app_reference)) as total_records
					...
					where (U.user_type='.B2C_USER.' OR BD.created_by_id=0 OR BD.created_by_id!=0) AND BD.booking_source='".$booking_source."' AND BD.supplier_id='".$this->entity_user_id."'  AND  BD.domain_origin=".get_domain_auth_id()." ".$condition."";
...
function b2b_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	...
	function b2b2b_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join hotel_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.sub_agent_id = U.user_id  where U.user_type='.SUB_AGENT.' AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			return $data['total_records'];
		} else {
			$this->load->library('booking_data_formatter');
			...
			$cancellation_details = array();
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from hotel_booking_details AS BD
					     left join user U on BD.sub_agent_id =U.user_id 					     
						 WHERE  U.user_type='.SUB_AGENT.' AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						 order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			$booking_details = $this->db->query($bd_query)->result_array();
			$app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
			if(empty($app_reference_ids) == false) {
...
/**
	 ...
	 */
	   ...
	   function corporate_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join hotel_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.created_by_id = U.user_id  where (U.user_type='.EMPLOYEE.' OR U.user_type='.CORPORATE_USER.' OR U.user_type='.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			return $data['total_records'];
		} else {
			$this->load->library('booking_data_formatter');
			...
			$cancellation_details = array();
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from hotel_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id WHERE  (U.user_type='.EMPLOYEE.' OR U.user_type='.CORPORATE_USER.' OR U.user_type='.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						 order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			$booking_details = $this->db->query($bd_query)->result_array();
			$app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
			if(empty($app_reference_ids) == false) {
	...
	function sub_corporate_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join hotel_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.created_by_id = U.user_id  where (U.user_type='.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			return $data['total_records'];
		} else {
			$this->load->library('booking_data_formatter');
			...
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name from hotel_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id 					     
						 WHERE  (U.user_type='.SUB_CORPORATE.') AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'
						 order by BD.created_datetime desc, BD.origin desc limit '.$offset.', '.$limit;
			$booking_details = $this->db->query($bd_query)->result_array();
			$app_reference_ids = $this->booking_data_formatter->implode_app_reference_ids($booking_details);
			if(empty($app_reference_ids) == false) {
