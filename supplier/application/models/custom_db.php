...
Class Custom_Db extends CI_Model
{
	...
	function get_tbranch_list($corp_id,$branch_id){
	
		$query = 'SELECT * FROM user where user_id='.$corp_id.' and user_type='.$branch_id.'';
		//echo $query;exit;
	...
	function get_parent_name($userid)
	{
		if($userid!='')
		{
			...
			$query='SELECT * from user where user_id='.$this->entity_user_id.'';
			return $this->db->query($query)->result_array();
		}
		//echo $query;exit;
	}
