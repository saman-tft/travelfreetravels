...
Class User_Model extends CI_Model
{
	...
	{
		$branch_id1="select supplier_id,branch_id from user where user_id=$branch_id";
		$branch=$this->db->query($branch_id1)->result_array();
		// print_r($branch);die;
		$b=$branch[0]['branch_id'];
		$s=$branch[0]['supplier_id'];
		$query = "SELECT * FROM user where branch_id=$branch_id and user_type=8 and supplier_id=$s";
		// echo $query;exit;
		return $this->db->query($query)->result_array();

	}
	function activate_account_status($status, $email_activation, $user_id)
	{
		$data = array(
				'status' => $status,
	...
	function get_domain_user_list($condition=array(), $count=false, $offset=0, $limit=10000000000, $order_by=array())
	{
		...
		if (!$count) {
			...
			//  debug("SELECT U.*, UT.user_type as user_type_name, ACL.country_code as country_code_value,
			// MAX(LM.login_date_time) as last_login, min(LM.logout_date_time) as logout_date_time
			// FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin INNER JOIN api_country_list AS ACL ON U.country_code=ACL.origin
			// LEFT JOIN login_manager LM ON LM.user_id=U.uuid  WHERE 1=1"
		 // . $filter_condition . ' group by U.user_id ' . $filter_order_by . ' limit ' . $limit . ' offset ' . $offset );exit();
	...
	function get_admin_details($data){
		$query = 'SELECT * FROM user U where U.password = "'.($data['password']).'" and U.email ="'.$data['user_name'].'" and U.status ='.$data['status'].' and U.user_type = 8'; 
	...
	function get_supplier_privillages($uid)
	{
		$query = "SELECT * FROM supplier_crs_privilage  where supplier_id='$uid'" ; 
...
function get_corporate_info($ut,$user_id){
		if($ut==MANAGER || $ut==EMPLOYEE || $ut==BRANCH)
		{
$query="select * from user where user_id=".$user_id;
$select=$this->db->query($query)->result_array();
$corp_id=$select[0]['corporate_id'];
		}
else
{
	$corp_id=$user_id;
		...
		// die;
		$query = 'select U.*,BU.logo from user AS U
					      join  corporate_user_details BU on U.user_id = BU.user_oid
				          join  currency_converter CUC on CUC.id = BU.currency_converter_fk
						  WHERE  U.user_type='.CORPORATE_USER.' AND U.user_id='.$corp_id;
...
public function get_corporate_detail($user_id=0)
	{
		$this->db->select('U.*,BU.logo');
		$this->db->from('user U');
		$this->db->join('corporate_user_details BU','U.user_id = BU.user_oid');
		$this->db->join('currency_converter CUC','CUC.id = BU.currency_converter_fk');
		$this->db->where('U.user_id',$user_id);
		return $this->db->get()->row_array();
	}


	function b2b_agency_list($condition = array(), $count = false, $offset = 0, $limit = 10000000000, $order_by = array()) {
		$agency_list = $this->get_b2b_user_list ( $condition, $count, $offset, $limit, $order_by );

		// debug($agency_list);exit();
		$reporting_to = array ();
		if ($count == false && valid_array ( $agency_list ) == true) {
			$tmp_user_list = array ();
			$user_id_lst = array ();
			...
			$reporting_to = $this->user_reporting_to ( $user_id_lst );
			$agency_list = array ();
			$agency_list ['agency_name'] = $reporting_to;
		}
		return $agency_list;
	}
	...
	function get_b2b_user_list($condition = array(), $count = false, $offset = 0, $limit = 20000000000, $order_by = array()) {
		$filter_condition = ' and ';
		if (valid_array ( $condition ) == true) {
			foreach ( $condition as $k => $v ) {
				$filter_condition .= implode ( $v ) . ' and ';
		...
		if (is_domain_user () == false) {
			// PROVAB ADMIN
			// GET ALL DOMAIN ADMINS DETAILS
			$filter_condition .= ' U.domain_list_fk > 0 and U.user_type = ' . ADMIN . ' and ';
		} else if (is_domain_user () == true) {
			// DOMAIN ADMIN
			// GET ALL DOMAIN USERS DETAILS
			$filter_condition .= ' U.domain_list_fk =' . get_domain_auth_id () . ' and U.user_type != ' . ADMIN . ' and ';
		}
		$filter_order_by = ' ORDER BY b2b_a.due_amount ASC, b2b_a.balance desc, b2b_a.credit_limit DESC ';
		if (valid_array ( $order_by ) == true) {
			foreach ( $order_by as $k => $v ) {
				$filter_order_by .= implode ( $v ) . ',';
			}
		...
		if (! $count) {
			// $limit = 16;
			$query = 'SELECT U.*, b2b_a.reporting_to_id, b2b_a.logo as agent_logo,(select (uuid) from user where user_id=b2b_a.reporting_to_id ) as reporting_to,
				b2b_a.balance as agent_balance, b2b_a.due_amount as agent_due_amount, b2b_a.credit_limit as agent_credit_limit,
				UT.user_type as user_type_name, ACL.country_code as country_code_value,
			MAX(LM.login_date_time) as last_login, min(LM.logout_date_time) as logout_date_time
			FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin INNER JOIN api_country_list AS ACL ON U.country_code=ACL.origin
			JOIN b2b_user_details AS b2b_a ON U.user_id=b2b_a.user_oid LEFT JOIN login_manager LM ON LM.user_id=U.uuid WHERE 1=1
		 	' . $filter_condition . ' group by U.user_id ' . $filter_order_by . ' limit ' . $limit . ' offset ' . $offset;
			// debug($query);exit;
			return $this->db->query ( $query )->result_array ();
		} else {
			$query = 'SELECT count(*) as total FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin INNER JOIN api_country_list AS ACL ON U.country_code=ACL.origin JOIN b2b_user_details AS b2b_a ON U.user_id=b2b_a.user_oid 
		 WHERE U.user_type=UT.origin
		 AND U.country_code=ACL.origin' . $filter_condition . ' limit ' . $limit . ' offset ' . $offset;
		 // debug($query);exit;
			return $this->db->query ( $query )->row ();
		}
	}

	function user_reporting_to($user_id_list) {
		// echo "<pre>"; print_r($user_id_list); echo "</pre>"; die();
		$user_rep = array ();
		$user_id_cond = '';
		if (valid_array ( $user_id_list ) == true) {
			for($i = 0; $i < count ( $user_id_list ); $i ++) {
				$query = "select `created_by_id` from `user` where `user_id`='$user_id_list[$i]'";
				$reporting_id_array = $this->db->query ( $query )->result_array ();
				$id = $reporting_id_array [0] ['created_by_id'];
				$query1 = "select `agency_name` from `user` where `user_id`='$id'";
				$reporting_id_array1 = $this->db->query ( $query1 )->result_array ();
				$user_rep [] = @$reporting_id_array1 [0] ['agency_name'];
			}
		}
		// echo "<pre>"; print_r($user_rep); echo "</pre>"; die();
		return $user_rep;
	}
