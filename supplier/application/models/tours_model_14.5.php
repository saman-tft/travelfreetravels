...
function booking($condition = array(), $count = false, $offset = 0, $limit = 100000000000) {
	// error_reporting(E_ALL);
		...
		$condition = $condition_static.$this->custom_db->get_custom_condition ( $condition );
		if ($count) {
			$query = 'select COUNT(*) AS total_records from package_booking_details AS BD WHERE 1=1 ' . $condition;
			$data = $this->db->query ( $query )->row_array ();
			return $data ['total_records'];
		} else {		
...
// debug($pack_attr);
// die;
			$bd_query = 'select BD.* from package_booking_details AS BD where 1=1 '. $condition . ' order by BD.origin desc limit ' . $limit;
			// debug($bd_query);
			// die;
			$booking_details = $this->db->query ( $bd_query )->result_array ();
$rr=0;
