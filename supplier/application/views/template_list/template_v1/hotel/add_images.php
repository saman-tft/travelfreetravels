...
<style> 
 ...
 .htlcrs_images .btn{ width: 100px;
    text-align: center;}
    .htlcrs_images button.btn.btn-green.my-actve {
    background-color: #007d3d !important;}
    .btn-danger.btn-icon.btn-sm.icon-left {padding-left: 5px;}
    .btn-sm, .btn-xs {
    padding: 2px 10px;}
</style>	
	...
	<div class="panel panel-default clearfix">
		...
		<!-- PANEL HEAD START -->
			...
			<div class="panel-body">
				<form method="post" id="hotel11" name="hotel11" action="<?php echo base_url()."/index.php/hotel/upload_hotel_image"; ?>" class="form-horizontal form-groups-bordered validate" enctype= "multipart/form-data">				
...
<input type="hidden" name="hotel_id" value="<?= $hotel_data->id?>">
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
									<!--<a href="<?php echo base_url()?>/hotels/hotel_crs_list/" class="btn btn-primary addnwhotl">Back to Hotels</a>-->
									<a href="<?php echo base_url().'index.php/hotel/room_crs_list/'.$hotel_data->id;?>" class="btn btn-info addnwhotl">Go to Rooms</a>
									</div>
								</div>
				</form>
			</div>
			...
			<div class="table-responsive">
				<form action="" method="POST" autocomplete="off">
					<table class="table table-striped htlcrs_images">
						<tr>
							<th>Sl No</th>
							<th>Image</th>
							<th>Status</th>
						...
						<tr>
							...
							<td class="center">
								...
								if($list->status == "ACTIVE")
									{ 
								?>
									<a href="<?php echo base_url()."index.php/hotel/inactive_hotel_image/".$list->hotel_id."/".$list->id; ?>" class="btn btn-orange btn-sm btn-icon icon-left my-inactve"><i class="entypo-eye"></i>InActive</a>
								<?php 
								}
								else
								{ ?>
									<a href="<?php echo base_url()."index.php/hotel/active_hotel_image/".$list->hotel_id."/".$list->id; ?>" class="btn btn-green btn-sm btn-icon icon-left my-actve"><i class="entypo-check"></i>Active</a>
								<?php 
								} 
								?>
									 <a href="<?php echo base_url()."index.php/hotel/delete_hotel_images/".$list->hotel_id."/".$list->id; ?>"  class="btn btn-danger btn-sm btn-icon icon-left"><i class="entypo-cancel"></i>Delete</a>
							</td>
						</tr>
					<?php 
					}
	...
	<script>
		 jQuery(document).ready(function($){
                    ...
                    if (country_id != '') {
                        ...
                        $.ajax({
                            url: '<?php echo base_url(); ?>/hotel/get_city_name/' + country_id + '/' + from_selected_city_id,
                    ...
                    if (city_id != '') {
                        ...
                        $.ajax({
                            url: '<?php echo base_url(); ?>/hotel/get_location_name/' + city_id + '/' + selected_location_id,
                            success: function (data, textStatus, jqXHR) {                            
		...
		function select_city(country_id){				 
		 if (country_id != '') {         	  
          var select1 = $('#city_name'); 
          $.ajax({
            url: '<?php echo base_url(); ?>/hotel/get_city_name/' + country_id,
            success: function (data, textStatus, jqXHR) {                                    
              select1.html('');
              select1.html(data);
              select1.trigger("chosen:updated");  
		...
		function select_location(city_id){				 
		 if (city_id != '') {         	  
          var location_select = $('#location_info');          
          $.ajax({
            url: '<?php echo base_url(); ?>/hotel/get_location_name/' + city_id,
            success: function (data, textStatus, jqXHR) {                                    
              location_select.html('');
              location_select.html(data);
              location_select.trigger("chosen:updated");  
	...
	function getAddress2(latLng) {				
	  ...
	  /*function remove_image(hotel_id,image_name,id){
		if(hotel_id != '' && image_name != '') {
			$.ajax({
						url: '<?php echo base_url(); ?>/hotel/unlink_image/' + hotel_id + '/' + image_name,
						success: function (data, textStatus, jqXHR) {                            
							  $("#img_" + id).remove();                                                                       		   			
						}
					 });
	  ...
	  function remove_image(hotel_id,image_name,id){
		if(hotel_id != '' && image_name != '') {
					...
					$.ajax({
						url: '<?php echo base_url(); ?>index.php/hotel/unlink_image/' + hotel_id + '/' + image_name,
						success: function (data, textStatus, jqXHR) {   
							$("#img_" + id).remove(); 
