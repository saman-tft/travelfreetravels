...
<!-- <li role="presentation" class="<?=$act_tab?>"><a href="<?=base_url()?>index.php/report/b2b_<?=($v)?><?= $append_query_string ?>"><i class="<?=get_arrangement_icon($k)?>"></i> <?='B2B '.$v.' Report'?></a></li> -->
			<?
		}
		else
		{
	?>
		<li role="presentation" class="<?=$act_tab?>"><a href="<?=base_url()?>index.php/report/b2b2b_<?=($v).'_report'?><?= $append_query_string ?>"><i class="<?=get_arrangement_icon($k)?>"></i> <?='B2B2B '.$v.' Report'?></a></li>
	<?php
	}
}
}
