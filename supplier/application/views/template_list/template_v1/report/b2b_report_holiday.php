...
$pack_attr=json_decode($parent_v['attributes'],1);
				...
				$action .= holiday_voucher($app_reference, $booking_source, $status,'b2b');
				$action.='<br/>';
				$action .= holiday_pdf($app_reference, $booking_source, $status,'b2b');
				$action.='<br/>';
...
$voucher_date=$parent_v['created_datetime'];
 $grand_total=$pack_attr['total_markup']+$pack_attr['cv']+$pack_attr['tour_totalamount']+$pack_attr['gst']-$pack_attr['discount'];
		$report_data .= '<tr>
					...
					<td>'.$currency.'&nbsp;'.$pack_attr['discount'].'</td>
