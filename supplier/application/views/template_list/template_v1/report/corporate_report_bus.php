...
if (is_array($search_params)) {
	...
	//echo '<pre>'; print_r($created_by_id);die;
}
	...
	<div class="panel panel-default clearfix"><!-- PANEL WRAP START -->
		...
		<div class="panel-body">
			...
			<div id="show-search" class="collapse">
				...
				<form method="GET" autocomplete="off" action="<?= base_url().'report/corporate_bus_report?'?>">
					...
					<div class="clearfix form-group">
						<div class="col-xs-4">
							...
							<select class="form-control" name="created_by_id">
								<option>All</option>
								<?=generate_options($agent_details, $created_by_id)?>
							</select>
						</div>
						<div class="col-xs-4">
							<label>
						...
						<div class="col-xs-4">
							...
							<select class="form-control" name="status">
								<option>All</option>
								<?=generate_options($status_options, $status)?>
							</select>
						</div>
						<div class="col-xs-4">
							<label>
						...
						<!-- <div class="col-xs-4">
							...
							<option value="01-04" <?= ($quarterly == '01-04') ? "selected" : '' ?>>JAN-APR</option>
							<option value="05-08" <?= ($quarterly == '05-08') ? "selected" : '' ?>>MAY-AUG</option>
							<option value="09-12" <?= ($quarterly == '09-12') ? "selected" : '' ?>>SEP-DEC</option>
							</select>
						</div>
						<div class="col-xs-4">
							<label>
							...
							<option value="">Select Month</option>
								<option value="01" <?= ($month_wise == '01') ? "selected" : '' ?>>JAN</option>
								<option value="02" <?= ($month_wise == '02') ? "selected" : '' ?>>FEB</option>
								<option value="03" <?= ($month_wise == '03') ? "selected" : '' ?>>MAR</option>
								<option value="04" <?= ($month_wise == '04') ? "selected" : '' ?>>APR</option>
								<option value="05" <?= ($month_wise == '05') ? "selected" : '' ?>>MAY</option>
								<option value="06" <?= ($month_wise == '06') ? "selected" : '' ?>>JUN</option>
								<option value="07" <?= ($month_wise == '07') ? "selected" : '' ?>>JUL</option>
								<option value="08" <?= ($month_wise == '08') ? "selected" : '' ?>>AUG</option>
								<option value="09" <?= ($month_wise == '09') ? "selected" : '' ?>>SEP</option>
								<option value="10" <?= ($month_wise == '10') ? "selected" : '' ?>>OCT</option>
								<option value="11" <?= ($month_wise == '11') ? "selected" : '' ?>>NOV</option>
								<option value="12" <?= ($month_wise == '12') ? "selected" : '' ?>>DEC</option>
 ...
 <script type="text/javascript">
      $('#year_wise').datepicker({
         ...
         format: 'yyyy'
					...
					<a href="<?=base_url().'report/corporate_bus_report?' ?>" id="clear-filter" class="btn btn-primary">Clear Filter</a>
					</div>
						...
						<?php
							...
							if(valid_array($table_data['booking_details']) == true) {
					        	...
					        	foreach($booking_details as $parent_k => $parent_v) {
									...
									$voucher_btn = bus_voucher($app_reference, $booking_source, $status,'corporate');
									$invoice = bus_invoice($app_reference, $booking_source, $status,'corporate');
									$cancel_btn = bus_cancel($app_reference, $booking_source, $status,'corporate');
									$pdf_btn= bus_pdf($app_reference, $booking_source, $status,'corporate');
									$email_btn = bus_email($app_reference, $booking_source,$status,provab_decrypt($email));
                                    ...
                                    $get_count= $this->custom_db->single_table_records ( 'bus_booking_customer_details','count(*)',array('app_reference'=> $app_reference));
								...
								?>
									<tr>
										...
										<td><?php echo date('d-m-Y', strtotime($booked_on))?></td>
										...
										<td><?php echo ($fare+$admin_markup+$gst)?></td>
		  ...
		  if(email != ''){
			  if(!emailReg.test(email)){
				  $('#mail_voucher_error_message').empty().text('Please Enter Correct Email Id');
					...
					var _opp_url = app_base_url+'index.php/voucher/b2e_bus_voucher/';
...
function bus_email($app_reference, $booking_source,$status,$recipient_email)
{
...
function get_cancellation_details_button($app_reference, $booking_source, $master_booking_status, $booking_customer_details)
{
	...
	if($status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'index.php/bus/cancellation_refund_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$master_booking_status.'" class="col-md-12 btn btn-sm btn-info "><i class="fa fa-info"></i> Cancellation Details</a>';
	}
}
?>
<script>
