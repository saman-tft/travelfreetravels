	...
	$report_data .= '<table class="table table-condensed table-bordered" id="b2c_report_hotel_table">
		...
		if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
		    ...
		    foreach($booking_details as $parent_k => $parent_v) { 
		        	...
		        	extract($parent_v);
		        //	debug($payment_details[0]['currency']);die;
//$currency=$payment_details[0]['currency'];
				$action = '';
				$email='';
				$tdy_date = date ( 'Y-m-d' );
		...
		$report_data .= '<tr>
					...
					<td>'.$payment_details[0]['currency'].''.($fare).'</td>
					<td class="hide">'.$currency.''.($itinerary_details[0]['TDS']).'</td>
					<td>'.$payment_details[0]['currency'].''.$admin_markup.'</td>
					<td>'.$payment_details[0]['currency'].''.$convinence_amount.'</td>
				
					<td>'.$payment_details[0]['currency'].''.$discount.'</td>
					<td>'.$payment_details[0]['currency'].''.$grand_total.'</td>
