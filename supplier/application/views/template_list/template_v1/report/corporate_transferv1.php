	...
	<div class="panel panel-default clearfix"><!-- PANEL WRAP START -->
		...
		<div class="panel-body">
			...
			<div id="show-search" class="collapse">
				...
				<form method="GET" autocomplete="off" action="<?= base_url().'report/corporate_transfers_report?'?>">
					...
					<a href="<?=base_url().'report/corporate_transfers_report?' ?>" id="clear-filter" class="btn btn-primary">Clear Filter</a>
					</div>
		...
		<div class="clearfix table-responsive"><!-- PANEL BODY START -->
					<div class="pull-left">
						<?php echo $this->pagination->create_links();?> <span class="">Total <?php echo $total_rows ?> Bookings</span>
									...
									<table class="table table-condensed table-bordered" id="corporate_report_airline_table">
						<thead>
							<tr>
								<th>Sno</th>
						...
						<?php
							...
							if(valid_array($table_data['booking_details']) == true) {
					        	...
					        	foreach($booking_details as $parent_k => $parent_v) {
									...
									$voucher_btn = transfers_voucher($app_reference, $booking_source, $status, 'corporate');
									
									if($status=='BOOKING_CONFIRMED'){
										$cancel_btn = cancel_transfers_booking($app_reference, $booking_source, $status,'corporate');
									}
									
									$pdf_btn= transfers_pdf($app_reference, $booking_source, $status,'corporate');
								...
								?>
									<tr>
										...
										<td><?php echo $gst ?></td>
										<td><?php echo $admin_net_fare+$admin_markup+$gst?></td>
		  ...
		  if(email != ''){
			  if(!emailReg.test(email)){
				  $('#mail_voucher_error_message').empty().text('Please Enter Correct Email Id');
					...
					var _opp_url = app_base_url+'index.php/voucher/b2e_transfers_voucher/';
