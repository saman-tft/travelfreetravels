	...
	<div class="panel panel-default clearfix"><!-- PANEL WRAP START -->
		...
		<div class="panel-body">
			...
			<div id="show-search" class="collapse">
				<form method="GET" autocomplete="off" action="<?= base_url().'report/corporate_hotel_report?'?>">
					...
					<a href="<?php echo base_url().'index.php/report/corporate_hotel_report?' ?>" id="clear-filter" class="btn btn-primary">Clear Filter</a>
					</div>
				</form>
			</div>
		</div>
	...
	$report_data .= '<table class="table table-condensed table-bordered" id="b2b_report_hotel_table">
		<thead>
			<tr>
				...
				<th>Fare</th>
				<th>Admin Markup</th>
				<!--<th>Corporate Markup</th>-->
				<th>GST</th>
				<!--<th>Amount Deducted<br/> from Corporate</th>-->
				<th>Total Amount</th>
				<th>BookedOn</th>
				<th>Status</th>
				<th>Action</th>
		...
		if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
		    ...
		    foreach($booking_details as $parent_k => $parent_v) { 
		       //debug($parent_v);exit;
				...
				$action .= hotel_voucher($app_reference, $booking_source, $status,'corporate');
				$action.='<br/>';
				$action .= hotel_pdf($app_reference, $booking_source, $status,'corporate');
				$action.='<br/>';
		...
		$report_data .= '<tr>
					...
					<td>'.$currency.''.round($fare + $admin_markup + $gst).'</td>
			 ...
			 if(email != ''){ 
				  if(!emailReg.test(email)){
					  $('#mail_voucher_error_message').empty().text('Please Enter Correct Email Id');
						...
						var _opp_url = app_base_url+'index.php/voucher/b2e_hotel_voucher/';
