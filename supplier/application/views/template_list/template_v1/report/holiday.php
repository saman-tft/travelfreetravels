...
<div class="bodyContent col-md-12">
    <div class="panel panel-default clearfix">
        ...
        <div class="panel-body">
            ...
            <form action="<?=base_url().'index.php/report/holiday/'.$module_type?>" method="GET" autocomplete="off">
                <div class="clearfix form-group">
                    <div class="col-xs-4">
                        <label> Reservation Code </label> <input type="text" class="form-control" name="app_reference"
                            value="<?=@$app_reference?>" placeholder="Reservation Code">
                    </div>
                    <div class="col-xs-4">
                        <label> Phone </label> <input type="text" class="form-control mobile" name="phone"
                            value="<?=@$phone?>" placeholder="Phone">
                    </div>
                    <div class="col-xs-4">
                        <label> Email </label> <input type="text" class="form-control" name="email" value="<?=@$email?>"
                            placeholder="Email">
                    </div>
                    <div class="col-xs-4">
                        <?php 
            ...
            ?>
                    ...
                    <div class="col-xs-4">
                        <label> Booked From Date </label> <input type="text" readonly id="created_datetime_from"
                            class="form-control" name="created_datetime_from" value="<?=@$created_datetime_from?>"
                            placeholder="Request Date">
                    </div>
                    <div class="col-xs-4">
                        <label> Booked To Date </label> <input type="text" readonly id="created_datetime_to"
          ...
          ?>
                    <ul class="cust_list">
                        <li><label><input type="checkbox" name="column_name[]" value="Sno"
                                    <?=check_colume('Sno')?>>Sno</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Action"
                                    <?=check_colume('Action')?>>Action</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Booking Reference"
                                    <?=check_colume('Booking Reference')?>> Reservation Code</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Status"
                                    <?=check_colume('Status')?>> Status</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Package Name"
                                    <?=check_colume('Package Name')?>>Package Name</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Email" <?=check_colume('Phone')?>>
                                Email</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Phone" <?=check_colume('Phone')?>>
                                Phone</label> </li>

                        <!--  <li><label><input type="checkbox" name="column_name[]" value="API Reference" <?=check_colume('Phone')?>> API Reference Number</label> </li> -->

                        <li><label><input type="checkbox" name="column_name[]" value="Passanger Name"
                                    <?=check_colume('Passanger Name')?>> Passanger Name</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Departure Date"
                                    <?=check_colume('Departure Date')?>> Departure Date</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Number Of Days"
                                    <?=check_colume('Number Of Days')?>> Number Of Days</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Package Price"
                                    <?=check_colume('Package Price')?>> Package Price</label> </li>
                        ...
                        <li><label><input type="checkbox" name="column_name[]" value="Promocode Amount"
                                    <?=check_colume('Promocode Amount')?>>Promocode Amount</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Markup"
                                    <?=check_colume('Markup')?>> Markup</label> </li>
                        <li><label><input type="checkbox" name="column_name[]" value="CF"> Convenience Fee</label> </li>
                        <li><label><input type="checkbox" name="column_name[]" value="VAT" <?=check_colume('VAT')?>>
                                VAT</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Total Fare"
                                    <?=check_colume('Total Fare')?>> Total Fare</label> </li>
                        <li><label><input type="checkbox" name="column_name[]" value="Grand_total">Grand Total</label>
                        </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Currency"
                                    <?=check_colume('Currency')?>>Currency</label> </li>

                        <!--  <li><label><input type="checkbox" name="column_name[]" value="Payment Mode" <?=check_colume('Payment Mode')?>> Payment Mode</label> </li>-->

                        <li><label><input type="checkbox" name="column_name[]" value="BookedOn"
                                    <?=check_colume('BookedOn')?>> BookedOn</label> </li>

                        <li><label><input type="checkbox" name="column_name[]" value="Billing Type"
                                    <?=check_colume('Billing Type')?>> Billing Type</label> </li>
      ...
      /*if (isset ( $table_data ) == true and valid_array ( $table_data ['booking_details'] ) == true) {
        ...
        <div class="row">
            <a
                href="<?php echo base_url(); ?>index.php/report/hotel/excel<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>">
                <button class="btn btn-primary btn-xs" type="button">Export to Excel</button>
            </a>
            <a href="<?php echo base_url(); ?>index.php/report/hotel/pdf<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>"
                target="_blank">
      ...
      ?>
        <div id="tableList" class="clearfix table-responsive">
            <div class="pull-left"><?=$GLOBALS ['CI']->pagination->create_links () ?><span class="">Total
                    <?=$total_records?> Bookings</span></div>
          ...
          echo $packagehtml;
                ?>
                  ...
                  if(check_colume('Status')){
                    ?>
                        <td><span
                                class="<?= booking_status_label ( $data['booking_details']['status'] )?>"><?=get_holiday_display_status( $data['booking_details']['status'] ) ?></span>
                        </td>
                        <?php
                  }
                   if(check_colume('Package Name')){
               ...
               /*    if(check_colume('Promocode'))
                   {
                    ?>
                      ...
                      {
                     echo isset($data['booking_details']['promocode'])? number_format(get_converted_currency_value ( $currency_obj->force_currency_conversion ( $data['booking_details']['promocode'] ) ), 2):0;
                    }
                    else
                    {
                      echo "NA";
                  ...
                  if(check_colume('Markup')){
                    ...
                    echo $markup;
                     ?></td>
                        ...
                        <td><?php  echo isset( $aed_attributes->aed_convenience_fee)? number_format($aed_attributes->aed_convenience_fee,2):0; ?>
                        </td>
...
$(function() {
    ...
    $("#created_datetime_to").datepicker().datepicker("setDate", new Date());
});
</script>