						...
						//	debug($table_data['booking_details']);exit;
							if(valid_array($table_data['booking_details']) == true) {
					        	...
					        	foreach($booking_details as $parent_k => $parent_v) {
									...
									$voucher_btn = holiday_voucher_link($app_reference, $booking_source, $status, 'b2b');
									//$invoice = flight_invoice($app_reference, $booking_source, $status);
									$cancel_btn = holiday_cancel($app_reference, $booking_source, $status);
									//$pdf_btn= holiday_pdf_link($app_reference, $booking_source, $status, 'b2b');
									$pdf_btn='';
					        		...
					        		// $action .=  '<br />'.$pdf_btn;
					        		$action .=  '<br />'.$email_btn;
									if($diff > 0 && $status != 'BOOKING_CANCELLED'){
										$action .= $cancel_btn;
									}
...
{
	return '<a href="'.holiday_cancel_url($app_reference, $booking_source, $status).'" target="_blank" class="btn btn-sm btn-warning"><i class="fa fa-arrows-alt"></i> Cancel</a>';

}
function holiday_cancel_url($app_reference, $booking_source='', $status='')
{
	return base_url().'index.php/package/pre_cancellation_b2b/'.$app_reference.'/'.$booking_source.'/'.$status;
}
...
function holiday_voucher_link($app_reference, $booking_source='', $status='',$module='')

{
	return '<a href="'.base_url().'index.php/voucher/package/'.$app_reference.'/'.$booking_source.'/'.$status.'/show_voucher" target="_blank" class="btn btn-sm btn-primary"><i class="fa fa-file-text" aria-hidden="true"></i> Voucher</a>';
}

function holiday_pdf_link($app_reference, $booking_source='', $status='',$module='')

{
	return '<a href="'.base_url().'index.php/voucher/activity_crs_pdf/'.$app_reference.'/'.$booking_source.'/'.$status.'/show_pdf" target="_blank" class="btn btn-sm btn-primary"><i class="fa fa-file-text" aria-hidden="true"></i> Pdf</a>';
}
