		...
		<div class="panel-body">
			...
			<div id="show-search" class="collapse">
				...
				<form method="GET" autocomplete="off" action="<?= base_url().'report/corporate_flight_report?'?>">
					...
					<a href="<?=base_url().'report/corporate_flight_report?' ?>" id="clear-filter" class="btn btn-primary">Clear Filter</a>
					</div>
						...
						<?php
							...
							if(valid_array($table_data['booking_details']) == true) {
					        	...
					        	foreach($booking_details as $parent_k => $parent_v) {
					        		$booking_transaction_details = $parent_v ['booking_transaction_details'];
$cost_attributes=json_decode($booking_transaction_details[0]['attributes'],1);
// debug($cost_attributes);
$base_fare=$cost_attributes['Fare']['BaseFare']+$cost_attributes['Fare']['Tax'];
$extra_cost=$parent_v['fare']-$base_fare;
$TotalFare=$base_fare+@$parent_v['admin_markup']+@$parent_v['gst']+$extra_cost;
					        		extract($parent_v);
					        		//debug($parent_v);exit;
									$action = '';
									$cancellation_btn = '';
									...
									$voucher_btn = flight_voucher($app_reference, $booking_source, $status, 'corporate');
									$invoice = flight_invoice($app_reference, $booking_source, $status);
									$cancel_btn = flight_executive_cancel($app_reference, $booking_source, $status,'corporate');
									$pdf_btn= flight_pdf($app_reference, $booking_source, $status, 'corporate');
		  ...
		  if(email != ''){
			  if(!emailReg.test(email)){
				  $('#mail_voucher_error_message').empty().text('Please Enter Correct Email Id');
					...
					var _opp_url = app_base_url+'index.php/voucher/b2e_flight_voucher/';
...
{
	if($courseType == 'BOOKING_CONFIRMED') {
