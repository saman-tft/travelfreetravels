...
function get_table($table_data, $total_rows)
{
	...
	$report_data .= '<table class="table table-condensed table-bordered" id="b2c_report_hotel_table">
		...
		$report_data .= '<tr>
					...
					<td>'.$currency.'</td>
					<td>'.roundoff_price($fare).'</td>
					<td class="hide">'.roundoff_price($itinerary_details[0]['admin_commission']).'</td>
					<td class="hide">'.roundoff_price($itinerary_details[0]['admin_tds']).'</td>
					<td>'.roundoff_price($admin_net_fare).'</td>
					<td>'.roundoff_price($admin_markup).'</td>
					<td>'.roundoff_price($convinence_amount).'</td>
					<td>'.roundoff_price($gst).'</td>
					<td>'.roundoff_price($discount).'</td>
					<td>'.roundoff_price($admin_net_fare+$admin_markup+$convinence_amount-$discount+$gst).'</td>
