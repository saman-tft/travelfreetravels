						...
						<tr>
							...
							<td class="center">
								...
								if($list->status == "ACTIVE")
									{ 
								?>
									<a href="<?php echo site_url()."/hotels/inactive_room_types/".base64_encode(json_encode($list->id)); ?>" class="btn btn-orange btn-sm btn-icon icon-left my-inactve"><i class="entypo-eye"></i>InActive</a>
								<?php 
								}
								else
								{ ?>
									<a href="<?php echo site_url()."/hotels/active_room_types/".base64_encode(json_encode($list->id)); ?>" class="btn btn-green btn-sm btn-icon icon-left my-actve"><i class="entypo-check"></i>Active</a>
								<?php 
								} 
								?>
								<a href="<?php echo site_url()."/hotels/edit_room_types/".base64_encode(json_encode($list->id)); ?>" class="btn btn-default btn-sm btn-icon icon-left"><i class="entypo-pencil"></i>Edit</a>	
