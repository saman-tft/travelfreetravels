				...
				<tbody>
					<?php if($room_rate_list!=''){  for($a=0;$a<count($room_rate_list);$a++){ 
						...
						<tr>
							...
							<td>
							  <?php  if($room_rate_list[$a]->default_nationality=='default') { ?>
							    <input type="radio" name="default_radio" checked id="default_radio" onclick="UpdateDefaultNationality('<?php echo site_url()."/roomrate/update_nationality/".base64_encode(json_encode($room_rate_list[$a]->hotel_room_rate_info_id))."/".$redirection_url."/".$room_rate_list[$a]->hotel_room_rate_id; ?>')">
							 <?php } else {?>
							  <input type="radio" name="default_radio" id="default_radio" onclick="UpdateDefaultNationality('<?php echo site_url()."/roomrate/update_nationality/".base64_encode(json_encode($room_rate_list[$a]->hotel_room_rate_info_id))."/".$redirection_url."/".$room_rate_list[$a]->hotel_room_rate_id; ?>')">
							...
							<td class="center">								
								<?php 
							//	if($room_rate_list[$a]->roomrate_status == "ACTIVE")
...
<script type="text/javascript">
	...
	function myFunctiondel(argument) {
		...
		function UpdateDefaultNationality(argument) 
		{
		//alert(argument);
	    if (confirm("Do You want to Set this Nationality as Default ?")) 
	    {
	        //alert( argument);
	        location.href = argument;
	    }
