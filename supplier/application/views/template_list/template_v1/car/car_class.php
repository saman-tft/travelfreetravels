...
$(document).ready(function() {
    $('.c_msg').fadeIn(1500).delay(3500).fadeOut(1000);
...
<ul class="nav nav-tabs" role="tablist" id="myTab">
	...
	<li role="presentation" class="<?=$tab1?>">
		<a id="fromListHead" href="#fromList" aria-controls="home" role="tab"	data-toggle="tab">Add Car Class</a>
	</li>
	<li role="presentation" class="<?=$tab2?>">
		<a	href="#tableList" aria-controls="profile" role="tab" data-toggle="tab">Car Class List </a>
	</li>
...
<div class="panel-body">
	<form method="post" action="<?php echo base_url();?>index.php/car_supplier/car_class/<?php echo $ID;?>" enctype="multipart/form-data">
		<div class='form-group'>
			<label class='control-label col-sm-3' for='validation_current'>								Class Name </label>
			<div class='col-sm-4 controls'>
				<div class="controls">
					<input type="text" name="class_name" id="name"
						data-rule-required='true' value="<?php echo isset($class_name) ? $class_name : '';?>" placeholder="Enter Car Class"
						class='form-control add_pckg_elements' onkeypress="return alphanumeric(event)" required>
				</div>
			</div>
			<span><?php echo form_error('class_name')?></span>
		</div>
			...
			<div class='row'>
				<div class='col-sm-9 col-sm-offset-3'>
					...
					<input class="btn btn-primary" type="submit" id=sup_submit value="Submit">&nbsp;&nbsp;
						<a class='btn btn-primary' href="<?php echo base_url(); ?>car_supplier/car_class"> Cancel</a>
				</div>
			</div>
		</div>
</form>
...
function get_table($promo_code_list)
{
	...
	if(valid_array($promo_code_list)) {
		...
		foreach($promo_code_list as $k => $v) {
			...
			$table .= '<td>'.ucfirst($class_name).'</td>';			
...
{
	return '<a role="button" href="'.base_url().'index.php/car_supplier/car_class/'.$origin.'" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>
		'.
		" ".get_app_message('AL0022').'</a>';
}
function get_delete_button($origin){
	return '<a role="button" href="#" class="btn btn-sm btn-danger del-car-asset-btn" data-id="'.$origin.'" data-table="car_class"><i class="fa fa-trash"></i>
