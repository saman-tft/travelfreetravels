...
<ul class="nav nav-tabs" role="tablist" id="myTab">
	...
	<li role="presentation" class="<?=$tab1?>">
		<a id="fromListHead" href="#fromList" aria-controls="home" role="tab"	data-toggle="tab">Add Car Type</a>
	</li>
	<li role="presentation" class="<?=$tab2?>">
		<a	href="#tableList" aria-controls="profile" role="tab" data-toggle="tab">Car Type List </a>
	</li>
...
<div class="panel-body">
	<form method="post" action="<?php echo base_url();?>index.php/car_supplier/car_make/<?php echo $ID;?>" enctype="multipart/form-data">
		<div class='form-group'>
			<label class='control-label col-sm-3' for='validation_current'>								Car Type </label>
			<div class='col-sm-4 controls'>
				<div class="controls">
					<input type="text" name="make_name" id="name"
						data-rule-required='true' value="<?php echo isset($make_name) ? $make_name : '';?>" placeholder="Enter Car Type"
						class='form-control add_pckg_elements' onkeypress="return alphanumeric(event)"  required>
				</div>
			</div>
			<span><?php echo form_error('make_name')?></span>
		</div>
			...
			<div class='row'>
				<div class='col-sm-9 col-sm-offset-3'>
					...
					<input class="btn btn-primary" type="submit" id=sup_submit value="Submit">&nbsp;&nbsp;
						<a class='btn btn-primary' href="<?php echo base_url(); ?>car_supplier/car_make"> Cancel</a>
				</div>
			</div>
		</div>
</form>
...
{
	return '<a role="button" href="'.base_url().'index.php/car_supplier/car_make/'.$origin.'" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>
		'.
		" ".get_app_message('AL0022').'</a>';
}
function get_delete_button($origin){
	return '<a role="button" href="#" class="btn btn-sm btn-danger del-car-asset-btn" data-id="'.$origin.'" data-table="car_make"><i class="fa fa-trash"></i>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$(".del-car-asset-btn").on("click",function(){
			...
			$.ajax({
				url:car_base_url+"car_supplier/delete_record/"+table+"/"+id,
		...
		$('.toggle-user-status').on('change', function(e) {
			...
			_opp_url = _opp_url+$(this).data('id')+'/car_make';
			toastr.info('Please Wait!!!');
