									...
									<div class="col-sm-5">
										 <select id="hotel_details_id" name="hotel_details_id" onChange="select_room(this.value);" class="form-control">
											...
											<?php foreach ($hotels_list as $hotel){ ?>
												<option value="<?php echo $hotel->hotel_details_id; ?>" <?php if(isset($hotel_details_id)) { if($hotel_details_id == $hotel->hotel_details_id){ echo "selected"; } } elseif( isset($seasons_list[0]->hotel_details_id)) { if($hotel->hotel_details_id == $seasons_list[0]->hotel_details_id ){ echo "selected"; }}?> data-iconurl=""><?php echo $hotel->hotel_name; ?></option>
											<?php } ?>
										</select>
	 ...
	 function select_cutoff(type){
	   ...
	   function select_room(hotel){	  		  	
	  	if(hotel != ""){
	  		...
	  		$.ajax({
            url: '<?php echo base_url(); ?>seasons/get_room_type/' + hotel,
            success: function (data, textStatus, jqXHR) {                     	            	
              select1.html('');
              select1.html(data);
