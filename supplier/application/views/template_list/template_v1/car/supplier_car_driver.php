...
<ul class="nav nav-tabs" role="tablist" id="myTab">
	...
	<li role="presentation" class="active">
		<a	href="#tableList" aria-controls="profile" role="tab" data-toggle="tab">Supplier driver & car List</a>
	</li>
...
function get_table($promo_code_list)
{
	...
	$table .= '<th>Supplier Phone Number</th>';
	$table .='<th>Country Name</th>';
	$table .= '<th>City Name</th>';
	...
	if(valid_array($promo_code_list)) {
		...
		foreach($promo_code_list as $k => $v) {
			...
			$action = '';
			extract($v);			
			$action .= get_car_list($user_id);
			$action .= get_driver_list($user_id);
			
			$table .= '<tr>';
			$table .= '<td>'.(++$current_record).'</td>';
			$table .= '<td>'.ucfirst($agency_name).'</td>';			
			$table .= '<td>'.$phone.'</td>';	
			$table .='<td>'.$s_country_name.'</td>';
...
function get_car_list($supplier_id){
	return '<a href="'.base_url().'index.php/car_supplier/get_all_supplier_driver_car/car/'.$supplier_id.'" class="btn btn-primary">View Cars</a>';
}
