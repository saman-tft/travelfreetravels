									...
									<div class='' style='margin-bottom: 0;'>
										<div class='box-content'>
											<form class='form form-horizontal validate-form'
												...
												action="<?php echo base_url(); ?>index.php/transfers/edit_transfer_vehicle_details/<?php echo $transfer_data->id;?>"
												...
												<?php
												if(isset($transfer_vehicle_data) && !empty($transfer_vehicle_data)){
													foreach ($transfer_vehicle_data as $key => $driver_info) {
														$count =$key+1;
														// debug($driver_info['driver_shift_days']);exit;
														$driver_shift_days = json_decode($driver_info->driver_shift_days,true);
												...
												<div class='form-group'>
													...
													<div class='col-sm-4 controls'>
														<input type="hidden" name="driver[<?=$key?>][id]" value="<?php echo $driver_info->id;?>">
														<input type="text" name="driver[<?=$key?>][driver_name]" id="driver_name"
															data-rule-number='true' data-rule-required='true'
															class='form-control'
															value="<?php echo $driver_info->driver_name;?>">
													</div>
												</div>
												...
												<div class='form-group'>
													...
													?>
		                                               <input class='form-control checkbox' id='monday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="1" type='checkbox' <?php echo $monday;?>>&nbsp;
			                                           <label for="monday_<?=$key?>">Mon</label>
			                                           &nbsp;&nbsp;
			                                           <input class='form-control checkbox' id='tuesday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="2" type='checkbox' <?php echo $tuesday;?>>&nbsp;
			                                           <label for="tuesday_<?=$key?>">Tue</label>
			                                           &nbsp;&nbsp;
			                                           <input class='form-control checkbox' id='wednesday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="3" type='checkbox' <?php echo $wednesday;?>>&nbsp;
			                                           <label for="wednesday_<?=$key?>">Wed</label>
			                                            &nbsp;&nbsp;
			                                           <input class='form-control checkbox' id='thursday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="4" type='checkbox' <?php echo $thursday;?>>&nbsp;
			                                           <label for="thursday_<?=$key?>">Thu</label>
			                                            &nbsp;&nbsp;
			                                           <input class='form-control checkbox' id='friday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="5" type='checkbox' <?php echo $friday;?>>&nbsp;
			                                           <label for="friday_<?=$key?>">Fri</label>
			                                            &nbsp;&nbsp;
			                                           <input class='form-control checkbox' id='saturday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="6" type='checkbox' <?php echo $saturday;?>>&nbsp;
			                                           <label for="saturday_<?=$key?>">Sat</label>
			                                            &nbsp;&nbsp;
			                                           <input class='form-control checkbox' id='sunday_<?=$key?>' name='driver[<?=$key?>][driver_shift_days][]' value="7" type='checkbox' <?php echo $sunday;?>>&nbsp;
			                                           <label for="sunday_<?=$key?>">Sun</label>
												...
												<div class='form-group'>
													...
													<div>
														...
														<input type="hidden" name="driver[<?=$key?>][id]" value="<?=$driver_info->id?>">
															<input type="text" name="driver[<?=$key?>][driver_shift_from]" id="driver_shift_from_<?=$key?>"
																data-rule-required='true' placeholder="Driver Shift From" value="<?=$driver_info->driver_shift_from?>"
																class='form-control vehicle_elements' required readonly>
														</div>
														<span style="margin-left: -166px; font-weight: bold;"> to </span>
														<div class='col-sm-2 controls'>
															<input type="text" name="driver[<?=$key?>][driver_shift_to]" id="driver_shift_to_<?=$key?>" value="<?=$driver_info->driver_shift_to?>"
												...
												<div class='form-group'>
													...
													<div class='col-sm-3 controls'>
														<input type="file" title='Image to add to add' class=''
															id='driver_license' name='driver_license_<?=$key?>'> <input type="hidden"
															name='driver[<?=$key?>][driver_license]' value="<?php echo $driver_info->driver_license; ?>">
														<img
															src="<?php echo $path.$driver_info->driver_license; ?>"
															width="100" name="photo">
													</div>
												</div> 
										...
										<div class="add_driver_btn"><span class="btn btn-primary pull-right" style="margin-right: 261px;">Add Driver</span></div>
