						   ...
						   <i class="fa fa-ellipsis-v"></i>  
						    <ul class="dropdown-menu sidedis" style="display: none;left: 30px;">
								...
								<td>
									...
									$p_status = '';
									$r_status = '';
									if($data->admin_approve_status == 1)
									{ 
										$a_status = 'selected';
										$approve = 'Approved';
										$pending = 'Pending';
										$reject = 'Reject';
									}else if($data->admin_approve_status == 0)
									{
										$p_status = 'selected';
										$approve = 'Approve';
										$pending = 'Pending';
										$reject = 'Reject';
									}else if($data->admin_approve_status == 2)
									{
										$r_status = 'selected';
										$approve = 'Approve';
										$pending = 'Pending';
									...
									?>
									<select class="admin_status_approve" id="admin_status" data-value="<?=$data->id?>" name="admin_status">
									<option value="1" <?=$a_status?> ><?=$approve?></option>;
									<option value="0" <?=$p_status?> ><?=$pending?></option>;
									<option value="2" <?=$r_status?> ><?=$reject?></option>;
									</select>
								</td>
...
$(".admin_status_approve").change(function()
    {  
    	var approve_status = $(this).val();
    	var transfer_id = $(this).data('value');
		var url="<?php echo base_url().'index.php/transfers/manage_supplier_approval'; ?>" ;
      	$.ajax({
              url :url,
              type: 'POST',
              data: {approve_status:approve_status,transfer_id:transfer_id},
              success: function(data)
              {
                location.reload()
              }
