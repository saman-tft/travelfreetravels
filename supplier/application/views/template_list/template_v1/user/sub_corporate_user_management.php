   ...
   <!-- <th>Action</th>-->
	...
	if (valid_array($table_data) == true) {
		...
		foreach ($table_data as $k => $v) {
			...
			if (is_active_airline_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'sub_corporate_flight_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			...
			if (is_active_hotel_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'sub_corporate_hotel_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			...
			if (is_active_bus_module()) {
				$booking_summ .= '<td><a target="_blank" href="'.$rep_url.'sub_corporate_bus_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}

			if (is_active_transferv1_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'sub_corporate_transfers_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			if (is_active_sightseeing_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'sub_corporate_sightseeing_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
...
function view_account_ledger($user_id,$date){
	...
	return '<a role="button" href="'.base_url().'management/account_ledger?agent_id='.$user_id.'&agent_type='.CORPORATE_USER.'" target="_blank" class="btn btn-sm btn-success" >
				<i class="fa fa-bar-chart"></i> Account Ledger
			</a>
		';	  
}
