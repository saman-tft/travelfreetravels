...
if($segment_2=='b2b_user' && !isset($_GET['eid']))
{
	$tab2 = " active ";
	$tab1 = "";
}
   ...
   // debug($table_data);exit;
	if (valid_array($table_data) == true) {
		...
		foreach ($table_data as $k => $v) {
			...
			if (is_active_airline_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'b2b_flight_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			...
			if (is_active_hotel_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'b2b_hotel_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			...
			if (is_active_bus_module()) {
				$booking_summ .= '<td><a target="_blank" href="'.$rep_url.'b2b_bus_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			...
			if (is_active_transferv1_module()) {
				$booking_summ .= '<td> <a target="_blank" href="'.$rep_url.'b2b_transfers_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
			...
			if (is_active_sightseeing_module()) {
				$booking_summ .= '<td><a target="_blank" href="'.$rep_url.'b2b_activities_report?created_by_id='.$v['user_id'].'">view</a></td>';
			}
...
function view_account_ledger_oldd($user_id,$date){
	...
	return '<a role="button" href="'.base_url().'index.php/management/account_ledger?agent_id='.$user_id.'" target="_blank" class="btn btn-sm btn-success" >
				<i class="fa fa-bar-chart"></i> Account Ledger
			</a>';	  
}
function view_account_ledger($user_id,$date){
	
	return '<a role="button" href="'.base_url().'management/account_ledger?agent_id='.$user_id.'&agent_type='.B2B_USER.'" target="_blank" class="btn btn-sm btn-success" >
				<i class="fa fa-bar-chart"></i> Account Ledger
			</a>
		';	  
}
