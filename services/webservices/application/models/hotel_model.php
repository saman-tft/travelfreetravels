...
Class Hotel_Model extends CI_Model
{
			...
			if (isset($temp_search_data['city']) == true) {
				...
				if (isset($temp_location[1]) == true) {
					//$clean_search['country_name'] = trim($temp_location[1], '() ');
                                    $clean_search['country_name'] = $temp_search_data['CountryCode'];
				} else {
					$clean_search['country_name'] = $temp_search_data['CountryCode'];
				}
			} else {
	...
	$hotel_check_in, $hotel_check_out, $payment_mode,	$attributes, $created_by_id, $currency_conversion_rate=1, $hotel_version=HOTEL_VERSION_1, $offline_book_status='')
	{
		...
		$data['offline_booking_status'] = $offline_book_status;
	...
	function save_search_history_data($search_request)
	{
		...
		$hotel_city_details = $this->db->query('select * from hotels_city where origin='.$hotel_city_id)->row_array();
