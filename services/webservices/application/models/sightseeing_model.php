...
Class Sightseeing_Model extends CI_Model {
  ...
  $travel_date,$grade_code,$image,$destnation_name,$grade_desc,$payment_mode, $attributes, $created_by_id, $currency_conversion_rate=1, $sightseen_version=SIGHTSEEING_VERSION_1,$gst=0,$admin_markup_gst=0,$admin_commission=0,$agent_commission=0,$admin_tds=0,$agent_tds=0,$book_total_net_fare)
  {
  ...
  function save_booking_pax_details($title,$app_reference, $first_name, $middle_name, $last_name, $phone, $email, $pax_type,$status)
  {
    ...
    $data['status'] = $status;       
    $status = $this->custom_db->insert_record('sightseen_booking_pax_details', $data);
    return $status;
  }
  /**
   * Elavarasi
  ...
  public function update_cancellation_details($AppReference, $cancellation_details)
  {
    ...
    //3.Update Pax Status
    $this->custom_db->update_record('sightseen_booking_pax_details', array('status' => $booking_status), array('app_reference' => $AppReference));//later
  }
  /**
  ...
  private function update_cancellation_refund_details($AppReference, $cancellation_details)
  {
    ...
    if($cancel_details_exists['status'] == true) {
      ...
      $this->custom_db->update_record('sightseeing_cancellation_details', $ss_cancellation_details, array('app_reference' => $AppReference));
    } else {
      //Insert Data
      ...
      $this->custom_db->insert_record('sightseeing_cancellation_details',$ss_cancellation_details);
    }
  }
  /**
  ...
  function update_refund_details($app_reference, $refund_status, $refund_amount,$cancellation_charge, $currency, $currency_conversion_rate)
  {
    ...
    $this->custom_db->update_record('sightseeing_cancellation_details', $refund_details, array('app_reference'=> $app_reference));
  }
  /**
   * Elavarasi
   * @param $id
   */
  function sightseeing_commission_details($id){
    $cols = 'DL.domain_name,BSCD.value, BSCD.api_value,BSCD.value_type';
    $query = 'select '.$cols.' from domain_list DL
          left join b2b_sightseeing_commission_details as BSCD on DL.origin = BSCD.domain_list_fk 
  ...
  {
    $query = 'select SCD.* from sightseeing_booking_details HB
          join sightseeing_cancellation_details SCD on SCD.app_reference=HB.app_reference
          where HB.app_reference='.$this->db->escape($app_reference).' and HB.domain_origin='.intval($domain_id).' and SCD.ChangeRequestId='.$this->db->escape($ChangeRequestId);
