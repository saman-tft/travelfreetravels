...
<div class="row details-wrapper">
	...
	<div class="col-md-offset-2 col-md-2">
		<form class="booking-form" action="<?=base_url().'bus/booking/'.$search_id?>" method="POST" target="">
			...
			<input type="hidden" name="booking_source" class="booking-source" value="<?=TRAVELYAARI_BUS_BOOKING_SOURCE?>">
			<input type="hidden" name="token" class="token" value="<?php echo $seat_no_summary = serialized_data($seat_no_summary)?>">
			<input type="hidden" name="token_key" class="token_key" value="<?=md5($seat_no_summary)?>">
			</div>
			<input type="button" class="btn btn-p b-r-0 booking-btn" value="Continue">
		</form>
	</div>
</div>