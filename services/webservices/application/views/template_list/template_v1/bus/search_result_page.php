...
echo $GLOBALS['CI']->template->isolated_view('bus/search_panel_summary.php');
?>
...
$(document).ready(function() {
	...
	*/
	function core_bus_loader(booking_source_id)
	{
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url()?>ajax/bus_list',
			async:true,
			cache: false,
			data: $.param(params),
			dataType: 'json',
			success: function(result) {
				...
				if (result.hasOwnProperty('status') == true && result.status == true) {
					$('#bus_search_result').append(result.data);
	...
	*/
	...
	var sliderCurrency = "<?php echo get_application_currency_preference(); ?>";
	...
	function filter_row_origin_marker(visibility)
	{
		...
		$('.result-row-index'+visibility).each(function(key, value) {
			//$.inArray($('.airline-name:first', this).data('airline-code'), airlineList) != -1
			if (
	...
	$(document).on('click', ".more-bus-content-btn", function(){
		$(".more-bus-content-container", $(this).closest('.result-row-index')).toggle();
	});
	...
	$(".price-l-2-h").click(function(){
		...
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.bus-price:first', item: '.result-row-index', order: 'asc', is_num: true });
	});

	$(".price-h-2-l").click(function(){
		$(this).addClass('hide');
		$('.price-l-2-h').removeClass('hide');
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.bus-price:first', item: '.result-row-index', order: 'desc', is_num: true});
	});
	...
	$(".travel-l-2-h").click(function(){
		...
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.travel-name:first', item: '.result-row-index', order: 'asc', is_num: false});
	});

	$(".travel-h-2-l").click(function(){
		$(this).addClass('hide');
		$('.travel-l-2-h').removeClass('hide');
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.travel-name:first', item: '.result-row-index', order: 'desc', is_num: false});
	});
	...
	$(".departure-l-2-h").click(function(){
		...
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.departure-time:first', item: '.result-row-index', order: 'asc', is_num: true});
	});

	$(".departure-h-2-l").click(function(){
		$(this).addClass('hide');
		$('.departure-l-2-h').removeClass('hide');
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.departure-time:first', item: '.result-row-index', order: 'desc', is_num: true});
	});
	...
	$(".arrival-l-2-h").click(function(){
		...
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.arrival-time:first', item: '.result-row-index', order: 'asc', is_num: true});
	});

	$(".arrival-h-2-l").click(function(){
		$(this).addClass('hide');
		$('.arrival-l-2-h').removeClass('hide');
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.arrival-time:first', item: '.result-row-index', order: 'desc', is_num: true});
	});
	...
	$(".seat-l-2-h").click(function(){
		...
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.available-seats:first', item: '.result-row-index', order: 'asc', is_num: true});
	});

	$(".seat-h-2-l").click(function(){
		$(this).addClass('hide');
		$('.seat-l-2-h').removeClass('hide');
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.available-seats:first', item: '.result-row-index', order: 'desc', is_num: true});
	});
	...
	$(".bus-type-l-2-h").click(function(){
		...
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.bus-type-count:first', item: '.result-row-index', order: 'asc', is_num: true});
	});

	$(".bus-type-h-2-l").click(function(){
		$(this).addClass('hide');
		$('.bus-type-l-2-h').removeClass('hide');
		$("#bus_search_result .result-wrapper-group").jSort({sort_by: '.bus-type-count:first', item: '.result-row-index', order: 'desc', is_num: true});
	});
	...
	$(document).on('click', '.inner-summary-btn', function(e) {
		e.preventDefault();
		var _inner_summary_toggle = $('.inner-summary-toggle', $(this).closest('.result-row-index'));
		...
		if (_inner_summary_toggle.is(':visible')) {
			...
			$('.inner-summary-content', _inner_summary_toggle).html(_booking_data).show();
			$('.loader-image', _inner_summary_toggle).hide();
		} else {
			$('.inner-summary-content', _inner_summary_toggle).html('').hide();
	...
	function get_inner_bus_details(params)
	{
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url()?>ajax/get_bus_details',
			async: false,
			cache: false,
			data: params,
			success: function(result) {
	...
	$(document).on('click', '.bus-info-btn', function(e) {
		...
		//load data
		clean_up_info_modal();
		var _bus_info_data = get_inner_bus_information($('.book-form', $(this).closest('.result-row-index')).serializeArray());
	});
	...
	function get_inner_bus_information(params)
	{
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url()?>ajax/get_bus_information',
			async: true,
			cache: true,
			data: params,
			success: function(result) {
	...
	$(document).on('click', '.bus-boarding-info-btn', function(e) {
		...
		var _bus_info_data = get_board_bus_information($('.book-form', $(this).closest('.result-row-index')).serializeArray());
	});
	...
	function get_board_bus_information(params)
	{
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url()?>ajax/get_bus_details/true',
			async: true,
			cache: true,
			data: params,
			success: function(result) {
