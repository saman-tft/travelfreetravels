...
<div class="modal fade" id="myModal_1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <a class="hand-cursor" href="<?=base_url().'auth/register'?>">Not Yet Registered ? Sign Up Now</a>
      </div>
      </div>
    </div>
  </div>
...
$(document).ready(function() {
	$('#login_submit').on('click', function(e) {
		...
		if (_username == '' || _password == '') {
			...
			$.post("<?php echo base_url().'auth/login/'?>", {username: _username, password: _password}, function(response) {
				if (response.status) {
					$('#myModal_1').hide();
	...
	$('#reset-password-trigger').on('click', function(e) {
		...
		$.post("<?php echo base_url().'auth/forgot_password/'?>", {email: $('#recover_email').val(), 'phone':$('#recover_phone').val()}, function(response) {
			if (response.status) {
