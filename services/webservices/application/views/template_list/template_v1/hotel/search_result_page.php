...
echo $GLOBALS['CI']->template->isolated_view('hotel/search_panel_summary.php');
?>
...
$(document).ready(function() {
	...
	*/
	function core_hotel_loader(booking_source_id)
	{
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url()?>ajax/hotel_list',
			async:true,
			cache: false,
			data: $.param(params),
			dataType: 'json',
	...
	$(document).on('click', ".more-hotel-content-btn", function(){
		$(".more-hotel-content-container", $(this).closest('.result-row-index')).toggle();
	});
	var room_list_cache = {};
	$(document).on('click', ".hotel-room-list-btn", function(e){
		e.preventDefault();
		var _cur_row_index = $(this).closest('.result-row-index');
		var _hotel_room_list = $(".hotel-room-list", _cur_row_index);
		...
		if (_hotel_room_list.is(':visible') == false) {
			...
			if ((_result_index in room_list_cache) == false) {
				var _hotel_name = $('.hotel-name',_cur_row_index).text();
				var _hotel_star_rating = $('.hotel-star-rating',_cur_row_index).text();
				$('.loader-image', _cur_row_index).show();
				$.post("<?php echo base_url().'ajax/get_room_details'?>", $('.room-details-form', _cur_row_index).serializeArray(), function(response) {
					if (response.hasOwnProperty('status') == true && response.status == true) {
						...
						$(".inner-summary-content", _cur_row_index).html(response.data);
				...
				$(".inner-summary-content", _cur_row_index).show();
			}
		} else {
