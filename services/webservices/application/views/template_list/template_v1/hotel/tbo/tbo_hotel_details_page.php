...
<div class="panel panel-default htl_dtls_cont clearfix">
	<div class="panel-body">
		<p class="text-primary"><?php echo strtoupper($sanitized_data['HotelName']);?> <span class="rating-no"><span class="hide" id="hotel-star-rating"><?=$sanitized_data['StarRating']?></span><?php echo print_star_rating($sanitized_data['StarRating']);?></span></p> <?php echo '<span>-'.$sanitized_data['Address']?></span>
		<!-- Hotel Basic Info -->
	</div>
	<div class="panel-body">
		<div class="row">
		...
		<div class="panel panel-default clearfix">
		  ...
		  <?php
		    if (valid_array($sanitized_data['HotelFacilities']) == true) {
		    	...
		    	//$color_code = string_color_code('Arjun');
		    	$color_code = '#00a0e0';
		    	?>
		    	<div class="panel-body">
...
$(document).ready(function() {
	...
	function load_hotel_room_details()
	{
		...
		$.post("<?php echo base_url().'ajax/get_room_details'?>", _q_params, function(response) {
			if (response.hasOwnProperty('status') == true && response.status == true) {
				$('#hotel-room-list').html(response.data);
