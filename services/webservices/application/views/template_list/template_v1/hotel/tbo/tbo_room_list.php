...
foreach ($_RoomCombinations['RoomCombination'] as $__rc_key => $__rc_value) {
	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		$temp_price_details = $GLOBALS['CI']->hotel_lib->update_markup_currency($temp_room_details['Price'], $currency_obj);
		...
		$room_panel_details .= '<p><h4>'.$currency_obj->to_currency.' <span class="h3 text-i hotel-price">'.$RoomPrice.'</span></h4></p>';
		$room_panel_details .= '</div>';


		if (intval($__room_index_key) == 0) {
			$temp_book_now_button = '';
			$temp_summary_room_list = array($temp_room_details['RoomTypeName']);
			$temp_summary_price_list = array($RoomPrice);
		} else {
			$temp_summary_room_list[] = $temp_room_details['RoomTypeName'];
		...
		if (intval($temp_current_combination_count) == intval($__room_index_key+1)) {
			...
			$room_panel_summary = '<p><span class="room-name">'.$temp_summary_room_list.'</span> : '.$application_preferred_currency.' <span class="h3 text-i room-price">'.$temp_summary_price_list.'</span> <h3 class="mrdtls_drp  hand-cursor">More Details <i class="fa fa-info"></i></i></h3></p>';
		}

		$dynamic_params_url[] = get_dynamic_booking_parameters($__room_index_value, $temp_room_details, $application_default_currency);
