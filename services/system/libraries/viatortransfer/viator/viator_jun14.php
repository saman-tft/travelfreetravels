...
class Viator extends Common_Api_Transferv1 {
    ...
    **/
    ...
    function format_search_data_response($search_result, $search_data) {       
        ...
        foreach ($search_result['data'] as $result_k => $value) {
          ...
          if(isset($value['data'])){
           ...
           // $response[$result_k]['Promotion'] = true;
           // $response[$result_k]['PromotionAmount'] =1;
    ...
    private function calculate_commission($price_arr){
        ...
        $price_details = array();
        $domain_id = get_domain_auth_id();       
        //$check_commission = $this->CI->sightseeing_model->check_commision($domain_id);
    ...
    public function get_product_details($request, $search_id) {
        ...
        if ($product_details_request ['status'] == SUCCESS_STATUS) {
            ...
            // $result = $this->CI->custom_db->single_table_records('provab_api_response_history','*',array('origin'=>6940));

            // $product_details_response =$result['data'][0]['response'];
    ...
    */
    ...
    public function admin_cancel_booking($request)
    {
        ...
        $booking_details = $this->CI->custom_db->single_table_records('sightseeing_booking_details', '*', array('app_reference' => $app_reference));
    ...
    public function cancel_booking($request)
    {
        ...
        if($booking_details['status'] == SUCCESS_STATUS && $booking_details['data'][0]['status'] == 'BOOKING_CONFIRMED'){
           ...
           // $total_booking_amount = round($booking_details['total_fare']+$booking_details['domain_markup']);
            ...
            round($booking_details['total_fare']-$booking_details['agent_commission']+$booking_details['agent_tds']+$booking_details['domain_markup']);
            ...
            //  echo $ChangeRequestId;
                ...
                //unset($response['data']['CancellationDetails']['B2B2BStatus'], $response['data']['CancellationDetails']['ResponseStatus'], $response['data']['CancellationDetails']['Error'], $response['data']['CancellationDetails']['TraceId']);
    ...
    */
    public function get_hold_booking_status($request){
        ...
        $data['status'] = false;
        $data['data'] =array();
        $get_booking_details = $this->CI->custom_db->single_table_records('sightseeing_booking_details','*',array('status'=>'BOOKING_CONFIRMED','app_reference'=>$app_reference));
