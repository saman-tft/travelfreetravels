...
class Viator extends Common_Api_Transferv1 {
    ...
    **/
    ...
    function format_search_data_response($search_result, $search_data) {       
        ...
        $product_list['TransferSearchResult']['TransferResults'] = $response;
        $product_list['TransferSearchResult']['Destination_Id'] = $search_data['destination_id'];      
        // debug($product_list);
        // exit;
        return $product_list;
    ...
    private function format_price($total_price, $API_Currency) {     
        ...
        $conversion_amount = $GLOBALS['CI']->domain_management_model->get_viator_currency_conversion_rate();
        $conversion_rate = $conversion_amount['value'];
    ...
    public function format_Tx_Cancellation_Policy($cancel_available,$booking_date,$price_details){
    ...
    public function get_product_details($request, $search_id) {
        ...
        if ($product_details_request ['status'] == SUCCESS_STATUS) {
            ...
            if (valid_array($product_details_response['data']) == true && isset($product_details_response['data']) == true) {
                $response ['status'] = SUCCESS_STATUS;
                $response ['data']['TransferInfoResult'] = $this->format_product_details($product_details_response,$product_photo_response,$product_date_available_response,$product_review_response,$search_id);
            } else {
    ...
    */
    ...
    private function format_product_details($product_details_response,$product_photos,$product_available_date,$product_review_response,$search_id) {
        ...
        $product_details_response_arr = array();
         ...
         if(count($product_photos['data'])>0){
            ...
            if($product_details_response['thumbnailHiResURL']){
                $arr=array(
                ...
                $product_details_response_arr['ProductPhotos'] =array();
           }
        ...
        $product_details_response_arr['TermsAndConditions'] =$product_details_response['termsAndConditions'];
         ...
         $detail['key'][0]['Additional_info'] = $product_details_response['additionalInfo'];
         $detail['key'][0]['Inclusions'] = $product_details_response['inclusions'];
         $detail['key'][0]['Exclusions'] = $product_details_response['exclusions'];
         $detail['key'][0]['ShortDescription'] = $product_details_response['shortDescription'];
        ...
        foreach ($tourgrade_response_arr['data'] as $result_k => $value) {
           ...
           // if($value['available']==true){
               ...
               // $tourgrade_list[$result_k]['currencyCode'] = $value['currencyCode'];
                ...
                $key['key'][$result_k]['additional_info'] = $available_date[0]['Additional_info'];
                
                $key['key'][$result_k]['exclusions'] = $available_date[0]['Exclusions'];
                $key['key'][$result_k]['inclusions'] = $available_date[0]['Inclusions'];
                 $key['key'][$result_k]['shortdescription'] = $available_date[0]['ShortDescription'];
    ...
    private function format_block_tourgrade_details($block_grade_response,$activity_info_data,$acitivity_data,$search_request,$search_params){
       ...
       // echo $format_block_response['HotelPickup'];
        ...
        $key['key'][0]['ProductName'] = $activity_info_data['ProductName'];
         ...
         $key['key'][0]['additional_info'] = $activity_info_data['additional_info'];
         $key['key'][0]['exclusions'] = $activity_info_data['exclusions'];
         $key['key'][0]['inclusions'] = $activity_info_data['inclusions'];
         $key['key'][0]['shortdescription'] = $activity_info_data['shortdescription'];         
    ...
    function process_booking($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if($book_service_response['status'] == SUCCESS_STATUS){
            ...
            $tour_attributes['Additional_info'] = $booking_params['ResultToken']['additional_info'];
            $tour_attributes['Exclusions'] = $booking_params['ResultToken']['exclusions'];
            $tour_attributes['Inclusions'] = $booking_params['ResultToken']['inclusions'];
            $tour_attributes['ShortDesc'] = $booking_params['ResultToken']['shortdescription'];
    ...
    private function book_service_request($booking_params,$search_id){
       ...
       if(strtolower(transferv1::get_credential_type()=='live')){
