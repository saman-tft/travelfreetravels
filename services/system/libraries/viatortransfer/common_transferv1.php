...
class Common_Transferv1 {
	...
	static function combined_booking_url($search_id) {
		return Common_Api_Transferv1::pre_booking_url ( $search_id );
	}
	...
	private function update_fare_markup_commission(& $FareDetails, $multiplier, $commission_percentage, $domain_currency_conversion, $booking_source, $OperatorCode='')
	{
		...
		#echo "total_markup".$total_markup;
		...
		if($domain_currency=='INR'){
			...
			#$domain_currency_obj = new Currency(array('from' => get_application_default_currency() , 'to' => domain_base_currency()));	
			
			
			 #$domain_converted_markup = 	get_converted_currency_value($domain_currency_obj->force_currency_conversion($total_markup));
		...
		if($porceed_to_save['status'] != SUCCESS_STATUS){
			...
			foreach($passenger_details as $pax_k => $passenger_v) {
				if(isset($passenger_v['Title'])){
					...
					$ci->transferv1_model->save_booking_pax_details($title,$app_reference,$first_name, $middle_name, $last_name, $phone, $email, $pax_type,$tour_booking_status);
			}
		}
		return $data;
	}
