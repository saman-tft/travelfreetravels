    ...
    function hold_ticket($booking_params, $app_reference, $sequence_number, $search_id)
    {
        ...
        if ($AirSellRecommdation['status'] == SUCCESS_STATUS) {
            ...
            if ($PNR_AddMultiElements_Option0['status'] == SUCCESS_STATUS) {
                ...
                $PNR_AddMultiElements_BookingClass = $this->PNR_AddMultiElements_BookingClass($FOP['data'], $flight_marketing_carrier, $search_id, $country);
    ...
    function process_request($request, $url, $remarks = '', $soap_url)
    {
        ...
        try {
            $headers = array(
                'Content-Type: text/xml; charset="utf-8"',
                'Content-Length: ' . strlen($request),
                'Host: nodeD1.test.webservices.amadeus.com',
                'POST: "https://nodeD1.test.webservices.amadeus.com/"',
                //'Accept-Encoding: gzip,deflate',
                'Accept: text/xml',
                'Cache-Control: no-cache',
                'Pragma: no-cache',
