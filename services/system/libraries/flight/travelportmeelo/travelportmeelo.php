...
class Travelportmeelo extends Common_Api_Flight {

    var $master_search_data;
    var $retunr_domestic;
    protected $token;
    function __construct() {
        parent::__construct(META_AIRLINE_COURSE, TRAVELPORT_MEELO_FLIGHT_BOOKING_SOURCE);
    ...
    public function get_fare_rules($flight_result) {
        ...
        if ($fare_rule_request['status'] == SUCCESS_STATUS) {
            ...
            $api_remarks = 'flight_rules(Travelport Meelo Flight)';
            ...
            if(!empty($low_fare_search_res['air:LowFareSearchRsp']['air:FlightDetailsList']['air:FlightDetails'])){
                ...
                if (isset($air_pricing_solutions_data) && valid_array($air_pricing_solutions_data)) {
                    foreach ($air_pricing_solutions_data as $ap_key => $pricing_array) {
                        $flight_data = $this->format_air_pricing_solution($pricing_array, $air_segment_list, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData, $air_flight_list_arr);
     ...
     * array $pricing_array
     */
    function format_air_pricing_solution($pricing_array, $air_segment_list1, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData, $air_flight_list) {
        ...
        $conversion_data = $this->CI->custom_db->single_table_records('currency_detail','value', array('f_currency' => $currency, 't_currency' => 'INR'));
        $conversion_amount = $conversion_data['data'][0]['value'];
        ...
        $approx_base_price = $approx_base_price*$conversion_amount;
        $total_tax = $total_tax*$conversion_amount;
        $display_price = $display_price*$conversion_amount;
        $yqvalue = $yqvalue*$conversion_amount;
        $YR_value = $YR_value*$conversion_amount;
    ...
    public function get_update_fare_quote($request, $search_id) {
        ...
        if ($update_fare_quote_request['status'] == SUCCESS_STATUS) {
            ...
            $api_remarks = 'flight_pricing(Travelport Meelo Flight)';
            $this->CI->api_model->store_api_request_booking($api_url, $api_request, $api_response, $api_remarks);
    ...
    private function send_change_request($request_params, $carrier) {
        ...
        if ($send_change_request['status'] == SUCCESS_STATUS) {
            ...
            $api_remarks = 'CancelBooking(Travelport Meelo Flight)';
            $this->CI->api_model->store_api_request_booking($api_url, $api_request, $api_response, $api_remarks);
    ...
    private function run_book_service($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if ($book_service_request['status'] == SUCCESS_STATUS) {
            ...
            $api_remarks = 'flight_booking(Travelport Meelo Flight)';
            $this->CI->api_model->store_api_request_booking($api_url, $api_request, $api_response, $api_remarks);
    ...
    private function run_book_service_request($params, $search_id) {
        ...
        //             <CreditCard BankCountryCode="'.$credit_card_info['bank_country_code'].'" BankName="'.$credit_card_info['bank_name'].'" ExpDate="'.$credit_card_info['exp_date'].'" Name="'.$credit_card_info['card_name'].'" Number="'.$credit_card_info['card_number'].'" Type="'.$credit_card_info['card_type'].'">
        //                 <BillingAddress>
        //                     <AddressName>Electronic City</AddressName>
        //                     <Street>provab technosoft</Street>
        //                     <City>Bangalore</City>
        //                     <State>KA</State>
    ...
    private function run_non_lcc_ticket_service($booking_params, $app_reference, $sequence_number) {
        ...
        if ($non_lcc_ticket_service_request['status'] == SUCCESS_STATUS) {
            ...
            $api_remarks = 'flight_gds_ticketing(Travelport Meelo Flight)';
            $this->CI->api_model->store_api_request_booking($api_url, $api_request, $api_response, $api_remarks);
