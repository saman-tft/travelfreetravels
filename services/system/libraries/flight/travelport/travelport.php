...
INR") {
    ...
    private function air_create_reservation_request($params, $token_data) {
        ...
        $AirCreateReservationReq = '<?xml version="1.0" encoding="UTF-8"?>
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                        <s:Header/>
                        <s:Body>
                          <AirCreateReservationReq xmlns="http://www.travelport.com/schema/universal_v41_0" RetainReservation="None" RuleName="COMM" TargetBranch="P7039804">
    ...
    private function format_search_data_response($low_fare_search_res_data, $search_data) {
        ...
        foreach ($low_fare_search_res_data as $key => $low_fare_search_res) {
            ...
            if(isset($low_fare_search_res['air:LowFareSearchRsp']['common_v41_0:ResponseMessage'])){
                $warning = force_multple_data_format($low_fare_search_res['air:LowFareSearchRsp']['common_v41_0:ResponseMessage']);
    ...
    function format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list1, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData, $air_flight_list) {
                                ...
                                if($check_segment == true){
                                    ...
                                    if ($flight_key == $segment_ref) {
                                        if (isset($air_code_share_info) && valid_array($air_code_share_info)) {
                                            ...
                                            /*$blocked_list = $this->CI->custom_db->single_table_records('blocked_airline', '*', array('airline_list' => $operator_code, 'booking_source' => $this->booking_source,'status'=> 0, 'domain_origin' => get_domain_auth_id()));*/
                   
                                            $check_airline_active = false;
                                            $check_blocked = true;
                                            /*if($blocked_list['status'] == false){
                                                $check_blocked = false;
                                                $blocked_list = $this->CI->custom_db->single_table_records('blocked_airline', '*', array('airline_list' => $operator_code, 'booking_source' => $this->booking_source,'status'=> 1, 'domain_origin' => get_domain_auth_id()));
                                                if($blocked_list['status'] == true){
                                                    $check_airline_active = true;
                                                }
                                            }*/
                                           
                                            /*if($check_airline_active == false && $check_blocked == false){
                                            
                                                $blocked_list = $this->CI->custom_db->single_table_records('blocked_airline', '*', array('airline_list' => $operator_code, 'status'=> 0, 'booking_source' => $this->booking_source,'domain_origin' => 0));
                                            }*/
                                            /*if($check_airline_active == true){
                                                $blocked_list['status'] = false;
                                                unset($blocked_list['data']);
                                            }*/
                                            $blocked_list['status'] = false;
                                            if($blocked_list['status']==true)
                                            {
        ...
        if (isset($air_pricing_info) && valid_array($air_pricing_info)) {
                    ...
                    foreach ($booking_details_flight as $b_fkey => $booking) {
                        ...
                        if(isset($flight_journey['FlightDetails']['Details'][0][0]['OperatorCode']) && $flight_journey['FlightDetails']['Details'][0][0]['OperatorCode'] == '6E'){
                            if($fare_type == "Lite Fare"){
                                ...
                                else if($search_data['from'] == 'CMB' || $search_data['from'] == 'DXB' || $search_data['from'] == 'MCT' || $search_data['from'] == 'SHJ' || $search_data['from'] == 'DOH' || $search_data['from'] == 'AUH' || $search_data['from'] == 'KWI' || $search_data['to'] == 'CMB' || $search_data['to'] == 'DXB' || $search_data['to'] == 'MCT' || $search_data['to'] == 'SHJ' || $search_data['to'] == 'DOH' || $search_data['to'] == 'AUH' || $search_data['to'] == 'KWI' || $search_data['from'] == 'IST' || $search_data['to'] == 'IST' || $search_data['from'] == 'HKG' || $search_data['to'] == 'HKG' || $search_data['from'] == 'HKG' || $search_data['to'] == 'HKG' || $search_data['from'] == 'RGN' || $search_data['to'] == 'RGN'){
                                    $arrBaggageData[$strFare_Ref_Key]['Baggage']= '30 Kg';
                                }
                                else if($search_data['from'] == 'JED' || $search_data['from'] == 'JED'){
                                    $arrBaggageData[$strFare_Ref_Key]['Baggage']= '25 Kg';
                                }
                                else{
            ...
            if(valid_array($flight_journey['FlightDetails'])){
                ...
                $tds_commission = ($agent_commssion + $yq_value) * 3.7 / 100;
                ...
                $flight_journey ['Price'] ['PriceBreakup'] ['YQValue'] = $yqvalue;
                  ...
                  $flight_journey ['Price'] ['PriceBreakup'] ['TdsOnCommission'] = ((@$iata_commssion * 3.7)/100);
                  $flight_journey ['Price'] ['PriceBreakup'] ['TdsOnPLB'] = (((@$PLB_commssion+$CashBackAmount) * 3.7)/100);
                
                
                $flight_journey ['Price'] ['PriceBreakup'] ['AgentCommission'] = @$iata_commssion + @$PLB_commssion + $CashBackAmount;
                $flight_journey ['Price'] ['PriceBreakup'] ['AgentTdsOnCommision'] = ((@$iata_commssion * 3.7)/100)+((@$PLB_commssion * 3.7)/100) + (($CashBackAmount*3.7)/100);
        ...
        if(valid_array($flight_journey['FlightDetails'])){
            ...
            $flight_journey ['api_name'] = 'galelio';
  ...
  private function flight_segment_summary($journey_detail, $air_seg_key, $air_price_result, $random_value, $search_id, $fare_info_response) {
        //debug($journey_detail);exit;
        $search_data = $this->search_data($search_id);
        ...
        if (isset($air_pricing_info) && valid_array($air_pricing_info)) {
            ...
            $fare_type = $fare_info_response[0]['@attributes']['FareFamily'];
            $arrBaggageData = array();
            ...
            if($flight_journey['FlightDetails']['Details'][0][0]['OperatorCode'] == '6E'){
                ...
                if($fare_type == "Lite Fare"){
                    $arrBaggageDataValue['Baggage']= '0 Kg';
                    $arrBaggageDataValue['CabinBaggage'] = '7 Kg';
                }
                else{
                    if($search_data['data']['is_domestic'] == 1){
                       $baggage = 15;
                       $arrBaggageDataValue ['Baggage'] = $baggage.' Kg';
                    }
                    else if($search_data['data']['from'] == 'CMB' || $search_data['data']['from'] == 'DXB' || $search_data['data']['from'] == 'MCT' || $search_data['data']['from'] == 'SHJ' || $search_data['data']['from'] == 'DOH' || $search_data['data']['from'] == 'AUH' || $search_data['data']['from'] == 'KWI' || $search_data['data']['to'] == 'CMB' || $search_data['data']['to'] == 'DXB' || $search_data['data']['to'] == 'MCT' || $search_data['data']['to'] == 'SHJ' || $search_data['data']['to'] == 'DOH' || $search_data['data']['to'] == 'AUH' || $search_data['data']['to'] == 'KWI' || $search_data['data']['to'] == 'KWI' || $search_data['data']['to'] == 'HKG' || $search_data['data']['from'] == 'HKG' || $search_data['data']['from'] == 'RUH' || $search_data['data']['to'] == 'RUH' || $search_data['data']['from'] == 'RGN' || $search_data['data']['to'] == 'RGN' || $search_data['data']['from'] == 'IST' || $search_data['data']['to'] == 'IST'){
                            $baggage = 30;
                            $arrBaggageDataValue['Baggage']= $baggage.' Kg';
                    }
                    else{
    ...
    private function flight_detail_format($segment, $flight_key, $trip_type, $is_leg) {
        ...
        $operated_by_airline= '';
        if(valid_array($segment ['air:CodeshareInfo'])){
            $operated_by_airline = $segment ['air:CodeshareInfo'] ['@attributes']['OperatingCarrier'];
            $operated_by_airline_name = $this->get_airline_name($operated_by_airline);
        }
        ...
        $flight_detail_arr ['Operatedbyairline'] = @$operated_by_airline;
        $flight_detail_arr ['Operatedbyairlinename'] = $operated_by_airline_name;
     ...
     */
     function flexi_flight_low_fare_search_req($search_data) {
        $response ['status'] = SUCCESS_STATUS;
        $response ['data'] = array();
        ...
        if($search_data['is_domestic'] == 1){
            $air_india = '<Carrier xmlns="http://www.travelport.com/schema/common_v41_0" Code="AI"/>';
        }
        else{
            $air_india = '';
        }
        ...
        $check_promo_from_date = '2019-10-20T00:00:00';
        $check_promo_to_date = '2019-11-01T00:00:00';
        ...
        if((strtotime($from_date) > strtotime($check_promo_from_date)) && (strtotime($from_date) < strtotime($check_promo_to_date)) ){
            $airpricing_modifiers = '<AirPricingModifiers>
                        <PromoCodes>
                            <PromoCode Code="PRIN0003" ProviderCode="ACH" SupplierCode="6E"/>
                        </PromoCodes>
                    </AirPricingModifiers>';
        }
        $prefered_carrier = "";
        ...
        if ($search_data['trip_type'] == 'return' && $search_data['is_domestic'] != 1) {
            ...
            $depart_date .= '<SearchExtraDays DaysBefore="3" DaysAfter="3" xmlns="http://www.travelport.com/schema/common_v50_0"/>';
            $depart_date .= '</SearchDepTime>';
            $arrival_date = date("Y-m-d", strtotime(str_replace('/', '-', $search_data ['return'])));
            $arrival_date = '<SearchDepTime PreferredTime="' . $arrival_date . '" >';
            $arrival_date .= '<SearchExtraDays DaysBefore="3" DaysAfter="3" xmlns="http://www.travelport.com/schema/common_v50_0"/>';
            $arrival_date .= '</SearchDepTime>';
            $circle = '<SearchAirLeg>
                            <SearchOrigin>
            ...
            $dep_date .= '<SearchExtraDays DaysBefore="3" DaysAfter="3" />';
            $dep_date .= '</SearchDepTime>';
        ...
        //if($search_data['is_domestic'] == 1 ||(get_domain_auth_id() == 257 || get_domain_auth_id() == 331)){
            $Ach_key = 1;
     ...
     */
     function flight_low_fare_search_req($search_data) {
        ...
        if (strlen($search_data["carrier"]) == 2) {
           $prefered_carrier = '<PermittedCarriers>
            ...
            /*if($search_data["carrier"] == 'AI'){
                ...
                $prefered_carrier = '<PermittedCarriers>
                <Carrier xmlns="http://www.travelport.com/schema/common_v41_0" Code="LLL"/>
                </PermittedCarriers>';   
            }*/
            // $prefered_carrier ='';
        ...
        /*else{
            ...
            if(get_domain_auth_id() == 257 || get_domain_auth_id() == 331){
                 $prefered_carrier = '';
            }
            else{
                 $prefered_carrier = '<PermittedCarriers>
                <Carrier xmlns="http://www.travelport.com/schema/common_v41_0" Code="AI"/>
                </PermittedCarriers>';
            }
        }*/
        ...
        //if($search_data['is_domestic'] == 1 ||(get_domain_auth_id() == 257 || get_domain_auth_id() == 331)){
            ...
            /*PrivateFaresOnly <AccountCodes>
									<AccountCode Code="ZGH12 " ProviderCode="1G" SupplierCode="FQ" xmlns="http://www.travelport.com/schema/common_v41_0"/>
								</AccountCodes>*/
        /*}
        else{
            $Ach_key = 0;
        ...
        if ($status = SUCCESS_STATUS) {
            ...
            if (isset($air_price_response ['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
                ...
                if (isset($air_price_result ['air:AirPricingInfo']) && valid_array($air_price_result ['air:AirPricingInfo'])) {
                    ...
                    $flight_details = $this->flight_segment_summary($itenary_details, $air_seg_key, $air_price_result, $random_value, $search_id, $fare_info_response);
            ...
            $travelport_price_xml_new_arr = array(
            ...
            if (!empty($check_price_xml)) {
                ...
                $CI->db->insert('travelport_price_xml_new', $travelport_price_xml_new_arr);
            }
    ...
    public function get_extra_services($request, $search_id, $result_token) {
        ...
        if($provider_code == '1G'){
            ...
            $ssr_request = $this->seat_request($request, $search_id, $check_price_xml[0]['itinerary_xml']);
            // $ssr_request['status'] = FAILURE_STATUS;
            // debug($ssr_request);exit;
            ...
            if($ssr_request['status'] == SUCCESS_STATUS){
                $response ['status'] = SUCCESS_STATUS;
                if( get_domain_auth_id()==406 || get_domain_auth_id()==1){
                     $response ['data']['ExtraServiceDetails']['Seat'] = $ssr_request['data'];
                }
            ...
            //         $response ['data']['ExtraServiceDetails']['Meals'] = $extra_services_data[0]['meal'];
            //     }
            // }
      ...
      public function seat_request($request, $search_id, $itinerary_xml) {
        ...
        $air_itineray = $arrResponse['AirItinerary'];
        $host_token = force_multple_data_format($air_itineray['common_v41_0:HostToken']);
        // debug($host_token);exit;
        $host_token_xml= '';
        foreach($host_token as $air_key => $air_iti){
            // debug($air_iti);exit;
             $host_token_xml .= '<HostToken xmlns="http://www.travelport.com/schema/common_v47_0" Key="'.$air_iti['@attributes']['Key'].'">'.$air_iti['@value'].'</HostToken>';
         // debug($air_itineray);exit;
        }
        // echo $host_token;exit;
        $search_passenger = '';
        $pax = 0;
        for($i=0;$i<$search_data['data']['adult_config']; $i++){
            $search_passenger .= '<air:SearchTraveler Code="ADT" Age="40" Key="'.($pax+1).'">
                                    <Name xmlns="http://www.travelport.com/schema/common_v47_0" Prefix="Mr" First="anitha" Last="Gangagaga" />
                                </air:SearchTraveler>';
            $pax++;
        }
        for($j=0;$i<$search_data['data']['child_config']; $j++){
            $search_passenger .= '<air:SearchTraveler Code="CNN" Age="9" Key="'.$pax.'">
                                    <Name xmlns="http://www.travelport.com/schema/common_v47_0" Prefix="Mr" First="John" Last="Smith" />
                                </air:SearchTraveler>';
            $pax++;
        }
        ...
        for ($se = 0; $se < count($seat_data_new); $se++) {
            ...
            // debug($seat_data_new);exit;
            $code_share_info = $seat_data_new[$se]['CodeshareInfo'];
            // debug($code_share_info);die;
            ...
            $HostTokenRef =  @$pmyseat['HostTokenRef'];
            ...
            $air_segment .= '<air:AirSegment Key="' . $AirSegment_Key . '" Group="' . $Group . '" Carrier="' . $Carrier . '" FlightNumber="' . $FlightNumber . '" Origin="' . $Origin . '" Destination="' . $Destination . '" DepartureTime="' . $DepartureTime . '" ArrivalTime="' . $ArrivalTime . '" FlightTime="' . $FlightTime . '" Equipment="' . $Equipment . '" ChangeOfPlane="' . $ChangeOfPlane . '" ClassOfService="' . $ClassOfService . '" HostTokenRef="' . $HostTokenRef . '" ProviderCode="' . $provider . '">
                <air:CodeshareInfo OperatingCarrier="'.$code_share_info['@attributes']['OperatingCarrier'].'" OperatingFlightNumber="'.$code_share_info['@attributes']['OperatingFlightNumber'].'" /></air:AirSegment>';
    ...
    // echo $seat_xml;exit;
            $seatRes = $this->process_request($seat_xml);
            $api_url = $this->config ['EndPointUrl'];
            $api_request = $update_fare_quote_request ['request'];
            $api_response = $price_response;
            $api_remarks = 'SeatMapping(Travelport Flight)';
            $this->CI->api_model->store_api_request_booking($api_url, $seat_xml, $seatRes, $api_remarks);
            // $seatRes = $this->CI->custom_db->get_static_response (1089);
            // $seatRes[$se] = file_get_contents('http://192.168.0.46/travelomatix_services/SeatMapRes.xml');
            // debug($seatRes[$se]);exit;

            $seatRes_arrar = Converter::createArray($seatRes);
            // debug($seatRes_arrar);die;
            if (isset($seatRes_arrar['SOAP:Body']['SOAP:Fault'])) {
                $seatResformated = $seatRes_arrar['SOAP:Body']['SOAP:Fault']['faultstring'];
            } else {
                $seatResformated = $this->seat_format_ACH($seatRes_arrar);
            }
            // debug($seatResformated);exit;
            if(valid_array($seatResformated)){
                $response['status'] = SUCCESS_STATUS;
                $response['data'] = $seatResformated;
            }
            // debug($response);exit;
            
        return $response;
    }
    public function seat_format_ACH($seat_res){
        $arrSeatData = force_multple_data_format($seat_res['SOAP:Envelope']['SOAP:Body']['air:SeatMapRsp']['air:Rows']);
        $air_segment_data = force_multple_data_format($seat_res['SOAP:Envelope']['SOAP:Body']['air:SeatMapRsp']['air:AirSegment']);
        $segment_data_arr = array();
        foreach($air_segment_data as $seg_key => $seg_data){
            $segment_data_arr[$seg_data['@attributes']['Key']] = $seg_data;
        }
        // debug($segment_data_arr);exit;
        $arrOServ = array();
        if (isset($seat_res['SOAP:Envelope']['SOAP:Body']['air:SeatMapRsp']['air:OptionalServices'])) {
            ...
            if (isset($arrOptionalServices['air:OptionalService'])) {
                ...
                $arrOptSer = force_multple_data_format($arrOptSer);
               
                // debug($arrOptSer);exit;
                foreach ($arrOptSer as $opt_key => $option_service) {
                    $arrOpt_services = $option_service['@attributes'];
                    $air_segment_ref = $option_service['common_v47_0:ServiceData']['@attributes']['AirSegmentRef'];
                    // debug($arrOpt_services);exit;
                    $strSeatKey = $arrOpt_services['Key'];
                    $arrOServ[$strSeatKey][$air_segment_ref] = $arrOpt_services;
                   
                }
            }
        }
         $new_arr_seat_data = array();
        foreach($segment_data_arr as $seg_key => $seg_value){
            foreach($arrSeatData as $s_key => $s_value){
                if($seg_key == $s_value['@attributes']['SegmentRef']){
                    $new_arr_seat_data[$s_key] = $s_value;
                }
            }    
        }
        // debug($new_arr_seat_data);exit;
        $new_arr_seat_data = array_values($new_arr_seat_data);
        // debug($new_arr_seat_data);exit;
        $seat_segment_counter = 0;
        $seat_data_final = array();
       
        foreach($new_arr_seat_data as $seat_key => $seat_data){
            $air_segment_ref = $seat_data['@attributes']['SegmentRef'];
            foreach($seat_data['air:Row'] as $row_key => $row_data){
                $seat_row_counter = 0;
                foreach($row_data['air:Facility'] as $row_key1 => $row_data_val){
                    // debug($arrOServ);exit;
                    if ($row_data_val ['@attributes']['Availability'] == 'Available') {
                        $availability = 1;
                    }
                    else{
                        $availability = 0;
                    }
                    $opt_key = $row_data_val['@attributes']['OptionalServiceRef']; 
                   
                    $total_price = substr($arrOServ[$opt_key][$air_segment_ref]['ApproximateTotalPrice'], 3);
                    $key ['key'][0] = $arrOServ[$opt_key][$air_segment_ref];
                    $key ['key'][0]['Type'] = 'dynamic';
                    $seat_id = serialized_data($key['key']);

                    $seat_row[$seat_row_counter]['AirlineCode'] =  $segment_data_arr[$air_segment_ref]['@attributes']['Carrier'];
                    $seat_row[$seat_row_counter]['FlightNumber'] = $segment_data_arr[$air_segment_ref]['@attributes']['FlightNumber'];
                    $seat_row[$seat_row_counter]['AvailablityType'] = $availability;
                    $seat_row[$seat_row_counter]['RowNumber'] = ( int ) $row_data['@attributes'] ['Number'];
                    $seat_row[$seat_row_counter]['Origin'] = $segment_data_arr[$air_segment_ref]['@attributes']['Origin'];;
                    $seat_row[$seat_row_counter]['Destination'] = $segment_data_arr[$air_segment_ref]['@attributes']['Destination'];;
                    $seat_row[$seat_row_counter]['Price'] = $total_price;
                    $seat_row[$seat_row_counter]['SeatNumber'] = $row_data_val['@attributes'] ['SeatCode'];
                    $seat_row[$seat_row_counter]['Type'] = 'dynamic';
                    $seat_row[$seat_row_counter]['SeatId'] = 'seat_id';
                    $seat_row_counter++;
                }
                if (valid_array($seat_row) == true) {
                    //assign the seat row
                    $seat_data_final[$seat_segment_counter][] = $seat_row;
                }
            }
            $seat_segment_counter++;
        }
        // debug($seat_data_final);exit;
        return $seat_data_final;
        // debug($seat_data_final);exit;
    }
    /**
     * Extra Services
     * @param unknown_type $request
     */
    public function get_extra_services_old($request, $search_id) {
    ...
    private function format_send_change_request($params, $carrier) {
        ...
        if($carrier == '6E'){
            ...
            $AirRefundQuoteReq = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v41_0" xmlns:air="http://www.travelport.com/schema/air_v41_0">
                                <soapenv:Header/>
                                <soapenv:Body>
            ...
            if(valid_array($AirRefundQuoteRes_array) &&(isset($AirRefundQuoteRes_array['SOAP:Envelope']['SOAP:Body']['air:AirRefundQuoteRsp']['air:TCRRefundBundle']['@attributes']['TCRNumber']))){
                ...
                $AirSegment = str_replace("<common_v41_0:HostToken", "<common_v41_0:HostToken xmlns:common_v41_0='http://www.travelport.com/schema/common_v41_0'", $AirSegment);
          
                $CancelBookingReq = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v41_0" xmlns:air="http://www.travelport.com/schema/air_v41_0" >
                                    <soapenv:Header/>
                                    <soapenv:Body>
                                   <air:AirRefundReq AuthorizedBy="user" TraceId="dsfdfd" TargetBranch="' . $this->config ['Target'] . '">
                                    <com:BillingPointOfSaleInfo xmlns:com="http://www.travelport.com/schema/common_v41_0" OriginApplication="UAPI" />
    ...
    function process_booking($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if ($book_service_response['status'] == SUCCESS_STATUS ||  $ach_hold == 'hold') {
            ...
            $book_response = $book_service_response['data']['book_response'];
            if($carrier == '6E'){
                if(isset($book_response['universal:AirCreateReservationRsp']['common_v41_0:ResponseMessage'])){
                    $warning_list = force_multple_data_format($book_response['universal:AirCreateReservationRsp']['common_v41_0:ResponseMessage']);
                    if($warning_list[0]['@value'] == 'Reservation declined by the Vendor' || $warning_list[1]['@value'] == 'Reservation is on hold. Add payment to complete booking'){
            ...
            if($search_data['from_country'] == 'Indonesia' || $search_data['to_country'] == 'Indonesia'){
                $ticketing_status = "HOLD";
            }
        ...
        // $CI = & get_instance();
        // $rand1 = $search_id . "_" . $random_value;
        // $CI->custom_db->delete_record('travelport_price_xml_new',array('serach_id' =>$rand1));
    ...
    private function run_book_service($booking_params, $app_reference, $sequence_number, $search_id, $ach_status='') {
        ...
        $response ['data'] = array(); // Data to be returned
        $t = time();
        $provider = $booking_params['ResultToken']['flight_list'][0]['flight_detail'][0]['provider_code'];
        $domain_origin = get_domain_auth_id();
        // if($domain_origin == 279){
        //    $ach_status = 'hold'; 
        // }
        ...
        if ($book_service_request['status'] == SUCCESS_STATUS) {
            ...
            //$book_service_response = $this->CI->custom_db->single_table_records('provab_api_response_history', '*', array('origin' => 5060))['data'][0]['response'];
    ...
    private function run_book_service_request($params, $search_id, $ach_status) {
        ...
        $CI = &get_instance();
        $check_price_xml = $CI->db_cache_api->get_travelport_flight_price_xml_new($search_id1);
        ...
        /*if($provider == 'ACH'){
            if((strtotime($from_date) > strtotime($check_promo_from_date)) && (strtotime($from_date) < strtotime($check_promo_to_date)) ){
                $airpricing_modifiers = '<AirPricingModifiers AccountCodeFaresOnly="true"><PromoCodes><PromoCode Code="PRIN0003" ProviderCode="ACH" SupplierCode="6E"/>
                            </PromoCodes>
        ...
        /**
         * updated by   indrajeet.marve@provabmail.com
         * date         12-07-2021
         * desc         un-commented above payment address as client had faced issue, and commected below payment address
        ...
        */
        ...
        foreach ($params['Passengers'] as $passenger) {
            ...
            if ($passenger['PaxType'] == 1) {
               ...
               if($provider == 'ACH' && get_domain_auth_id()==395)
                {
                     $email = $passenger['Email'];
                }
                ...
                $adults .= '<BookingTraveler Key="' . $paxId . '" TravelerType="ADT" DOB="' . $pax_dob . '" Gender="' . $gender . '" xmlns="http://www.travelport.com/schema/common_v41_0">
        ...
        $continuity_seg = '<ContinuityCheckOverride xmlns="http://www.travelport.com/schema/common_v41_0" Key="1">pass</ContinuityCheckOverride>';
           ...
           if($ach_status == 'hold'){
                ...
                if($carrier == '6E'){
                    $payment = '<FormOfPayment Type="AgencyPayment" xmlns="http://www.travelport.com/schema/common_v41_0">
                       <AgencyPayment AgencyBillingIdentifier="'.$api_config['api_agent_id'].'" AgencyBillingPassword="'.$api_config['api_password'].'" />
                       </FormOfPayment>';
                      //   $payment = '<FormOfPayment Type="AgencyPayment" xmlns="http://www.travelport.com/schema/common_v41_0">
    ...
    private function save_flight_ticket_details($booking_params, $book_response, $ticket_response, $app_reference, $sequence_number, $search_id){
        ...
        foreach ($passenger_details as $pax_k => $pax_v) {
            ...
            if(empty($ticket_number[$pax_k]['ticket_id']) == false){
                $ticket_id = $ticket_number[$pax_k]['ticket_id'].'1';
     ...
     * @param unknown_type $search_id
     */
    public function get_flexi_search_request($search_id) {
        ...
        if ($search_data ['status'] == SUCCESS_STATUS) {
            ...
            $search_request = $this->flexi_flight_low_fare_search_req($search_data ['data']);
            // debug($search_request);exit;            
            if ($search_request ['status'] = SUCCESS_STATUS) {
                $response ['status'] = SUCCESS_STATUS;
    ...
    private function save_gds_book_response_details($booking_params, $book_response, $app_reference, $sequence_number) {
        ...
        exit;*/
        //added by dinesh
        // $pnr = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['@attributes']['LocatorCode'];    
        $update_data['pnr'] = $pnr;
        
        $update_data['book_id'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['@attributes']['LocatorCode'];
        
        /*added below loator code for cancellation*/
        $book_id_new = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
        
        $update_data['book_id'] = $update_data['book_id'].'_'.$book_id_new;
    ...
    function pre_booking_process($unique_onward_return_booking_source, $source_index, $token_param, $transaction_id, $search_id, & $flight_data) { // debug($source_index);exit;
        ...
        if ($air_price_info ['status'] == SUCCESS_STATUS) {
            ...
            if (!empty($air_price_info ['data'] ['price_xml'])) {
                ...
                $CI = &get_instance();
                $CI->db->insert('tmp_flight_pre_booking_details', $travelport_price_xml_new_arr);
                // $response['status'] = SUCCESS_STATUS;
            }
        }
        return $response;
