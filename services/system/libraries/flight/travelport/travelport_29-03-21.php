      ...
      function format_fare_rule_response($fare_rule_response) {
        ...
        foreach ($fare_rule_response as $k => $v) {
            ...
            foreach ($FareRulesArray as $key => $value) {
                ...
                if (trim($FareRulesArray[$key])=="INR" || trim($FareRulesArray[$key])=="Rs" || trim($FareRulesArray[$key])=="-
INR") {
    ...
    private function fare_rule_request($arrSegmentData) {
        ...
        if($arrSegmentData[0]['ProviderCode'] == '1G'){
            $strRequest = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                <soapenv:Header/>
                <soapenv:Body>
                    <ns2:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v47_0" xmlns:ns2="http://www.travelport.com/schema/air_v47_0" FareRuleType="long" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" AuthorizedBy="' . $this->config ['UserName'] . '">
            ...
            $strRequest = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                <soapenv:Header/>
                <soapenv:Body>
                    <air:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v47_0" xmlns:air="http://www.travelport.com/schema/air_v47_0" FareRuleType="long" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" AuthorizedBy="' . $this->config ['UserName'] . '">
                      <BillingPointOfSaleInfo OriginApplication="uAPI" />';
            //debug($arrSegmentData);exit;
                // for ($s = 0; $s < count($arrSegmentData); $s++) {
    ...
    private function air_create_reservation_request($params, $token_data) {
        ...
        for ($i = 0; $i < $adult_config; $i ++) {
            ...
            $adults .= '<BookingTraveler xmlns="http://www.travelport.com/schema/common_v47_0" Key="0" TravelerType="ADT" DOB="' . date('Y-m-d', strtotime(str_replace('/', '-', $passenger_date_of_birth_arr [$i]))) . '" Gender="' . $gender_pax . '"     >
        ...
        for ($j = 0; $j < $child_config; $j ++) {
            // NEED TO FIX
            // $child_patch .= '<SearchPassenger Code="CNN" Age="10" xmlns="http://www.travelport.com/schema/common_v47_0" />';
        }
        // infant
        $infants = '';
        ...
        if ($provider == "1G") {
            $payment = '<FormOfPayment xmlns="http://www.travelport.com/schema/common_v47_0"  Type="Cash"/>';
            // QueueCategory="01"
            ...
            // $ActionStatus = '<ActionStatus ProviderCode="' . $provider . '" TicketDate="'.$ticketDate.'" Type="TAU" xmlns="http://www.travelport.com/schema/common_v47_0" ></ActionStatus>';Type="ACTIVE"
            $ActionStatus = '<ActionStatus xmlns="http://www.travelport.com/schema/common_v47_0" ProviderCode="' . $provider . '" TicketDate="' . $ticketDate . '" Type="TAU" QueueCategory="01"/>';
        } else {
            ...
            $ActionStatus = '<ActionStatus xmlns="http://www.travelport.com/schema/common_v47_0" ProviderCode="' . $provider . '" TicketDate="T*" Type="TAU" QueueCategory="01"/>';
            // $ActionStatus = '<ActionStatus ProviderCode="' . $provider . '" TicketDate="' . $LatestTicketingTime . '" Type="TAW" xmlns="http://www.travelport.com/schema/common_v47_0" ></ActionStatus>';
        }
        ...
        $AirCreateReservationReq = '<?xml version="1.0" encoding="UTF-8"?>
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                        <s:Header/>
                        <s:Body>
                          <AirCreateReservationReq xmlns="http://www.travelport.com/schema/universal_v47_0" RetainReservation="None" RuleName="COMM" TargetBranch="P7039804">
                            <BillingPointOfSaleInfo xmlns="http://www.travelport.com/schema/common_v47_0" OriginApplication="UAPI"/>
                            ' . $adults . '
                            ' . $childs . '
                            ' . $infants . '
                            ' . $payment . '
    ...
    private function format_search_data_response($low_fare_search_res_data, $search_data) {
        ...
        foreach ($low_fare_search_res_data as $key => $low_fare_search_res) {
            ...
            if(isset($low_fare_search_res['air:LowFareSearchRsp']['common_v47_0:ResponseMessage'])){
                $warning = force_multple_data_format($low_fare_search_res['air:LowFareSearchRsp']['common_v47_0:ResponseMessage']);
                foreach ($warning as $w_key => $w_value) {
                   $needle = 'NNS6E : IndiGo Flights may not be available for'; 
                   if(strpos($w_value['@value'],$needle)  !== false){
                        $response['status'] = FALSE;
                        return $response;
                   }
                }
            ...
            if(!empty($low_fare_search_res['air:LowFareSearchRsp']['air:FlightDetailsList']['air:FlightDetails'])){
                ...
                $air_flight_list = $low_fare_search_res ['air:LowFareSearchRsp'] ['air:FlightDetailsList']['air:FlightDetails'];
                $air_flight_list_arr = array();
                foreach($air_flight_list as $air_f_key => $air_flight_data){
                    $air_flight_list_arr[$air_flight_data['@attributes']['Key']] = $air_flight_data['@attributes'];
                }
                ...
                if((isset($air_host_token_list['common_v47_0:HostToken'][0]['@attributes'])) && valid_array($air_host_token_list)){
                    
                    foreach($air_host_token_list['common_v47_0:HostToken'] as $host_key => $host_value){
                ...
                if (isset($air_pricing_solutions_data) && valid_array($air_pricing_solutions_data)) {
                    foreach ($air_pricing_solutions_data as $ap_key => $pricing_array) {
                        $flight_data = $this->format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData, $air_flight_list_arr);
                        if(valid_array($flight_data['FlightDetails'])){
                             $flight_data_list [] = $flight_data;
                        }
                    } 
                }
     ...
     * array $pricing_array
     */
    function format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list1, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData, $air_flight_list) {
       // debug($pricing_array);exit;
        ...
        if (isset($pricing_array['air:AirPricingInfo'][0])) {
            ...
            if (isset($AirPricingInfo_Attr->Refundable) && ( $AirPricingInfo_Attr->Refundable == "true")) {
                $Refundable = true;
            } else {
                $Refundable = false;
            }
            if($AirPricingInfo_Attr->ProviderCode == 'ACH'){
                $Refundable = true;
                $provider_code = 'ACH';
            }
        ...
        $flight_journey_price ['Price']['PassengerBreakup']['ADT']['PassengerCount'] = $passenger_count['ADT'];
        $flight_journey_price ['Price']['PassengerBreakup']['ADT']['BasePrice'] = $passenger_count['ADT']*$Pass_BasePrice['ADT'];
        $flight_journey_price ['Price']['PassengerBreakup']['ADT']['Tax'] = $passenger_count['ADT']*($Pass_Taxes['ADT']+$Pass_Fees['ADT']);
        $flight_journey_price ['Price']['PassengerBreakup']['ADT']['TotalPrice'] = $passenger_count['ADT']*($Pass_BasePrice['ADT'] + $Pass_Taxes['ADT']+$Pass_Fees['ADT']);
        if (isset($passenger_count['CNN'])) {
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['PassengerCount'] = $passenger_count['CNN'];
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['BasePrice'] = $passenger_count['CNN']*$Pass_BasePrice['CNN'];
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['Tax'] = $passenger_count['CNN']*($Pass_Taxes['CNN']+$Pass_Fees['CNN']);
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['TotalPrice'] = $passenger_count['CNN']*($Pass_BasePrice['CNN'] + $Pass_Taxes['CNN']+$Pass_Fees['CNN']);
        } 
        
        else if (isset($passenger_count['CHD'])) {
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['PassengerCount'] = $passenger_count['CHD'];
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['BasePrice'] = $passenger_count['CHD']*$Pass_BasePrice['CHD'];
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['Tax'] = $passenger_count['CHD']*($Pass_Taxes['CHD']+$Pass_Fees['CHD']);
            $flight_journey_price ['Price']['PassengerBreakup']['CHD']['TotalPrice'] = $passenger_count['CHD']*($Pass_BasePrice['CHD'] + $Pass_Taxes['CHD']+$Pass_Fees['CHD']);
        }
       
      
        if (isset($passenger_count['INF'])) {
            $flight_journey_price ['Price']['PassengerBreakup']['INF']['PassengerCount'] = $passenger_count['INF'];
            $flight_journey_price ['Price']['PassengerBreakup']['INF']['BasePrice'] = $passenger_count['INF']*$Pass_BasePrice['INF'];
            $flight_journey_price ['Price']['PassengerBreakup']['INF']['Tax'] = $passenger_count['INF']*($Pass_Taxes['INF']+$Pass_Fees['INF']);
            $flight_journey_price ['Price']['PassengerBreakup']['INF']['TotalPrice'] = $passenger_count['INF']*($Pass_BasePrice['INF']+ $Pass_Taxes['INF']+$Pass_Fees['INF']);
        }
                            ...
                            foreach ($air_segment_list as $as_key => $segment) {
                                if($provider_code == 'ACH'){
                                    if($search_data['trip_type'] == 'multicity'){
                                        if($j_key == 0 && $segment['@attributes']['Group'] ==1){
                                            $check_segment = true;
                                        }
                                        else if($j_key !=0){
                                            $check_segment = true;
                                ...
                                $operator_code = $segment['@attributes']['Carrier'];
                                if($check_segment == true){
                                    $air_segment_arr ['flight_detail_ref'] = $flight_detail_key = @$segment ['air:FlightDetailsRef'] ['@attributes'] ['Key'];
                                    $flight_details_ref = $segment ['air:FlightDetailsRef'];
                                    $flight_details_ref = force_multple_data_format($flight_details_ref);
                                    ...
                                    if ($flight_key == $segment_ref) {
                                        if (isset($air_code_share_info) && valid_array($air_code_share_info)) {
                                         ...
                                         # For Blocking Airline from Specific API
                                          ...
                                          // if($segment ['@attributes'] ['Carrier'] == '6E'){
                                          //   $Refundable = true;
                                          // } 
                                        if(count($flight_details_ref)  == 1){
                                        // booking_code
                                        ...
                                        if(count($flight_details_ref) > 1){
                                            foreach($flight_details_ref as $flight_d_key => $flight_det_ref){
                                                $segment1 = $segment;
                                                ...
                                                $flight_list_on_out ['connection'] = $connection;
                                                $segment_list = $air_flight_list[$flight_det_ref['@attributes']['Key']];
                                                $segment ['@attributes'] ['Origin'] = $segment_list['Origin'];
                                                $segment ['@attributes'] ['Destination'] = $segment_list['Destination'];
                                                $segment ['@attributes'] ['DepartureTime'] = $segment_list['DepartureTime'];
                                                $segment ['@attributes'] ['ArrivalTime'] = $segment_list['ArrivalTime'];
                                                $segment ['@attributes'] ['FlightTime'] = $segment_list['FlightTime'];
                                                $segment ['@attributes'] ['TravelTime'] = $segment_list['TravelTime'];
                                                $segment ['@attributes'] ['Equipment'] = $segment_list['TravelTime'];
        ...
        if (isset($air_pricing_info) && valid_array($air_pricing_info)) {
            ...
            $fareinfo_list = force_multple_data_format($AirPricingInfo['air:FareInfoRef']);
            $fare_type_display ='';
            if(empty($fare_info_key_array[$fareinfo_list[0]['@attributes']['Key']]) == false){
                $fare_type = $fare_info_key_array[$fareinfo_list[0]['@attributes']['Key']];
                $fare_type_display = $fare_info_key_array[$fareinfo_list[0]['@attributes']['Key']];
            }
            else{
            ...
            if (isset($booking_details_flight) && valid_array($booking_details_flight)) {
                ...
                if(valid_array($flight_key_list)){
                    foreach ($booking_details_flight as $b_fkey => $booking) {
                        ...
                        if(isset($flight_journey['FlightDetails']['Details'][0][0]['OperatorCode']) && $flight_journey['FlightDetails']['Details'][0][0]['OperatorCode'] == '6E'){
                            if($fare_type == "Lite Fare"){
                                $arrBaggageData[$strFare_Ref_Key]['Baggage']= '0 Kg';
                            }
                            else{
                                if($search_data['is_domestic'] == 1){
                                    $arrBaggageData[$strFare_Ref_Key]['Baggage']= '15 Kg';
                                }
                                else if($search_data['from'] == 'CMB' || $search_data['from'] == 'DXB' || $search_data['from'] == 'MCT' || $search_data['from'] == 'SHJ' || $search_data['from'] == 'DOH' || $search_data['from'] == 'SIN' || $search_data['from'] == 'KWI'
                                   || $search_data['to'] == 'CMB' || $search_data['to'] == 'DXB' || $search_data['to'] == 'MCT' || $search_data['to'] == 'SHJ' || $search_data['to'] == 'DOH' || $search_data['to'] == 'SIN' || $search_data['to'] == 'KWI'){
                        ...
                        if($hand_baggage['status'] == SUCCESS_STATUS){
                            $arrBaggageData[$strFare_Ref_Key]['CabinBaggage']= $hand_baggage['data'][0]['weight'].' Kg';
                        }
            ...
            if(valid_array($flight_journey['FlightDetails'])){
                $carrier = @$flight_journey['FlightDetails']['Details'][0][0]['OperatorCode'];
                $check_comm = $CI->flight_model->check_commision_travelport($carrier, $search_data['is_domestic'], $this->booking_source);
                ...
                $flight_journey['Price']['PassengerBreakup'] = $flight_journey_price['Price']['PassengerBreakup'];
                // $flight_journey ['Price'] ['pax_breakup'] = array();
                $flight_journey ['Attr']['IsRefundable'] = $Refundable;
                if($provider_code == 'ACH'){
                    $flight_journey ['Attr']['IsLCC'] = true;
                }
                else{
                    $flight_journey ['Attr']['IsLCC'] = false;
                }
                $airline_remark ='';
                if($fare_type == "Lite Fare"){
                    $airline_remark = 'This is a hand bag fare only result. No checked in baggage is allowed.';
                }
                $flight_journey ['Attr']['AirlineRemark'] = $airline_remark;
                $flight_journey ['Attr']['FareType'] = $fare_type_display;
            }
        ...
        if(valid_array($flight_journey['FlightDetails'])){
           ...
           // $flight_journey ['booking_source'] = $this->booking_source;
            ...
            if($search_data['trip_type'] == 'return' && empty($search_data['is_domestic'] == true)){
                ...
                if(count($flight_journey['FlightDetails']['Details']) > 1){
  ...
  private function flight_segment_summary($journey_detail, $air_seg_key, $air_price_result, $random_value, $search_id) {
        ...
        $air_segment_list_price = force_multple_data_format($air_segment_list);
       
        $carrier = $air_segment_list_price[0]['@attributes']['Carrier'];
        if (isset($air_pricing_info) && valid_array($air_pricing_info)) {
            if (isset($air_pricing_info[0] ['air:BookingInfo'])) {
                $booking_details_flight = force_multple_data_format(@$air_pricing_info[0] ['air:BookingInfo']);
            }
        ...
        if (isset($air_pricing_info) && valid_array($air_pricing_info)) {
            ...
            $carrier = $flight_journey['FlightDetails']['Details'][0][0]['OperatorCode'];
            if($flight_journey['FlightDetails']['Details'][0][0]['OperatorCode'] == '6E'){
                $pass_count = $search_data['data']['adult_config']+$search_data['data']['child_config'];
                if($search_data['data']['is_domestic'] == 1){
                   $baggage = 15;
                   $arrBaggageDataValue ['Baggage'] = $pass_count*$baggage.' Kg';
                }
                else if($search_data['data']['from'] == 'CMB' || $search_data['data']['from'] == 'DXB' || $search_data['data']['from'] == 'MCT' || $search_data['data']['from'] == 'SHJ' || $search_data['data']['from'] == 'DOH' || $search_data['data']['from'] == 'SIN' || $search_data['data']['from'] == 'KWI' || $search_data['data']['to'] == 'CMB' || $search_data['data']['to'] == 'DXB' || $search_data['data']['to'] == 'MCT' || $search_data['data']['to'] == 'SHJ' || $search_data['data']['to'] == 'DOH' || $search_data['data']['to'] == 'SIN' || $search_data['data']['to'] == 'KWI'){
                        $baggage = 30;
                        $arrBaggageDataValue['Baggage']= $pass_count*$baggage.' Kg';
                }
                else{
            ...
            $check_comm = $CI->flight_model->check_commision_travelport($carrier, $is_domestic, $this->booking_source);
        ...
        //debug($token_data);exit;
        $flight_journey ['token_data'] = $token_data;
        $flight_journey ['Attr'] = @$arrBaggageDataValue;
        $flight_journey ['Attr']['IsRefundable'] = $Refundable;
        $flight_journey ['Attr']['AirlineRemark'] = 'Flight Booking';
        // $flight_journey ['booking_source'] = $this->booking_source;
        // $flight_journey ['token_key'] = md5($flight_journey ['ResultToken']);
        // $flight_journey ['flight_details']['summery'] = $flight_list_on_out;
    ...
    private function flight_detail_format($segment, $flight_key, $trip_type, $is_leg) {
        ...
        $flight_detail_arr ['Duration'] = @$duration_sec;
     ...
     */
     function flight_low_fare_search_req($search_data) {
        ...
        if($search_data['is_domestic'] == 1){
            $air_india = '<Carrier xmlns="http://www.travelport.com/schema/common_v47_0" Code="AI"/>';
        ...
        if($search_data['trip_type'] == 'multicity'){
            $from_date = $search_data['depature'][0];
        }
        else{
            $from_date = $search_data['depature'];
        }
        $check_promo_from_date = '2019-06-03T00:00:00';
        $check_promo_to_date = '2019-06-30T00:00:00';
        $airpricing_modifiers ='';
        if((strtotime($from_date) > strtotime($check_promo_from_date)) && (strtotime($from_date) < strtotime($check_promo_to_date)) ){
            $airpricing_modifiers = '<AirPricingModifiers>
                        <PromoCodes>
                            <PromoCode Code="AGTSPL01" ProviderCode="ACH" SupplierCode="6E"/>
                        </PromoCodes>
                    </AirPricingModifiers>';
        }
        /* paxes details */
        $prefered_carrier1 = "";
        // debug($search_data);exit;
        if (strlen($search_data["carrier"]) == 2) {
           $prefered_carrier1 = '<PermittedCarriers>
                        <Carrier xmlns="http://www.travelport.com/schema/common_v47_0" Code="' . $search_data["carrier"] . '"/>
                        </PermittedCarriers>';
            if($search_data["carrier"] == 'AI'){
                ...
                $prefered_carrier = '<PermittedCarriers>
                <Carrier xmlns="http://www.travelport.com/schema/common_v47_0" Code="LLL"/>
                </PermittedCarriers>';   
            }
            // $prefered_carrier ='';
            ...
            $prefered_carrier = '<PermittedCarriers>
            <Carrier xmlns="http://www.travelport.com/schema/common_v47_0" Code="AI"/>
            </PermittedCarriers>';
        }

        
        $AirSearchModifiers = '<AirSearchModifiers MaxSolutions="100"><PreferredProviders>
                     <Provider xmlns="http://www.travelport.com/schema/common_v47_0" Code="1G" />
                     </PreferredProviders>
                     ' . $prefered_carrier . '
                    </AirSearchModifiers>';
         $AirSearchModifiers1 = '<AirSearchModifiers MaxSolutions="200"><PreferredProviders>
                     <Provider xmlns="http://www.travelport.com/schema/common_v47_0" Code="ACH" />
                     </PreferredProviders>
                     ' . $prefered_carrier1 . '
                    </AirSearchModifiers>';
         $cabin_tag="";
        ...
        $LegModifiers = '<AirLegModifiers><PermittedCabins>
                            <CabinClass Type="' . $cabin_class . '" xmlns="http://www.travelport.com/schema/common_v47_0"></CabinClass>
                        </PermittedCabins>
                        </AirLegModifiers>
                        ';
        
        $LegModifiers1 = '<AirLegModifiers><PermittedCabins>
                            <CabinClass Type="' . $cabin_class . '" xmlns="http://www.travelport.com/schema/common_v47_0"></CabinClass>
                       </PermittedCabins>
                        </AirLegModifiers>';
        $infant = '';
        ...
        for ($i = 0; $i < $search_data ['adult']; $i ++) {
            $adult .= '<SearchPassenger Code="ADT" xmlns="http://www.travelport.com/schema/common_v47_0" />';
        }
        for ($i = 0; $i < $search_data ['child']; $i ++) {
            $child .= '<SearchPassenger Code="CNN" Age="10" xmlns="http://www.travelport.com/schema/common_v47_0" />';
        }
        for ($i = 0; $i < $search_data ['child']; $i ++) {
            $child_ach .= '<SearchPassenger Code="CHD" Age="10" xmlns="http://www.travelport.com/schema/common_v47_0" />';
        }
        for ($i = 0; $i < $search_data ['infant']; $i ++) {
            $infant .= '<SearchPassenger Code="INF" Age="01" xmlns="http://www.travelport.com/schema/common_v47_0" />';
        }
        ...
        if ($search_data['trip_type'] == 'return' && $search_data['is_domestic'] != 1) {
            ...
            $circle = '<SearchAirLeg>
                            <SearchOrigin>
                                <CityOrAirport Code="' . $origin_code . '" xmlns="http://www.travelport.com/schema/common_v47_0" />
                            </SearchOrigin>
                            <SearchDestination>
                                <CityOrAirport Code="' . $desti_code . '" xmlns="http://www.travelport.com/schema/common_v47_0" />
                            </SearchDestination>
                             ' . $depart_date . '
                             ' . $LegModifiers . '
                        </SearchAirLeg>';
            ...
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                ...
                <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                    <' . $search_method . ' SolutionResult="true" AuthorizedBy="user" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v47_0">
            ...
            $request1 ['payload_return'][$ach_return_key] = $request_ach;
            // debug($request1);exit;
            ...
            for ($mul = 0; $mul < count($arrMultiFrmCity); $mul++) {
                ...
                $oneway_gds .= '<SearchAirLeg>
                                    <SearchOrigin>
                                        <CityOrAirport Code="' . $origin_code_m . '" xmlns="http://www.travelport.com/schema/common_v47_0" />
                                    </SearchOrigin>
                                    <SearchDestination>
                                        <CityOrAirport Code="' . $desti_code_m . '" xmlns="http://www.travelport.com/schema/common_v47_0" />
                                    </SearchDestination>
                                    ' . $dep_date_m . '
                                    ' . $LegModifiers . '
                                    </SearchAirLeg>';
        ...
        $travelport_request[$Ach_key] = '<?xml version="1.0" encoding="utf-8"?>
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                <s:Header>
                    <Action s:mustUnderstand="1" xmlns="http://schemas.microsoft.com/ws/2005/05/addressing/none">localhost:8080/kestrel/AirService</Action>
                </s:Header>
    ...
    public function format_update_fare_quote_response($air_price_response, $price_response, $search_id, $flight_details_key, $price_request, $api_request) {
        // echo $api_request;exit;
        ...
        if ($status = SUCCESS_STATUS) {
            ...
            if (isset($air_price_response ['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
                ...
                // AirPricingSolution
                $air_pricing_sol ['AirPricingSolution'] ['@attributes'] ['xmlns'] = "http://www.travelport.com/schema/air_v47_0";
                    ...
                    foreach ($air_price_result ['air:AirPricingInfo'] as $ap_key => $air_price_sol) {
                        ...
                        if (isset($fare_air_pass_info) && valid_array($fare_air_pass_info)) {
                            foreach ($fare_air_pass_info as $fapass_k => $fare_passenger_info) {
                                ...
                                $passenger_list[$fare_passenger_info['@attributes']['BookingTravelerRef']] = $fapass_k.$fare_passenger_info['@attributes']['Code'];
                            }
                        }
                        // air:ChangePenalty
                    ...
                    if (isset($itenary_details ['air:AirSegment']) && valid_array($itenary_details ['air:AirSegment'])) {
                        ...
                        foreach ($itenary_details as $it_key => $air_segment) {
                            if (in_array($air_segment ['@attributes'] ['Key'], $air_seg_key)) {
                                $segment_list[$air_segment ['@attributes'] ['Key']]['Origin'] = $air_segment ['@attributes'] ['Origin'];
                                $segment_list[$air_segment ['@attributes'] ['Key']]['Destination'] = $air_segment ['@attributes'] ['Destination'];
                            ...
                            /*if (isset($air_price_result ['air:OptionalServices']) && valid_array($air_price_result ['air:OptionalServices'])) {
                                ...
                                foreach($booking_traveller_key as $b_key => $b_value){
                                    $passenger_xml .= '<SearchPassenger BookingTravelerRef="' . $b_value . '" Code="'.$b_key.'" xmlns="http://www.travelport.com/schema/common_v47_0" ></SearchPassenger>';
                                }
                                ...
                                foreach ($xp->query('//AirPriceReq') as $key => $tn) {
                                    $tn->setAttribute('xmlns','http://www.travelport.com/schema/air_v47_0');
                                    $tn->setAttribute('xmlns:common_v47_0','http://www.travelport.com/schema/common_v47_0');
                                }
                                ...
                                foreach ($air_price_result ['air:OptionalService'] as $opt_key => $opt_service) {
                                    ...
                                    $opt_service_xml = str_replace('common_v47_0:', '', $opt_service_xml);
                                    ...
                                    if ($opt_service['@attributes']['Type'] == 'MealOrBeverage') {
                                        ...
                                        $AirSegmentRef = $opt_service['common_v47_0:ServiceData']['@attributes']['AirSegmentRef'];
                                        if($AirSegmentRef == $air_segment['@attributes']['Key']){
                                            if(!in_array($opt_service['common_v47_0:ServiceInfo']['common_v47_0:Description'], $meal_desc_array)){
                                                ...
                                                $meal_array [$it_key][$op]['Description'] = $opt_service['common_v47_0:ServiceInfo']['common_v47_0:Description'];
                                                ...
                                                $key ['key'][$op]['Description'] = $opt_service['common_v47_0:ServiceInfo']['common_v47_0:Description'];
                                                $meal_id = serialized_data($key['key']);
                                                $meal_array[$it_key][$op]['MealId'] = $meal_id;
                                                array_push($meal_desc_array, $opt_service['common_v47_0:ServiceInfo']['common_v47_0:Description']);
                                                $op++;
                                            }
               ...
               // debug($meal_array);exit;
                ...
                $air_price_result ['common_v47_0:HostToken'] = force_multple_data_format($air_price_result ['common_v47_0:HostToken']);
                if (isset($air_price_result ['common_v47_0:HostToken']) && valid_array($air_price_result ['air:FareNote'])) {
                    foreach ($air_price_result ['common_v47_0:HostToken'] as $af_not_k => $air_host_token) {
                ...
                //$air_pricing_xml = str_replace('air:', '', str_replace('common_v47_0:', '', $air_pricing_xml));
            ...
            /* meal & baggage for ACH Indigo */
            
            $optional_services = $air_price_result['air:OptionalServices']['air:OptionalService'];
            $baggage_array = array();
            $baggage_booking_data = array();

            // debug($optional_services);exit;
            // $baggage_ii = 0 ;
            foreach($optional_services as $opt_key => $option){

                if($option['@attributes']['Type'] == 'Baggage'){
                    // debug($option);exit;
                    $type = 'domestic';
                    if (strpos($option['@attributes']['ProviderDefinedType'], 'IN') !== false) {
                        $type = 'international';
                    }
                    if($type != 'international'){
                        $passenger_code = '';
                        $price = substr($option['@attributes']['ApproximateTotalPrice'], 3);
                        if($price > 0){
                            $bagagge = substr($option['@attributes']['ProviderDefinedType'],-2);
                            $AirSegmentRef = $option['common_v47_0:ServiceData']['@attributes']['AirSegmentRef'];
                            $BookingTravelerRef = $option['common_v47_0:ServiceData']['@attributes']['BookingTravelerRef'];
                            $origin = $segment_list[$AirSegmentRef]['Origin'];
                            $destination = $segment_list[$AirSegmentRef]['Destination'];
                            $passenger_code = $passenger_list[$BookingTravelerRef];
                            $baggage_ii = $bagagge.' Kg';
                            $baggage_booking_data[$origin.'-'.$destination][$passenger_code][$baggage_ii]['OptionalServiceKey'] = $option['@attributes']['Key'];
                            $baggage_booking_data[$origin.'-'.$destination][$passenger_code][$baggage_ii]['OptionalServiceKey'] = $option['@attributes']['Key'];
                            $baggage_booking_data[$origin.'-'.$destination][$passenger_code][$baggage_ii]['BookingTravelerRef'] = $BookingTravelerRef;
                            $baggage_booking_data[$origin.'-'.$destination][$passenger_code][$baggage_ii]['TaxInfoKey'] = $option['air:BrandingInfo']['@attributes']['Key'];
                            // $baggage_ii++;
                        }
                    }
                }
            ...
            $display_text = array();
            $baggage_i = 0;
            foreach($baggage_booking_data as $bg_key => $bg_data){
                $bg_data = array_values($bg_data);
                $baggage_booking_data[$bg_key] = $bg_data;
            }
            // debug(array_values($baggage_booking_data));exit;
            foreach($optional_services as $opt_key => $option){
                if($option['@attributes']['Type'] == 'Baggage'){
                    $type = 'domestic';
                    if (strpos($option['@attributes']['ProviderDefinedType'], 'IN') !== false) {
                    ...
                    if($type != 'international'){
                        ...
                        if($price > 0){
                            $origin = $segment_list[$AirSegmentRef]['Origin'];
                            $display_txt = $option['@attributes']['DisplayText'].'_'.$origin;
                            
                            if(!in_array($display_txt, $display_text)){
                                $AirSegmentRef = $option['common_v47_0:ServiceData']['@attributes']['AirSegmentRef'];
                                $BookingTravelerRef = $option['common_v47_0:ServiceData']['@attributes']['BookingTravelerRef'];
                                $bagagge = substr($option['@attributes']['ProviderDefinedType'],-2);
                                
                                $destination = $segment_list[$AirSegmentRef]['Destination'];
                                $xml = ArrayToXML::createXML('OptionalService', $option);
                                $opt_service_xml = str_replace('<?xml version="1.0" encoding="UTF-8"?>', '', $xml->saveXML());
                                $opt_service_xml = str_replace('common_v41_0:', '', $opt_service_xml);
                                $opt_service_xml = str_replace('air:', '', $opt_service_xml);
                                $passenger_code = $passenger_list[$BookingTravelerRef];
                                
                                $key_baggage ['key'][0]['Code'] = $option['@attributes']['Key'];
                                $key_baggage ['key'][0]['Weight'] = $bagagge.' Kg';
                                $key_baggage ['key'][0]['Baggagexml'] = $opt_service_xml;
                                $key_baggage ['key'][0]['baggage_book_data'] = array_values($baggage_booking_data);
                                $baggage_id = serialized_data($key_baggage['key']);
                                
                                $baggage_array[$origin.'-'.$destination][$baggage_i]['Code'] = $option['@attributes']['Key'];
                                $baggage_array[$origin.'-'.$destination][$baggage_i]['Price'] = $price;
                                $baggage_array[$origin.'-'.$destination][$baggage_i]['Weight'] = $bagagge.' Kg';
                                $baggage_array[$origin.'-'.$destination][$baggage_i]['Origin'] = $origin;
                                $baggage_array[$origin.'-'.$destination][$baggage_i]['Destination'] = $destination;
                                $baggage_array[$origin.'-'.$destination][$baggage_i]['BaggageId'] = $baggage_id;
                                $baggage_i++;

                                array_push($display_text, $display_txt);
                            }
                        }
            ...
            $baggage_array= array_values($baggage_array);
            // debug($baggage_array);exit;
            $token_data = $flight_details['JourneyList'][0][0]['token_data'];
            $token_data[0]['bagagge'] = $baggage_array;
            unset($flight_details['JourneyList'][0][0]['token_data']);
            $flight_details['JourneyList'][0][0]['ResultToken'] = serialized_data($token_data);
            $flight_details ['bagagge'] = $baggage_array;
        }
    ...
    public function get_update_fare_quote($request, $search_id) {
        ...
        if ($update_fare_quote_request['status'] == SUCCESS_STATUS) {
            ...
            $price_response = file_get_contents(FCPATH."travelport_xmls/travelport_meals.xml");
            ...
            if (valid_array($update_fare_quote_response) == true && isset($update_fare_quote_response['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
                $response ['status'] = SUCCESS_STATUS;
                $response ['data']['FareQuoteDetails'] = $this->format_update_fare_quote_response($update_fare_quote_response, $price_response, $search_id, $flight_details_key, $update_fare_quote_request['request_opt'], $api_request);
            } else {
                $response ['message'] = 'Not Available';
            }
        } else {
    ...
    private function update_fare_quote_request($token, $search_id) {
        ...
        if ($adult_count != 0) {
            for ($i = 0; $i < $adult_count; $i ++) {
                $adults .= '<SearchPassenger BookingTravelerRef="' . $paxId . '" Code="ADT" xmlns="http://www.travelport.com/schema/common_v47_0" ></SearchPassenger>';
                $paxId ++;
            }
        }
        if ($child_count != 0) {
            for ($k = 0; $k < $child_count; $k ++) {
                $childs .= '<SearchPassenger BookingTravelerRef="' . $paxId . '" Code="'.$child_type.'" Age="10" xmlns="http://www.travelport.com/schema/common_v47_0" ></SearchPassenger>';
                $paxId ++;
            }
        }
        if ($infant_count != 0) {
            for ($j = 0; $j < $infant_count; $j ++) {
                $infants .= '<SearchPassenger BookingTravelerRef="' . $paxId . '" Code="INF" Age="01" xmlns="http://www.travelport.com/schema/common_v47_0" ></SearchPassenger>';
                $paxId ++;
            }
        }
                ...
                if (isset($flights_list ['flight_detail']) && valid_array($flights_list ['flight_detail'])) {
                    ...
                    // debug($flight);exit;
                        ...
                        if (isset($flight['host_token_key']) && !empty($flight['host_token_key'])) {
                            $host_token .= '<HostToken xmlns="http://www.travelport.com/schema/common_v47_0" Key="'.$flight['host_token_key'].'">'.$flight['host_token_value'].'</HostToken>';
        ...
        $PermittedCabins = '<PermittedCabins>
                            <CabinClass xmlns="http://www.travelport.com/schema/common_v47_0" Type="'.$cabin_class.'"/>
                            </PermittedCabins>';
        $all_price_modfier ='';
        $form_of_payment ='';
        if ($provider_code == '1G') {
            $all_price_modfier = '<AirPricingModifiers FaresIndicator="AllFares">'.$PermittedCabins.'</AirPricingModifiers>';
            $form_of_payment = '<FormOfPayment xmlns="http://www.travelport.com/schema/common_v47_0"  Type="Cash"></FormOfPayment>';
        }
        if($provider_code == 'ACH'){
            $gst_number = '27AABCA9521E1ZX'; //29AANCA8324M2Z0
            $company_name = 'ITQ'; //ACCENTRIA SOLUTIONS PRIVATE LIMITED
            $gst_email = 'anitha.g.provab//gmail.com'; // vinay//travelomatix.com
            $gstn = '/IND/'.$gst_number.'/'.$company_name;
            $gste = '/IND/'.$gst_email;
            $form_of_payment = '<com:SSR Type="GSTN" Carrier="6E" Key="11" Status="HK" FreeText="'.$gstn.'" />
            <com:SSR Type="GSTE" Carrier="6E" Key="22" Status="HK" FreeText="'.$gste.'" />';
           
        }
        if($search_data['data']['trip_type'] == 'multicity'){
            $from_date = $search_data['data']['depature'][0];
        }
        else{
            $from_date = $search_data['data']['depature'];
        }
        ...
        $search_passenger = '<SearchPassenger Key="0" Code="ADT" xmlns="http://www.travelport.com/schema/common_v47_0" ></SearchPassenger>';
        ...
        $air_price_req = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
            ...
            <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                <AirPriceReq TraceId="394d96c00971c4545315e49584609ff6" CheckOBFees="All" AuthorizedBy="user" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v47_0" xmlns:common_v47_0="http://www.travelport.com/schema/common_v47_0" xmlns:com="http://www.travelport.com/schema/common_v47_0">
                    <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v47_0"></BillingPointOfSaleInfo>
                    <AirItinerary>';
        ...
        $air_price_req .=  $host_token .'
                      ...
                      ' . $childs . '
                      ' . $infants . '
                    <AirPricingCommand xmlns="http://www.travelport.com/schema/air_v47_0" CabinClass="'.$cabin_class.'">' . $AirSegmentPricingModifiers1.'</AirPricingCommand>
                   ' . $form_of_payment . ' 
                </AirPriceReq>
            </s:Body>
        </s:Envelope>';
    ...
    public function get_extra_services($request, $search_id, $result_token) {
        ...
        if($provider_code == '1G'){
            ...
            if ($result_token['status'] == SUCCESS_STATUS) {
                ...
                $extra_services_data = unserialized_data($flight_search_data['ResultToken']);
                // debug($extra_services_data);exit;
                if (isset($extra_services_data[0]['bagagge']) && valid_array($extra_services_data[0]['bagagge'])) {
                    $response ['status'] = SUCCESS_STATUS;
                    $response ['message'] = ''; // Message to be returned
                    $response ['data']['ExtraServiceDetails']['Baggage'] = $extra_services_data[0]['bagagge'];
                }
            }
    ...
    public function seat_request($request, $search_id, $itinerary_xml) {
        ...
        for ($se = 0; $se < count($seat_data_new); $se++) {
            ...
            $seat_xml = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                            <soapenv:Header />
                            <soapenv:Body>
                                <air:SeatMapReq TargetBranch="' . $this->config ['Target'] . '" TraceId="394d96c00971c4545315e49584609ff6" ReturnSeatPricing="true" AuthorizedBy="uAPI" xmlns:com="http://www.travelport.com/schema/common_v47_0" xmlns:univ="http://www.travelport.com/schema/universal_v47_0" xmlns:air="http://www.travelport.com/schema/air_v47_0">
                                    <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v47_0" />
    ...
    public function cancel_booking($request) {
        ...
        if ($elgible_for_ticket_cancellation['status'] == SUCCESS_STATUS) {
            ...
            $carrier = $booking_details['booking_itinerary_details'][0]['operating_carrier'];
            ...
            if($carrier == '6E'){
                if($request['IsFullBookingCancel'] == 1){
                    $send_change_request = $this->send_change_request($request_params, $carrier);
                }
                else{
                    $send_change_request['status'] = FAILURE_STATUS;
                    $send_change_request['message'] = 'Indigo Partial Cancellation will not be happen';
                }
            }
     ...
     */
    private function send_change_request($request_params, $carrier) {
       
        $response ['status'] = FAILURE_STATUS; // Status Of Operation
        $response ['message'] = ''; // Message to be returned
        $response ['data'] = array(); // Data to be returned
        $send_change_request = $this->format_send_change_request($request_params, $carrier);
        // debug($send_change_request);
        if ($send_change_request['status'] == SUCCESS_STATUS) {
            
           
            if($carrier !='6E'){
                $api_url = 'https://americas.universal-api.travelport.com/B2BGateway/connect/uAPI/UniversalRecordService';
            }
            else{
                $api_url = $this->config ['EndPointUrl'];
            }
            $send_change_response = $this->process_request($send_change_request ['request'], $api_url);
            $api_request = $send_change_request ['request'];
            $api_response = $send_change_response;
            $api_remarks = 'CancelBooking(Travelport Flight)';
            $this->CI->api_model->store_api_request_booking($api_url, $api_request, $api_response, $api_remarks);
            // debug($send_change_response);exit;
            ...
            if (valid_array($send_change_response) == true) {
                $provider_code = $send_change_request['provider_code'];
               
                if($provider_code == '1G'){
                   
                    if($send_change_response['SOAP:Envelope']['SOAP:Body']['universal:UniversalRecordCancelRsp']['universal:ProviderReservationStatus']['@attributes']['Cancelled'] == true){
                        $response ['status'] = SUCCESS_STATUS;
                        $response ['data']['send_change_response'] = $send_change_response;
                    } 
                }
                else{
                    if($send_change_response['SOAP:Envelope']['SOAP:Body']['air:AirRefundRsp']['air:TCR']['@attributes']['Status'] == "Refunded"){
                        $response ['status'] = SUCCESS_STATUS;
                        $response ['data']['send_change_response'] = $send_change_response;
                    }
                }
     ...
     * @param unknown_type $request
     */
    private function format_send_change_request($params, $carrier) {
        ...
        if($carrier == '6E'){
            $tcrn_number = $params['booking_transaction_details'][0]['book_id'];
            $AirRefundQuoteReq = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v47_0" xmlns:air="http://www.travelport.com/schema/air_v47_0">
                                <soapenv:Header/>
                                <soapenv:Body>
                                <air:AirRefundQuoteReq AuthorizedBy="user" TraceId="564d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '">
                                <com:BillingPointOfSaleInfo OriginApplication="UAPI"/>
                                <air:TCRNumber>'.$BookingId.'</air:TCRNumber>
                                </air:AirRefundQuoteReq>
                                </soapenv:Body>
                                </soapenv:Envelope>';
            
            $AirRefundQuoteRes = $this->process_request($AirRefundQuoteReq);
           
            $AirRefundQuoteRes_array = Converter::createArray($AirRefundQuoteRes);
            if(valid_array($AirRefundQuoteRes_array) &&(isset($AirRefundQuoteRes_array['SOAP:Envelope']['SOAP:Body']['air:AirRefundQuoteRsp']['air:TCRRefundBundle']['@attributes']['TCRNumber']))){
                $AirRefundQuoteRes = str_replace('SOAP:', '', $AirRefundQuoteRes);
                $AirRefundQuoteRes = str_replace('air:', '', $AirRefundQuoteRes);
                $AirRefundQuoteRes = new SimpleXMLElement($AirRefundQuoteRes);
                $AirSegment = $AirRefundQuoteRes->Body->AirRefundQuoteRsp->TCRRefundBundle;
                unset($AirSegment->AirRefundInfo);
                unset($AirSegment->AirSegment->Connection);
                foreach($AirSegment as $seg_key => $seg_value){
                    if(valid_array($seg_value['AirSegment'])){
                        foreach($segment_list as $ss_key => $ss_value){
                           
                            if($ss_value->Connection){
                               
                                unset($ss_value->Connection);
                            }
                        }
                    }
                }
                $AirSegment = $AirSegment->asXML();
                $array_sr = array('<TCRRefundBundle TCRNumber="'.$BookingId.'" RefundType="Auto">', '</TCRRefundBundle>');
                $AirSegment = str_replace($array_sr, '', $AirSegment);
                $AirSegment = str_replace("AirSegment", "air:AirSegment", $AirSegment);
                $AirSegment = str_replace("CodeshareInfo", "air:CodeshareInfo", $AirSegment);
                $AirSegment = str_replace("<common_v47_0:HostToken", "<common_v47_0:HostToken xmlns:common_v47_0='http://www.travelport.com/schema/common_v47_0'", $AirSegment);
          
                $CancelBookingReq = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v47_0" xmlns:air="http://www.travelport.com/schema/air_v47_0" >
                                    <soapenv:Header/>
                                    <soapenv:Body>
                                   <air:AirRefundReq AuthorizedBy="user" TraceId="dsfdfd" TargetBranch="' . $this->config ['Target'] . '">
                                    <com:BillingPointOfSaleInfo xmlns:com="http://www.travelport.com/schema/common_v47_0" OriginApplication="UAPI" />
                                    <air:TCRRefundBundle RefundType="Auto" TCRNumber="'.$tcrn_number.'">
                                        <air:AirRefundInfo Refund="true" />'.$AirSegment.'
                                    </air:TCRRefundBundle>
                                </air:AirRefundReq>
                                    </soapenv:Body>
                                    </soapenv:Envelope>';
            ...
            $CancelBookingReq = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v47_0" xmlns:univ="http://www.travelport.com/schema/universal_v47_0">
            <soapenv:Header/>
            <soapenv:Body>
            <univ:UniversalRecordCancelReq AuthorizedBy="user" TraceId="564d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" UniversalRecordLocatorCode="' . $BookingId . '" Version="1">
            <com:BillingPointOfSaleInfo OriginApplication="UAPI"/>
            ...
            $provider_code = '1G';
            $request ['request'] = $CancelBookingReq;
            $request ['url'] = 'https://americas.universal-api.travelport.com/B2BGateway/connect/uAPI/UniversalRecordService';
            $request ['remarks'] = 'SendChangeRequest(Travelport)';
            $request ['status'] = SUCCESS_STATUS;
            $request ['provider_code'] = $provider_code;
        }
    ...
    public function save_ticket_cancellation_details($cancellation_details, $passenger_origin, $pass_count) {
        $tcrn_number = $cancellation_details['SOAP:Envelope']['SOAP:Body']['air:AirRefundRsp']['air:TCR']['@attributes']['TCRNumber'];
        $refund_amount = substr($cancellation_details['SOAP:Envelope']['SOAP:Body']['air:AirRefundRsp']['air:TCR']['@attributes']['RefundAmount'], 3);
        $cancellation_chargers = substr($cancellation_details['SOAP:Envelope']['SOAP:Body']['air:AirRefundRsp']['air:TCR']['@attributes']['RefundFee'], 3);
        $cancelltion_tax = substr($cancellation_details['SOAP:Envelope']['SOAP:Body']['air:AirRefundRsp']['air:TCR']['@attributes']['ForfeitAmount'], 3);
        $data['cancellation_processed_on'] = date('Y-m-d H:i:s');
        $data['RequestId'] = $tcrn_number;
        $data['API_RefundedAmount'] = $refund_amount/$pass_count;
        $data['API_CancellationCharge'] = $cancellation_chargers/$pass_count;
        $data['API_ServiceTaxOnRefundAmount'] = $cancelltion_tax/$pass_count;
    ...
    function hold_ticket($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if ($book_service_response['status'] == SUCCESS_STATUS) {
            ...
            $this->save_gds_book_response_details($booking_params, $book_response, $app_reference, $sequence_number);
            ...
            $ticket_request_params['BookingId'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v47_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
    ...
    function process_booking($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if($carrier == '6E'){
            if(isset($book_service_response['data']['book_response']['SOAP:Envelope']['SOAP:Body']['SOAP:Fault']['detail']['common_v47_0:ErrorInfo']['common_v47_0:Service'])){
                $fault_provider = $book_service_response['data']['book_response']['SOAP:Envelope']['SOAP:Body']['SOAP:Fault']['detail']['common_v47_0:ErrorInfo']['common_v47_0:Service'];
            ...
            if($ach_hold !='hold'){
                ...
                if($provide_code  == '1G'){
                    ...
                    foreach($flights_data as $flight_data){
                       $all_hold_airline_list = $this->CI->custom_db->single_table_records('hold_airline_list', '*', array('status' => 1, 'domain_origin' => get_domain_auth_id(), 'code' => $flight_data['carrier']));
                ...
                $payment_desc = $book_service_response['data']['book_response']['SOAP:Envelope']['SOAP:Body']['SOAP:Fault']['detail']['common_v47_0:ErrorInfo']['common_v47_0:Description'];
    ...
    private function run_book_service_request($params, $search_id, $ach_status) {
        ...
        if($provider == '1G'){
            $credit_card_info = $this->CI->custom_db->single_table_records('test_info', '*', array('booking_source' => 'PTBSID0000000007'));
            if($credit_card_info['status'] == true){
                $credit_card_info = $this->credit_decrypt($credit_card_info['data'][0]['data']);
                $credit_card_info = json_decode($credit_card_info, true);
            }
        ...
        if($provider == 'ACH'){
             // if(array_key_exists('GST', $params)){
                // $gst_number = $params['GST']['Gstnumber']; //29AANCA8324M2Z0
                // $company_name = $params['GST']['GstComapnyName']; //ACCENTRIA SOLUTIONS PRIVATE LIMITED
                // $gstn = '/IND/'.$gst_number.'/'.$company_name;
                // $gste = '/IND/'.$gst_email;
                $gst_number = '27AABCA9521E1ZX'; //29AANCA8324M2Z0
                $company_name = 'ITQ'; //ACCENTRIA SOLUTIONS PRIVATE LIMITED
                ...
                $airprice_request = $check_price_xml[0]['air_price_request'];
                $doc = new DOMDocument();
                $doc->loadXML($airprice_request);
                $domds = $doc;
                $ssr_length = $domds->getElementsByTagName("SSR");
                for($i=0; $i<$ssr_length->length; $i++){
                    $node = $ssr_length->item($i);
                    if($i==0){
                        $node->setAttribute('FreeText',$gstn);
                    }
                    else{
                        $node->setAttribute('FreeText',$gste);
                    }
                }
                $BaggageDetails = '';
                foreach ($params['Passengers'] as $pass_key => $passenger) {
                    if (isset($passenger['BaggageId']) == true && valid_array($passenger['BaggageId']) == true) {
                        $BaggageDetails .= $this->baggage_request_details($passenger['BaggageId'], $pass_key);
                    }
                }
                $BaggageDetails = str_replace('<?xml version="1.0"?>', '', $BaggageDetails);
               
                $fragment = $doc->createDocumentFragment();
                $fragment->appendXML($BaggageDetails);
                $air_price_request = $doc->saveXML();
                debug($air_price_request);exit;
                $price_response = $this->process_request($air_price_request);
            // }
            // else{
            //     $gst_number = '27AABCA9521E1ZX'; //29AANCA8324M2Z0
            //     $company_name = 'ITQ'; //ACCENTRIA SOLUTIONS PRIVATE LIMITED
        ...
        $AirItinerary_xml = str_replace("common_v47_0:", '', $AirItinerary_xml);
        $AirPricingSolution_xml = $AirPricingSolution_xml1 = str_replace('common_v47_0:', '', $AirPricingSolution_xml);
        ...
        if($provider == 'ACH'){
            if((strtotime($from_date) > strtotime($check_promo_from_date)) && (strtotime($from_date) < strtotime($check_promo_to_date)) ){
                $airpricing_modifiers = '<AirPricingModifiers AccountCodeFaresOnly="true"><PromoCodes><PromoCode Code="AGTSPL01" ProviderCode="ACH" SupplierCode="6E"/>
                            </PromoCodes>
                        </AirPricingModifiers></AirPricingInfo>';
            }
            else{
                $airpricing_modifiers = '</AirPricingInfo>';
            }
        }
        else{
            $airpricing_modifiers = '</AirPricingInfo>';
        ...
        foreach ($xp->query('//TaxInfoRef') as $key => $tn) {
             $tn->setAttribute('xmlns','http://www.travelport.com/schema/common_v47_0');
        }
        if($provider == 'ACH'){
        ...
        $AirPricingSolution_xml = str_replace("</AirPricingInfo>", $airpricing_modifiers, $AirPricingSolution_xml);
        // debug($AirPricingSolution_xml);exit;
        ...
        $Payment_address = '<AddressName>A111,krishvigavakshi</AddressName>
                            <Street>#77/3 to 77/6 Panathur Village , Varthur Hobli</Street>
                            <City>Bangalore</City>
                            <PostalCode>560037</PostalCode>
                            <Country>IN</Country>';
        ...
        foreach ($params['Passengers'] as $passenger) {
            ...
            if ($passenger['PaxType'] == 1) {
                ...
                if (isset($passenger['DateOfBirth']) == true && empty($passenger['DateOfBirth']) == false) {
                    $Passenger_DOBp = date('dMy', strtotime($passenger['DateOfBirth']));
                    $pax_dob = $passenger['DateOfBirth'];
                    $dob = ' DOB="' . $pax_dob . '" ';
                    $doc_db = $Passenger_DOBp . '/';
                } else {
                    if($provider == 'ACH'){
                        if($search_data['is_domestic'] == 1){
                            $dob =' ';
                            ...
                            $pax_dob = $this->get_pax_default_dob($passenger['PaxType']);
                            $Passenger_DOBp = date('dMy', strtotime($pax_dob));
                            $dob = ' DOB="' . $pax_dob . '" ';
                            $doc_db = $Passenger_DOBp . '/';
                        }
                    }
                ...
                $adults .= '<BookingTraveler Key="' . $paxId . '" TravelerType="ADT"'.$dob.'Gender="' . $gender . '" xmlns="http://www.travelport.com/schema/common_v47_0">
                ...
                if ($passenger['PassportNumber'] != "" && $passenger['PassportExpiry'] != "") {
                    $adults .= '<SSR Carrier="' . $air_line_pcode . '" FreeText="P/' . $Passenger_PassportNationality . '/' . $adult_passport_number . '/' . $Passenger_PassportIssuedBy . $doc_db. '/' . $Passenger_Genderp . '/' . $Passenger_DOEp . '/' . $Passenger_Lastnamep . '/' . $Passenger_Firstnamep . '" Status="HK" Type="DOCS"></SSR>';
                }
                if($provider == 'ACH'){
                    
                   if($i == 0){
                        $adults.= '<SSR Key="11" Type="GSTN" Carrier="6E" Status="NN" FreeText="'.$gstn.'"/>
                                    <SSR Key="22" Type="GSTE" Carrier="6E" Status="NN" FreeText="'.$gste.'"/>';
                   }
                ...
                $Passenger_Firstnamep = $passenger['FirstName'];
                $Passenger_Lastnamep = $passenger['LastName'];
                $childs .= '<BookingTraveler Key="' . $paxId . '" TravelerType="'.$type.'" DOB="' . $passenger['DateOfBirth'] . '" Gender="' . $gender . '" xmlns="http://www.travelport.com/schema/common_v47_0">
                ...
                $Passenger_Firstnamep = $passenger['FirstName'];
                $Passenger_Lastnamep = $passenger['LastName'];
                $infants .= '<BookingTraveler Key="' . $paxId . '" TravelerType="INF" DOB="' . $passenger['DateOfBirth'] . '" Gender="' . $gender . '" xmlns="http://www.travelport.com/schema/common_v47_0">
        ...
        $continuity_seg = '<ContinuityCheckOverride xmlns="http://www.travelport.com/schema/common_v47_0" Key="1">pass</ContinuityCheckOverride>';

        if ($provider == "ACH") {
            $api_data = $this->CI->custom_db->single_table_records('api_config', '*', array('booking_source_fk' => 107));
            if($api_data['status'] == SUCCESS_STATUS){
                $api_config = $this->decrypt_api_data($api_data['data'][0]['config']);
                $api_config = json_decode($api_config, True);
            }
           if($ach_status == 'hold'){
                $payment = '';
            }
                ...
                if($carrier == '6E'){
                      $payment = '<FormOfPayment Type="Credit" xmlns="http://www.travelport.com/schema/common_v47_0" Key="446408">
                    <CreditCard BankCountryCode="US" CVV="123" ExpDate="2025-09" Name="TEST TRAVELPORT" Number="5200000000000007" Type="MC">
                      ...
                      //   $payment = '<FormOfPayment Type="AgencyPayment" xmlns="http://www.travelport.com/schema/common_v47_0">
                      // <AgencyPayment AgencyBillingIdentifier="IGS4275" AgencyBillingPassword="INDacc@@2019" />
                      //  </FormOfPayment>';
                }
                else if($carrier == 'I5' || $carrier == 'AK' || $carrier == 'FD' || $carrier == 'Z2'){
                    $payment = '';
                }
            }
            ...
            // $payment = '<FormOfPayment xmlns="http://www.travelport.com/schema/common_v47_0" Key="' . $arrPriceKey . '" Type="Cash" />';
               $payment = '<FormOfPayment Type="Credit" xmlns="http://www.travelport.com/schema/common_v47_0" Key="446408">
                    <CreditCard BankCountryCode="US" CVV="123" ExpDate="2025-09" Name="TEST TRAVELPORT" Number="5200000000000007" Type="MC">
                        <BillingAddress>
                            <AddressName>Home</AddressName>
        ...
        $ActionStatus = '<ActionStatus ProviderCode="' . $provider . '" TicketDate="T*" Type="ACTIVE" QueueCategory="01" xmlns="http://www.travelport.com/schema/common_v47_0" ></ActionStatus>';

        $AirCreateReservationReq = '<?xml version="1.0" encoding="utf-8"?>
        <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                        <s:Header/>
                        <s:Body>
                            <univ:AirCreateReservationReq xmlns:air="http://www.travelport.com/schema/air_v47_0"  xmlns:common_v47_0="http://www.travelport.com/schema/common_v47_0" xmlns:univ="http://www.travelport.com/schema/universal_v47_0" AuthorizedBy="user" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" RetainReservation="None">
                            <BillingPointOfSaleInfo  xmlns="http://www.travelport.com/schema/common_v47_0" OriginApplication="UAPI" ></BillingPointOfSaleInfo>
                            ' . $adults . '
                            ' . $infants . '
                            ' . $childs . '  
                            ' . $continuity_seg . '                          
        ...
        $AirCreateReservationReq = str_replace("HostToken Key", "HostToken xmlns='http://www.travelport.com/schema/common_v47_0' Key", $AirCreateReservationReq);
        $AirCreateReservationReq = str_replace("<AirPricingSolution", "<AirPricingSolution xmlns='http://www.travelport.com/schema/air_v47_0'", $AirCreateReservationReq);
        // debug($AirCreateReservationReq);exit;
    ...
    private function run_non_lcc_ticket_service_request($params) {
        ...
        $ticket_request ='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v47_0" 
            xmlns:univ="http://www.travelport.com/schema/universal_v47_0">
               <soapenv:Header/>
               <soapenv:Body>
                  <air:AirTicketingReq BulkTicket="false" ReturnInfoOnFail="true" TargetBranch="' . $this->config ['Target'] . '" xmlns:air="http://www.travelport.com/schema/air_v47_0">
                     <com:BillingPointOfSaleInfo OriginApplication="UAPI"/>
    ...
    private function save_flight_ticket_details($booking_params, $book_response, $ticket_response, $app_reference, $sequence_number, $search_id){
        ...
        foreach($airsegment_list as $seg_key => $seg_value){
            ...
            $update_itinerary_data['airline_pnr'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v47_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
     ...
     * Meal Details For Non-LCC Flights
     */
    private function baggage_request_details($BaggageDetails, $pass_key) {
        ...
        if (valid_array($BaggageDetails) == true) {
            foreach ($BaggageDetails as $bag_k => $bag_v) {
                if (empty($bag_v) == false) {
                    $baggae_data = Common_Flight::read_record($bag_v);
                      
                    if (valid_array($baggae_data) == true) {
                        $baggae_data = json_decode($baggae_data[0], true);
                        $temp_baggage = array_values(unserialized_data($baggae_data['BaggageId']));
                        $opt_key = $temp_baggage[0]['baggage_book_data'][$bag_k][$pass_key][$temp_baggage[0]['Weight']]['OptionalServiceKey'];
                        $baggage_xml = str_replace("common_v47_0:", '', $temp_baggage[0]['Baggagexml']);
                        $doc = new DOMDocument();
                        $doc->loadXML($baggage_xml);
                        $xp = new DOMXPath($doc);
                        foreach ($xp->query('//OptionalService') as $key => $tn) {
                           $tn->setAttribute('Key',$opt_key);
                        }
                        $baggage_xml = $doc->saveXML();
                        $optional_services .= $baggage_xml;
                    }
                }
            }
        }
     ...
     * @param unknown_type $sequence_number
     */
    private function save_gds_book_response_details($booking_params, $book_response, $app_reference, $sequence_number) {
        
        $update_data = array();
        $update_condition = array();
        $update_data['book_id'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
        $update_data['airline_pnr'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v47_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
     ...
     private function save_ach_book_response_details($booking_params, $book_response, $app_reference, $sequence_number, $search_id){
        ...
        $update_data = array();
        $update_condition = array();
        $update_data['book_id'] = $book_id = @$book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['air:DocumentInfo']['air:TCRInfo']['@attributes']['TCRNumber'];
        $update_data['airline_pnr'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v47_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
        $gds_pnr_data = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['universal:ProviderReservationInfo'];
        if(isset($gds_pnr_data[0])){
            $gds_pnrs = $gds_pnr_data;
    ...
    function flyariana_login(){
        $response['status'] = SUCCESS_STATUS;
        $response['data']   = array();
        $user_name = 'trip';
        $password = 'Pinky678';
        $request= 'login=trip&password=Pinky678&ta_login_action=dologin' ;
       
        $url = 'https://www.flyariana.com/agents/bookings/flight_selection.aspx?TT=RT&SS=&RT=&FL=on&DC=KBL&AC=KDH&AM=2019-07&AD=24&RM=2019-07&RD=25&PA=1&PC=&PI=&CC=&NS=&CD=';
        $url = 'https://www.flyariana.com/agents/default.asp';
        ...
        CURLOPT_TIMEOUT => 30,
        CURLOPT_COOKIE => "ASP.NET_SessionId=BHMMOPFAAIELDKBPBPBABPCA; path=/",
