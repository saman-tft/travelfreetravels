...
$(document).ready(function () {
  ...
  $("#flight_fare_calendar").on("click", function (e) {
    ...
    var url =
      app_base_url +
      "index.php/flight/pre_calendar_fare_search?" +
      $.param(data);
    window.open(url);
  });
  ...
  $("#flight-form-submit").on("click", function (e) {
    ...
    if (trip_type == "oneway" || trip_type == "circle") {
      ...
      var _to_return = $("#f anlight_form #flight_datepicker2").val();
      if (_from_loc == "") {
        show_alert_content(
          "Please Enter From location.",
          "#flight-alert-box-from"
        );
        e.preventDefault();
        // return ''
      } else {
      ...
      if (_from_loc1 == "") {
        show_alert_content(
          "Please enter From 1st location.",
          "#flight-alert-box"
        );
        e.preventDefault();
        return "";
      ...
      if (m_flight_datepicker1 == "") {
        show_alert_content(
          "Please sleect 1st Departure date.",
          "#flight-alert-box"
        );
        e.preventDefault();
        return "";
      ...
      if (_from_loc2 == "") {
        show_alert_content(
          "Please enter From 2nd location.",
          "#flight-alert-box"
        );
        e.preventDefault();
        return "";
      ...
      if (m_flight_datepicker2 == "") {
        show_alert_content(
          "Please sleect 2nd Departure date.",
          "#flight-alert-box"
        );
        e.preventDefault();
        return "";
      ...
      if ($("#multi_city_container_3:visible").length > 0) {
        if (_from_loc3 == "") {
          show_alert_content(
            "Please enter From 3rd location.",
            "#flight-alert-box"
          );
          e.preventDefault();
          return "";
        ...
        if (m_flight_datepicker3 == "") {
          show_alert_content(
            "Please sleect 3rd Departure date.",
            "#flight-alert-box"
          );
          e.preventDefault();
          return "";
      ...
      if ($("#multi_city_container_4:visible").length > 0) {
        if (_from_loc4 == "") {
          show_alert_content(
            "Please enter From 4th location.",
            "#flight-alert-box"
          );
          e.preventDefault();
          return "";
        ...
        if (m_flight_datepicker4 == "") {
          show_alert_content(
            "Please sleect 4th Departure date.",
            "#flight-alert-box"
          );
          e.preventDefault();
          return "";
      ...
      if ($("#multi_city_container_5:visible").length > 0) {
        if (_from_loc5 == "") {
          show_alert_content(
            "Please enter From 5th location.",
            "#flight-alert-box"
          );
          e.preventDefault();
          return "";
        ...
        if (m_flight_datepicker5 == "") {
          show_alert_content(
            "Please sleect 5th Departure date.",
            "#flight-alert-box"
          );
          e.preventDefault();
          return "";
  ...
  $("#flight-small").on("click", function (e) {
    var trip_type = $('[name="trip_type"]:checked').val();
    if (trip_type == "oneway" || trip_type == "circle") {
      var _from_loc = $("#flight_form #from").val();
      var _to_loc = $("#flight_form #to").val();
  ...
  $(".fromflight, .departflight")
    .catcomplete({
      open: function (event, ui) {
        $(".ui-autocomplete").off(
          "menufocus hover mouseover mouseenter mouseout"
        );
      },
      source: function (request, response) {
        ...
        if (term in cache) {
          ...
          $.getJSON(
            app_base_url + "index.php/ajax/get_airport_code_list",
            request,
      ...
      minLength: 0,
      //for removing autoFocus from search bar.
      autoFocus: false,
    ...
    var auto_suggest_value = highlight_search_text(
      this.term.trim(),
      item.value,
      item.label,
      item.country_code
...
$("#add_city").click(function (e) {
  ...
  if (segment_count <= max_multicity_segments) {
    ...
    for (var i = 1; i <= segment_count; i++) {
      if (
        $("#multi_city_container_" + i).hasClass("inactive_segment") == false
      ) {
...
$(".remove_city").click(function (e) {
  ...
  if (segment_count > min_multicity_segments) {
    ...
    seg_object.hide();
    $("input, select", seg_object)
      .val("")
      .attr("disabled", "disabled")
      .addClass("inactive_segment");
    seg_object.insertAfter(
      $(".multi_city_container", "#multi_way_fieldset").last()
    );
    segment_count = segment_count - 1;
    $("#multicity_segment_count").val(segment_count);
    validate_multicity_segments(segment_count);
...
function validate_multicity_segments(segment_count) {
  for (var i = parseInt(segment_count) + 1; i <= max_multicity_segments; i++) {
    if ($("#multi_city_container_" + i).is(":visible") == false) {
      $("input, select", $("#multi_city_container_" + i)).attr(
        "disabled",
        "disabled"
      );
    }
  ...
  for (var i = parseInt(segment_count); i >= min_multicity_segments; i--) {
    if ($("#multi_city_container_" + i).is(":visible") == true) {
      $("input, select", $("#multi_city_container_" + i)).removeAttr(
        "disabled"
      );
    }
  }
...
function validate_segment_dates() {
  $(".multi_city_container").each(function () {
    ...
    var next_departure_id = $(this)
      .next(".multi_city_container")
      .find(".m_depature_date")
      .attr("id");
    auto_set_dates(
      $("#" + current_departure_id).datepicker("getDate"),
      next_departure_id,
      "minDate",
      0
    );
