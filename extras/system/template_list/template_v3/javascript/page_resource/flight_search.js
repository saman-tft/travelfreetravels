$(document).ready(function() {
		...
		window.process_result_update = function(result, run_api_cnt) {
			$('.loader-image').hide();
			if (result.hasOwnProperty('status') == true && result.status == true) {
				if ( $(".search_append").length ) {
					//if(run_api_cnt > 1){
						$(".search_append").append(result.data);
					/*}else{
						$(".search_append").html(result.data);	
					}*/
				}else{
					/*
					if(run_api_cnt > 1){*/
		...
		//	console.log(airlineList);
			...
			//cloneSliderv2(airlineList);
			cloneSliderv2(airlineList);
			//console.log(cloneList);
			$('#core_minimum_range_value', '#core_min_max_slider_values').val(Math.round(minPrice));
			$('#core_maximum_range_value', '#core_min_max_slider_values').val(Math.round(maxPrice));
		}
			...
			if ($.isPlainObject(stopCountList)) {
				...
				$('#stopCountWrapper .stopcount').each(function(key, value) {
					...
					if (stopCountListArray.indexOf(stopCat) == -1) {
						//disabled
						$(this).attr('disabled', 'disabled');
						$(this).closest('.stop-wrapper').removeClass('enabled');
		...
		function set_slider_label(val1, val2,original_minPrice='',original_maxPrice='')
		{
			...
			console.log('sideslidertest');
			console.log(original_maxPrice);
			console.log(val2);
		...
		function cloneSlider(slides) {
			...
			airline_name = $('#flight_search_result .r-r-i:nth-child(' + k + ') .a-n:first').text();
				...
				if(airline_name!="" && airline_name!=null)
				{
			...
			function cloneSliderv2(airlineList) {
			var cloneList = {};
			console.log('slider');
			...
			$.each(airlineList, function(key,value) {
				...
				list += '<div class="item hand-cursor">';
					list += '<div class="airlinesd">';
						list += '<input type="checkbox" class="airline-slider" value="'+key+'">';
						list += '<div class="imgemtrx"><img  alt="Flight" src="https://travelfreetravels.com/extras/system/library/images/airline_logo/' +key+ '.gif"></div>';
						list += '<div class="alsmtrx">';
							list += '<strong>' +value+ '</strong>';
					//	list += '<span class="mtrxprice"><span class="clone_side_org_price hide">test</span><span class="clone_side_curr">' +sliderCurrency+'</span> <span class="clone_side_price">'+airline_price + '</span></span>';
						list += '</div>';
						//Important For Clone
						//list += '<div class="cloneForm" style="display:none;">';
							//list += $('#flight_search_result .r-r-i:nth-child(' + k + ') .form-wrapper').html();
    ...
    responsive:{
		...
		**/
		...
		function update_col2x_flight(segment, trip_way_indicator) {
			...
			console.log(trip_way_indicator);
		...
		function default_col2x() {
			...
			var col_count = $('.r-w-g').length;
                console.log("col_countouttest");
                console.log(parseInt(col_count));
