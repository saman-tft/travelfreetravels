...
// debug($post_params);exit();
        ...
        // exit();
				switch($post_params['payment_method']) {
						...
						// break;
						 ...
						 redirect(base_url().'index.php/hotelcrs/process_booking/'.$book_id.'/'.$book_origin);
	...
	function process_bookingold($book_id, $temp_book_origin){
	...
	//	debug($currency_obj);exit;
				...
				//	$currency_obj = new Currency(array('module_type' => 'hotel', 'from' => get_api_data_currency(), 'to' => get_application_currency_preference()));
					...
					//	debug($page_data['pre_booking_params']);die;
						...
						$temp_record = $this->custom_db->single_table_records('api_country_list', '*');
						$page_data['phone_code'] =$temp_record['data'];
						$page_data['user_country_code'] = '+1';
						// debug($page_data);exit;
						$this->template->view('hotel/tbo/crs_booking_page', $page_data);
					}
				} else {

					echo  "booking api url";die;
					redirect(base_url().'index.php/hotel/exception?op=Data Modification&notification=Data modified while transfer(Invalid Data received while validating tokens)');
				}
			} else {
			    
					echo  "booking api url2";die;
				redirect(base_url());
			}
		} else {
		    
					echo  "booking api url3";die;
			redirect(base_url());
		}
	}
	function bookingoldtest($search_id)
	{
		...
		if ($domain_balance_status) {
			...
			if ($temp_booking != false) {
				...
				if ($booking['status'] == SUCCESS_STATUS) {
					...
					// debug($data);exit();
					$this->domain_management_model->update_transaction_details('hotel', $book_id, $data['fare'], $data['admin_markup'], $data['agent_markup'], $data['convinence'], $data['discount'],$data['transaction_currency'], $data['currency_conversion_rate'], 'api_booking' );


					$site_url = str_replace('/agent/','',base_url());
					$text="Dear ".$data['name'].", 
Thank you for choosing Alkhaleej Tours services. Your booking reference id is ".$book_id.". 
 click to view ticket ".$site_url."/index.php/voucher/hotel/".$book_id."/".$temp_booking ['booking_source'].".";
                        // $url = "http://www.smsalert.co.in/api/push.json?apikey=$this->apikey&sender=$this->sender&mobileno=$mobileno&text=".urlencode($text);
