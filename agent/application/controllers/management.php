...
class Management extends CI_Controller {
	...
	function b2b_hotelcrs_markup() {
		...
		// Hotel would have All - general and domain wise markup
		$data_list = $this->domain_management_model->hotelcrs_markup ();
...
{ 
	...
	function b2b_transfercrs_markup() {
		// Transfers would have All - general and domain wise markup
		$markup_module_type = 'b2b_transfercrs';
		$page_data ['form_data'] = $this->input->post ();
		if (valid_array ( $page_data ['form_data'] ) == true) {
			switch ($page_data ['form_data'] ['form_values_origin']) {
				case 'generic' :
		...
		// Transfers would have All - general and domain wise markup
		$data_list = $this->domain_management_model->transfercrs_markup ();
		$this->template->view ( 'management/b2b_transfercrs_markup', $data_list );
	}
	...
	function b2b_carcrs_markup() {
		...
		// Hotel would have All - general and domain wise markup
		$data_list = $this->domain_management_model->carcrs_markup ();
...
//debug($export_data);die;
	...
	*/
	...
	//new function to load the b2b topup page
		function b2b_topup()
	{
		$page_data = array();
		 $datetime = date("YmdHis");
 		$validationId = 'TOPUP-'. $datetime;
			$insertStatus = $this->transaction_model->save_validation($validationId);
			if (is_array($insertStatus) && $insertStatus['status'] === 1) {
				$id = $insertStatus['insert_id'];
				$page_data['data']['amount'] = 0;
				$page_data['data']['id'] = $id;
				$page_data['data']['validation_id'] = $validationId;
			} else {
				throw new Exception("Something went wrong with payment intitiation.");
			}
		$this->template->view('management/b2b_topup', $page_data);
	}
}
