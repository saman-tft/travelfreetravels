...
class Report extends CI_Controller {
	public function __construct()
	{
		...
		$this->load->model('privatecar_model');
	...
	function transfers_crs($offset=0)
	{
		//$emulate_status =0;
		...
		$page_data = array();
		$condition = array();
		$filter_conditionn = array();
		$usrId = $this->entity_user_id;
		$filter_data = $this->format_basic_search_filters('transfers_crs');
		// debug($filter_data);exit;
		$agent_ids = $this->transfer_model->get_agent_ids($this->entity_user_id);
			if($agent_ids){

			$agentId_ary = '';
			// debug($agent_ids);exit;
			for($i=0;$i<count($agent_ids);$i++){
				if($i==0){
				$agentId_ary.= $agent_ids[$i]['user_id'];
				}else{
				$agentId_ary.= ','.$agent_ids[$i]['user_id'];
				}
			}
			$agentId_ary.= ','.$usrId;
			// $ids = implode(', ', $agentId_ary);
			// debug($agentId_ary);exit;
			$filter_conditionn[] = array('BD.created_by_id', 'IN', '('.$agentId_ary.')');
			}else{
			$filter_conditionn[] = array('BD.created_by_id','=', $usrId);
			}
		
		$page_data['from_date'] = $filter_data['from_date'];
		$page_data['to_date'] = $filter_data['to_date'];
		$condition = $filter_data['filter_condition'];
		$condition = $filter_conditionn;
		//debug($condition);exit;
		// if(isset($get_data['filter_report_data']) == true || empty($get_data['filter_report_data']) == false || isset($get_data['created_datetime_from']) == true || empty($get_data['created_datetime_from']) == false  || isset($get_data['created_datetime_to']) == true || empty($get_data['created_datetime_to']) == false || isset($get_data['status']) == true || empty($get_data['status']) == false ) {
		// 	$search_filter_condition=$this->search_filter('transfers_crs');
		...
		// 	$table_data = $this->transfer_model->filter_booking_report($search_filter_condition,false, $offset, RECORDS_RANGE_2);
		// 	$total_records = $table_data['data']['count'][0]['total_records'];
		// 	//($table_data);exit;
		// } else {
		...
		$config['base_url'] = base_url().'index.php/report/transfers_crs/';
		...
		$this->template->view('report/transfers_crs', $page_data);
	}
function activities_crs()
	{
		...
		$condition = array();
		$emulate_status=0;
		$filter_data = $this->format_basic_search_filters('activities_crs');
		$filter_conditionn = array();
		$usrId = $this->entity_user_id;
		// debug($filter_data);exit;
		$agent_ids = $this->transfer_model->get_agent_ids($this->entity_user_id);
		...
		// 	if(isset($get_data['filter_report_data']) == true || empty($get_data['filter_report_data']) == false || isset($get_data['created_datetime_from']) == true || empty($get_data['created_datetime_from']) == false  || isset($get_data['created_datetime_to']) == true || empty($get_data['created_datetime_to']) == false || isset($get_data['status']) == true || empty($get_data['status']) == false ) {
		// 	$search_filter_condition=$this->search_filter('activities_crs');
		// 	//debug($search_filter_condition);exit;
		// 	$total_records = $this->activity_model->b2c_holiday_report_filter($search_filter_condition, true);
		// 	$table_data = $this->activity_model->b2c_holiday_report_filter($search_filter_condition, false, $offset, RECORDS_RANGE_2);
		// }
		// else
		// {
		...
		$this->load->library('pagination');
		if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
		$config['base_url'] = base_url().'index.php/report/activities_crs/';
		$config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
		$page_data['total_rows'] = $config['total_rows'] = $total_records;
		$config['per_page'] = RECORDS_RANGE_2;
		$this->pagination->initialize($config);
...
function bus($offset=0)
{
		...
		if(isset($get_data['filter_report_data']) == true && empty($get_data['filter_report_data']) == false) {
			...
			$table_data = $this->bus_model->filter_booking_report($search_filter_condition, false, $offset, RECORDS_RANGE_1);
		} else {
	...
	function villasapartment($offset=0)
	{
		...
		if(isset($get_data['filter_report_data']) == true && empty($get_data['filter_report_data']) == false) {
			...
			$table_data = $this->hotel_model->filter_booking_report($search_filter_condition, false, $offset, RECORDS_RANGE_1);
		} else {
        ...
        foreach ($flight_booking_data as $k => $v) {
           if ($op == 'csv') {
            ...
            $export_data[$k]['journey_from'] = $v['journey_from'];
           	...
           	$export_data[$k]['convinence_amount'] = $v['convinence_amount'];
           	$export_data[$k]['discount'] = $v['discount'];
           	$export_data[$k]['segment_discount'] = $v['segment_discount'];
           	$export_data[$k]['grand_total'] = ($v['grand_total'] - $v['segment_discount']);
           	$export_data[$k]['booked_date'] = date('d-m-Y', strtotime($v['booked_date']));
        ...
        if ($op == 'excel') { // excel export
           $headings = array('a1' => 'Sl. No.',
        	...
        	// echo "dd";exit;
               ...
               $headings = array("Sl. No.","Appreference","Lead Pax Name","Lead Pax Email","Lead Pax Phone","GDS PNR","Airline PNR","Airline Code","From","To","Form Date","To Date","Commission Fare","Commission","TDS","Net Fare","GST","Convinence Amount","Discount","Segment Discount","Customer Paid","Booked Date"); 
	...
	function car($offset=0)
	{
		...
		$table_data = $this->car_model->booking($condition, false, $offset, RECORDS_RANGE_1);
	...
	function privatecar($offset=0)
	{
		validate_user_login();
		$condition = array();
		$total_records = $this->privatecar_model->booking($condition, true);
		$table_data = $this->privatecar_model->booking($condition, false, $offset, RECORDS_RANGE_1);
		$table_data = $this->booking_data_formatter->format_car_booking_datas($table_data, 'b2c');
		$page_data['table_data'] = $table_data['data'];
		/** TABLE PAGINATION */
		$this->load->library('pagination');
	...
	function activities($offset=0)
	{
		...
		if(isset($get_data['filter_report_data']) == true && empty($get_data['filter_report_data']) == false) {
			...
			$table_data = $this->sightseeing_model->booking($condition, false, $offset, RECORDS_RANGE_1);
		}
	...
	function transfers($offset=0)
	{
		...
		if(isset($get_data['filter_report_data']) == true && empty($get_data['filter_report_data']) == false) {
			...
			$table_data = $this->transferv1_model->booking($condition, false, $offset, RECORDS_RANGE_1);
		}
