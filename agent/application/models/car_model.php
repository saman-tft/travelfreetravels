	...
	$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records
					...
					where BD.domain_origin='.get_domain_auth_id().' and BD.booking_source="'.PROVAB_CAR_BOOKING_SOURCE.'" and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.''.$condition;
	...
	$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records
					...
					join car_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code
					where BD.booking_source="PTBCSID00000000017" AND BD.domain_origin='.get_domain_auth_id().' and BD.booking_source="'.PROVAB_CAR_BOOKING_SOURCE.'" and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.''.$condition;
