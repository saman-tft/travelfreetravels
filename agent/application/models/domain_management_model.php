...
function transfercrs_markup()
	 ...
	 */
	function update_transaction_detailsflighting($transaction_type, $app_reference, $fare, $domain_markup=0, $level_one_markup=0,$convinence=0, $discount=0, $currency='NPR', $currency_conversion_rate=1)
	{
		$status = FAILURE_STATUS;
	...
	function update_transaction_details($transaction_type, $app_reference, $fare, $domain_markup=0, $level_one_markup=0,$convinence=0, $discount=0, $currency='NPR', $currency_conversion_rate=1)
	{
		$status = FAILURE_STATUS;
		$remarks = $transaction_type.' Transaction was Successfully done';
		$amount = $this->agent_buying_price($transaction_type, $app_reference);
		...
		debug($this->config->item('current_module'));
		if($this->config->item('current_module')=="flight")
		{
		    $amount=$fare;
		}
	//echo "domain".$amount;die;
		if ($this->verify_domain_balance == true) {
			//$amount = floatval($fare+$domain_markup);
			if ($amount > 0) {
