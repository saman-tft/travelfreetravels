...
Class Hotels_Model extends CI_Model
{
	...
	function clean_search_data($temp_search_data)
	{
		...
		//Occupancy
	//	array_pop($temp_search_data['adult']);
	//	array_pop($temp_search_data['adult']);
	//	array_pop($temp_search_data['child']);
		//	array_pop($temp_search_data['child']);
...
function b2c_villa_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
    ...
    //	$this->db->where('s.nationality',$search_data['data']['nationality']);
    	...
    	if($query->num_rows()>0)
    	{
    		...
    		foreach ($query->result() as $key => $value) 
    		{
    			...
    			$RoomPrice= $this->get_first_room_price($value->id,$value->price_id,$search_data);
    			// debug($RoomPrice);die;
    			if ($RoomPrice==false) {
    				continue;
    			}
...
WHERE find_in_set('".$search_data['data']['nationality']."',all_nationality_country.include_countryCodes) and module='hotel'";
   $query = $this->db->query($sql);
    $natprice = $query->result();
		$q3=$this->db->get_where('crs_room_price',array('nationality'=>$natprice[0]->name,'hotel_id'=>$hotel_code));
		if($q3->num_rows()<1)
		{
			return	false;
		}
...
{	
		...
		$RoomPrice = ($adult_price+$child_price)*$search_data['data']['no_of_nights']*$search_data['data']['room_count'];
		$price=array();
...
$hotel_data['LastCancellationDate']=$canelation[0]['FromDate'];
    			...
    			if($hotel_data['Price']!=""  && $hotel_data['Price']!=0)
    			{
    			    $hotel_list[]=$hotel_data; 
