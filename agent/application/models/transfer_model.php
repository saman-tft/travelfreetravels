...
// debug($this->db->last_query());exit;
    ...
    public function admin_markup($min_price, $supplier, $cntry_code, $city_id='', $product_id='',$module_api=''){
				...
				ML.module_type = "b2b_transferv1" and ML.markup_level = "level_3" and DL.origin=ML.domain_list_fk and ML.domain_list_fk != 0 and ML.reference_id=0 and ML.domain_list_fk = '.get_domain_auth_id().' '.$where.'order by DL.created_datetime DESC'; 
				// debug($query);  exit;
				...
				if($specific_data_list){
					...
					ML.origin AS markup_origin, ML.value, ML.value_type,  ML.markup_currency AS markup_currency
					FROM domain_list AS DL JOIN markup_list AS ML where ML.value != "" and
					ML.module_type = "b2b_transferv1" and ML.markup_level = "level_3" and DL.origin=ML.domain_list_fk and ML.domain_list_fk != 0 and ML.reference_id=0 and ML.domain_list_fk = '.get_domain_auth_id().' and ML.product_id is NULL  and ML.city="0" order by DL.created_datetime DESC';
					// debug($query);  exit;
	...
	public function agent_markup($min_price, $cntry_code='', $city_id='', $module_api=''){
	 		...
	 		//		$where .=" and ML.country='$cntry_code'";
				...
				if($specific_data_list){
					...
					ML.origin AS markup_origin, ML.value, ML.value_type,  ML.markup_currency AS markup_currency
					FROM domain_list AS DL JOIN markup_list AS ML where ML.value != "" and
					ML.module_type = "b2b_transferv1" and ML.markup_level = "level_4" and DL.origin=ML.domain_list_fk and ML.domain_list_fk != 0 and ML.reference_id=0 and ML.user_oid ='.$GLOBALS['CI']->entity_user_id.' and  ML.domain_list_fk = '.get_domain_auth_id().' order by DL.created_datetime DESC ';
					// debug($query);  exit;
					$specific_data_list1 = $this->db->query($query)->row_array();
					// debug($specific_data_list1);exit;
					if($specific_data_list1){
