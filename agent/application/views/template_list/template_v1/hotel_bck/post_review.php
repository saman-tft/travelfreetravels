...
<div class="mlgnformin">
    <form class="form-horizontal" role="form" id="post_review">
        ...
        <div class="form-group">
            <label class="control-label col-md-5 col-xs-4" for="">Have you booked with us before?</label>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#post_review").submit(function(event) {
            ...
            $.ajax({
                type: "POST",
                url: '<?=base_url()?>index.php/hotel/set_post_review',
                data: data,
                dataType:'json',
                cache:false,
                success: function(result){
                    ...
                    if(result['status'] == 1)
                    {
                        $("#post_review").before('<img style="text-align: center; margin: 10px auto; display: block;" src="<?=$GLOBALS['CI']->template->template_images('tick.png'); ?>"> <p class="alert alert-success" id="post_msg">Review submitted successfully.</p>');
                        $("#post_review").find("input[type=text], input[type=email], input[type=number], textarea").val("");
                        $('#post_review').addClass('hide');
                        $("#post_msg").fadeOut(6000);
                    }else
                    {
                        $("#post_review").before('<p class="alert alert-danger">Review submit failed.</p>');
                        $('#post_review').removeClass('hide');
                    }
                    // $('#post_review').unbind('submit');
                }
            });
            return false;
        });
