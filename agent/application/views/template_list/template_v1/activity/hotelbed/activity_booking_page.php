...
$travel_date = explode("-",$pre_booking_params['activity_date']);
$trip_total_price = $product_details->Price->TotalDisplayFare - $pre_booking_params['Price']['_GST'];
$user_country_code = $traveller_details[0]['country_code_value'];
$tax_total = $pre_booking_params['Price']['_GST'];
$convenience_fees = $pre_booking_params['Price']['ConvinenceFee'];
$grand_total_vall = $trip_total_price + $tax_total;
$grand_total = $trip_total_price + $tax_total + $convenience_fees;
$adult = $pre_booking_params['adult'];
$child = $pre_booking_params['child'];
$total_pax = $pre_booking_params['total_pax'];
$modalities = $pre_booking_params['modalities'];
$question = $product_details->modalities[$modalities]->questions;
$questions = json_decode(json_encode($question), True);
$remarks = $product_details->modalities[$modalities]->comments[0]->text;
 // debug($convenience_fees);debug($grand_total); die;
			...
			<div class="bookcol">
			  <div class="hotelistrowhtl">
				...
				<div class="col-md-12 col-sm-8 col-xs-8 padall10 xcel">
				  <div class="hotelhed">Title: <?=$product_details->ProductName?></div>
				  <div class="hotelhed1">Tour Code: <?=$product_details->ProductCode?></div>

				  <div class="clearfix"></div>
				  <div class="mensionspl"> <?
				  if($product_details->Location){
				  	echo $product_details->Location;
				  }
				  ?> </div>
			  ...
			  <div class="ffty">
				...
				Travel Date</span>
				  <div class="fuldate_book"> <span class="bigdate_book"><?= $travel_date[0] ?></span>
					<div class="biginre_book"> <?= date('M',strtotime($travel_date[1])); ?><br>
					  <?= $travel_date[2] ?> </div>
				  </div>
				</div>
			  </div>
							...
							<div class="insidechs">
								...
								<div class="cntgust">
									...
									<div class="continye col-xs-8 nopad">
										...
										<a href="<?= base_url() ?>"><button class="bookcont" id=""><i class="fa fa-home"></i> Home</button></a>
									</div>
								</div>
								<div class="alrdyacnt">
		   ...
		   <div id="slidebarscr" style="width:370px;">
		 	<table class="table table-condensed tblemd">
			 	...
			 	<!--   <tr class="frecanpy">
			      ...
			      <?php if($convenience_fees > 0){ ?>
			      <tr class="texdiv">
			        <th>Convinence Fee</th>
			        <td><?=$this->currency->get_currency_symbol(@$pre_booking_params['default_currency'])?> <?=$convenience_fees?></td>
			      </tr>
			      <?php } ?>
			      <tr class="grd_tol">
			        <th>Grand Total</th>
			...
			<div class="col-xs-12 nopad">
			  ...
			  <div class="hotelistrowhtl">
				...
				<div class="col-md-8 padall10 xcel">
				  <div class="hotelhed"><?=$product_details->ProductName?></div>
				  <div class="clearfix"></div>
...
$activity_datepicker = array(array('activity_datepicker', FUTURE_DATE_DISABLED_MONTH));
$this->current_page->set_datepicker($activity_datepicker);
...
$lead_pax_details = @$pax_details[0];
 ?>
<form action="<?=base_url().'index.php/activity/pre_booking/'.$search_id?>" method="POST" autocomplete="off">
...
<?php
	...
	// debug($pre_booking_params['AgeBands']);exit;
	//echo $total_pax;
	foreach ($pre_booking_params['AgeBands'] as $p_key => $p_value) {
...
?>
				...
				<div class="col-md-5 nopad nopadding">
				 ...
				 <div class="col-md-7 nopad nopadding">
				 ...
				 <div class="inptalbox">
					<div class="col-xs-3 col-sm-3 col-md-3 spllty">
						...
						<?php if($p_value['bandId']==1){ echo $adult_title_options;}else{echo $child_title_options;}?>
						</select>
					</div>
					<div class="col-xs-4 col-md-5 spllty">
						  <input value="<?=@$cur_pax_info['first_name']?>" required="required" type="text" name="first_name[]" id="passenger-first-name-<?=$name_index?>" class="clainput alpha_space <?=$traveller_class?>"  minlength="2" maxlength="45" placeholder="Enter First Name" data-row-id="<?=($pax_index);?>" onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123)"/>
					</div>
					<div class="col-xs-4 col-md-4 spllty">
					 	<input value="<?=@$cur_pax_info['last_name']?>" required="required" type="text" name="last_name[]" id="passenger-last-name-<?=$name_index?>" class="clainput alpha_space last_name" minlength="2" maxlength="45" placeholder="Enter Last Name" onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123)"/>
					 </div>
					 <?php 
					 	// debug($pre_booking_params['BookingQuestions']);
					 	// exit;
                    ...
                    <?php foreach($questions as $q_key=>$q_value): $requierd='';?>
                        ...
                        <div class="pasngrinput _passenger_hiiden_inputs">
                             <div class="col-md-9 nopad">
                                   <div class="col-md-5 nopad">
                                       <div class="labbkdiv">
                                        <label><?=ucfirst(strtolower($q_value['text']))?></label><!-- ucwords -->
                                   ...
                                   <div class="col-md-7 nopad">
                                    <div class="mr-inp-div">
                                      ...
                                      <input type="text"  class="chk normalinput auto-focus hand-cursor form-control b-r-0" id="activity_datepicker"  name="answer[<?=@$q_key?>][]" placeholder="" <?=$requierd?>>
                                    </div>  
                                   </div>
