...
foreach ($_RoomCombinations['RoomCombination'] as $__rc_key => $__rc_value) {
	...
	$specific_markup_config[]=array('ref_id'=>$_HotelRoomsDetails[0]['HOTEL_CODE'],'category'=>'hotel_wise');
...
// debug($specific_markup_config);die;
	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		//debug($temp_room_details);exit;
			...
			$temp_price_details = $GLOBALS['CI']->hotel_lib->update_room_markup_currency($temp_room_details['Price'], $currency_obj, $search_id,false,true,$specific_markup_config);
	...
	<span class="romtyprice"><i class="" aria-hidden="true"></i>&nbsp;'.$currency_obj->to_currency.' '.roundoff_number($RoomPrice).'<p class="ninenyt">( '.$no_of_nights.' '.$night_str.' )</p></span>
...
$currency_symbol = $currency_obj->to_currency;
