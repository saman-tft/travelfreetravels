...
// debug($raw_sightseeing_list); die;
foreach ($raw_transfer_list['SSSearchResult']['TransferResults'] as $product_key=>$product) {

	// debug($product);exit;
	
	$booking_source= isset($product['booking_source'])?PROVAB_TRANSFER_SOURCE_CRS:HOTELBED_TRANSFER_BOOKING_SOURCE;
		$current_hotel_rate = $product['StarRating'];

?>
<div class="rowresult r-r-i col-xs-4"  >

	<div class="madgrid forhtlpopover" data-key="<?=$product_key?>" data-product-name="<?=$product['transfer_name']?>" data-product-code="<?=@$product['ProductCode']?>" data-result-token="<?=$product['ResultToken']?>">
		...
		<div class="col-xs-4 nopad listimage">
			<div class="imagehtldis">
				...
				<?php if($product['ImageVehicleUrl']):?>
					
					<a href="<?php echo base_url()?>index.php/transfer/transfer_details?op=get_details&search_id=<?=$search_id?>&booking_source=<?=urlencode($booking_source)?>&price_id=<?=@$product['price_id']?>&result_token=<?=$product['ResultToken']?>">

					<img src="<?=$product['ImageVehicleUrl']?>" alt="Image" data-src="<?=$product['ImageVehicleUrl']?>" title="<?=$product['transfer_name']?>" class="lazy h-img">
				...
				<?php else:?>				 		
				 ...
				 <img src="<?php echo $GLOBALS['CI']->template->template_images('no_image_available.jpg'); ?>" alt="Image" data-src="<?=$product['ImageUrl']?>" title="<?=$product['transfer_name']?>" class="lazy h-img">
			...
			<div class="sidenamedesc">
				<div class="celhtl width70">
					<div class="innd acttbosrch">
						...
						<div class="shtlnamehotl">
							<span class="h-name"><a  href="<?php echo base_url()?>index.php/transfer/transfer_details?op=get_details&search_id=<?=$search_id?>&booking_source=<?=urlencode($booking_source)?>&price_id=<?=@$product['price_id']?>&result_token=<?=$product['ResultToken']?>"><?php echo $product['transfer_name']?></a></span>
							<?php
						...
						<div class="strrat">
							...
							<ul class="std">
							 ...
							 {
							 ?>
							 <li><span class="review"><?=$product['StarRating']?> Reviews</span></i></li>
                        ...
                        <div class="col-md-12 col-xs-12 nopad">
						   <div class="loc_see">
							<?php if($product['Duration']):?>
								<span>Duration: <?=$product['Duration']?> hr</span>
							<?php endif;?>
						</div>
						</div>
				...
				<div class="celhtl width30">
					<div class="sidepricewrp">					
						...
						<form method="GET" action="<?php echo base_url().'index.php/transfer/transfer_details'?>">
							<div class="hide">
								...
								<input type="hidden" class="price_id" name="price_id" value="<?=@$product['price_id']?>">
