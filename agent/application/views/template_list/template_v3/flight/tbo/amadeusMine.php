                ...
                <div class="padpaspotr">
                    <div class="col-xs-8 nopadding tab_pasnger">
                        <div class="fligthsdets">
                            ...
                            // debug($year_options_citizenship);die;
                            /**
...
<script>
    function showModal() {
        ...
        const fetchUrl = baseUrl + "insurance/getavailableplans";
        ...
        fetch(fetchUrl)
            ...
            .then(plans => {
                ...
                plans.forEach(plan => {
                    ...
                    planItem.textContent = plan.name; // Assuming each plan object has a 'name' property
...
<!-- <script>
    ...
    $("#yesBtn").click(function(){
        // Show buffering gif
        $('#insurancePlans').html('<img src="<?php echo base_url('assets/img/buffering.gif'); ?>">');

        // Make AJAX request
        $.post("<?php echo base_url('insurance/getAvailablePlans'); ?>", function(data){
            // Stop buffering gif
            $('#insurancePlans').empty();

            // Display plans
            $('#insurancePlans').html(data);

            // Handle plan selection
            $('#insurancePlans').on('click', '.plan', function(){
                var selectedPlan = $(this).text();
                $('#insuranceSection').html('<p>Selected plan: ' + selectedPlan + '</p>');
            });
        });
    });

    $("#noBtn").click(function(){
        // Hide insurance section if user doesn't want insurance
