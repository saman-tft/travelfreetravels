 ...
 * @version    V1
 */
class Tripxmlcar extends Common_Api_Grind {
...
debug($res);die;
    ...
    function credentials($service) {
        switch ($service) {
            case 'Search':
                $this->service_url = $this->Url . 'car/search';
                break;
            case 'CarDetails':
                $this->service_url = $this->Url . 'car/details';
                break;
            case 'CarRule':
                $this->service_url = $this->Url . 'cart/car/add';
                break;
            case 'CommitBooking':
...
// debug($response);exit;
    ...
    function get_car_list($search_id = '') {
        ...
        if ($search_data['status'] == true) {
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                $this->generate_static_response(json_encode($search_request['data']['request']),'tripxml car search request',$search_id);
                ...
                if ($search_request['status']) {
                    ...
                    $this->generate_static_response(json_encode($search_response),'tripxml car search response',$search_id);
    ...
    function tripxml_to_tmx_format($search_response,$search_data){
        $cars = force_multple_data_format($search_response['Cars']);
        $CarResults = array();
        $SessionId = $search_response['SessionId'];
        // debug($search_response);exit;
        if(valid_array($cars)){
            foreach($cars as $c_k => $c_v){
                $CarResults[$c_k]['PickUpDateTime'] = $c_v['PickupDate'];
                $CarResults[$c_k]['ReturnDateTime'] = $c_v['DropOffDate'];
                $VehicleInfo = force_multple_data_format($c_v['VehicleInfo']);
                $VehicleInfo = $VehicleInfo[0];
                $CarResults[$c_k]['Status']         = $VehicleInfo['Status'];
                $AirConditioning = false;
                if($VehicleInfo['AirConditioning']==1){
                    $AirConditioning = true;
                }
                $CarResults[$c_k]['AirConditionInd'] = $AirConditioning;
                $CarResults[$c_k]['TransmissionType'] = $VehicleInfo['TransmissionType'];
                $CarResults[$c_k]['FuelType'] = '';
                $CarResults[$c_k]['PassengerQuantity'] = $VehicleInfo['Seats'];
                $CarResults[$c_k]['BaggageQuantity'] = '';

                $CarResults[$c_k]['VendorCarType'] = $VehicleInfo['CarType'];
                $CarResults[$c_k]['VehicleCategoryName'] = $VehicleInfo['VehCategory'];
                $CarResults[$c_k]['DoorCount'] = '';
                $CarResults[$c_k]['VehClassSizeName'] = $VehicleInfo['VehClass'];
                $CarResults[$c_k]['Name'] = $VehicleInfo['VehModel'];
                $CarResults[$c_k]['PictureURL'] = $VehicleInfo['PictureURL'];
                $UnlimitedRate = false;
                if($VehicleInfo['UnlimitedRate']==1){
                    $AirConditioning = true;
                }
                $CarResults[$c_k]['Unlimited'] = $UnlimitedRate;
                $CarResults[$c_k]['DistUnitName'] = $VehicleInfo['DistanceUnit'];


                $CarResults[$c_k]['RateComments'] = $VehicleInfo['RateCategory'];
                $CarResults[$c_k]['RateRestrictions'] = '';
                $CarResults[$c_k]['reference_url'] = '';
                $CarResults[$c_k]['Vendor'] = $VehicleInfo['VendorName'];
                $CarResults[$c_k]['DropOffLocation'] = $c_v['DropOffCity'];
                $CarResults[$c_k]['PaymentRules'] = '';

                $CarResults[$c_k]['TPA_Extensions']['TermsConditions'] = $VehicleInfo['CarRules'];
                $CarResults[$c_k]['TPA_Extensions']['SupplierLogo'] = $c_v['CarVendorLogo'];
                $CarResults[$c_k]['PricedCoverage'] = '';
                $CarResults[$c_k]['CancellationPolicy'] = '';
                $CarResults[$c_k]['ResultToken'] = '';
                $CarResults[$c_k]['TotalCharge']['CurrencyCode'] = $VehicleInfo['CurrencyCode'];
                $CarResults[$c_k]['TotalCharge']['EstimatedTotalAmount'] = $VehicleInfo['RateTotalAmount'];
                $CarResults[$c_k]['TotalCharge']['Pricebreakup']['RentalPrice'] = $VehicleInfo['RateTotalAmount'];
                $CarResults[$c_k]['TotalCharge']['Pricebreakup']['OnewayFee'] = 0;
                $CarResults[$c_k]['TotalCharge']['Pricebreakup']['OtherTaxes'] = 0;
                $CarResults[$c_k]['TotalCharge']['Pricebreakup']['YoungDriverAmount'] = 0;
                $CarResults[$c_k]['ResultToken'] = $SessionId.'***'.$c_v['Id'].'***'.$VehicleInfo['Id'];
            }
        }

        $response['Search']['CarSearchResult']['CarResults'] = $CarResults;
    ...
    function tripxml_to_tmx_rate_rules_format($search_response,$search_data){
        $cars = force_multple_data_format($search_response['ShoppingCart']['Cars']);
        $CarResults = array();
        $SessionId = $search_response['SessionId'];
        if(valid_array($cars)){
            foreach($cars as $c_k => $c_v){
        ...
        $response['RateRule']['CarRuleResult'] = $CarResults;
        $response['Status'] = SUCCESS_STATUS;
        $response['Message'] = '';
        // debug($response);exit;
        return $response;
    ...
    function car_search_request($search_params) {
        ...
        $request_params['pickupcity'] = $search_params['pickup_loc_code'];
        // $request_params['to1'] = $search_params['return_loc_code'];
        // $request_params['pickup_loc_id'] = $search_params['pickup_loc_code'];
        // $request_params['return_loc_id'] = $search_params['return_loc_code'];
        $request_params['pickupdate'] = date("Y-m-d", strtotime($search_params['depature'])) ;
        $request_params['dropoffdate'] = date("Y-m-d", strtotime($search_params['return'])) ;
        // $request_params['country'] = $search_params['country'];
        // $request_params['driver_age'] = $search_params['driver_age'];
        // $request_params['Adults'] = '1';
    ...
    public function process_booking($book_id, $booking_params) {
        ...
        if (valid_array($booking_params)) {
            ...
            $this->generate_static_response(json_encode($booking_request['data']['request']),'tripxml booking request',0);
        	$header = $this->get_header();
        	if ($booking_request['status']) {
        		$car_book_response = $GLOBALS ['CI']->api_interface->post_tripxml_json_response ( $booking_request ['data'] ['service_url'], $booking_request ['data'] ['request'], $header );
                /*$res = '{
                  "ReferenceNumber": "22NXPH",
                ...
                $car_book_response = json_decode(($res),true);*/
                // debug($car_book_response);exit;
				$this->generate_static_response(json_encode($car_book_response),'tripxml booking response',0);
        		/**    PROVAB LOGGER * */
                ...
                if (valid_array($car_book_response) == true && $car_book_response['IsSuccessful'] == true) {
                    $response['data']['result'] = $car_book_response;
                } else {
                    $response['status'] = false;
                }
        	}
    ...
    function BookCar_request($booking_params, $booking_source, $booking_id) {
        ...
        $pax_type = '';
        if($booking_params['pax_title']==1){
            $pax_type = 'ADT';
        }else if($booking_params['pax_title']==2){
            $pax_type = 'CHD';
        }else if($booking_params['pax_title']==3){
            $pax_type = 'INF';
        }
        ...
        $tmp_passenger[0]['IndexInSequence'] = 0;
        $tmp_passenger[0]['Title'] = "Mr";
        $tmp_passenger[0]['TypeCode'] = $pax_type;
        $tmp_passenger[0]['FirstName'] = ((strlen($booking_params['first_name']) < 2) ? str_repeat($booking_params['first_name'], 2) : $booking_params['first_name']);
        $tmp_passenger[0]['MiddleName'] = "";
        $tmp_passenger[0]['LastName'] = ((strlen($booking_params['last_name']) < 2) ? str_repeat($booking_params['last_name'], 2) : $booking_params['last_name']);
        $tmp_passenger[0]['Email'] = $booking_params['billing_email'];
        $tmp_passenger[0]['CountryCode'] = $country_data['data'][0]['iso_country_code'];
        $tmp_passenger[0]['Phone'] = $booking_params['passenger_contact'];
        $tmp_passenger[0]['PhoneCode'] = "null";
        $tmp_passenger[0]['SkypeID'] = "null";
        $tmp_passenger[0]['MealsNSeatReq'] = "null";
        $tmp_passenger[0]['Gender'] = "true";
        $date_of_birth = $booking_params['date_of_birth'];
        $date_of_birth = explode('-', $date_of_birth);
        $tmp_passenger[0]['DateOfBirthString'] = $date_of_birth[2].'-'.$date_of_birth[1].'-'.$date_of_birth[0];
        $tmp_passenger[0]['PaymentInfoObject'] = "null";
        $tmp_passenger[0]['BaggagePiecesQty'] = 0;
        $tmp_passenger[0]['LoyalityProgram'] = "null";
    
        $tmp_passenger[0]['PassportNo'] = "null";

        $tmp_passenger[0]['PassportIssuingCountry'] = $country_data['data'][0]['iso_country_code'];
        $tmp_passenger[0]['CountryOfNationality'] = $country_data['data'][0]['iso_country_code'];
        $tmp_passenger[0]['PassportIssuingDate'] = "null";
  ...
  //   	$request ['AppReference'] = $booking_id;
        ...
        $ResultToken = explode('***', $booking_params['token']['ResultToken']);

        $request_params = array();
        $SessionId = $ResultToken[0];
        ...
        $request_params['Travellers'] = $tmp_passenger;
        $request_params['PaymentDetails'][0]['TravelType'] = "CAR";
        ...
        $request_params['PaymentDetails'][0]['CardInfo']['CardholderName'] = "Badri";
        $request_params['PaymentDetails'][0]['CardInfo']['CardType'] = "VI";
        $request_params['PaymentDetails'][0]['CardInfo']['CardTypeCode'] = "VI";
        $request_params['PaymentDetails'][0]['CardInfo']['CardNumber'] = "4111111111111111";
        $request_params['PaymentDetails'][0]['CardInfo']['ExpirationMonth'] = "1";
        $request_params['PaymentDetails'][0]['CardInfo']['ExpirationYear'] = "2025";
        $request_params['PaymentDetails'][0]['CardInfo']['CVV'] = "123";
        $request_params['PaymentDetails'][0]['CardInfo']['BankName'] = "test";
...
// echo 'herrerre'.$admin_markup;exit;
        $booking_id = $params ['result']['ReferenceNumber'];
		$booking_reference = $params ['result']['ReferenceNumber'];
        $account_info = '';
		...
		$dob = date("Y-m-d", strtotime($params['temp_booking']['book_attributes']['date_of_birth']));
    ...
    function format_search_response($hl, $cobj, $sid, $module = 'b2c', $fltr = array()) {
        ...
        foreach ( $hl ['CarSearchResult'] ['CarResults'] as $cr => $cd ) {
           ...
           $ac_var = ($cd['AirConditionInd']=='true')? 'AC': 'Non AC';
            ...
            $CarResults [$cr] = $cd;
            $CarResults [$cr]['booking_source'] = PROVAB_TRIPXML_CAR_BOOKING_SOURCE;
            $frc ++;
            //echo 'count'.$frc;
        }
        // SORTING STARTS
        ...
        $hl ['CarSearchResult'] ['booking_source'] = PROVAB_TRIPXML_CAR_BOOKING_SOURCE;
...
//                  debug($category_list); exit;
	...
	function get_rate_rules($ResultIndex) {
        ...
        $this->generate_static_response(json_encode($car_rule_request['data']['request']),'tripxml car add to cart req',0);
		if ($car_rule_request ['status']) {
			$car_rule_response = $GLOBALS ['CI']->api_interface->post_tripxml_json_response ( $car_rule_request ['data'] ['service_url'], $car_rule_request ['data'] ['request'], $header );
			$this->generate_static_response ( json_encode ( $car_rule_response ) ,'tripxml car add to cart res',0);
			if ($this->valid_car_rules ( $car_rule_response )) {

                $car_rule_response = $this->tripxml_to_tmx_rate_rules_format($car_rule_response,array());
	...
	private function valid_car_rules($car_rules) {
		$status = false;
		if (valid_array ( $car_rules ) == true and isset ( $car_rules ['SessionId'] ) == true) {
			$status = true;
		}
		return $status;
	}

    private function car_details_request($ResultToken) {
        $response ['status'] = true;
        $this->credentials ( 'CarDetails' );
        $response ['data'] = array ();
        $ResultToken = explode('***', $ResultToken);
        $request_params ['sessionid'] = $ResultToken[0];
        $request_params ['carid'] = $ResultToken[1];
        $request_params = "?" . http_build_query($request_params);
        $url = $this->service_url . $request_params;
        // debug($url);exit;
        $response ['data'] ['request'] = $url;
	...
	private function car_rule_request($ResultToken) {
        ...
        $ResultToken = explode('***', $ResultToken);
        $request_params ['SessionId'] = $ResultToken[0];
		$request_params ['ItemId'] = $ResultToken[1];
		$response ['data'] ['request'] = json_encode($request_params);
		$response ['data'] ['service_url'] = $this->service_url;
		return $response;
	}
    ...
    function valid_api_response($api_response) {
        if (empty($api_response) == false && valid_array($api_response) == true and isset($api_response['Cars']) == true) {
            return true;
        } else {
            return false;
        }
