...
class Api_Interface {
    ...
    function rest_service($method, $params = array()) {
        ...
        // debug($system);exit;
        $params = array('domain_key' =>'TMX1512291534825461', 'username' => $username, 'password' => $password, 'system' => $system);
        // debug($params);exit;
    ...
    public function get_json_response($url, $request = array(), $header_details) {
         ...
         //debug($url);debug($request);debug($header_details);die;
        // $CI = &get_instance();
       ...
       // debug($res);exit;
        ...
        if($_SERVER['REMOTE_ADDR'] == "192.168.0.88")
        {
            // debug($res);exit;
            // debug($request);exit;
           	if($url != 'http://192.168.0.87/travelomatix_live_services/webservices/index.php/flight/service/UpdateFareQuote'){
           		 // debug($res);exit;
           	}
          // debug($res);exit;
        }
