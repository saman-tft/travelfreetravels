 ...
 * @version V1
 */
class Resvoyage_hotel extends Common_Api_Grind {
	private $ClientId;
	private $UserName;
	private $Password;
	private $service_url;
    ...
    //    echo $url; exit();
        ...
        $GLOBALS ['CI']->session->set_userdata('tb_auth_token',$res['Token']);
    ...
    function get_destination($city) {
        ...
        $CI =& get_instance();
        $authorization = "Authorization: Bearer ".$GLOBALS ['CI']->session->userdata ( 'tb_auth_token' );
        $header = array(
            'Content-Type: application/json; charset="utf-8"', 
        'Accept-Encoding: gzip,deflate',
            $authorization  
        );
        $url = "http://rest.resvoyage.com/api/v1/hotel/references/destination/".$city."";
       // echo $url;
        $ch = curl_init($url);
        ...
        if($res[$i]['Code']=="")
        {
            if($res[$i]['PlaceId']!="")
            {
              $res[$i]['Code']="pl-".$res[$i]['PlaceId'];
            }
        }
            $newcity=array(
                "placeid"=>$res[$i]['PlaceId'],
                "city_name"=>$res[$i]['Name'],
                "country_name"=>$res[$i]['CountryCode'],
                "stateprovince"=>$res[$i]['Region'],
                "origin"=>$res[$i]['Code'],
                );
                array_push($data,$newcity);
        }
        //debug($data);die;
        curl_close($ch);
        return $data;
	...
	function credentials($service) {
		switch ($service) {
			case 'GetHotelResult' :
				$this->service_url = $this->Url . 'hotel/search';
				break;
			case 'GetHotelImages' :
				$this->service_url = $this->Url.'GetHotelImages';
				break;
			case 'GetHotelInfo' :
				$this->service_url = $this->Url . 'hotel/details';
				break;
			case 'GetHotelRoom' :
				$this->service_url = $this->Url . 'RoomList';
				break;
			case 'BlockRoom' :
				$this->service_url = $this->Url . 'cart/hotel/add';
				break;
			case 'Book' :
	...
	//	$this->get_auth_token();
		 ...
		 */
		 $resexp=explode("pl-",$search_params ['hotel_destination']);
		 //	debug($search_params);die;
//	 debug($resexp);die;
	 if($resexp[0]=="")
	 {
		$request ['HotelCityCode'] =""; // dd/mm/yyyy
	 }
	 else
	 {
	     $request ['HotelCityCode'] =$resexp[0];
	 }
	 
		$request ['CheckInDate'] =date("Y-m-d",strtotime($search_params ['raw_from_date'])); // Min 1
		$request ['CheckoutDate'] =date("Y-m-d",strtotime($search_params ['raw_to_date'])); // ISO Country Code of Destination
		$request ['RoomCount'] = intval ( $search_params ['room_count'] );
	
		 if($resexp[1]=="")
	 {
		$request ['PlaceId'] =""; // dd/mm/yyyy
	 }
	 else
	 {
	     $request ['PlaceId'] =$resexp[1];
	 }
		
		//$request ['token'] =  ;
		$room_index = $temp_child_index = 0;
		for($room_index = 0; $room_index < $request ['RoomCount']; $room_index ++) {
		 //   echo "sdfsd";die;
			$temp_room_config = array();
			if($room_index==0)
			{
    			$request ['Adults'] = intval ( $search_params ['adult_config'] [$room_index] );
    			$request ['Children'] = intval ( $search_params ['child_config'] [$room_index] );
    			if ($search_params ['child_config'] [$room_index] > 0) {
    				$request ['childrenAge'] = array_slice ( $search_params ['child_age'], $temp_child_index, intval ( $search_params ['child_config'] [$room_index] ) );
    				$temp_child_index += intval ( $search_params ['child_config'] [$room_index] );
    			}
			}
			if($room_index==2)
			{
    			$request ['Adults2'] = intval ( $search_params ['adult_config'] [$room_index] );
    			$request ['Children2'] = intval ( $search_params ['child_config'] [$room_index] );
    			if ($search_params ['child_config'] [$room_index] > 0) {
    				$request ['childrenAge'] = array_slice ( $search_params ['child_age'], $temp_child_index, intval ( $search_params ['child_config'] [$room_index] ) );
    				$temp_child_index += intval ( $search_params ['child_config'] [$room_index] );
    			}
			...
			if($room_index==3)
			{
    			$request ['Adults3'] = intval ( $search_params ['adult_config'] [$room_index] );
    			$request ['Children3'] = intval ( $search_params ['child_config'] [$room_index] );
    			if ($search_params ['child_config'] [$room_index] > 0) {
    				$request ['childrenAge3'] = array_slice ( $search_params ['child_age'], $temp_child_index, intval ( $search_params ['child_config'] [$room_index] ) );
    				$temp_child_index += intval ( $search_params ['child_config'] [$room_index] );
    			}
			}
			if($room_index==4)
			{
    			$request ['Adults4'] = intval ( $search_params ['adult_config'] [$room_index] );
    			$request ['Children4'] = intval ( $search_params ['child_config'] [$room_index] );
    			if ($search_params ['child_config'] [$room_index] > 0) {
    				$request ['childrenAge4'] = array_slice ( $search_params ['child_age'], $temp_child_index, intval ( $search_params ['child_config'] [$room_index] ) );
    				$temp_child_index += intval ( $search_params ['child_config'] [$room_index] );
    			}
			}
		...
		$response ['data'] ['request'] = json_encode ( $request );
		    $response ['data'] ['token']=$GLOBALS ['CI']->session->userdata ( 'tb_auth_token' );
		$this->credentials ( 'GetHotelResult' );
		$response ['data'] ['service_url'] = $this->service_url.'?'.http_build_query($request);
	//	debug($response);die;
		return $response;
	}
	 ...
	 */
	private function hotel_details_request($ResultToken,$hotelcode) {
		$response ['status'] = true;
		$response ['data'] = array ();
		$request ['SessionId'] = $ResultToken;
		$request ['HotelId'] = $hotelcode;
		$response ['data'] ['request'] = json_encode ( $request );
		$response ['data'] ['token']=$GLOBALS ['CI']->session->userdata ( 'tb_auth_token' );
	...
	private function get_block_room_request($booking_params)
	{
		...
		$response ['status'] = true;
		$response ['data'] = array ();
		$request ['SessionId'] = $booking_params['sessionid'];
	$request ['Adults'] = $booking_params['adult'];
	$request ['Children'] =$booking_params['child'];
	$request ['RoomId'] = $booking_params['ResultIndex'];
	...
	{	
		...
		for($i = 0; $i < $NO_OF_ROOMS; $i ++) {
			...
			foreach ($booking_params['name_title'] as $bk => $bv){
				...
				$booking_params ['first_name'][$bk]="Arunachalam";
				if(intval($pax_type) == 1 && intval($assigned_pax_type_count) < intval($room_adult_count)){//Adult
					$room_wise_passenger_info[$i]['Title'][]			= $booking_params ['name_title'][$bk];
					$room_wise_passenger_info[$i]['FirstName'][]			= $booking_params ['first_name'][$bk];
					$room_wise_passenger_info[$i]['MiddleName'][]		= $booking_params ['middle_name'][$bk];
					$room_wise_passenger_info[$i]['LastName'][]			= $booking_params ['last_name'][$bk];
					$room_wise_passenger_info[$i]['Phone'][]	= $booking_params ['passenger_contact'];
					$room_wise_passenger_info[$i]['Email'][]		= $booking_params ['billing_email'];
					$room_wise_passenger_info[$i]['TypeCode'][]		= $booking_params ['passenger_type'][$bk];
					$room_wise_passenger_info[$i]['DateOfBirthString'][]		= $booking_params ['date_of_birth'][$bk];
					...
					$room_wise_passenger_info[$i]['MiddleName'][]		= $booking_params ['middle_name'][$bk];
					$room_wise_passenger_info[$i]['LastName'][]			= $booking_params ['last_name'][$bk];
					$room_wise_passenger_info[$i]['passenger_contact'][]	= $booking_params ['Phone'];
					$room_wise_passenger_info[$i]['Email'][]		= $booking_params ['billing_email'];
					$room_wise_passenger_info[$i]['TypeCode'][]		= $booking_params ['passenger_type'][$bk];
					$room_wise_passenger_info[$i]['DateOfBirthString'][]		= $booking_params ['date_of_birth'][$bk];
		...
		$request ['SessionId'] = $booking_params ['token'] ['BlockRoomId'];
		$request['PaymentDetails'][0]['TravelType'] = "HOTEL";
		$request ['PaymentDetails'][0]['PaymentOption'] ="PayPal";
		$request ['PaymentDetails'][0]['Address'] =array(
		     "CountryCode"=>"US",
            "CountryName"=>"USA",
            "State"=>"Maryland",
            "RegionName"=>"Maryland",
            "ZIP"=>"12345",
            "CityName"=>"Miami",
            "StreetAddress"=>"Zarka Pucara"
		    );
		$request['PaymentDetails'][0]['CardInfo'] = array(
		...
		$room_details = array ();
		$k = 0;
	//	debug($room_wise_passenger_info);die;
	$request['Travellers']=array();
		for($i = 0; $i < $NO_OF_ROOMS; $i ++) {
			for($j = 0; $j < $booking_params ['token'] ['token'] [$i] ['no_of_pax']; $j ++) {
				...
				$pax_list ['IndexInSequence'] = $j;
				$pax_list ['Title'] = $pax_title;
				$pax_list ['FirstName'] = $room_wise_passenger_info [$i] ['FirstName'] [$j];
				$pax_list ['TypeCode'] = ($room_wise_passenger_info [$i] ['TypeCode'] [$j]==1)?"ADT":"CDT";
				$pax_list ['MiddleName'] = $room_wise_passenger_info [$i] ['MiddleName'] [$j];
				$pax_list ['LastName'] = $room_wise_passenger_info [$i] ['LastName'] [$j];
				$pax_list ['Phone'] = $room_wise_passenger_info [$i] ['Phone'][$j];
				
				$pax_list ['Email'] = $room_wise_passenger_info [$i] ['Email'][$j];
				$pax_list ['CountryOfNationality'] ="US";
				$pax_list ['Gender'] = "true";
				$pax_list ['PhoneCode'] ='';
				$pax_list ['CountryCode'] ='';
				$pax_list ['DateOfBirthString'] ="1982-04-04T09:42:53.998Z";
				...
				array_push($request['Travellers'],$pax_list);
				$k ++;
			}
		}
	...
	private function cancel_booking_request_params($app_reference) {
		...
		$response ['data'] ['service_url'] = $this->service_url.'/'.$request ['AppReference'].'/cancel';
		// debug($response);
		// exit;
		return $response;
	}
	...
	*/
	...
	function get_hotel_list($search_id = '') {
		...
		if ($search_data ['status'] == true) {
			if ($cache_search === false || ($cache_search === true && empty ( $cache_contents ) == true)) {
				...
				if ($search_request ['status']) {					
					...
					$search_response = $GLOBALS['CI']->api_interface->get_json_response_tripxml( $search_request ['data'] ['service_url'], $search_request ['data'] ['request'], $header,$search_request ['data'] ['token']);
				//	 $search_response = Converter::createArray($search_response);
					$search_response = $this->hotel_result_tmx_format($search_response, $search_id, 'hotel');
					// exit;
		...
		public function hotelblock_result_tmx_format($hotel_response, $search_id, $module){
     
        $response = array();
        $response['Status'] = FAILURE_STATUS;
        $response['Message'] = '';

        $raw_hotel_lists = $hotel_response['ShoppingCart']['Hotels']; 
        ...
        if(!empty($raw_hotel_lists)){
            ...
            foreach ($raw_hotel_lists as $r_hl_k => $r_hl_v) {
                ...
                $list_data['SessionId'] = $hotel_response['SessionId'];
                $list_data['AvailabilityType'] ='';
                $list_data['RequireAllPaxDetails'] = '';
                $list_data['RoomId'] = $r_hl_v['Rooms'][0]['Id'];
                $list_data['RoomStatus'] = $r_hl_v['Rooms'][0]['Status'];
                $list_data['RoomTypeCode'] =$r_hl_v['Rooms'][0]['RoomTypeCode'];
                $list_data['RoomDescription'] = $r_hl_v['Rooms'][0]['RoomText'][1];
                $list_data['RoomTypeName'] =$r_hl_v['Rooms'][0]['Category'];
                $list_data['RatePlanCode'] =$r_hl_v['Rooms'][0]['RatePlanCode'];
                $list_data['RatePlan'] = '';
                $list_data['RatePlanName'] =$r_hl_v['Rooms'][0]['GuaranteeType'];
                $list_data['InfoSource'] ='';
                $list_data['SequenceNo'] = '';
                $list_data['IsPerStay'] ='';
                $list_data['SupplierPrice'] ='';

                $api_price = roundoff_number($r_hl_v['Rooms'][0]['RoomRate']);
                ...
                $list_data['RoomPromotion'] ='';
                $list_data['Amenities'] =array();
                $i=0;
                foreach ($r_hl_v['RoomAmenitiesCollection'] as $k => $v) {
                   $list_data['Amenities'][$i]=$v['Name'];
                   $i++;
                }
                $list_data['Amenity'] =array();
                $i=0;
                foreach ($r_hl_v['RoomAmenitiesCollection'] as $k => $v) {
                   $list_data['Amenity'][$i]=$v['Name'];
                   $i++;
                }
                $list_data['SmokingPreference'] ='';
                $list_data['BedTypes'] = $r_hl_v['Rooms'][0]['DailyRate'];
                $list_data['HotelSupplements'] = '';

                $list_data['LastCancellationDate'] ='';
                $list_data['CancellationPolicies'] =$r_hl_v['Rooms'][0]['CancellationPenalties'];

                

                $list_data['LastVoucherDate'] =$r_hl_v['Rooms'][0]['CancellationPenalties'][0]['DeadlineDate'];
                $list_data['CancellationPolicy'] = $r_hl_v['Id'];
                $list_data['Inclusion'] ='';
                $list_data['IsPassportMandatory'] ='';
                $list_data['IsPANMandatory'] = '';
                $list_data['TBO_RoomIndex'] =$r_hl_v['Rooms'][0]['ResultIndex'];
                $list_data['TBO_RoomTypeName'] =$r_hl_v['Rooms'][0]['Category'];
                $list_data['HotelCode'] =$r_hl_v['HotelCode'];
                $list_data['SEARCH_ID'] ='';
                 $list_data['API_raw_price'] =1;
                $list_data['IsPackageFare'] ='';
                $list_data['IsPackageDetailsMandatory'] = '';
            ...
            $Results['BlockRoomResult']['BlockRoomId']= $hotel_response['SessionId'];
            $Results['BlockRoomResult']['IsPriceChanged']='';
            $Results['BlockRoomResult']['IsCancellationPolicyChanged']='';
            $Results['BlockRoomResult']['HotelRoomsDetails']= $HotelResults;
        }else{
            $response['Status'] = FAILURE_STATUS;
            $response['Message'] = '';
        }
	...
	public function hotel_result_tmx_format($hotel_response, $search_id, $module){
     ...
     //  debug($hotel_response);
       // die('=================');
        ...
        $raw_hotel_lists = $hotel_response['Hotels']; 
        ...
        if(!empty($raw_hotel_lists)){
            ...
            //    debug($r_hl_v);die();
               ...
               //debug($hotel_info);
                ...
                $list_data['HotelName'] = $r_hl_v['HotelName'];
                $list_data['HotelCategory'] = '';
                $list_data['StarRating'] = $r_hl_v['HotelAwards'][0]['Rating'];
                $list_data['HotelDescription'] = $r_hl_v['HotelDescription'][0];
                $list_data['HotelPromotion'] = '';
                $list_data['HotelPolicy'] ='';

                $api_price = roundoff_number($r_hl_v['DailyRatePerRoom']);
                ...
                $list_data['HotelPicture'] = $r_hl_v['HotelMainImage'];
                $list_data['HotelAddress'] = $r_hl_v['HotelAddress']['StreetAddress'];
                $list_data['HotelPhone'] =$r_hl_v['HotelMainImage'];
                $list_data['HotelMap'] ='';
                $list_data['Latitude'] = $r_hl_v['latitude'];
                $list_data['Longitude'] = $r_hl_v['longitude'];

                $list_data['HotelLocation'] ='';
                $list_data['SupplierPrice'] ='';

                $AmentiesDetails = $r_hl_v['HotelAmenities'];
                $format_room_details = array();
                $i=0;
                foreach ($AmentiesDetails as $k => $v) {
                   $format_room_details[$i]=$v;
                   $i++;
                }
                ...
                $list_data['HotelAmenities'] = $format_room_details;
	...
	*/
	 ...
	 */
	function get_hotel_details($ResultIndex,$hotelcode) {
		$header = $this->get_header ();
		$response ['data'] = array ();
		$response ['status'] = false;
		$hotel_details_request = $this->hotel_details_request ($ResultIndex,$hotelcode);
		// debug($hotel_details_request);exit;
		if ($hotel_details_request ['status']) {
			// get the response for hotel details
			$hotel_details_response = $GLOBALS ['CI']->api_interface->get_json_response_tripxml ( $hotel_details_request ['data'] ['service_url'], $hotel_details_request ['data'] ['request'], $header,$hotel_details_request ['data'] ['token']);
	...
	public function format_hotel_details($hotel_details_response){
        ...
        if(!empty($hotel_details_response)){
            ...
            $response_details = $hotel_details_response['Hotels'][0];
   ...
   //     debug($response_details);die();
            ...
            $HotelDetails = array();
                ...
                $HotelDetails['HotelCode'] = $response_details['HotelCode'];
                $HotelDetails['HotelName'] = $response_details['HotelName'];
                $HotelDetails['StarRating'] = '5';
                $HotelDetails['HotelURL'] = '';
                $HotelDetails['Description'] = $response_details['HotelDescription'][0];
                $HotelDetails['Attractions'] = array();
$HotelDetails['SessionId'] = $hotel_details_response['SessionId'];

                $HotelDetails['HotelFacilities'] = '';
                if(isset($response_details['Recreation'])){
                    $HotelDetails['HotelFacilities'] = $response_details['Recreation'];
                   /* $i=0;
                foreach ($response_details['HotelAmenities'] as $k => $v) {
                   $HotelDetails['HotelFacilities'][$i]=$v;
                   $i++;
                }*/
                }else{
                    $HotelDetails['HotelFacilities'] ='';
                ...
                $HotelDetails['HotelPicture'] = ['HotelMainImage'];
               
                $HotelDetails['Images'] = array();    
                if(isset($response_details['HotelImages'])){
                    $HotelDetails['Images'] = $response_details['HotelImages'];
                }else{
                    $HotelDetails['Images'] = array(
                        '0' => $response_details['HotelMainImage'],
                    );
                }    
 
                if(isset($response_details['Address'])){
                    $HotelDetails['Address'] = $response_details['Address'];
                }else{
                    $HotelDetails['Address'] = $response_details['Address'];
                }    
                
                $HotelDetails['CountryName'] = $response_details['HotelAddress']['CountryName'];
                $HotelDetails['PinCode'] = $response_details['HotelAddress']['ZIP']; 
                $HotelDetails['HotelContactNo'] = $response_details['HotelPhone'];
                $HotelDetails['FaxNumber'] = $response_details['HotelFax'];
                $HotelDetails['Email'] = $response_details['Email'];
                $HotelDetails['Latitude'] = $response_details['Latitude'];
                $HotelDetails['Longitude'] = $response_details['Longitude'];
                $HotelDetails['RoomData'] = '';

                $HotelDetails['RoomFacilities'] = ''; 
                if(isset($response_details['RoomAmenitiesCollection'])){
                     $i=0;
                foreach ($response_details['RoomAmenitiesCollection'] as $k => $v) {
                 
                   $HotelDetails['RoomFacilities'][$i]=$v['Name'];
                   $i++;
                }
                }else{
                    $HotelDetails['RoomFacilities'] = explode(',', $response_details['RoomAmenitiesCollection']);
                }  

                $HotelDetails['Services'] = ''; 
                $HotelDetails['Amenities'] = '';
                if(isset($response_details['HotelAmenitiesCollection'])){
                     $i=0;
                foreach ($response_details['HotelAmenitiesCollection'] as $k => $v) {
                //    debug($v);die;
                   $HotelDetails['Amenities'][$i]=$v['Name'];
                   $i++;
                }
                }else{
                    $HotelDetails['Amenities'] = explode(',', $response_details['HotelAmenitiesCollection']);
                }
            ...
            $HotelDetails['checkin'] = date("Y-m-d",strtotime($response_details ['CheckInDate']));
          
            $HotelDetails['checkout'] = date("Y-m-d",strtotime($response_details ['CheckOutDate']));
            
            $RoomDetails = $response_details['Rooms'];
              
          // debug($RoomDetails[0]);die;
             
                $api_price = roundoff_number($RoomDetails[0]['RoomRate']);
                $price = array(
                    'TBO_RoomPrice' => $api_price,
                    'TBO_OfferedPriceRoundedOff' => $api_price,
                    'TBO_PublishedPrice' => $api_price,
                ...
                $room_name = $RoomDetails[0]['BedType'];
                $Room_data = array(
                        'RoomUniqueId' => $RoomDetails[0]['Id'],
                        'rate_key' => '1',
                        'group_code' => '',
                    );
	...
	function block_room($pre_booking_params) {
		...
		//	debug($block_room_request ['data']);
				$block_room_response = $GLOBALS ['CI']->api_interface->post_tripxml_json_response_sec( $block_room_request ['data'] ['service_url'], $block_room_request ['data'] ['request'], $header,$block_room_request ['data'] ['token']);
				$block_room_response = $this->hotelblock_result_tmx_format($block_room_response, $search_id, 'hotel');
...
//debug($book_request ['data'] ['request']);
//debug($book_request ['data'] ['service_url']);
     	$book_responsexml= $GLOBALS ['CI']->api_interface->post_tripxml_json_response_sec ( $book_request ['data'] ['service_url'], $book_request ['data'] ['request'], $header,$book_request ['data'] ['token']);
     $book_response=array(
            "status"=>1,
            "Message"=> "",
            "CommitBooking"=>array(
                     "BookingDetails"=>array(
                     "ConfirmationNo"=>$book_responsexml["ReferenceNumber"],
                     "BookingRefNo"=>$book_responsexml["ReferenceNumber"],
                     "BookingId"=>$book_responsexml["ReferenceNumber"],
                     "SupplierVatId"=> "",
...
//debug($params ['book_response']);die;
        if($booking_source==PROVAB_RESVOYAGE_BOOKING_SOURCE)
        {
             $app_booking_id= $params ['book_response'] ['BookResult'] ['BookingRefNo'];
        }
	...
	//	debug($cancel_booking_request);die;
		if ($cancel_booking_request ['status']) {
			...
			$cancel_booking_response = $GLOBALS ['CI']->api_interface->post_tripxml_json_response_sec ( $cancel_booking_request ['data'] ['service_url'], $cancel_booking_request ['data'] ['request'], $header,$cancel_booking_request ['data'] ['token']);
		//	debug($cancel_booking_response);die;
			if($cancel_booking_response['sucess'])
			{
			$cancel_booking_response= array(
             ...
             "CancelBooking"=>array(
                     ...
                     "ChangeRequestId"=>3,
                     "RefundedAmount"=>$booking_details['grand_total'],
