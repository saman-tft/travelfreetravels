...
//	debug($search_data);die;
	...
	*/
	...
	function booking_url($search_id) {
		return base_url () . 'index.php/villasapartment/booking/' . intval ( $search_id );
	}
	 ...
	 */
	function update_markup_currency(& $price_summary, & $currency_obj, $no_of_nights = 1, $level_one_markup = false, $current_domain_markup = true,$specific_markup_config=array()) {
		...
		foreach ( $price_summary as $__k => $__v ) {
			...
			if (in_array ( $__k, $markup_list )) {
				...
				$agent_markup=$temp_price['original_markup'];
			} else {
			...
			if (in_array ( $__k, $tax_removal_list )) {
				$markup_summary [$__k] = roundoff_number($temp_price ['default_value'] + $tax_service_sum);
			} else {

				$markup_summary [$__k] = roundoff_number($temp_price ['default_value']);
			}
	    ...
	    if($Markup > 0 ){
	        ...
	        if($gst_details['status'] == true){
	            if($gst_details['data'][0]['gst'] > 0){
					$gst_value = ($convinence_amount*$gst_details['data'][0]['gst']) /100;
	            }
	        }
	     }
       ...
       */
       ...
       // debug($markup_summary['PublishedPrice']);
        ...
        $markup_summary['OfferedPrice'] =  round($markup_summary['OfferedPrice'] );
