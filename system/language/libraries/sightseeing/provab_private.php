	...
	*/
	...
	function save_booking($app_booking_id, $params, $module = 'b2c') {
       ...
       /*if($gst_value_con >0){
		...
		if($module == 'b2c'){
        	$GLOBALS ['CI']->sightseeing_model->save_booking_itinerary_details ( $app_reference, $location, $travel_date,$grade_code, $grade_desc, $status,$commissionable_fare,$admin_net_fare_markup,$admin_markup+$gst_value, $agent_markup, $currency, $attributes, @$book_total_fare,$agent_commission,$agent_tds,$admin_commission,$admin_tds,$api_raw_fare,$agent_buying_price, $gst_value_con);
        }
        else if($module == 'b2b'){
        ...
        if($module == 'b2c'){
        	//adding gst
	      	if($Markup > 0 ){
	        	...
	        	$search_data = $GLOBALS['CI']->private_management_model->sightseeing_convinence_fees();
