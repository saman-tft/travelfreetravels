...
class Flight_crs_data extends Common_api_flight {
	...
	public function __construct()
	{
		...
		$this->CI->load->model('flight_model');
			$this->CI->load->model('flight_crs_model');
...
/**
	...
	function save_booking($app_booking_id, $book_params, $currency_obj, $module='b2c')
	{
		...
		foreach ($token['FlightDetails']['Details'] as $token_index => $token_value) {
			...
			// debug($pnr1);${"pnr".$x}
			// debug($pnr2);
			$transaction_insert_id = $GLOBALS['CI']->flight_crs_model->save_flight_booking_transaction_details(
		...
		//	debug($transaction_insert_id); 
			...
			for ($i=0; $i<$total_pax_count; $i++)
			{
				...
				//SAVE Pax Details
				$pax_insert_id = $GLOBALS['CI']->flight_crs_model->save_flight_booking_passenger_details(
				...
				$passenger_ticket_info = $GLOBALS['CI']->flight_crs_model->save_passenger_ticket_info($pax_insert_id['insert_id']);
			...
			foreach($segment_details as $seg_k => $seg_v) {
				...
				foreach($seg_v as $ws_key => $ws_val) {
					...
					//SAVE ITINERARY
					$GLOBALS['CI']->flight_crs_model->save_flight_booking_itinerary_details(
		...
		$cabin_class='NULL';
		$GLOBALS['CI']->flight_crs_model->save_flight_booking_details(
		...
		$GLOBALS['CI']->flight_crs_model->add_extra_service_price_to_published_fare($app_reference);
		
		//Adding Extra services Total Price
		$extra_services_total_price = $GLOBALS['CI']->flight_crs_model->get_extra_services_total_price($app_reference);
		...
		$GLOBALS['CI']->transaction->update_convinence_discount_details('crs_flight_booking_details', $app_reference, $discount, $promo_code, $convinence, $convinence_value, $convinence_type, $convinence_per_pax);
	...
	{
		$stored_booking_details = $GLOBALS['CI']->flight_crs_model->get_booking_details($app_reference);
			...
			foreach($booking_transaction_details as $tr_k => $tr_v){
				...
				foreach ($book_params["baggage_$baggage_index"] as $bag_k => $bag_v){
					...
					if(empty($bag_v) == false&& isset($baggage_details[$bag_v]) == true && $baggage_details[$bag_v]['JourneyType'] == $journy_type){
						...
						$GLOBALS['CI']->flight_crs_model->save_passenger_baggage_info($passenger_fk, $from_airport_code, $to_airport_code, $description, $price, $code);
					}
				}
			}
			$baggage_index++;
			...
			foreach($booking_transaction_details as $tr_k => $tr_v){
				...
				foreach ($book_params["meal_$meal_index"] as $meal_k => $meal_v){
					...
					if(empty($meal_v) == false&& isset($meal_details[$meal_v]) == true && $meal_details[$meal_v]['JourneyType'] == $journy_type){
						...
						$GLOBALS['CI']->flight_crs_model->save_passenger_meals_info($passenger_fk, $from_airport_code, $to_airport_code, $description, $price, $code);
					}
				}
			}
			$meal_index++;
			...
			foreach($booking_transaction_details as $tr_k => $tr_v){
				...
				foreach ($book_params["seat_$seat_index"] as $seat_k => $seat_v){
					...
					if(empty($seat_v) == false&& isset($seat_details[$seat_v]) == true && $seat_details[$seat_v]['JourneyType'] == $journy_type){
						...
						$GLOBALS['CI']->flight_crs_model->save_passenger_seat_info($passenger_fk, $from_airport_code, $to_airport_code, $description, $price, $code, 'dynamic', $airline_code, $flight_number);
					}
				}
			}
			$seat_index++;
			...
			foreach($booking_transaction_details as $tr_k => $tr_v){
				...
				foreach ($book_params["meal_pref$meal_index"] as $meal_k => $meal_v){
					...
					if(empty($meal_v) == false&& isset($meal_details[$meal_v]) == true && $meal_details[$meal_v]['JourneyType'] == $journy_type){
						...
						$code = 				$meal_details[$meal_v]['Code'];
						//Save passenger meal information
						$GLOBALS['CI']->flight_crs_model->save_passenger_meals_info($passenger_fk, $from_airport_code, $to_airport_code, $description, $price, $code, 'static');
					}
				}
			}
			$meal_index++;
			...
			foreach($booking_transaction_details as $tr_k => $tr_v){
				...
				foreach ($book_params["seat_pref$seat_index"] as $seat_k => $seat_v){
					...
					if(empty($seat_v) == false&& isset($seat_details[$seat_v]) == true && $seat_details[$seat_v]['JourneyType'] == $journy_type){
						...
						//Save passenger seat information
						$GLOBALS['CI']->flight_crs_model->save_passenger_seat_info($passenger_fk, $from_airport_code, $to_airport_code, $description, $price, $code, 'static');
					}
				}
			}
			$seat_index++;
    ...
    {
        $saved_booking_data = $GLOBALS['CI']->flight_crs_model->get_booking_details($book_id);
		...
		$c_fsid = $GLOBALS['CI']->flight_crs_model->get_fsid_for_pnr_int($c_date1,$fsid_for_oneway);
		...
		$seat_status_query = $GLOBALS['CI']->flight_crs_model->update_crs_seat($fdid_for_oneway, $pax_count, $fsid_for_oneway);
		...
		for($i=0;$i<count($fsid);$i++){
			...
			$GLOBALS['CI']->flight_crs_model->save_flight_crs_booking_details($crs_array);
		}
	...
	public function update_booking_details($book_id, $book_params, $ticket_details, $module='b2c')
	{
		...
		$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));
		...
		$type_for_crs = count($book_params['token']['token']);
		// debug($type_for_crs); exit;
		$fsid_for_oneway = $book_params['token']['token']['FlightDetails']['Details'][0][0]['Origin']['fsid'];
		// debug($fsid_for_oneway); exit;
		$fsid_for_roundway = $book_params['token']['token']['FlightDetails']['Details'][1][0]['Origin']['fsid'];
		// debug($fsid_for_roundway); exit;
		...
		if($c_is_domestic != '' && $type_for_crs == 1)
		{
			...
			$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_transaction_details', $update_transaction_data, $update_transaction_condition);
			...
			for($i=0;$i<count($itinerary_origins);$i++){
				$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_itinerary_details',$update_itenerary_data,array('origin' => $itinerary_origins[$i]));
			}
		}
		...
		if($c_is_domestic != '' && $type_for_crs == 2)
		{
			...
			//$itinerary_origins
			for($i=0;$i<count($itinerary_origins);$i++){
				$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_itinerary_details',array('airline_pnr' => $array_pnr[$i]),array('origin' => $itinerary_origins[$i]));
			}
	...
	$c_details = $book_params['token']['token']['flight_details']['summary'][0]['origin']['datetime'];
			$c_date = current($c_details)['OriginDetails']['_Date'];
			$c_date1 = date("Y-m-d", strtotime($c_date));
			$c_fsid = $GLOBALS['CI']->flight_crs_model->get_fsid_for_pnr_int($c_date1,$fsid_for_oneway);
			$c_pnr = $c_fsid['pnr'];
		...
		if($is_domestic_crs == '')
		{
	    	...
	    	$fsid =$book_params['token']['token']['FlightDetails']['Details'][0][0]['Origin']['fsid'];
	    	
	    	// debug($fsid); exit;

	    	$seat_status_query = $GLOBALS['CI']->flight_crs_model->update_crs_seat($dat, $pax_count, $fsid, $dat1);
	    	$fsid = array($fsid);
		...
		for($k=0;$k<count($itineary_details);$k++){
			...
			foreach($passenger_details as $pax_k => $pax_v){
				...
				if($trans_id == $pax_v['flight_booking_transaction_details_fk']){
					...
					$GLOBALS['CI']->custom_db->update_record('crs_flight_passenger_ticket_info',$update_passenger_data,$itinary_update_condition);

					$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_passenger_details',array('status'=>$status, 'attributes' => $passenger_price),array('origin' => $pax_v['origin']));
				}
		...
		$safe_search_data['is_domestic_one_way_flight'] = $GLOBALS['CI']->flight_crs_model->is_domestic_flight($from_loc, $to_loc);
		if ($safe_search_data['is_domestic_one_way_flight'] == false && strtolower($from_to_trip_type) == 'circle') {
			$multiplier = $pax_count * 2;//Multiply with 2 for international round way
		} else if(strtolower($from_to_trip_type) == 'multicity'){
			$multiplier = $pax_count * count($safe_search_data['from']);
		...
		if(valid_array($ticket_details) == true) {
			...
			foreach ($ticket_details as $ticket_index => $ticket_value) {
				...
				if (($ticket_value['Status']) == true) {//IF Ticket is HOLD/CONFIRMED
					...
					$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_passenger_details', $update_passenger_data, $update_passenger_condition);
					...
					foreach($passenger_details as $pax_k => $pax_v){
						...
						$GLOBALS['CI']->flight_crs_model->update_passenger_ticket_info($passenger_fk, $TicketId, $TicketNumber, $IssueDate, $Fare,
					...
					foreach($segment_details as $seg_k => $seg_v) {
						foreach($seg_v as $ws_key => $ws_val) {
							...
							$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_itinerary_details', $update_segement_data, $update_segment_condition);
						}
					}
				} else {//IF Ticket is Failed
					$GLOBALS['CI']->flight_crs_model->update_flight_booking_transaction_failure_status($app_reference, $transaction_details_origin);
					//For Transaction Log
 ...
 //    }
    ...
    public function update_pnr_booking_details($book_id, $pnr_no)
    {
    	...
    	$GLOBALS['CI']->custom_db->update_record('crs_flight_booking_itinerary_details',$update_itenerary_data,array('app_reference' => $book_id));
    	return true;
     }
