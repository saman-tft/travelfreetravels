...
class Flight_crs_01_06_2021.php extends Common_Api_Grind {
...
//debug($flight_result);exit();
	...
	function make_flight_details_query($dep_date,$origin,$destination,$segment_type,$strClass,$strTripType="oneway",$ret_date='',$module="b2c"){
				...
				if(valid_array($arrData)){
					foreach($arrData as $FKey=>$F_Val){
						...
						for($fv=0; $fv<count($F_Val); $fv++){
							...
							$strAdultFare  = $arrDataH['adult_basefare']+$arrDataH['adult_tax'];
							$strChildFare  = $arrDataH['child_basefare']+$arrDataH['child_tax'];
							$strInfantFare = $arrDataH['infant_basefare']+$arrDataH['infant_tax'];

							$strTotalFare  = ($strAdultFare*$strAdult)+($strChildFare*$strChild)+($strInfantFare*$strInfant);
							$strTotalTax   = ($arrDataH['adult_tax']*$strAdult)+($arrDataH['child_tax']*$strChild)+($arrDataH['infant_tax']*$strInfant);

							$arrFlight_fare['price']['api_currency']  			= $arrDataH['crs_currency'];
							$arrFlight_fare['price']['api_total_display_fare']  = $strTotalFare;
							$arrFlight_fare['price']['total_breakup']['api_total_tax']  	= $strTotalTax;
							$arrFlight_fare['price']['total_breakup']['api_total_fare']  	= $strTotalFare-$strTotalTax;
							$arrFlight_fare['price']['pax_breakup']  = array();
...
/**
	...
	public function format_search_response($search_result, $currency_obj, $search_id, $module, $from_cache = false, $search_hash = '')
	{
		...
		if ($_SERVER['REMOTE_ADDR']=='192.168.0.77') {
			echo 'herre';
			debug($search_result);exit;
		}
