 ...
 * @version    V1
 */
class Aerocrs extends Common_Api_Grind {
    ...
    private function set_api_credentials() {
        ...
        $this->Url = 'https://api.aerocrs.com/v5/';

        // live key
        $this->UserName = 'CC3F793E-DC63-4B46-A29B-672B09F8026B';
        
        // test key
        //$this->UserName = '93FB37CB-7600-4583-AA45-D0CDBBE5280E';
        $this->Password = '4UgfU47C#b!PLvhs';
    ...
    function credentials($service) {
        switch ($service) {
            case 'Search':
                $this->service_url = $this->Url . 'getDeepLink';
                break;
            case 'FareRule':
                $this->service_url = $this->Url . 'getFareRules';
                break;
            case 'UpdateFareQuote':
                $this->service_url = $this->Url . 'getFlight';
                break;
            ...
            case 'HoldTicket':
                ...
                break;
            case 'CommitBooking':
                $this->service_url = $this->Url . 'confirmBooking';
                break;
    ...
    function flight_search_request($search_params) {
        ...
        //$segments = array();
        // https://api.aerocrs.com/v5/getDeepLink?from=BCN&to=MAD&start=2019/12/01&
        //adults=1&child=0&infant=0&end=2019/12/02

        // for ($i = 0; $i < count($search_params['from']); $i++) {
        //     $segments[$i]['from'] = $search_params['from'][$i];
        //     $segments[$i]['to'] = $search_params['to'][$i];
        //     $segments[$i]['start'] = $search_params['depature'][$i];
        //     if ($search_params['type'] == 'Return') {
        //         $segments[$i]['end'] = $search_params['return'][$i];
        //     }
        // }

        $from = $search_params['from'][0];
        $to = $search_params['to'][0];
        $start = date('Y/m/d', strtotime($search_params['depature'][0]));
       
        $adults = $search_params['adult'];
        $child = $search_params['child'];
        $infant = $search_params['infant'];

        $c_class = 'ECO';
        if($search_params['v_class']=='Business'){
        $c_class = 'BIZ';
        }else if($search_params['v_class']=='PremiumEconomy'){
        $c_class = 'PRM';
        }else if($search_params['v_class']=='First'){
        $c_class = 'FST';
        }
        $req = $this->service_url."?from=".$from."&to=".$to."&start=".$start."&adults=".$adults."&child=".$child."&infant=".$infant."&cabinCode=".$c_class;

        if ($search_params['type'] == 'Return') {
            $end = date('Y/m/d', strtotime($search_params['return'][0]));
            $req.="&end=".$end;
        }
    ...
    function fare_quote_request($data_key, $search_key,$safe_search_data) {
        ...
        if (empty($data_key) == false) {
            ...
            $params['adults'] = $safe_search_data['adult_config'];
            $params['child'] = $safe_search_data['child_config'];
            $params['infant'] = $safe_search_data['infant_config'];
            $params['fromcode'] = $safe_search_data['from'];
            $params['tocode'] = $safe_search_data['to'];
            $params['flightid1'] = $data_key[0]['Attr']['flightid'];
            $params['fareid1'] = $data_key[0]['Attr']['fareid'];
            ...
            $request_params['aerocrs']['parms'] = $params;
        } else {
            $response['status'] = FAILURE_STATUS;
        }
...
// debug($request_booking_params);exit;
    ...
    private function commit_booking_request($booking_params, $app_reference, $flight_data) {
        ...
        $flight_bookingid = $flight_data['token']['token'][0]['Attr']['bookingid'];
        $request_params['bookingid'] = ($flight_bookingid);
        $request_params['agentconfirmation'] = $app_reference;
        $request_params['confirmationemail'] = 'faisalhhh28@gmail.com';
        $request_params['paxcarringinfant'] = 1;

        $request_params['passenger'] = $booking_params['Passenger'];
        $request['aerocrs']['parms'] = $request_params;
        $response['data']['request'] = json_encode($request);
    ...
    function get_flight_list($search_id = '') {
        ...
        if ($search_data['status'] == true) {
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                $this->generate_static_response(json_encode($search_request['data']['request']),'search request',$search_id);

                // debug($search_request);exit;
                //get data
                if ($search_request['status']) {
                    $search_response = $this->CI->api_interface->get_search_response($search_request['data']['service_url'], $search_request['data']['request'], $header_info);

                    $this->generate_static_response(json_encode($search_response),'search response',$search_id);
                    ...
                    if ($this->valid_api_response($search_response)) {
                        if($search_data['data']['trip_type']=='circle'){
                            ...
                            $search_response = $this->aerocrs_to_tmx_format($search_response,$search_data['data']);
                        }
...
// debug($FinalResponse);exit;
    ...
    function aerocrs_to_tmx_format($search_response,$search_data){
        $flights = force_multple_data_format($search_response['aerocrs']['flights']['flight']);
        $JourneyList = array();
        ...
        foreach($flights as $f_k =>$f_v){
            ...
            $FlightList[$f_k]['FlightDetails']['Details'][0][0] = $SegmentDetails;
            ...
            $FlightList[$f_k]['Attr']['FareType'] = '';
            $FlightList[$f_k]['Attr']['fareid'] = $first_price['fareid'];
            $FlightList[$f_k]['Attr']['flightid'] = $first_price['flightid'];
    ...
    function aerocrs_to_fare_quote_tmx_format($search_response,$search_data,&$SegmentDetails){
        $flights = force_multple_data_format($search_response['aerocrs']['getFlight']['flight']);
        $JourneyList = array();
        ...
        foreach($flights as $f_k =>$f_v){
            ...
            $fare = $first_price;
            if($search_data['adult_config']>0){
                $BasicFare += $fare['agtfare_adult']*$search_data['adult_config'];
                $Tax +=   $fare['tax']*$search_data['adult_config'];
                $TotalDisplayFare += $fare['agtfare_adult']*$search_data['adult_config']+$fare['tax']*$search_data['adult_config'];
                $Adt_BasicFare +=$fare['agtfare_adult']*$search_data['adult_config'];
                $Adt_Tax += $fare['tax']*$search_data['adult_config'];
                $Adt_TotalPrice += $fare['agtfare_adult']*$search_data['adult_config']+$fare['tax']*$search_data['adult_config'];
            }
            
            if($search_data['child_config']>0){

                $BasicFare += $fare['agtfare_child']*$search_data['child_config'];
                $Tax +=   $fare['tax']*$search_data['child_config'];
                $TotalDisplayFare += $fare['agtfare_child']*$search_data['child_config']+$fare['tax']*$search_data['child_config'];
                $Chd_BasicFare +=$fare['agtfare_child']*$search_data['child_config'];
                $Chd_Tax += $fare['tax']*$search_data['child_config'];
                $Chd_TotalPrice += $fare['agtfare_child']*$search_data['child_config']+$fare['tax']*$search_data['child_config'];
            }
            if($search_data['infant_config']>0){

                $BasicFare += $fare['agtfare_infant']*$search_data['infant_config'];
                $Tax +=   $fare['tax']*$search_data['child_config'];
                $TotalDisplayFare += $fare['agtfare_infant']*$search_data['child_config']+$fare['tax']*$search_data['infant_config'];
                $Inf_BasicFare +=$fare['agtfare_infant']*$search_data['infant_config'];
                $Inf_Tax += $fare['tax']*$search_data['infant_config'];
                $Inf_TotalPrice += $fare['agtfare_infant']*$search_data['infant_config']+$fare['tax']*$search_data['infant_config'];
            ...
            if($search_data['adult_config']>0){
                $PassengerBreakup['ADT']['PassengerCount'] = $search_data['adult_config'];
                $PassengerBreakup['ADT']['BasePrice'] = $Adt_BasicFare;
                $PassengerBreakup['ADT']['Tax'] =  $Adt_Tax;
                $PassengerBreakup['ADT']['TotalPrice'] = $Adt_TotalPrice;

                $PassengerFareBreakdown['ADT']['PassengerCount'] = $search_data['adult_config'];
                $PassengerFareBreakdown['ADT']['BaseFare'] = $Adt_BasicFare;
                $PassengerFareBreakdown['ADT']['Tax'] =  $Adt_Tax;
                $PassengerFareBreakdown['ADT']['TotalPrice'] = $Adt_TotalPrice;
            }
            
            if($search_data['child_config']>0){
                $PassengerBreakup['CHD']['PassengerCount'] = $search_data['child_config'];
                $PassengerBreakup['CHD']['BasePrice'] = $Chd_BasicFare;
                $PassengerBreakup['CHD']['Tax'] =  $Chd_Tax;
                $PassengerBreakup['CHD']['TotalPrice'] = $Chd_TotalPrice;

                $PassengerFareBreakdown['CHD']['PassengerCount'] = $search_data['child_config'];
                $PassengerFareBreakdown['CHD']['BaseFare'] = $Chd_BasicFare;
                $PassengerFareBreakdown['CHD']['Tax'] =  $Chd_Tax;
                $PassengerFareBreakdown['CHD']['TotalPrice'] = $Chd_TotalPrice;
            }
            if($search_data['infant_config']>0){
                $PassengerBreakup['INF']['PassengerCount'] = $search_data['infant_config'];
                $PassengerBreakup['INF']['BasePrice'] = $Inf_BasicFare;
                $PassengerBreakup['INF']['Tax'] =  $Inf_Tax;
                $PassengerBreakup['INF']['TotalPrice'] = $Inf_TotalPrice;

                $PassengerFareBreakdown['INF']['PassengerCount'] = $search_data['infant_config'];
                $PassengerFareBreakdown['INF']['BaseFare'] = $Inf_BasicFare;
                $PassengerFareBreakdown['INF']['Tax'] =  $Inf_Tax;
                $PassengerFareBreakdown['INF']['TotalPrice'] = $Inf_TotalPrice;
            }
            ...
            $SegmentDetails[0]['api_original_price_details']['TotalDisplayFare'] = $TotalDisplayFare;
            $SegmentDetails[0]['api_original_price_details']['AgentCommission'] = 0;
            $SegmentDetails[0]['api_original_price_details']['BasicFare'] = $BasicFare;
            $SegmentDetails[0]['api_original_price_details']['Tax'] = $Tax;

            $SegmentDetails[0]['FareDetails']['BaseFare'] = $BasicFare;
            $SegmentDetails[0]['FareDetails']['Tax'] = $Tax;
            $SegmentDetails[0]['FareDetails']['PublishedFare'] = $TotalDisplayFare;
            $SegmentDetails[0]['FareDetails']['OfferedFare'] = $TotalDisplayFare;

            $SegmentDetails[0]['PassengerFareBreakdown']= $PassengerFareBreakdown;
    }
    /**
    ...
    private function run_fare_quote($data_key, $search_access_key,$safe_search_data) {
        ...
        $this->generate_static_response(($api_request['data']['request']),'getflight request',$search_id);
        ...
        if ($api_request['status']) {
            ...
            $this->generate_static_response(json_encode($api_response),'getflight response',$search_id);
            ...
            if ($this->valid_api_response($api_response)) {
                $fare_quote_res = $this->aerocrs_to_fare_quote_tmx_format($api_response,$safe_search_data['data'],$data_key);
                // debug($data_key);exit;
                $createbooking_request = $this->fare_quote_booking_request($data_key, $search_access_key,$safe_search_data);

                $this->generate_static_response(($createbooking_request['data']['request']),'createbooking request',$search_id);
                $createbooking_response = $this->CI->api_interface->post_json_response($createbooking_request['data']['service_url'], $createbooking_request['data']['request'], $header_info);

                 $this->generate_static_response(json_encode($createbooking_response),'createbooking response',$search_id);
                if ($this->valid_api_response($createbooking_response)) {
                    if(valid_array($createbooking_response['aerocrs']['booking']['items']) && isset($createbooking_response['aerocrs']['booking']['bookingid'])){
                        
                        $data_key[0]['Attr']['bookingid'] = $createbooking_response['aerocrs']['booking']['bookingid'];
    ...
    function run_commit_booking($op, $book_id, $token, $passenger, $search_access_key,$flight_data) {
        ...
        if ($api_request['status']) {
            ...
            $this->generate_static_response(($api_request['data']['request']),'confirmBooking request',$search_id);

            $api_response = $this->CI->api_interface->post_json_response($api_request['data']['service_url'], $api_request['data']['request'], $header_info);


            $this->generate_static_response(json_encode($api_response),'confirmBooking response',$search_id);
            ...
            if ($this->valid_api_response($api_response) == true) {
                ...
                $update_transaction_data['pnr'] = $api_response['aerocrs']['pnrref'];
                $update_transaction_data['book_id'] = $api_response['aerocrs']['bookingid'];
                $update_transaction_data['ref_id'] = $api_response['aerocrs']['bookingconfirmation'];
                $update_transaction_data['status'] = 'BOOKING_HOLD';

                $GLOBALS['CI']->custom_db->update_record('flight_booking_itinerary_details', array('airline_pnr' => $api_response['aerocrs']['pnrref']), array('app_reference' => $book_id));
...
//debug($pax_ticket_ids);exit;
    ...
    private function WSPassenger($passenger) {
        ...
        for ($i = 0; $i < $total_pax_count; $i++) {
            ...
            if($passenger['passenger_type'][$i]==1){
                if($passenger['name_title'][$i]=="Mr" || $passenger['name_title'][$i]=="Mstr" ){
                    $title = 'Mr.';
                }else if($passenger['name_title'][$i]=="Mrs"){
                    $title = 'Mrs.';
                }else if($passenger['name_title'][$i]=="Miss" || $passenger['name_title'][$i]=="Ms" ){
                    $title = 'Miss.';
                }
                if($inf_flag==1){
                    if(isset($infant_count[$i])){
                        $tmp_passenger[$i]['paxcarringinfant'] = 1;
                    }
                }
                $from = new DateTime(date('Y/m/d', strtotime($passenger['date_of_birth'][$i])));
                $to   = new DateTime('today');
                $age = (string)$from->diff($to)->y;
            ...
            $tmp_passenger[$i]['paxtitle'] = $title;
            ...
            $tmp_passenger[$i]['paxage'] = $age;
            
            $tmp_passenger[$i]['paxnationailty'] = $passenger['passenger_nationality'][$i];
            $tmp_passenger[$i]['paxdoctype'] = 'PP';

        
            if (empty($passenger['passport_number'][$i]) == false and empty($passenger['passport_expiry_date'][$i]) == false) {
                $tmp_passenger[$i]['paxdocnumber'] = $passenger['passport_number'][$i];
                $tmp_passenger[$i]['paxdocexpiry'] = date('Y/m/d', strtotime($passenger['passport_expiry_date'][$i]));
            } else {
                $tmp_passenger[$i]['paxdocnumber'] = '';
                $tmp_passenger[$i]['paxdocexpiry'] = null;
            }

            $tmp_passenger[$i]['paxdocissuer'] = $passenger['passenger_nationality'][$i];
            $tmp_passenger[$i]['paxbirthdate'] = date('Y/m/d', strtotime($passenger['date_of_birth'][$i]));

            //$tmp_passenger[$i]['CountryName'] = $passenger['billing_country_name'];
            $tmp_passenger[$i]['paxphone'] = $passenger['passenger_contact'];
            ...
            $tmp_passenger[$i]['paxemail'] = $passenger['billing_email'];
    ...
    function valid_api_response($api_response) {
        if (empty($api_response) == false && valid_array($api_response['aerocrs']) == true and isset($api_response['aerocrs']['success']) == true and $api_response['aerocrs']['success'] == SUCCESS_STATUS) {
            return true;
        } else {
            return false;
        }
            ...
            if ($clean_search_details['status'] == true) {
                ...
                switch ($clean_search_details['data']['trip_type']) {
                    case 'oneway':
                        ...
                        // if(strlen($clean_search_details['data']['from'])>=6){
                        //     debug($clean_search_details['data']['from']);exit;
                        //     $response['data']['from'] = substr(chop(substr($clean_search_details['data']['from'], -6), ')'), -4);
                        //     $response['data']['to'] = substr(chop(substr($clean_search_details['data']['to'], -6), ')'), -4);
                        // }else{
                        ...
                        $airport_code_from = chop(substr($clean_search_details['data']['from'], -5), ')');
                        $airport_code_to = chop(substr($clean_search_details['data']['to'], -5), ')');
                        
                        $response['data']['from'] = rtrim(ltrim($airport_code_from,'('),')');
                        $response['data']['to'] = rtrim(ltrim($airport_code_to,'('),')');
                        
                        // if(strlen($airport_code_from)<5){
                        //     $response['data']['from'] = substr(chop(substr($airport_code_from, -5), ')'), -3);
                        // }else{
                        //     $response['data']['from'] = substr(chop(substr($airport_code_from, -5), ')'), -4);
                        // }
                        
                        // if(strlen($airport_code_to)<5){
                        //     $response['data']['to'] = substr(chop(substr($airport_code_to, -5), ')'), -3);
                        // }else{
                        //     $response['data']['to'] = substr(chop(substr($airport_code_to, -5), ')'), -4);
                        // }
     ...
     * return booking form
     */
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '', $promotional_plan_type = '', $booking_source = PROVAB_AEROCRS_BOOKING_SOURCE) {
        $booking_form = '';
    ...
    public function extract_flight_segment_fare_details($flights, $currency_obj, $search_id, $module, $ins_token = false, & $token = array(), $flight_index = 0) {
        ...
        foreach ($flights as $list_k => $list_v) {
          ...
          // debug($list_v);exit;
            ...
            $flight_list[$list_k]['booking_source'] = PROVAB_AEROCRS_BOOKING_SOURCE;
