        ...
        if ($search_data['status'] == true) {
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                if ($search_request['status']) {
                    ...
                    // $this->CI->custom_db->generate_static_response(json_encode($search_response));
                    // $search_response = $this->CI->flight_model->get_static_response(1803);//336=>oneway;=>domestic roundway;=> Multicity
...
//debug($pax_ticket_ids);exit;
    ...
    function b2b_price_details($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $admin_markup_gst = 0;
        // debug($total_price['_OrgAdminMarkup']);
        // exit;
        if($total_price['_OrgAdminMarkup'] > 0 ){
            ...
            if($gst_details['status'] == true){
                if($gst_details['data'][0]['gst'] > 0){
                    $admin_markup_gst = ($total_price['_OrgAdminMarkup']/100) * $gst_details['data'][0]['gst'];
                    $admin_markup_gst  = roundoff_number($admin_markup_gst);
                }
            }
        }
        ...
        $agent_markup_gst = 0;
        if($total_price['_AgentMarkup'] > 0 ){
             // $total_markup = $total_price['_OrgAdminMarkup'];
            $gst_details = $GLOBALS['CI']->custom_db->single_table_records('gst_master', '*', array('module' => 'flight'));
            if($gst_details['status'] == true){
                if($gst_details['data'][0]['gst'] > 0){
                    $agent_markup_gst = ($total_price['_AgentMarkup']/100) * $gst_details['data'][0]['gst'];
                    $agent_markup_gst  = roundoff_number($agent_markup_gst);
                }
            }
        }
        $gst_value=$agent_markup_gst;
        $total_price['_AgentBuying'] = $admin_price_details['OfferedFare']+$agent_markup_gst+$admin_markup_gst;
        ...
        $total_price['_AgentEarning'] = $total_price['_Commission'] + $total_price['_Markup']+ $agent_markup_gst - $total_price['_tdsCommission'];
        $total_price['_TaxSum'] = $agent_price_details['PublishedFare'] - $agent_price_details['BaseFare'] + $agent_markup_gst+$admin_markup_gst;
        $total_price['_CustomerBuying'] =  $total_price['_CustomerBuying'] + $agent_markup_gst+$admin_markup_gst;
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c') {
        ...
        foreach ($token as $token_index => $token_value) {
            ...
            if($admin_markup > 0 ){
                ...
                if($gst_details['status'] == true){
                    if($gst_details['data'][0]['gst'] > 0){
                        $gst_value_admin_markup = ($admin_markup/100) * $gst_details['data'][0]['gst'];
                    }
                }
            }
            if($agent_markup > 0 ){
           ...
           $gst_value_con=0;
            /*$convinence = $currency_obj->convenience_fees($trans_total_fare+$final_booking_price_details['admin_markup']+$gst_value_admin_markup+$extra_services_total_price, $master_search_id);
            $convinence_row = $currency_obj->get_convenience_fees();
            $convinence_value = $convinence_row['value'];
            
            debug($extra_services_total_price);exit;
            ...
            {
                $transaction_insert_id = $GLOBALS['CI']->flight_model->save_flight_booking_transaction_details(
                    $app_reference, $transaction_status, $transaction_description, $pnr, $book_id, $source, $ref_id, json_encode($tranaction_attributes), $sequence_number, $currency, $commissionable_fare, $admin_markup+$gst_value_admin_markup, $agent_markup, $admin_commission, $agent_commission, $getbooking_StatusCode, $getbooking_Description, $getbooking_Category, $admin_tds, $agent_tds,$gst_value_agent_markup
                );
            }
        ...
        if ($module == 'b2c') {
            ...
            // debug($book_transaction_id);exit;
