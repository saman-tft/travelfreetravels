 ...
 * @version    V1
 */
class Plazmaxml extends Common_Api_Grind
{
    ...
    private function set_api_credentials()
    {
        ...
        $this->UserName = PLAZMAUSERID;
        $this->Password = PLAZMAPASSWORD;
        $this->strClientIP = PLAZMAUSERID;
        $this->strAgencyId = PLAZMAAGENCY;
        //$this->Url =  $this->CI->flight_url;
        $this->ClientId = $this->CI->domain_key;

        $this->Url = PLAZMAURL;
    }
    //changes added new function for agent price details 
    ...
    function credentials($service)
    {
        switch ($service) {
            case 'SectorCode':
                $this->service_url = $this->Url . 'SectorCode';
                break;
            case 'CheckBalance':
                $this->service_url = $this->Url . 'CheckBalance';
                break;
            case 'FlightAvailability':
                $this->service_url = $this->Url . 'FlightAvailability';
                break;
            case 'Reservation':
                $this->service_url = $this->Url . 'Reservation';
                break;
            case 'IssueTicket':
                $this->service_url = $this->Url . 'IssueTicket';
                break;
            case 'GetItinerary':
                $this->service_url = $this->Url . 'GetItinerary';
                break;
            case 'GetFlightDetail':
                $this->service_url = $this->Url . 'GetFlightDetail';
                break;
            case 'GetPnrDetail':
                $this->service_url = $this->Url . 'GetPnrDetail';
                break;
            case 'SalesReport':
                $this->service_url = $this->Url . 'SalesReport';
                break;
        }
    }
    ...
    private function get_header($search_id = 0)
    {
        ...
        $response[PLAZMAPASSWORD] = $this->Password;
    ...
    function flight_search_request($search_params)
    {
        ...
        $this->credentials('FlightAvailability');
        $request_params = array();
        //Converting to an array


        $nationality = $GLOBALS['CI']->custom_db->single_table_records('api_country_list', '*', array('name' => $search_params['country']))['data'][0]['iso_country_code'];
        if ($search_params['country'] == '') {
            // $name =$GLOBALS['CI']->session->userdata('oldsearchid');
            //debug($name);

            $safe_search_data1 = $GLOBALS['CI']->flight_model->get_safe_search_data($name);
            // debug($safe_search_data1['data']['country']);die;
            $nationality1 = $GLOBALS['CI']->custom_db->single_table_records('api_country_list', '*', array('name' => $safe_search_data1['data']['country']));
            $nationality = $nationality1['data'][0]['isocode'];
        } else {
            $nationality = $GLOBALS['CI']->custom_db->single_table_records('api_country_list', '*', array('name' => $search_params['country']))['data'][0]['iso_country_code'];
        }
        ...
        if ($search_params['trip_type_label'] != "Round") {
            $traveltype = 'O';
            $returndate = '';
        } else {
            $traveltype = 'R';
            $returndate = date('d-m-Y', strtotime($request_params['DepartureDate2']));
        }

        $xmlrequest = " <soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/' xmlns:book='http://booking.us.org/'>
                    <soapenv:Header/>
                        <soapenv:Body>
                        <book:FlightAvailability>
                                <strUserId>" . $this->strClientIP . "</strUserId>
                                <strPassword>" . $this->Password . "</strPassword>
                                <strAgencyId>" . $this->strAgencyId . "</strAgencyId>
                                <strSectorFrom>" . $request_params['from1'] . "</strSectorFrom>
                                <strSectorTo>" . $request_params['to1'] . "</strSectorTo>
                                <strFlightDate>" . date('d-m-Y', strtotime($request_params['DepartureDate1'])) . "</strFlightDate>
                                <strReturnDate>" . $returndate . "</strReturnDate>
                                <strTripType>" . $traveltype . "</strTripType>
                                <strNationality>" . $nationality . "</strNationality>
                                <intAdult>" . $request_params['Adults'] . "</intAdult>
                                <intChild>" . $request_params['children'] . "</intChild>
                                <strClientIP>" . $this->strClientIP . "</strClientIP>
                          </book:FlightAvailability>
                         </soapenv:Body>
                            </soapenv:Envelope> ";
        ...
        $response['data']['xmlrequest'] = $xmlrequest;
        $response['data']['service_url'] = $url;
        //$GLOBALS['CI']->session->unset_userdata('oldsearchid');
        return $response;
    }
     ...
     */
    private function commit_booking_request($booking_params, $app_reference, $flight_data, $flightid = '', $returnflightid = '')
    {
        $response['status'] = SUCCESS_STATUS;
        $response['data'] = array();
        $request_params = array();
        $this->credentials('IssueTicket');
        $passengerdata = '';
        //     DEBUG($booking_params);DIE;
        for ($i = 0; $i < count($booking_params['Passenger']); $i++) {
            if ($booking_params['Passenger'][$i]['Gender'] == true) {
                $gender = 'M';
            } else {
                $gender = 'F';
            }

            if ($booking_params['Passenger'][$i]['TypeCode'] == 'ADT') {
                $paxtype = 'ADULT';
            }
            if ($booking_params['Passenger'][$i]['TypeCode'] == 'CHD') {
                $paxtype = 'CHILD';
            }
            if ($booking_params['Passenger'][$i]['TypeCode'] == 'INF') {
                $paxtype = 'INFANT';
            }
            if ($paxtype != 'INFANT') {
                $passengerdata .= " <Passenger>
        <PaxType>" . $paxtype . "</PaxType>
        <Title>" . $booking_params['Passenger'][$i]['Title'] . "</Title>
        <Gender>" . $gender . "</Gender>
        <FirstName>" . $booking_params['Passenger'][$i]['FirstName'] . "</FirstName>
        <LastName>" . $booking_params['Passenger'][$i]['LastName'] . "</LastName>
        <Nationality>" . $booking_params['Passenger'][$i]['CountryCode'] . "</Nationality>
        <PaxRemarks>XYZ</PaxRemarks>
        </Passenger>
                     ";
            }
        }
        $commitrequest = "
            ...
            xmlns:book='http://booking.us.org/'>
             ...
             <strContactName>" . $booking_params['Passenger'][0]['FirstName'] . "</strContactName>
             <strContactEmail>" . $booking_params['Passenger'][0]['Email'] . "</strContactEmail>
             <strContactMobile>" . $booking_params['Passenger'][0]['Phone'] . "</strContactMobile>
             <strPassengerDetail>
                
                  <![CDATA[<?xml version='1.0'?>   <PassengerDetail> " . $passengerdata . "
                  </PassengerDetail>]]>
              </strPassengerDetail>
             </book:IssueTicket>
             </soapenv:Body>
        ...
        $response['data']['request'] = $commitrequest;
    ...
    function cancel_booking_request($cancell_request_params, $app_reference)
    {
        ...
        $cancelxml = "<soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/' xmlns:book='http://booking.us.org/'>
              <soapenv:Header/>
            <soapenv:Body>
             <book:GetPnrDetail>
             <strUserId>" . $this->strClientIP . "</strUserId>
             <strPassword>" . $this->Password . "</strPassword>
             <strAgencyId>" . $this->strAgencyId . "</strAgencyId>
             <strPnrNo>" . $cancell_request_params['PNR'] . "</strPnrNo>
             <strLastName>" . $cancell_request_params['SequenceNumber'] . "</strLastName>
             </book:GetPnrDetail>
             </soapenv:Body>
            </soapenv:Envelope>";
        $response['data']['request'] = $cancelxml;
        $response['data']['service_url'] = $this->service_url;
        return $response;
    }
     ...
     */
    //changes added new param module for b2b search
    function get_flight_list($search_id = '', $module = '')
    {
        ...
        if ($search_data['status'] == true) {
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                //get data
                if ($search_request['status']) {
                    $search_response = $this->flight_processRequest($search_request['data']['xmlrequest'], $search_request['data']['service_url']);

                    $search_response_format = Converter::createArray($search_response);
                    //        debug($search_response_format);
                    $search_response_array = Converter::createArray($search_response_format['S:Envelope']['S:Body']['ns2:FlightAvailabilityResponse']['return']);
                    //     debug($search_response_array);die;
                    ...
                    /*if($_SERVER['REMOTE_ADDR'] == "106.207.126.104")
                   {
                    ...
                    if (valid_array($search_response_array)) {
                        //debug($search_response_array);die
                        //   if($_SERVER['REMOTE_ADDR']=="223.187.120.51")
                        //   {
                        //changes added module for b2b search
                        $search_response = $this->tripxml_to_tmx_formatversion2($search_response_array, $search_data['data'], $module);
                        // }
                        //else
                        // {
                        // $search_response = $this->tripxml_to_tmx_format($search_response_array,$search_data['data']);
                        //}
    ...
    {
        $flights = $search_response['Flightavailability']['Outbound']['Availability'];
        //echo json_encode( $search_response );die;
        $JourneyList = array();
        ...
        foreach ($flights as $f_k => $f_v) {
            // debug($f_v);die;
            $currency_obj = new Currency(array('module_type' => 'flight', 'from' => $f_v['Currency'], 'to' => 'NPR'));
            //  debug($currency_obj);die;
            $FlightDetails = array();


            if ($f_v['Airline'] == "U4") {
                $airlinename = 'Buddha Air'; //removed . $f_v['Airline']
            }
            if ($f_v['Airline'] == "S1") {
...
Airlines';
            ...
            if ($f_v['Airline'] == "YT") //removed this cond && $f_v['Airline']==""
            {
                $airlinename = 'Yeti Airlines';
            }
            if ($f_v['Airline'] == "GA") {
                $airlinename = 'Goma Airlines';
            }
            if ($f_v['Airline'] == "SHA") {
                $airlinename = 'Shree
Airlines';
            ...
            if ($f_v['Airline'] == "ST") {
                $airlinename = 'Sita Air';
            }
            $SegmentDetails = array();

            $SegmentDetails['Origin']['AirportCode'] = $search_data['from_loc'];
            $origin_airport_dtl = $f_v['Departure'];
            $SegmentDetails['Origin']['CityName'] = $f_v['Departure'];
            $SegmentDetails['Origin']['AirportName'] = $f_v['Departure'];
            $SegmentDetails['Origin']['DateTime'] = date("Y-m-d", strtotime($f_v['FlightDate'])) . ' ' . $f_v['DepartureTime'];
            $SegmentDetails['Origin']['FDTV'] = $f_v['Departure'];

            $SegmentDetails['Destination']['AirportCode'] = $search_data['to_loc'];
            $destination_airport_dtl = $f_v['Arrival'];

            $SegmentDetails['Destination']['CityName'] = $f_v['Arrival'];
            $SegmentDetails['Destination']['AirportName'] = $f_v['Arrival'];
            $SegmentDetails['Destination']['DateTime'] = date("Y-m-d", strtotime($f_v['FlightDate'])) . ' ' . $f_v['ArrivalTime'];
            $SegmentDetails['Destination']['FDTV'] = $f_v['Arrival'];


            $SegmentDetails['Duration'] = '';
            $SegmentDetails['OperatorCode'] = $f_v['Airline'];
            $SegmentDetails['airline_logo'] = $f_v['AirlineLogo'];
            $SegmentDetails['OperatorName'] = $airlinename;
            $SegmentDetails['FlightNumber'] = $f_v['FlightNo'];
            $SegmentDetails['Operatedbyairline'] = $f_v['Airline'];
            $SegmentDetails['Operatedbyairlinename'] = $f_v['Airline'];
            if ($f_v['BookingClass'] == "F") {
                $s_v['BookingClass'] = "First class";
            }
            if ($f_v['BookingClass'] == "J") {
                $s_v['BookingClass'] = "Business class";
            }
            if ($f_v['BookingClass'] == "W") {
                $s_v['BookingClass'] = "Premium economy";
            }
            if ($f_v['BookingClass'] == "Y") {
                $s_v['BookingClass'] = "Economy class";
            }

            $SegmentDetails['CabinClass'] = '';


            $SegmentDetails['Attr']['Baggage'] = $f_v['FreeBaggage'];
            ...
            $FlightDetails['Details'][0][0] = $SegmentDetails;

            if ($search_data['trip_type'] == "circle") {
                $SegmentDetails1 = array();
                //  debug($search_response['Flightavailability']['Inbound']['Availability']);
                $inboundair = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Airline'];
                if ($inboundair == "U4") {
                    $airlinenamein = 'Buddha Air-' . $f_v['Airline'];
                }
                if ($inboundair == "S1") {
                    $airlinenamein = 'Saurya Airlines';
                }
                if ($inboundair == "RMK") {
                    $airlinenamein = 'Simrik
Airlines';
                }
                if ($inboundair == "YT" && $inboundair == "") {
                    $airlinenamein = 'Yeti Airlines';
                }
                if ($inboundair == "GA") {
                    $airlinenamein = 'Goma Airlines';
                }
                if ($inboundair == "SHA") {
                    $airlinenamein = 'Shree
Airlines';
                ...
                if ($inboundair == "ST") {
                    $airlinenamein = 'Sita Air';
                }


                $SegmentDetails1['Origin']['AirportCode'] = $search_data['from_loc'];
                $origin_airport_dtl = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Departure'];
                $SegmentDetails1['Origin']['CityName'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Departure'];
                $SegmentDetails1['Origin']['AirportName'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Departure'];
                $SegmentDetails1['Origin']['DateTime'] = date("Y-m-d", strtotime($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['FlightDate'])) . ' ' . $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['DepartureTime'];
                $SegmentDetails1['Origin']['FDTV'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Departure'];

                $SegmentDetails1['Destination']['AirportCode'] = $search_data['to_loc'];
                $destination_airport_dtl = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Arrival'];

                $SegmentDetails1['Destination']['CityName'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Arrival'];
                $SegmentDetails1['Destination']['AirportName'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Arrival'];
                $SegmentDetails1['Destination']['DateTime'] = date("Y-m-d", strtotime($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['FlightDate'])) . ' ' . $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['ArrivalTime'];
                $SegmentDetails1['Destination']['FDTV'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Arrival'];


                $SegmentDetails1['Duration'] = '';
                $SegmentDetails1['OperatorCode'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Airline'];
                $SegmentDetails1['airline_logo'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['AirlineLogo'];
                $SegmentDetails1['OperatorName'] = $airlinenamein;
                $SegmentDetails1['FlightNumber'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['FlightNo'];
                $SegmentDetails1['Operatedbyairline'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Airline'];
                $SegmentDetails1['Operatedbyairlinename'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Airline'];
                if ($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['BookingClass'] == "F") {
                    $s_v['BookingClass'] = "First class";
                }
                if ($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['BookingClass'] == "J") {
                    $s_v['BookingClass'] = "Business class";
                }
                if ($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['BookingClass'] == "W") {
                    $s_v['BookingClass'] = "Premium economy";
                }
                if ($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['BookingClass'] == "Y") {
                    $s_v['BookingClass'] = "Economy class";
                }

                $SegmentDetails1['CabinClass'] = '';


                $SegmentDetails1['Attr']['Baggage'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['FreeBaggage'];
                $SegmentDetails1['Attr']['CabinBaggage'] = '';


                $SegmentDetails1['Attr']['AvailableSeats'] = $s_v['SeatsLeft'];
                //  debug( $SegmentDetails1);die;

                $FlightDetails['Details'][1][0] = $SegmentDetails1;
            }
            ...
            if ($search_data['trip_type'] == "circle") {
                ...
                $inadult = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare']));
                $inchild = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare']));
                $inInfant = 0;
                $outadult = get_converted_currency_value($currency_obj->force_currency_conversion($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['AdultFare']));
                $outchild = get_converted_currency_value($currency_obj->force_currency_conversion($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['ChildFare']));
                $outInfant = 0;
                $inTax = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                $outTax = get_converted_currency_value($currency_obj->force_currency_conversion($search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Tax']));

                for ($i = 0; $i < count($search_data['adult_config']); $i++) {
                    $PassengerBreakup['ADT']['PassengerCount'] = $search_data['adult_config'];
                    $PassengerBreakup['ADT']['BasePrice'] = ($inadult + $outadult) * $search_data['adult_config'];

                    $PassengerBreakup['ADT']['Tax'] = $inTax + $outTax;
                    $PassengerBreakup['ADT']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice'])) + $inTax + $outTax;
                }
                for ($i = 0; $i < count($search_data['child_config']); $i++) {


                    $PassengerBreakup['CHD']['PassengerCount'] = $search_data['child_config'];
                    $PassengerBreakup['CHD']['BasePrice'] = ($inchild + $outchild) * $search_data['child_config'];

                    $PassengerBreakup['CHD']['Tax'] = $inTax + $outTax;
                    $PassengerBreakup['CHD']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice'])) + $inTax + $outTax;
                    /*
                       $PassengerBreakup['CHD']['PassengerCount'] = $search_data['child_config'];
                       $PassengerBreakup['CHD']['BasePrice'] =get_converted_currency_value($currency_obj->force_currency_conversion(($f_v['ChildFare']+$search_response['Flightavailability']['Inbound']['Availability'][$inbound]['ChildFare'])*$search_data['child_config']));
                       $PassengerBreakup['CHD']['Tax'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']+$search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Tax']));
                       $PassengerBreakup['CHD']['TotalPrice'] =get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice']+$f_v['Tax']+$search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Tax']));

                       */
                }
                for ($i = 0; $i < count($search_data['infant_config']); $i++) {

                    $PassengerBreakup['INF']['PassengerCount'] = $search_data['infant_config'];
                    $PassengerBreakup['INF']['BasePrice'] = ($inInfant + $outInfant) * $search_data['infant_config'];

                    $PassengerBreakup['INF']['Tax'] = $inTax + $outTax;
                    $PassengerBreakup['INF']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice'])) + $inTax + $outTax;

                    /*

                                                                     $PassengerBreakup['INF']['PassengerCount'] = $search_data['infant_config'];
                                                                     $PassengerBreakup['INF']['BasePrice'] =get_converted_currency_value($currency_obj->force_currency_conversion(($f_v['InfantFare']+$search_response['Flightavailability']['Inbound']['Availability'][$inbound]['InfantFare'])*$search_data['infant_config']));
                                                                     $PassengerBreakup['INF']['Tax'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']+$search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Tax']));
                                                                     $PassengerBreakup['INF']['TotalPrice'] =get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice']+$f_v['Tax']+$search_response['Flightavailability']['Inbound']['Availability'][$inbound]['Tax']));
                ...
                $price['TotalDisplayFare'] = (($inadult + $outadult) * $search_data['adult_config'] + ($inchild + $outchild) * $search_data['child_config'] + ($inInfant + $outInfant) * $search_data['infant_config']) + ($inTax + $outTax);



                $price['PriceBreakup']['Tax'] = $inTax + $outTax;


                $price['PriceBreakup']['BasicFare'] = ($inadult + $outadult) * $search_data['adult_config'] + ($inchild + $outchild) * $search_data['child_config'] + ($inInfant + $outInfant) * $search_data['infant_config'];

                $price['PriceBreakup']['YQValue'] = 0;

                $price['PriceBreakup']['CommissionEarned'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'] + $f_v['ChildCommission'] + $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['AgencyCommission'] + $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['ChildCommission']));
                $price['PriceBreakup']['PLBEarned'] = 0;
                $price['PriceBreakup']['TdsOnCommission'] = 0;
                $price['PriceBreakup']['TdsOnPLB'] = 0;

                $price['PriceBreakup']['AgentCommission'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'] + $f_v['ChildCommission'] + $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['AgencyCommission'] + $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['ChildCommission']));
                $price['PriceBreakup']['AgentTdsOnCommision'] = 0;
                //       debug($price);die;
            } else {
                for ($i = 0; $i < count($search_data['adult_config']); $i++) {
                    $PassengerBreakup['ADT']['PassengerCount'] = $search_data['adult_config'];
                    $PassengerBreakup['ADT']['BasePrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config'];
                    $PassengerBreakup['ADT']['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                    $PassengerBreakup['ADT']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice'])) + $f_v['Tax'];
                }
                for ($i = 0; $i < count($search_data['child_config']); $i++) {
                    $PassengerBreakup['CHD']['PassengerCount'] = $search_data['child_config'];
                    $PassengerBreakup['CHD']['BasePrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config'];
                    $PassengerBreakup['CHD']['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                    $PassengerBreakup['CHD']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice'])) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                }
                for ($i = 0; $i < count($search_data['infant_config']); $i++) {
                    $PassengerBreakup['INF']['PassengerCount'] = $search_data['infant_config'];
                    $PassengerBreakup['INF']['BasePrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['InfantFare'])) * $search_data['infant_config'];
                    $PassengerBreakup['INF']['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                    $PassengerBreakup['INF']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['PassengerType']['BasePrice'])) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                }

                $price = array();
                $price['Currency'] = $fare['CurrencyCode'];
                $price['TotalDisplayFare'] = (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['InfantFare'])) * $search_data['infant_config']) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                $price['PriceBreakup']['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']));
                $price['PriceBreakup']['BasicFare'] = (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['InfantFare'])) * $search_data['infant_config']);
                $price['PriceBreakup']['YQValue'] = 0;
                $price['PriceBreakup']['CommissionEarned'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'])) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildCommission']));
                $price['PriceBreakup']['PLBEarned'] = 0;
                $price['PriceBreakup']['TdsOnCommission'] = 0;
                $price['PriceBreakup']['TdsOnPLB'] = 0;
                $price['PriceBreakup']['AgentCommission'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'])) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildCommission']));
                $price['PriceBreakup']['AgentTdsOnCommision'] = 0;
            }
            $price['PassengerBreakup'] = $PassengerBreakup;

            $price['faretoken'] = serialized_data($PassengerBreakup);
            ...
            $FlightList[$f_k]['Attr']['IsRefundable'] = $f;

            $FlightList[$f_k]['Attr']['IsLCC'] = '';
            $FlightList[$f_k]['Attr']['AirlineRemark'] = '';
            $FlightList[$f_k]['Attr']['logo'] = $f_v['AirlineLogo'];
            $FlightList[$f_k]['Attr']['FareType'] = $fare['FareType'];
            $FlightList[$f_k]['Attr']['Id'] = $f_v['Id'];
            $FlightList[$f_k]['Attr']['SessionId'] = $SessionId;
            $FlightList[$f_k]['Attr']['FlightId'] = $f_v['FlightId'];
            $FlightList[$f_k]['Attr']['returnFlightId'] = $search_response['Flightavailability']['Inbound']['Availability'][$inbound]['FlightId'];
            $FlightList[$f_k]['ResultToken'] = serialized_data($FlightList[$f_k]);
    ...
    //changes added module here as well for b2b search
    function tripxml_to_tmx_formatversion2($search_response, $search_data,  $module = '')
    {
        ...
        $flights = $search_response['Flightavailability']['Outbound']['Availability'];
        $flightsin = $search_response['Flightavailability']['Inbound']['Availability'];
        $JourneyList = array();
        $FinalResponse = array();
        $FlightList = array();
        $FlightListin = array();
        ...
        Airlines';
                ...
                /* removed this for loop for child fare commission breakup and added the for loop below this one
                     for($i=0;$i<count($search_data['adult_config']);$i++)
                     {
                         if($_SERVER["REMOTE_ADDR"]=="14.97.94.42")
                       {
                       //echo $test=  get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission']))*$search_data['adult_config'];
                         }
                         //in response  only subtranscting commission --
             $test=  get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission']))*$search_data['adult_config'];
                      $PassengerBreakup['ADT']['PassengerCount'] = $search_data['adult_config'];
                      $PassengerBreakup['ADT']['BasePrice'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare']))* $search_data['adult_config']+(get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge']))* $search_data['adult_config']);
                      $PassengerBreakup['ADT']['Tax'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']))*$search_data['adult_config'];
                         $PassengerBreakup['ADT']['BasePrice']=$PassengerBreakup['ADT']['BasePrice']-$test;
                      $PassengerBreakup['ADT']['TotalPrice'] =get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['ADT']['BasePrice']))+(get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']))** $search_data['adult_config']);
                      }  
                      for($i=0;$i<count($search_data['child_config']);$i++)
                     {
                      $PassengerBreakup['CHD']['PassengerCount'] = $search_data['child_config'];
                      $PassengerBreakup['CHD']['BasePrice'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare']))*$search_data['child_config']+(get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge']))* $search_data['child_config']);
                      $PassengerBreakup['CHD']['Tax'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']))*$search_data['child_config'];
                      $PassengerBreakup['CHD']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['CHD']['BasePrice']))+(get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']))*$search_data['child_config']);
                      } 

                      $search_data['infant_config']=0;
                          for($i=0;$i<count($search_data['infant_config']);$i++)
                     {
                      $PassengerBreakup['INF']['PassengerCount'] = $search_data['infant_config'];
                      $PassengerBreakup['INF']['BasePrice'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['InfantFare']))*$search_data['infant_config']+get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge']))*$search_data['infant_config'];
                      $PassengerBreakup['INF']['Tax'] =get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']))*$search_data['infant_config'];
                      $PassengerBreakup['INF']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['INF']['BasePrice']))+get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax']))*$search_data['infant_config'];
                      }  
     */
                for ($i = 0; $i < count($search_data['adult_config']); $i++) {
                    $PassengerBreakup['ADT']['PassengerCount'] = $search_data['adult_config'];
                    if ($module != 'b2b') {
                        $PassengerBreakup['ADT']['BasePrice'] = (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config'] + (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge'])) * $search_data['adult_config'])) - (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'])) * $search_data['adult_config']);
                    } else {
                        $PassengerBreakup['ADT']['BasePrice'] = (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config'] + (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge'])) * $search_data['adult_config']));
                    }
                    $PassengerBreakup['ADT']['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax'])) * $search_data['adult_config'];
                ...
                for ($i = 0; $i < count($search_data['child_config']); $i++) {
                    $PassengerBreakup['CHD']['PassengerCount'] = $search_data['child_config'];
                    if ($module != 'b2b') {
                        $PassengerBreakup['CHD']['BasePrice'] = (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config'] + (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge'])) * $search_data['child_config'])) - (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildCommission'])) * $search_data['child_config']);
                        // echo $f_v['ChildCommission']; die;
                    } else {
                        $PassengerBreakup['CHD']['BasePrice'] = (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config'] + (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge'])) * $search_data['child_config']));
                    }
                    // debug($PassengerBreakup);die();
                    $PassengerBreakup['CHD']['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax'])) * $search_data['child_config'];
                    $PassengerBreakup['CHD']['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerBreakup['CHD']['BasePrice'])) + (get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax'])) * $search_data['child_config']);
                }
                $search_data['infant_config'] = 0;
                ...
                $price['TotalDisplayFare'] = (((get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config']) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge'])) * $search_data['adult_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax'])) * $search_data['adult_config']) + ((get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config']) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax'])) * $search_data['child_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge']))) * $search_data['child_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['InfantFare'])) * $search_data['infant_config']);
                $passengertaxcount = $search_data['child_config'] + $search_data['adult_config'] + $search_data['infant_config'];
                $price['PriceBreakup']['Tax'] =
                    get_converted_currency_value($currency_obj->force_currency_conversion($f_v['Tax'])) * $passengertaxcount;

                $price['PriceBreakup']['BasicFare'] = (((get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AdultFare'])) * $search_data['adult_config']) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge'])) * $search_data['adult_config']) + ((get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildFare'])) * $search_data['child_config']) + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['FuelSurcharge']))) * $search_data['child_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['InfantFare'])) * $search_data['infant_config']);
                $price['PriceBreakup']['YQValue'] = 0;
                $price['PriceBreakup']['CommissionEarned'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'])) * $search_data['adult_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildCommission'])) * $search_data['child_config'];
                $price['PriceBreakup']['PLBEarned'] = 0;
                $price['PriceBreakup']['TdsOnCommission'] = 0;
                $price['PriceBreakup']['TdsOnPLB'] = 0;
                $price['PriceBreakup']['AgentCommission'] = get_converted_currency_value($currency_obj->force_currency_conversion($f_v['AgencyCommission'])) * $search_data['adult_config'] + get_converted_currency_value($currency_obj->force_currency_conversion($f_v['ChildCommission'])) * $search_data['child_config'];
                $price['PriceBreakup']['AgentTdsOnCommision'] = 0;
                ...
                $FlightList[$f_k]['Attr']['SessionId'] = $SessionId;
                $FlightList[$f_k]['Attr']['FlightId'] = $f_v['FlightId'];
                $FlightList[$f_k]['Attr']['outFlightId'] = $f_v['FlightId'];
                // debug($FlightList[$f_k]);die;
        ...
        if (count($flightsin) > 0) {
            foreach ($flightsin as $f_k => $f_v) {
                $currency_obj = new Currency(array('module_type' => 'flight', 'from' => $f_v['Currency'], 'to' => 'NPR'));
                $FlightDetailsin = array();
                /*  if($f_v['Airline']=="U4" || $f_v['Airline']=="GUA")
                  {
                      $airlinename='Buddha Air - '.$f_v['Airline'];
                  }*/
                if ($f_v['Airline'] == "U4") {
                    $airlinename = 'Buddha Air'; // removed . $f_v['Airline']
                }
        ...
        Airlines';
                ...
                $SegmentDetails['Origin']['AirportCode'] = $search_data['to_loc'];
                $origin_airport_dtl = $f_v['Departure'];
                $SegmentDetails['Origin']['CityName'] = $f_v['Departure'];
                $SegmentDetails['Origin']['AirportName'] = $f_v['Departure'];
                $SegmentDetails['Origin']['DateTime'] = date("Y-m-d", strtotime($f_v['FlightDate'])) . ' ' . $f_v['DepartureTime'];
                ...
                $SegmentDetails['Destination']['AirportCode'] = $search_data['from_loc'];
                $destination_airport_dtl = $f_v['Arrival'];
                ...
                $FlightDetailsin['Details'][0][0] = $SegmentDetails;

                //  debug($SegmentDetails);die;
                ...
                $FlightListin[$f_k]['FlightDetails'] = $FlightDetailsin;
                $FlightListin[$f_k]['Price'] = $price;
                if ($f_v['Refundable'] == 'F') {
                    $f = 0;
                } else {
                    $f = 1;
                ...
                $FlightListin[$f_k]['Attr']['IsRefundable'] = $f;

                $FlightListin[$f_k]['Attr']['IsLCC'] = '';
                $FlightListin[$f_k]['Attr']['AirlineRemark'] = '';
                $FlightListin[$f_k]['Attr']['logo'] = $f_v['AirlineLogo'];
                $FlightListin[$f_k]['Attr']['FareType'] = $fare['FareType'];
                $FlightListin[$f_k]['Attr']['Id'] = $f_v['Id'];
                $FlightListin[$f_k]['Attr']['SessionId'] = $SessionId;
                $FlightListin[$f_k]['Attr']['FlightId'] = $f_v['FlightId'];
                $FlightListin[$f_k]['Attr']['returnFlightId'] = $f_v['FlightId'];
                $FlightListin[$f_k]['ResultToken'] = serialized_data($FlightList[$f_k]);
        ...
        $FinalResponse['Message'] = '';
        $FinalResponse['Search']['FlightDataList']['JourneyList'][0] = $FlightList;
        $FinalResponse['Search']['FlightDataList']['JourneyList'][1] = $FlightListin;
        // debug($FinalResponse);die;
    ...
    function tripxml_to_fare_quote_tmx_format($search_response, $search_data, &$SegmentDetails)
    {
        ...
        foreach ($flights as $f_k => $f_v) {
            ...
            $SegmentDetails[0]['Price'] = $SegmentDetails['Price'];
            $SegmentDetails[0]['api_original_price_details']['TotalDisplayFare'] = $SegmentDetails['api_original_price_details']['TotalDisplayFare'];
            $SegmentDetails[0]['api_original_price_details']['AgentCommission'] = 0;
            $SegmentDetails[0]['api_original_price_details']['BasicFare'] = $SegmentDetails['api_original_price_details']['BasePrice'];
            $SegmentDetails[0]['api_original_price_details']['Tax'] = $SegmentDetails['api_original_price_details']['Tax'];

            $SegmentDetails[0]['FareDetails']['BaseFare'] = $SegmentDetails['FareDetails']['BasePrice'];
            $SegmentDetails[0]['FareDetails']['Tax'] = $SegmentDetails['FareDetails']['Tax'];
            $SegmentDetails[0]['FareDetails']['PublishedFare'] = $SegmentDetails['FareDetails']['PublishedFare'];
            $SegmentDetails[0]['FareDetails']['OfferedFare'] = $SegmentDetails['FareDetails']['OfferedFare'];

            $SegmentDetails[0]['PassengerFareBreakdown'] = $SegmentDetails['Price']['PassengerBreakup'];
        }
    }
    /**
     ...
     */
    function fare_quote_details($flight_booking_details, $module = '')
    {
        //  echo "fqrequoteintegrationsta";die;
    ...
    private function run_fare_quote($data_key, $search_access_key, $safe_search_data)
    {
        ...
        if ($api_request['status']) {
            ...
            if (true) {
                $fare_quote_res = $this->tripxml_to_fare_quote_tmx_format($api_response, $safe_search_data['data'], $data_key[0]);
    ...
    public function process_booking($book_id, $booking_params)
    {
        ...
        $book_response = array();
        $reservattionxml = "<soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/'   xmlns:book='http://booking.us.org/'>
             <soapenv:Header/>
             <soapenv:Body>
             <book:Reservation>
             
             <strFlightId>" . $booking_params['token']['token'][0]['Attr']['FlightId'] . "</strFlightId>
             <strReturnFlightId >" . $booking_params['token']['token'][1]['Attr']['FlightId'] . "</strReturnFlightId>
             </book:Reservation>
             </soapenv:Body>
            </soapenv:Envelope>";
        //   $search_response_array = Converter::createArray ($search_response_format['S:Envelope']['S:Body']['ns2:FlightAvailabilityResponse']['return']);
        ///debug( $reservattionxml);
        $this->credentials('Reservation');
        $reservationurl = $this->service_url;
        $reservation_response = $this->flight_processRequest($reservattionxml, $reservationurl);

        $reservation_response_format = Converter::createArray($reservation_response);

        $reservation_response_array = Converter::createArray($reservation_response_format['S:Envelope']['S:Body']['ns2:ReservationResponse']['return']);

        //!isset($reservation_response_array['Error']))

        if (TRUE) {
            $book_response = $this->book_flight($book_id, $booking_params, $booking_params['token']['token'][0]['Attr']['FlightId'], $booking_params['token']['token'][1]['Attr']['FlightId']);
            ...
            $response['status'] = FAILURE_STATUS;
            $response['message'] = $reservation_response_array['Error'];
        }
     ...
     */
    function book_flight($book_id, $booking_params, $flightid = '', $returnflightid = '')
    {
        $response['status'] = FAILURE_STATUS;
        ...
        if (count($unique_search_access_key) == 1) { // Single session is one request
            if (count($token_wrapper['search_access_key']) == 1) {
                ...
                if (isset($booking_params['ticket_method']) && $booking_params['ticket_method'] === 'hold_ticket') { //HOLD TICKET
                    ...
                    $tmp_res = $this->run_commit_booking($op, $book_id, $token_wrapper['token'][0], $passenger, $token_wrapper['search_access_key'][0], $booking_params, $flightid, $returnflightid);
                }
            ...
            foreach ($token_wrapper['token'] as $___k => $___v) {
                ...
                $tmp_resp = $this->run_commit_booking($op, $book_id, $___v, $passenger, $token_wrapper['search_access_key'][$___k], $booking_params, $flightid, $returnflightid);
                // debug( $tmp_resp );die;
                if (true) {
     ...
     */
    //new run_commit_booking function
    function run_commit_booking($op, $book_id, $token, $passenger, $search_access_key, $flight_data, $flightid = '', $returnflightid = '')
    {
        $response['data'] = array();
        $response['status'] = FAILURE_STATUS;
        $response['message'] = '';
        ...
        $flight_booking_detail = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_details', '*', array('app_reference' => $book_id))['data'][0];

        if ($flight_booking_detail['trip_type'] == 'circle') {
            $response['data'] = array();
            $response['status'] = FAILURE_STATUS;
            $response['message'] = '';
            $SequenceNumber = $token['SequenceNumber'];
            ...
            $api_request = $this->commit_booking_request($booking_params, $book_id, $flight_data, $flightid, $returnflightid);
            //  debug($api_request);
            ...
            if ($api_request['status']) {
                ...
                $api_response = $this->flight_processRequest($api_request['data']['request'], $api_request['data']['service_url']);
                $commit_response_format = Converter::createArray($api_response);
                $api_response = Converter::createArray($commit_response_format['S:Envelope']['S:Body']['ns2:IssueTicketResponse']['return']);

                $this->generate_static_response(json_encode($api_response), 'Tripxml booking response', $search_id);

                if (valid_array($api_response) == true) {
                    // changes start of change for plazma round trip 1
                    $flight_booking_detail = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_details', '*', array('app_reference' => $book_id))['data'][0];
                    // changes end of change for plazma round trip 1


                    if (isset($api_response['Itinerary']['Passenger'][0]['PnrNo'])) {
                        // changes start adjusting for plazma round trip 2 
                        // $confirmed_passenger_exists = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_passenger_details', '*', array('app_reference' => $book_id))['data'];
                        // for ($i = 0; $i < count($confirmed_passenger_exists); $i++) {
                        $confirmed_passenger_exists = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_passenger_details', '*', array('app_reference' => $book_id, 'passenger_type !=' => 'Infant'))['data'];
                        $flight_booking_itinerary_detail = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_itinerary_details', '*', array('app_reference' => $book_id))['data'];
                        $flight_booking_transaction_detail = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_transaction_details', '*', array('app_reference' => $book_id))['data'];
                        // debug($booking_params);die;
                        if ($flight_booking_detail['trip_type'] == 'circle') {
                            for ($i = 0; $i < count($confirmed_passenger_exists) / 2; $i++) {
                                if ($api_response['Itinerary']['Passenger'][$i]['TicketNo'] != '') {
                                    // changes end adjusting for plazma round trip 2 
                                    $GLOBALS['CI']->custom_db->update_record('flight_booking_passenger_details', array('ticket_no' => $api_response['Itinerary']['Passenger'][$i]['TicketNo']), array('origin' => $confirmed_passenger_exists[$i]['origin']));
                                }
                            }
                            // changes start adjusting for plazma round trip 3

                            // $airpnr = $api_response['Itinerary']['Passenger'][0]['PnrNo'];

                            // $ticket = $api_response['Itinerary']['Passenger'][0]['TicketNo'];
                            $j = 1;
                            for ($i = count($confirmed_passenger_exists) / 2; $i < count($confirmed_passenger_exists); $i++) {
                                if ($api_response['Itinerary']['Passenger'][$i]['TicketNo'] != '') {
                                    $GLOBALS['CI']->custom_db->update_record(
                                        'flight_booking_passenger_details',
                                        array('ticket_no' => $api_response['Itinerary']['Passenger'][count($confirmed_passenger_exists) - $j]['TicketNo']),
                                        array('origin' => $confirmed_passenger_exists[$i]['origin'])
                                    );
                                }
                                $j++;
                            }
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_itinerary_details', array('airline_pnr' => $api_response['Itinerary']['Passenger'][0]['PnrNo']), array('origin' => $flight_booking_itinerary_detail[0]['origin']));
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_itinerary_details', array('airline_pnr' => $api_response['Itinerary']['Passenger'][count($confirmed_passenger_exists) / 2]['PnrNo']), array('origin' => $flight_booking_itinerary_detail[1]['origin']));
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_transaction_details', array('pnr' => $api_response['Itinerary']['Passenger'][0]['PnrNo'], 'book_id' => $book_id, 'ref_id' => $api_response['Itinerary']['Passenger'][0]['PnrNo'], 'status' => 'BOOKING_CONFIRMED'), array('origin' => $flight_booking_transaction_detail[0]['origin']));
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_transaction_details', array('pnr' => $api_response['Itinerary']['Passenger'][count($confirmed_passenger_exists) / 2]['PnrNo'], 'book_id' => $book_id, 'ref_id' => $api_response['Itinerary']['Passenger'][count($confirmed_passenger_exists) / 2]['PnrNo'], 'status' => 'BOOKING_CONFIRMED'), array('origin' => $flight_booking_transaction_detail[1]['origin']));
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_details', array('ticket' => $api_response['Itinerary']['Passenger'][0]['TicketNo']), array('app_reference' => $book_id));
                            // changes end adjusting for plazma round trip 3                
                        } else {
                            // changes start adjusting for plazma round trip 3
                            // $airpnr = $api_response['Itinerary']['Passenger']['PnrNo'];
                            // $ticket = $api_response['Itinerary']['Passenger']['TicketNo'];
                            // $confirmed_passenger_exists = $GLOBALS['CI']->custom_db->single_table_records('flight_booking_passenger_details', '*', array('app_reference' => $book_id))['data'][0];
                            // $GLOBALS['CI']->custom_db->update_record('flight_booking_passenger_details', array('ticket_no' => $ticket), array('origin' => $confirmed_passenger_exists['origin']));
                            // changes end adjusting for plazma round trip 3
                            for ($i = 0; $i < count($confirmed_passenger_exists); $i++) {
                                if ($api_response['Itinerary']['Passenger'][$i]['TicketNo'] != '') {
                                    $GLOBALS['CI']->custom_db->update_record('flight_booking_passenger_details', array('ticket_no' => $api_response['Itinerary']['Passenger'][$i]['TicketNo']), array('origin' => $confirmed_passenger_exists[$i]['origin']));
                            ...
                            // changes start adjusting for plazma round trip 4
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_itinerary_details', array('airline_pnr' => $api_response['Itinerary']['Passenger'][0]['PnrNo']), array('origin' => $flight_booking_itinerary_detail[0]['origin']));
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_transaction_details', array('pnr' => $api_response['Itinerary']['Passenger'][0]['PnrNo'], 'book_id' => $book_id, 'ref_id' => $api_response['Itinerary']['Passenger'][0]['PnrNo'], 'status' => 'BOOKING_CONFIRMED'), array('origin' => $flight_booking_transaction_detail[0]['origin']));
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_details', array('ticket' => $api_response['Itinerary']['Passenger'][0]['TicketNo']), array('app_reference' => $book_id));
                        }
                    }
                    // changes end adjusting for plazma round trip 4
                    // $update_transaction_data['pnr'] = $airpnr;
                    // $update_transaction_data['book_id'] = $book_id;
                    // $update_transaction_data['ref_id'] = $airpnr;
                    // $update_transaction_data['status'] = 'BOOKING_CONFIRMED';
                    // $GLOBALS['CI']->custom_db->update_record('flight_booking_details', array('ticket' => $ticket), array('app_reference' => $book_id));
                    // $GLOBALS['CI']->custom_db->update_record('flight_booking_itinerary_details', array('airline_pnr' => $airpnr), array('app_reference' => $book_id));
                    // changes end adjusting for plazma round trip 4
                    ...
                    if (true) {
                        ...
                        if (valid_array($api_response) == true) {
                            $GLOBALS['CI']->custom_db->update_record('flight_booking_details', array('status' => 'BOOKING_CONFIRMED'), array('app_reference' => $book_id));

                            $GLOBALS['CI']->custom_db->update_record('flight_booking_transaction_details', array('status' => 'BOOKING_CONFIRMED'), array('app_reference' => $book_id));
    ...
    function cancel_booking($master_booking_details, $passenger_origin, $passenger_ticket_id)
    {
        ...
        foreach ($booking_transaction_details as $transaction_details_k => $transaction_details_v) {
            ...
            if (isset($passenger_ticket_id[$transaction_origin]) == true && valid_array($passenger_ticket_id[$transaction_origin]) == true) {
                ...
                //debug($transaction_details_v['booking_customer_details'][0]['last_name']);die;
                ...
                $cancell_request_params['SequenceNumber'] = $transaction_details_v['booking_customer_details'][0]['last_name'];
                ...
                if ($cancel_booking_request['status']) {
                    $header_info = $this->get_header();
                    debug($cancel_booking_request);
                    $send_change_response = $this->flight_processRequest($cancel_booking_request['data']['service_url'], $cancel_booking_request['data']['request']);
                    //  $cancel_response_format = Converter::createArray ( $send_change_response );
    ...
    function update_markup_currency(&$price_summary, &$currency_obj, $level_one_markup = false, $current_domain_markup = true, $multiplier = 1, $specific_markup_config = array(), $module = '', $search_id = '')
    {
        ...
        foreach ($price_summary as $__k => $__v) {
            //changes added here for proper markup calculation fix
            $tempFare = $price_summary['OfferedFare'];

            $price_summary['OfferedFare'] = $price_summary['BaseFare'];
            //upto here
            if (is_numeric($__v) == true) {
                ...
                //changes added following for markup calculation
                ...
                if (in_array($__k, $markup_list)) {
                    ...
                    $price_summary['OfferedFare'] = $price_summary['BaseFare'];
                    //upto here
                    //changes changed the following to the below line
                    // $temp_price = $currency_obj->get_currency($__v, true, $level_one_markup, $current_domain_markup, $multiplier, $specific_markup_config);

                    $temp_price = $currency_obj->get_currency(
                        $price_summary['OfferedFare'],
                        true,
                        $level_one_markup,
                        $current_domain_markup,
                        $multiplier,
                        $specific_markup_config
                    );
                ...
                //changes added the following for proper markup calculation
                $price_summary['OfferedFare'] = $tempFare;
            }
        }
        ...
        if (isset($markup_summary['OfferedFare'])) {
            //changed the following for proper markup calculation
            // $Markup = $markup_summary['OfferedFare'] - $price_summary['OfferedFare'];
            $Markup = $markup_summary['OfferedFare'] - $price_summary['BaseFare'];
    ...
    {
        if ($_SERVER['REMOTE_ADDR'] == "106.211.215.247") {
            //  debug($agent_price_details);
            //  exit;
        }
        // debug($agent_price_details);exit;
        $total_price['BaseFare'] = round($api_price_details['BaseFare']);
        $total_price['_CustomerBuying'] = round($agent_price_details['PublishedFare']);
        $_AgentBuying = $admin_price_details['OfferedFare'];
        $total_price['_AdminBuying'] = round($api_price_details['OfferedFare']);
        ...
        $total_price['_AgentBuying'] = round($admin_price_details['OfferedFare'] + $gst_value);
        $total_price['_Commission'] = round($agent_price_details['ORG_AgentCommission']);
        ...
        $total_price['_CustomerBuying'] = round($total_price['_CustomerBuying'] + $gst_value);
     ...
     * return booking form
     */
    //  changes start session: added param $search_hash="" in function booking_form
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '', $promotional_plan_type = '', $booking_source = PLAZMA_BOOKING_SOURCE, $search_hash = "")
        //  changes end session: add param $search_hash="" in function booking_form
    {
        $booking_form = '';
    ...
    //changes added new function for b2b farequote
    ...
    public function extract_flight_segment_fare_details($flights, $currency_obj, $search_id, $module, $ins_token = false, &$token = array(), $flight_index = 0)
    {
        ...
        foreach ($flights as $list_k => $list_v) {
            ...
            $list_v['api_original_price_details']['PriceBreakup']['BasicFare'] = $list_v['api_original_price_details']['PriceBreakup']['BasicFare'] - $list_v['api_original_price_details']['PriceBreakup']['CommissionEarned'];
            //    debug($list_v['api_original_price_details']);exit;
            ...
            $flight_list[$list_k]['booking_source'] = PLAZMA_BOOKING_SOURCE;
    ...
    public function extract_flight_segment_fare_details2($flights, $currency_obj, $search_id, $module, $ins_token = false, &$token = array(), $flight_index = 0)
    {
        ...
        foreach ($flights as $list_k => $list_v) {
            ...
            $flight_list[$list_k]['AirlineRemark'] = $this->filter_airline_remark(@$list_v['Attr']['AirlineRemark'], $module);
            // debug($list_v);exit;
            $flight_list[$list_k]['FareDetails'] = $this->get_fare_object2($list_v, $currency_obj, $search_id, $module);
    ...
    private function get_fare_object2($flight_details, $currency_obj, $search_id, $module)
    {
        ...
        if ($module == 'b2c') {
            //$flight_details['FareDetails']['BaseFa$flight_detailre']=s['api_original_price_details']['BasicFare'];
            //$flight_details['FareDetails']['Tax']=$flight_details['api_original_price_details']['Tax'];
            //$flight_details['FareDetails']['PublishedFare']=$flight_details['api_original_price_details']['TotalDisplayFare'];
            //$flight_details['FareDetails']['OfferedFare']=$flight_details['api_original_price_details']['TotalDisplayFare'];
        }
        ...
        if ($module == 'b2c') {
            ...
            //debug($flight_details['FareDetails']);die;
            ...
            $b2c_price_details['BaseFare'] = round($api_price_details['BaseFare']);
            $b2c_price_details['TotalTax'] = round($o_Total_Tax);
            $b2c_price_details['TotalFare'] = $b2c_price_details['BaseFare'] + $b2c_price_details['TotalTax'];
            ...
            $api_price_details['BaseFare'] = round($api_price_details['BaseFare']);
            $api_price_details['PublishedFare'] = round($api_price_details['PublishedFare']);
            $FareDetails['b2c_PriceDetails'] = $b2c_price_details; //B2C PRICE DETAILS

            // $b2c_price_details['BaseFare'] = $flight_details['FareDetails']['BaseFare']-$flight_details['FareDetails']['AgentCommission'];
            //  $b2c_price_details['TotalTax'] =round($o_Total_Tax);
            //echo "testtax";
            //debug( $b2c_price_details['TotalTax']);
            if ($b2c_price_details['TotalTax'] == 0 || $b2c_price_details['TotalTax'] == "") {
                //echo "duc";
                //debug($flight_details['api_original_price_details']);
                //$b2c_price_details['TotalTax']=$flight_details['api_original_price_details']['PriceBreakup']['Tax'];
            }
            ...
            //   debug(  $agent_price_details);
            //   debug($admin_price_details);debug($agent_price_details);
    ...
    private function get_fare_object($flight_details, $currency_obj, $search_id, $module)
    {
        ...
        if ($module == 'b2c') {
            ...
            // echo $o_Total_Fare;exit;
            //   debug($flight_details['FareDetails']);die;-$flight_details['FareDetails']['AgentCommission']
            $b2c_price_details['BaseFare'] = round($api_price_details['BaseFare']);
            $b2c_price_details['TotalTax'] = round($o_Total_Tax);
            $b2c_price_details['TotalFare'] = $b2c_price_details['BaseFare'] + $b2c_price_details['TotalTax'];
            $b2c_price_details['Currency'] = $api_price_details['Currency'];
    ...
    private function preferred_currency_fare_object($fare_details, $currency_obj, $default_currency = '')
    {
        ...
        $FareDetails['BaseFare'] = get_converted_currency_value($currency_obj->force_currency_conversion($base_fare)) - get_converted_currency_value($currency_obj->force_currency_conversion($agent_commission));
        $FareDetails['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($tax));
        $FareDetails['PublishedFare'] = get_converted_currency_value($currency_obj->force_currency_conversion($published_fare)) - get_converted_currency_value($currency_obj->force_currency_conversion($agent_commission));
    ...
    private function get_final_booking_price_details($Fare, $multiplier, $specific_markup_config, $currency_obj, $deduction_cur_obj, $module)
    {
        ...
        if ($module == 'b2c') {
            ...
            $markup_total_fare = $currency_obj->get_currency($Fare['BaseFare'], true, false, true, $multiplier, $specific_markup_config);

            $ded_total_fare = $deduction_cur_obj->get_currency($Fare['BaseFare'], true, true, false, $multiplier, $specific_markup_config);
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c')
    {
        ...
        foreach ($token as $token_index => $token_value) {
            ...
            //changes added the following for proper markup calculation
            $Fare['BaseFare'] = $Fare['BaseFare'] + $Fare['AgentCommission'];

            $tmp_domain_markup = 0;
            $tmp_level_one_markup = 0;
            //changes changed the following from offeredfare to publishedfare
            $itinerary_price = $Fare['PublishedFare'];
            ...
            if ($module == 'b2c') {
                //echo $commissionable_fare+$admin_commission;die;
                $transaction_insert_id = $GLOBALS['CI']->flight_model->save_flight_booking_transaction_details(
                    ...
                    $sequence_number,
                    $currency,
                    $commissionable_fare + $admin_commission,
            ...
            for ($i = 0; $i < $total_pax_count; $i++) {
                ...
                $passenger_ticket_info = $GLOBALS['CI']->flight_model->save_passenger_ticket_info_plazma($pax_insert_id['insert_id'], $s_master_data['ticket']);
        ...
        $payment_mode = $book_params['payment_method'];
           // changes start user crm: commented this line and added new line
    ...
    function set_flight_search_session_expiry($from_cache = true, $search_hash)
    {
        ...
        $response['search_hash'] = $search_hash;
        // changes start session: added plazma_search_hash in $response
        // $response['plazma_search_hash'] = $search_hash;
        // changes end session: added plazma_search_hash in $response
        return $response;
    }
