...
//debug($pax_ticket_ids);exit;
    ...
    public function search_data($search_id) {
        ...
        if (empty($this->master_search_data) == true and valid_array($this->master_search_data) == false) {
            $is_roundtrip = false;
            $is_multicity = false;
            $clean_search_details = $this->CI->flight_model->get_safe_search_datatmx($search_id);
   ...
   //      debug($api_price_details);
     //    debug($admin_price_details);
        ...
        $total_price['_CustomerBuying'] = $api_price_details['PublishedFare'];
        $_AgentBuying = $api_price_details['OfferedFare'];
        $total_price['_AdminBuying'] = $api_price_details['OfferedFare'];
        if($admin_price_details['markup_type']=="")
        {
            $admin_price_details['_Markup']=0;
        }
        $total_price['_AgentMarkup'] = $agent_price_details['_Markup']-$admin_price_details['_Markup'];
        $total_price['_AdminMarkup'] = $admin_price_details['_Markup'];
        $total_price['_OrgAdminMarkup'] = $admin_price_details['_Markup'];
        $total_markup = $total_price['_AdminMarkup'] + $total_price['_AgentMarkup'];
   //   debug( $total_markup );
     
        $total_price['_AgentBuying'] =  $api_price_details['PublishedFare']+round($total_price['_AdminMarkup']);
        $total_price['_Commission'] = round($api_price_details['AgentCommission'], 3);
        $total_price['_tdsCommission'] = $currency_obj->calculate_tds($total_price['_Commission']); //Includes TDS ON PLB AND COMMISSION
        $total_price['_AgentEarning'] = $total_price['_Commission'] + round($total_price['_AgentMarkup'])- $total_price['_tdsCommission'];
        $total_price['_TaxSum'] = $api_price_details['Tax'] ;
        $total_price['_CustomerBuying'] =  $total_price['_CustomerBuying']+$total_markup;
         $total_price['_Markup'] =round($total_markup);
        $total_price['_BaseFare'] = $api_price_details['BaseFare']+$total_price['_AgentMarkup']+ $total_price['_AdminMarkup'];
        $total_price['_TotalPayable'] = $api_price_details['OfferedFare'] + $total_price['_tdsCommission']+round($total_price['_AdminMarkup']);
...
public function extract_flight_segment_fare_details2($flights, $currency_obj, $search_id, $module, $ins_token = false, & $token = array(), $flight_index = 0) {
    ...
    private function get_fare_object($flight_details, $currency_obj, $search_id, $module) {
      ...
      //  debug();
        if ($module == 'b2c') {
$flight_details['FareDetails']['BaseFare']=$flight_details['api_original_price_details']['BasicFare'];
        ...
        if ($module == 'b2c') {
          ...
          ;
           ...
           // echo $o_Total_Fare;exit;
            $b2c_price_details['BaseFare'] =round($flight_details['FareDetails']['BaseFare']);
