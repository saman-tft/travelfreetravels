...
$get = curl_exec($ch);
     ...
     */
    function get_currency($value, $add_markup = true, $parent_markup = false, $seller_markup = true, $markup_multiplier = 1, $specific_markup_config = array(), $booking_source='tmx', $base_fare=0) {
        if(($parent_markup == true) && ($seller_markup == true)){
            $this->set_markup_details($parent_markup, $seller_markup);
        }else if($booking_source == 'amadeus'){
            $this->set_amadeus_markup_details($parent_markup, $seller_markup);
        }else{
            $this->set_markup_details($parent_markup, $seller_markup);    
        }
        ...
        if ($add_markup) {
            $value1=$value;
            if($booking_source == 'amadeus'){
                $value = $this->add_amadeus_markup($value, $markup_multiplier, $parent_markup, $seller_markup, $specific_markup_config, $base_fare);
            }else{
    ...
    protected function add_amadeus_markup($value, $markup_multiplier = 1, $parent_markup, $seller_markup, $specific_markup_config = array(), $base_fare=0) {
        // echo $markup_multiplier;exit;
        if (floatval($value) > 0) {
            $markup_list = array();
            if ($parent_markup == true && valid_array($this->level_one_markup) == true) {
                        ...
                        foreach ($temp_markup_list as $__ik => $__iv) {
                         ...
                         // debug($__iv);
                            ...
                            switch ($__iv['value_type']) {
                                case 'percentage' :
                                    //Just need to calculate percentage of the values
                                    $markup_value = (($base_fare / 100) * $__iv['value']);
                                    $original_markup = $__iv['value'];
                                    $markup_type = 'Pecentage';
                                    break;
                                case 'plus' :
    ...
    protected function set_amadeus_markup_details($parent_markup = true, $seller_markup = true) {
        $CI = &get_instance();
        if ($parent_markup == true and empty($this->level_one_markup)) {
            //LEVEL EVEN(1, 3...)
            //odd nobody(b2c), admin(b2b)b2c user
            $this->level_one_markup = $CI->private_management_model->get_amadeus_markup($this->module_name);            
        }
        if ($seller_markup == true and empty($this->current_domain_markup)) { 
            //LEVEL EVEN(2, 4...)
            //even admin(b2c), agent(b2b)b2b user
            $this->current_domain_markup = $CI->domain_management_model->get_amadeus_markup($this->module_name);
        }
    }
    /**
     * get entity markup
    ...
    function convenience_fees($amount, $search_id, $pax_count ='') {
        ...
        switch ($this->module_name) {
            case 'flight' : $convinence_fees = $this->airline_convinence_fees($amount, $this->convinence_fees_row, $search_id);
                ...
                break;
             case 'plazmaflight' : $convinence_fees = $this->airline_convinence_fees($amount, $this->convinence_fees_row, $search_id);
                //Calculate Convenience fees
                break;
