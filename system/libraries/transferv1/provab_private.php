...
class Provab_private extends Common_Api_Grind {
	...
	function credentials($service) {
		switch ($service) {
				case 'Authenticate' :				
				$this->service_url ='http://prod.services.travelomatix.com/webservices/transferv1/service/Authenticate';			
				break;
			case 'CategoryList':
				$this->service_url ='http://prod.services.travelomatix.com/webservices/transferv1/service/CategoryList';
				break;
			case 'Search':
				$this->service_url ='http://prod.services.travelomatix.com/webservices/transferv1/service/Search';
				break;
			case 'ProductDetails':
				$this->service_url = 'http://prod.services.travelomatix.com/webservices/transferv1/service/ProductDetails';
				break;
			case 'TripList':
				$this->service_url ='http://prod.services.travelomatix.com/webservices/transferv1/service/TripList';
				break;
			case 'BlockTrip':
				$this->service_url = 'http://prod.services.travelomatix.com/webservices/transferv1/service/BlockTrip';
				break;
			case 'Book':
				$this->service_url ='http://prod.services.travelomatix.com/webservices/transferv1/service/CommitBooking';
				break;
			case 'CancelBooking':
				$this->service_url = 'http://prod.services.travelomatix.com/webservices/transferv1/service/CancelBooking';
				break;
			case 'UpdateHoldBooking':
				$this->service_url = 'http://prod.services.travelomatix.com/webservices/transferv1/service/UpdateHoldBooking';
				break;
			case 'CancellationRefundDetails':
				$this->service_url = 'http://prod.services.travelomatix.com/webservices/transferv1/service/CancellationRefundDetails';
				break;
		}
	}
	...
	*/
    ...
    function b2b_price_details($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
    	...
    	$total_price['NetFare'] = $_AgentBuying+$gst_value;
    ...
    function b2b_price_details_old($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        if($total_markup > 0 ){
            ...
            if($gst_details['status'] == true){
                if($gst_details['data'][0]['gst'] > 0){
                    $gst_value = ($total_price['_AdminMarkup']/100) * $gst_details['data'][0]['gst'];
                	$gst_value  = roundoff_number($gst_value);
                }
            }
    	}
	...
	*/
	...
	function save_booking($app_booking_id, $params, $module = 'b2c') {
        ...
        $pax_data = $params['booking_params']['token']['AgeBands'];
			$total_transaction_amount = $trans_total_fare;
        ...
        if($convinence > 0 ){
            ...
            if($gst_details['status'] == true){
                if($gst_details['data'][0]['gst'] > 0){
                	...
                	$gst_value_con  = roundoff_number($gst_value_con);
                }
          	}
        }
        if($gst_value_con >0){
		...
		if($module == 'b2c'){
        	$GLOBALS ['CI']->transferv1_model->save_booking_itinerary_details ( $app_reference, $location, $travel_date,$grade_code, $grade_desc, $status,$commissionable_fare,$admin_net_fare_markup,$admin_markup+$gst_value, $agent_markup, $currency, $attributes, @$book_total_fare,$agent_commission,$agent_tds,$admin_commission,$admin_tds,$api_raw_fare,$agent_buying_price, $gst_value_con);
        }
        else if($module == 'b2b'){
	...
	*/
	...
	function update_markup_currency(& $price_summary, & $currency_obj, $no_of_nights = 1, $level_one_markup = false, $current_domain_markup = true, $module='') {
        ...
        if($module == 'b2c'){
        	...
        	$search_data = $GLOBALS['CI']->private_management_model->transfers_convinence_fees();
	...
	function format_search_response($sl, $cobj, $sid, $module = 'b2c', $fltr = array()) {
		...
		$price_array=array_column($SightseeingResults,'Fare');
        array_multisort($price_array, SORT_ASC,$SightseeingResults);
		// SORTING STARTS
