...
class Sightseeing_crs extends Common_Api_Grind {
	private $ClientId;
	private $UserName;
	private $Password;
	private $service_url;
	...
	*/
	public function get_sightseeing_list($safe_search_data,$search_result)
	{
		...
		if($safe_search_data['data']){
			if ($cache_search === false || ($cache_search === true && empty ( $cache_contents ) == true)) {
				...
				if($search_request['status']==SUCCESS_STATUS)
				{
					...
					$packages = $GLOBALS['CI']->sightseeing_model->activitysearch($safe_search_data); 
				
				$search_country=$safe_search_data['data']['nationality'];
					
				$country_id = $GLOBALS['CI']->sightseeing_model->get_country_id( $search_country );
					//echo "asdf";die;
				// debug($safe_search_data);exit;
				 $cntt = 0;
            foreach ($packages as $keyy => $value) 
            {  
				...
				$cancellation_policy = $GLOBALS['CI']->sightseeing_model->get_cancellation_policy($value->package_id,$from_date);
				// debug($value->refundable);die;
				if(count($cancellation_policy)>0 && $value->refundable != "N")
                {
                    $cancellation_available = 1;
                 }else{
                 	$cancellation_available = 0;
                 }
                // debug($safe_search_data['data']);die;
				$package_price = $GLOBALS['CI']->sightseeing_model->get_package_price_by_app_currency_nationality($value->package_id,$safe_search_data['data']['nationality'],$seasons_date_value);
						//	debug($package_price);die;
				...
				if(count($package_price) >0)
				{
					...
					foreach ($package_price as $key => $price) {
						if($price['transfer_option']=='W'){
							...
							$transfer_price_ary[] = array('transfer_option'=>$price['transfer_option'],'total_price'=>$total_without_transfers);
						}
						if($price['transfer_option']=='S'){
							$adult_total_price = $price['adult_price']*$safe_search_data['data']['adult'];
							$child_total_price = $price['child_price']*$safe_search_data['data']['child'];
							$total_sharing_transfers = $adult_total_price+$child_total_price;
							$transfer_price_ary[] = array('transfer_option'=>$price['transfer_option'],'total_price'=>$total_sharing_transfers);	
						}
						if($price['transfer_option']=='P'){
							$adult_total_price = $price['adult_price']*$safe_search_data['data']['adult'];
							$child_total_price = $price['child_price']*$safe_search_data['data']['child'];
							$total_private_transfers = $adult_total_price+$child_total_price;
							$transfer_price_ary[] = array('transfer_option'=>$price['transfer_option'],'total_price'=>$total_private_transfers);
						}
					}
                ...
                $search_result['data']['SSSearchResult']['SightSeeingResults'][$key]['ImageUrl']=$path.$value->image;
                $search_result['data']['SSSearchResult']['SightSeeingResults'][$key]['ImageHisUrl']=$path.$value->image;
                ...
                $search_result['data']['SSSearchResult']['SightSeeingResults'][$key]['Cancellation_available']=$cancellation_available;
                ...
                $search_result['data']['SSSearchResult']['SightSeeingResults'][$key]['Duration']=$value->duration.' Days ('.sprintf("%02d",$value->activity_duration_hour).':'.sprintf("%02d",$value->activity_duration_min).' Hrs)';
...
// error_reporting('E_ALL');debug(1);exit;
	...
	*/
	...
	public function search_data_in_preferred_currency($search_result, $currency_obj,$module='b2c') {
		...
		foreach ( $sightseeing as $hk => $hv ) {
			...
			//$sightseeing_list [$hk] ['Price'] = $this->module_preferred_currency_fare_object ( $hv ['Price'], $currency_obj,'INR',$module );
			$sightseeing_list [$hk] ['Price'] = $this->module_preferred_currency_fare_object ( $hv ['Price'], $currency_obj,'NPR',$module );
		}
		// debug($sightseeing_list [$hk] ['Price']);exit;
	...
	*/
	...
	function get_convinence_fees($module_name="",$adult_count=0,$total_amount=0){
	    $convinience_fee=$GLOBALS['CI']->private_management_model->get_convinence_fees_new($module_name);
	    // debug($convinience_fee);exit;
	    if($total_amount>0){
		    if($convinience_fee['type']=='plus'){
		        if($convinience_fee['per_pax']==1){
		            return ($adult_count*$convinience_fee['value']);
		        }else{
		            return $convinience_fee['value'];
		        }
		    }else{
		        return (($total_amount/100)*$convinience_fee['value']);
		    }
		}else{
			return 0;
		}
	...
	private function module_preferred_currency_fare_object($fare_details, $currency_obj, $default_currency = '',$module='b2c') {		
		...
		$admin_profit = 0;
		$show_net_fare = 0;
		$convinence_fee=$this->get_convinence_fees('sightseeing',1,$fare_details['TotalDisplayFare']);
		// debug($convinence_fee);exit;
		if(isset($fare_details['PriceBreakup'])){
			$admin_commission = $fare_details['PriceBreakup']['AgentCommission'];
			$admin_tds =$fare_details['PriceBreakup']['AgentTdsOnCommision'];
		}else{
		...
		debug($currency_obj->force_currency_conversion ( $net_fare));exit;*/
		$price_details ['Currency'] = empty ( $default_currency ) == false ? $default_currency : get_application_currency_preference ();
		...
		$price_details['AgentTdsOnCommision'] =get_converted_currency_value ( $currency_obj->force_currency_conversion ( $agent_tdson_commission) );
		//$price_details['AdminCommProfit'] = get_converted_currency_value ( $currency_obj->force_currency_conversion ( $admin_profit) );
		$price_details['ConvinienceFee']=get_converted_currency_value ( $currency_obj->force_currency_conversion ( $convinence_fee) );
    ...
    function b2b_price_details($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $total_price['_CustomerBuying'] =round($agent_price_details['TotalDisplayFare']);
        ...
        $total_price['_AdminBuying'] = round($api_price_details['NetFare']);
//debug( $currency_obj);
        $total_price['_AgentMarkup'] = get_converted_currency_value ( $currency_obj->force_currency_conversion ($agent_price_details['_Markup']));
        $total_price['_AdminMarkup'] =get_converted_currency_value ( $currency_obj->force_currency_conversion ($admin_price_details['_Markup']));
        $total_price['_OrgAdminMarkup'] =get_converted_currency_value ( $currency_obj->force_currency_conversion ($admin_price_details['_Markup']));
       	$total_price['_Markup'] = $total_markup = $total_price['_OrgAdminMarkup'] + $total_price['_AgentMarkup'];
  //   	 debug($total_price);
   //      exit;
        $gst_value = 0;
        if($total_markup > 0 ){
        ...
        $total_price['_TaxSum'] = 0;
       	...
       	$total_price['_AgentBuying'] =round($admin_price_details['NetFare'])+$total_price['_tdsCommission']+$gst_value;
       	$total_price['_TotalPayable'] = round($total_price['_AgentBuying']);
        $total_price['_CustomerBuying'] =round($agent_price_details['TotalDisplayFare']+$gst_value);
       	$total_price['TotalDisplayFare'] = round($total_price['_CustomerBuying']+ get_converted_currency_value ( $currency_obj->force_currency_conversion ($admin_price_details['_Markup'])));
        $total_price['_GST'] = $gst_value;
        $convinence_fee=$this->get_convinence_fees('sightseeing',1,$total_price['TotalDisplayFare']);
        $total_price['ConvinienceFee'] =ceil(get_converted_currency_value ( $currency_obj->force_currency_conversion ($convinence_fee)));
	...
	*/
	...
	private function valid_search_result($search_result) {
		if(valid_array($search_result['data']['SSSearchResult']['SightSeeingResults'])&&$search_result['Status']==SUCCESS_STATUS){
			return true;
		}else{
			return false;
		}
	...
	*/
	...
	function update_search_markup_currency(& $price_summary, & $currency_obj, $search_id, $level_one_markup = false, $current_domain_markup = true, $module='') {		
		...
		// debug($search_id);exit;
			//Added by Chalapathi for MArkup Custamization
		if(isset($search_id))
		{ 
			$this->CI->product_search_id = $search_id;
			$this->CI->product_supplier = 1;
		}
	...
	function update_booking_markup_currency(& $price_summary, & $currency_obj, $search_id, $level_one_markup = false, $current_domain_markup = true, $module='') {
		...
		// debug($this->product_search_id);
		// debug($this->product_supplier);
		// exit();
	...
	function update_markup_currency(& $price_summary, & $currency_obj, $no_of_nights = 1, $level_one_markup = false, $current_domain_markup = true, $module='') {
        ...
        if (isset($markup_summary['NetFare'])) {
        	// echo $markup_summary['NetFare'];exit; 
        	// debug($markup_summary['NetFare']);exit;
	 ...
	 */
	function format_search_response($sl, $cobj, $sid, $module = 'b2c', $fltr = array(), $user_type=B2B_USER) {
		// debug($sl);exit;
		$CI=&get_instance();
		$level_one = true;	
		$current_domain = true;
		...
		$sc = 0;
		$frc = 0;
		$safe_search_data = $GLOBALS ['CI']->activity_model->get_safe_search_data($sid,$modeule_id='b2b');
		$safe_search_data_markup = $GLOBALS ['CI']->sightseeing_model->get_safe_search_data($sid,META_SIGHTSEEING_COURSE);
        $country_city_data = $GLOBALS['CI']->activity_model->get_country_city($safe_search_data['data']['destination_id']);
		...
		foreach ( $sl ['SSSearchResult'] ['SightSeeingResults'] as $hr => $hd ) {
			...
			$hd['booking_source']=PROVAB_SIGHTSEEN_SOURCE_CRS;
            $product_id = '';
			$supplier = 'Voyage CRS';
                if(!empty($hd['booking_source'])){
                    if($hd['booking_source'] == PTBSID00000000012){
                        $supplier = 'Voyage CRS';
                        if(!empty($hd['ProductCode'])){
                        $product_id = $hd['ProductCode'];
                        }
                    }else{
                ...
                if($hd['booking_source'] == PROVAB_SIGHTSEEN_SOURCE_CRS)
                {

                    if($hd['ProductCode'])
                    { 
                        $this->CI->product_id = $hd['ProductCode']; 
                         // debug( $GLOBALS['CI']);exit();
                    } 
                         // $GLOBALS['CI']->product_id = '39'; 
                } 
			...
			if($module=='b2b'){
                ...
                $agent_markup_details = $GLOBALS ['CI']->activity_model->get_markup_for_admin ( $hd ['Price']['TotalDisplayFare'], $supplier, $cntry_code, $city_id, $product_id);
                	$admin_markup_details = $GLOBALS ['CI']->activity_model->get_markup_for_admin ( $hd ['Price']['TotalDisplayFare'], $supplier, $cntry_code, $city_id, $product_id);
                $agent_price_details['Currency'] = 'NPR';
                $agent_price_details['TotalDisplayFare'] = $hd ['Price']['TotalDisplayFare']+$agent_markup_details+$search_level_markup+$admin_markup_details;
				...
				$admin_markup_details = $GLOBALS ['CI']->activity_model->get_markup_for_admin ( $hd ['Price']['TotalDisplayFare'], $supplier, $cntry_code, $city_id, $product_id);
                $admin_price_details['Currency'] =  get_application_currency_preference();
                ...
                $agent_markup_details = $GLOBALS ['CI']->activity_model->agent_markup ( $hd ['Price']['TotalDisplayFare'], $cntry_code, $city_id);
                $agent_price_details['Currency'] = get_application_currency_preference();
		...
		//	debug($hd ['Price']);die;
for($i=0;$i<count($hd['transport_price']);$i++)
{
$hd ['transport_price'][$i]['total_price']=round(round(get_converted_currency_value ( $currency_obj->force_currency_conversion ($hd ['transport_price'][$i]['total_price'])))+$hd ['Price']['_AdminMarkup']); 
}
//debug($hd ['transport_price']);die;
			// if($hd['ProductCode'] =='39')
			// {
