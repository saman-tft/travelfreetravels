...
class Booking_Data_Formatter {
	...
	function format_activity_booking_data($complete_booking_details, $module)
	{
		$response['status']	= SUCCESS_STATUS;
		$response['data']	= array();
		$booking_details = array();
	   ...
	   // debug($master_booking_details);exit;
		...
		foreach($master_booking_details as $book_k => $book_v) {
			...
			$fare += $itinerary_v['total_fare'];
				$admin_markup += floatval($core_booking_details['admin_markup']);
				$agent_markup += floatval($core_booking_details['agent_markup']);
				$agent_commission +=floatval($core_booking_details['agent_commission']);
				$admin_commission +=floatval($core_booking_details['admin_commission']);
				$admin_tds +=floatval($core_booking_details['admin_tds']);
				$agent_tds +=floatval($core_booking_details['agent_tds']);
				$gst += floatval($core_booking_details['gst_percentage']);
			if($module == 'b2b' || $module == 'b2c' ){
				//get the converted currency rate for booking transaction
			...
			// debug($booking_itinerary_details);
			// exit;
			//Calculating Price
			// debug($master_booking_detail);exit;
			// foreach($master_booking_details as $itinerary_k => $itinerary_v) {
			...
			// 	$gst += floatval($itinerary_v['gst_percentage']);
			...
			if($module == 'b2b' || $module=='admin') {
				...
				$agent_buying_price_amt = $core_booking_details['basic_fare']+$core_booking_details['convenience_fee']+$core_booking_details['gst']+$core_booking_details['admin_markup'];
				$core_booking_details['agent_buying_price'] = roundoff_number($agent_buying_price_amt);
			} else {
	...
	function format_sightseeing_booking_data_hb($complete_booking_details, $module)
	{
	...
	function format_sightseeing_booking_data_hb_crs($complete_booking_details, $module)
	{
	...
	//	$data['bus_booking_count'] = $GLOBALS['CI']->bus_model->booking($condition, true);
		...
		$data['sightseeing_booking_count'] = $GLOBALS['CI']->activity_model->b2c_holidaycrs_report($condition, true);

		$data['transfer_booking_count'] = $GLOBALS['CI']->transferv1_model->booking($condition, true);
			//$data['private_transfer_booking_count'] = $GLOBALS['CI']->package_model->b2b_holiday_report($condition, true);
			//	$data['private_car_booking_count'] = $GLOBALS['CI']->car_model->privatebooking($condition, true);
					//$data['villas_booking_count'] = $GLOBALS['CI']->hotel_model->bookingcrs($condition, true);
	 ...
	 */
    	...
    	function format_transfer_booking_data_crs($complete_booking_details, $module)
	{
		$response['status']	= SUCCESS_STATUS;
		$response['data']	= array();
		$booking_details = array();
	   ...
	   // debug($complete_booking_details);exit;
		...
		$cancellation_details = $this->format_transferv1_cancellation_details($complete_booking_details['data']['booking_details']);
		...
		foreach($master_booking_details as $book_k => $book_v) {
			...
			foreach($booking_itinerary_details as $itinerary_k => $itinerary_v) {
				...
				$agent_tds +=floatval($itinerary_v['agent_tds']);
				$gst += floatval($itinerary_v['gst']);
				$agent_buying_price += floatval($itinerary_v['agent_buying_price']);
				//$admin_net_fare_markup +=floatval($itinerary_v['admin_net_markup']);
			...
			//Booking Details
			$attributes = json_decode($core_booking_details['attributes'], true);
			$attributes_itinerary = json_decode($booking_itinerary_details[0]['attributes']);
			// debug($attributes_itinerary);exit;
			$core_booking_details['ProductImage'] = $booking_itinerary_details[0]['vehicle_image'];
			$core_booking_details['Destination'] = $booking_itinerary_details[0]['location'];
			
			$core_booking_details['ProductSpecifications'] = $attributes['ProductSpecifications'];
			$core_booking_details['TransferPickupTime'] = $booking_itinerary_details[0]['travel_time'];
			$core_booking_details['TransferPickupInformation'] = $attributes['TransferPickupInformation'];

			$core_booking_details['DeparturePointAddress'] = $booking_itinerary_details[0]['travel_from'];
			$core_booking_details['TM_Cancellation_Policy'] = $attributes_itinerary->cancellation_details;
			$core_booking_details['adult_count'] = $customer_details[$app_reference][0]['adult'];
			$core_booking_details['child_count'] = $customer_details[$app_reference][0]['child'];
			$core_booking_details['youth_count'] = $youth_count;
			$core_booking_details['senior_count'] = $senior_count;
			$core_booking_details['infant_count'] = $infant_count;
			...
			// echo $admin_markup;
			// exit;
			$core_booking_details['product_total_price'] =roundoff_number($agent_buying_price+$agent_markup);//without convience fee

			// debug($core_booking_details);
			// exit;
			$admin_buying_price = $fare+$admin_markup;
			$core_booking_details['admin_buying_price'] = roundoff_number($admin_buying_price);
